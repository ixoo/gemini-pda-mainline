...
| Selected built-in source conditionally reaches the gen3 WLAN initializer through connection and WLAN wrappers; wrapper results are arithmetically aggregated | Pinned detector Makefile and complete selected lifecycle wrappers; source fact | Actual weak/ordinary final linkage, the externally supplied chip value and every gen3 exit invocation/order remain unresolved. The exact retained reconstructed ELF has one `GLOBAL FUNC` entry for each required wrapper/init/exit symbol, but all four sizes are zero; synthesized binding and neighboring addresses cannot establish original strength or exact ranges. See the [drv-init audit](../../experiments/2026-09-06-mt6797-wlan-drv-init-lifecycle-source-attribution/README.md) and [retained-ELF boundary result](../../experiments/2026-09-06-mt6797-wlan-final-linkage-teardown-attribution/README.md) |
