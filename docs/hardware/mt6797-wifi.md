...
| Selected built-in source conditionally reaches the gen3 WLAN initializer through connection and WLAN wrappers; wrapper results are arithmetically aggregated | Pinned detector Makefile and complete selected lifecycle wrappers; source fact | Actual weak/ordinary final linkage, the externally supplied chip value and every gen3 exit invocation/order remain unresolved; see the [drv-init audit](../../experiments/2026-09-06-mt6797-wlan-drv-init-lifecycle-source-attribution/README.md) |
| The detector's level-6 built-in initcall registers its character interface and SDIO detector, while a later `COMBO_IOCTL_DO_MODULE_INIT` request passes its scalar argument to `do_connectivity_driver_init` and returns the integer aggregate unchanged through the compat handler | Pinned selected detector/stub bodies and direct registration/header/cleanup edges; source fact | Registration does not initialize connectivity. The actual userspace issuer, value, order and return/retry policy are unresolved; configuration alone does not force `0x6797`. The cleanup ioctl only establishes SDIO-detector unregister, not a gen3 teardown join; see the [producer audit](../../experiments/2026-09-06-mt6797-connectivity-producer-source-attribution/README.md) |
