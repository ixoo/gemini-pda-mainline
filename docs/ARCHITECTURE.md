...
property. Its two builds are byte-identical, all semantic/container gates pass,
and its logical-`boot2` write has a matching full readback; runtime is pending.
The exact kernel compiles fbcon rotation out, so sideways output is expected
until display retention is stable and a separate configuration-only follow-up
is justified.
