...
not establish repeatability or stable display retention. Because the kernel's
unused-clock cleanup completes before external `/init`, a broad
`clk_ignore_unused` candidate is deferred. Candidate I instead preserves H's
exact kernel and DTB and exact initramfs tree except `/init`, then emits one
tty0 line per second through `T+60` before a silent static hold. It is built
reproducibly, exported, synchronized and fully read back from logical `boot2`;
runtime is pending. The exact kernel compiles fbcon rotation out, so sideways
output is expected until display retention is stable and a separate
configuration-only follow-up is justified.
