...
| DVFSP handoff and AP-DMA ownership | `partial` | `local` | `none` | The local handoff passed late validation and released I2C6 for the read-only DA921x provider while preserving the shared AP-DMA owner used by I2C5. A separate enabled clock-backend node is not composable yet: its independent CSPM claim produced `-EBUSY` at the handoff, deferred I2C6, and removed the DA921x client. One CSPM owner or an explicit shared-access contract is required before any protected clock read. Resume and error-recovery ownership remain open. See the [resource map](hardware/mt6797-live-resource-map.md) and [clock-entry result](../experiments/2026-08-23-mainline-clock-backend-first-dmesg-entry/results/runtime-attempt-1-read-free-pass-resource-conflict-20260823.txt). |
