...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | Three fresh exact patch-`0481` current-mainline attempts passed pristine single triggers with CPUs `0-9` online and independent CPU8/CPU9 scheduler accounting. The third also pinned work to each A72 and passed bidirectional 1.9 MiB volatile-RAM writer/reader checks with every hash matching, cleanup complete, no device-storage access, and changed-ID recovery with unchanged `boot2` and terminal retained proofs. The exact DT lacks `cpu-map`, so Linux deterministically reports a flat package with core siblings `0-9`; topology is not yet modeled correctly. This is bounded dual-A72 execution and RAM-integrity support on the named device, not stable/default support or general coherency stress. CPU-map repair, concurrent load, safe CPU_OFF/hotplug, cpufreq/OPP, thermal, suspend, default integration, and upstream review remain open. See the [RAM-integrity/topology result](../experiments/2026-09-02-mainline-dual-a72-ram-coherency/results/runtime-attempt-1-ram-integrity-pass-flat-topology-20260902.txt) and [DA921x/A72 boundary](hardware/da921x-i2c6-a72.md). |
