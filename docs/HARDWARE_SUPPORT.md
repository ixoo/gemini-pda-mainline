...
is a failed enumeration test, not proof that Linux did not execute.
...
power domains. The exact kernel compiles fbcon rotation out, so rotation is a
separate follow-up. None of this is native display support or proof that every
...
by the standing `boot2` opt-in. The completed reassessment selected
[Candidate K](../experiments/2026-07-17-fbcon-newline-boundary-diagnostic/README.md),
an exact-J kernel/DTB/container-layout derivative whose initramfs changes only
`/init`. Its 20 one-second fixed-width carriage-return updates deliberately use
no newline, followed by a distinct transition and 12 controlled one-second
newline lines; asynchronous kernel printk remains a confounder. Two final VM
builds are byte-identical. K's raw SHA-256 is
`83704cde0e3e4ed897990b230a817a1c7618201a6b8a33a86a2e19c8e07a07cb`.
After a fresh exact-J backup, it was synchronized, flushed and fully read back
from logical `boot2` with padded SHA-256
`959092428f849c5ee2612c352ac4e4f707a4e0ec8696bda6632252e7194a7927`.
The device remains in Gemian. Runtime is pending; one attended K selection is
the next gate. See the [K write/readback](../experiments/2026-07-17-fbcon-newline-boundary-diagnostic/results/boot2-write-candidate-k-20260717.txt).
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Boot | Planet LK development path | all | boot chain | Retained Android 8 LK requires `bootopt=64...`, gzip plus an appended DTB, its pre-jump CPU/reserved-memory/SCP DT shapes, and a decompressed payload within the MT6797 50 MiB buffer. The stock Gemian Image uses `text_offset=0x80000` with `kernel_addr=0x40080000`; the current Image uses `text_offset=0` and the header-derived `0x40200000`, satisfying the 2 MiB arm64 placement rule. The one-CPU, storage-inert handoff package and diagnostic wrappers pass exact flags, DT-delta, Android-v0 layout/ID, gzip/FDT, size, checksum, and reproducibility gates. The USB candidate remained dark with no enumeration; a timed-reboot initramfs produced strong indirect `/init` evidence but did not restart. Candidate E's raw marker remained black. Candidate F retained only `CLK_INFRA_DISP_PWM` through simplefb and produced sideways fbcon for about one second before black. Candidate G removed every raw framebuffer access and reproduced sideways scrolling for 1–2 seconds before black with the backlight apparently off. Candidate H added `CLK_TOP_MUX_MM`; two attempts approximately exposed its `/init` marker but later attempts did not. Candidate I's intended selection was directly black with no marker. Candidate J added forced `clk_ignore_unused`; its reports provisionally map to two tick-04-compatible visible-to-black outcomes and one direct-black/no-console outcome. Candidate K keeps exact J's kernel/DTB/container layout and changes only `/init` to separate 20 CR/no-newline updates from 12 newline lines; asynchronous printk remains a confounder. K is reproducibly built, synchronized, flushed and fully read back from logical `boot2`; runtime is pending. PSCI/TOPRGU restart behavior and an independent post-black signal remain unresolved | `observed` | `missing` | `required-nonfree` | [Current handoff candidate](../experiments/2026-07-16-lk-handoff-alignment/README.md); [USB result](../experiments/2026-07-16-usb-gadget-diagnostic/results/runtime-usb-enumeration-attempt-20260716.txt); [timed-reboot result](../experiments/2026-07-16-timed-reboot-diagnostic/results/runtime-timed-reboot-attempt-20260716.txt); [Candidate F runtime](../experiments/2026-07-16-screen-clock-retention-diagnostic/results/runtime-candidate-f-20260716.txt); [Candidate G runtime](../experiments/2026-07-16-fbcon-text-diagnostic/results/runtime-candidate-g-20260716.txt); [Candidate H runtime](../experiments/2026-07-16-simplefb-mm-root-retention/results/runtime-candidate-h-20260716.txt); [Candidate I](../experiments/2026-07-16-fbcon-refresh-timing-diagnostic/README.md); [Candidate J](../experiments/2026-07-17-clk-ignore-unused-diagnostic/README.md); [Candidate J first runtime](../experiments/2026-07-17-clk-ignore-unused-diagnostic/results/runtime-candidate-j-attempt-1-20260717.txt); [Candidate J repeat report](../experiments/2026-07-17-clk-ignore-unused-diagnostic/results/runtime-candidate-j-repeat-report-20260717.txt); [Candidate K](../experiments/2026-07-17-fbcon-newline-boundary-diagnostic/README.md); [Candidate K write/readback](../experiments/2026-07-17-fbcon-newline-boundary-diagnostic/results/boot2-write-candidate-k-20260717.txt); [restart source audit](../experiments/2026-07-16-timed-reboot-diagnostic/results/restart-path-source-audit-20260716.txt); [Boot contract recovery](../experiments/2026-07-12-boot-contract-recovery/README.md); perform one attended K selection after a known-good boot and normal temperature; no J repetition or matched-I rollback |
...
| Watchdog | MT6797 TOPRGU at `0x10007000` | all | TOPRGU / SPI137 bark IRQ | Linux 7.1.3 `mtk_wdt` already covers the MT6797 register and reset-controller protocol; the Gemini board DTS patch adds the vendor-confirmed pretimeout IRQ without importing the vendor WDK ABI. The current package also enables `WATCHDOG_HANDLE_BOOT_ENABLED`, so a firmware-running timer will be serviced and its timeout rewritten during probe. A timed `RB_AUTOBOOT` attempt reached an off-like state but did not restart; the next Gemian boot reported no PMIC/AED watchdog reset. Gemian sets TOPRGU mode bit 4 to bypass the power key for normal reboot, while mainline's restart handler preserves that bit. Because PSCI runs first, a successful key-gated TOPRGU reset is a source-backed explanation but is not yet proven | `unknown` | `local` | `none` | [Watchdog recovery](../experiments/2026-07-12-mt6797-watchdog-recovery/README.md); [current boot-policy audit](../experiments/2026-07-12-mt6797-watchdog-recovery/results/mainline-watchdog-current-72-policy-20260714.txt); [timed-reboot runtime result](../experiments/2026-07-16-timed-reboot-diagnostic/results/runtime-timed-reboot-attempt-20260716.txt); [restart source audit](../experiments/2026-07-16-timed-reboot-diagnostic/results/restart-path-source-audit-20260716.txt); isolate PSCI from TOPRGU and test bit-4 policy before separately testing pretimeout/restart with a recovery path |
...
| Display pipeline | MT6797 MMSYS/SMI/IOMMU/CMDQ/MM mutex/OVL/PQ/RDMA/DSI | all | multimedia | Local series adds disabled SMI/IOMMU, GCE, mutex, 29 routes and 64 resets; patches 33–43 add native component/DSI/PHY data, the recovered 12-block DRM master path, disabled SoC nodes, and a descriptor-ready NT36672E framework; current package audit confirms DRM/DSI/PHY are module-only and every risky native display node remains disabled. Candidate E's bounded raw simplefb fill remained black. Candidate F changed only the simplefb `CLK_INFRA_DISP_PWM` property and produced about one second of sideways fbcon before black. Candidate G removed all raw framebuffer access and reproduced sideways scrolling for 1–2 seconds before black with the backlight apparently off. Candidate H added `CLK_TOP_MUX_MM`; two attempts approximately exposed its initramfs marker, while later attempts did not. Candidate I was directly black with no marker. Candidate J added forced `clk_ignore_unused`; its reports provisionally map to two tick-04-compatible visible-to-black outcomes and one direct-black/no-console outcome. Candidate K retains exact J's kernel/DTB/container layout and changes only `/init`: 20 fixed-width CR/no-newline updates, then 12 controlled newline lines. Asynchronous printk remains a confounder. K is synchronized and fully read back from `boot2`; runtime is pending. This remains diagnostic loader scanout, not native DRM/DSI/panel ownership; broad retention neither enables already-off clocks nor prevents explicit disables or retains regulators/power domains. The exact kernel compiles rotation out, so a later isolated config change must enable `CONFIG_FRAMEBUFFER_CONSOLE_ROTATION` before `fbcon=rotate:3` can work. The bsg100 generic OVL/CRTC, DSI-command-mode, D-PHY timing and MIPI-TX findings remain the starting point for a native port; its SSD2092 consumer remains variant-gated. Mutex is implicitly enabled but its probe only maps/registers a clock | `unknown` | `local` | `build-only` | [DRM component recovery](../experiments/2026-07-12-mt6797-drm-component-recovery/README.md); [Candidate F runtime](../experiments/2026-07-16-screen-clock-retention-diagnostic/results/runtime-candidate-f-20260716.txt); [Candidate G runtime](../experiments/2026-07-16-fbcon-text-diagnostic/results/runtime-candidate-g-20260716.txt); [Candidate H runtime](../experiments/2026-07-16-simplefb-mm-root-retention/results/runtime-candidate-h-20260716.txt); [Candidate I](../experiments/2026-07-16-fbcon-refresh-timing-diagnostic/README.md); [Candidate J](../experiments/2026-07-17-clk-ignore-unused-diagnostic/README.md); [Candidate J first runtime](../experiments/2026-07-17-clk-ignore-unused-diagnostic/results/runtime-candidate-j-attempt-1-20260717.txt); [Candidate J repeat report](../experiments/2026-07-17-clk-ignore-unused-diagnostic/results/runtime-candidate-j-repeat-report-20260717.txt); [Candidate K](../experiments/2026-07-17-fbcon-newline-boundary-diagnostic/README.md); [Candidate K write/readback](../experiments/2026-07-17-fbcon-newline-boundary-diagnostic/results/boot2-write-candidate-k-20260717.txt); [bsg100 fbcon commit audit](../experiments/2026-07-13-bsg100-gemini-linux-comparison/results/fbcon-commit-035d4b0-20260716.md); [current display/input package audit](../experiments/2026-07-12-input-backlight-recovery/results/mainline-display-input-current-72-package-20260714.txt); perform one attended K selection, then reassess; attach native panel, bias, reset, backlight, SMI/IOMMU, mutex, and graph contracts only in separate experiments |
