...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | Four fresh current-mainline attempts passed pristine single triggers with CPUs `0-9` online and independent CPU8/CPU9 scheduler accounting. The patch-`0481` RAM attempt also pinned work to each A72 and passed bidirectional 1.9 MiB volatile-RAM writer/reader checks with every hash matching, cleanup complete, no device-storage access, changed-ID recovery, unchanged `boot2`, and terminal retained proofs. Canonical patch `0482` adds the previously missing 4+4+2 `cpu-map`; its first exact runtime attempt preserved admission and produced 102 accounting ticks on each A72, but automatic return removed USB before the separate topology/RAM session, so live package/cluster/core/thread acceptance remains unmeasured rather than failed. This is bounded dual-A72 execution and RAM-integrity support on the named device, not stable/default support or general coherency stress. Runtime CPU-map acceptance, concurrent load, safe CPU_OFF/hotplug, cpufreq/OPP, thermal, suspend, default integration, and upstream review remain open. See the [RAM-integrity result](../experiments/2026-09-02-mainline-dual-a72-ram-coherency/results/runtime-attempt-1-ram-integrity-pass-flat-topology-20260902.txt) and [CPU-map follow-up](../experiments/2026-09-02-mainline-mt6797-cpu-map/README.md). |
