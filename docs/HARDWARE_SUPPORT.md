...
is the current build-only observability gate, adding UART0 GPIO97/98 pinmux,
an exact mainline-console/active-Gemian primary `console-ramoops` alignment, and
MT6797 watchdog auto-restart plus IRQ-dependent dual-stage policy. Mainline pmsg is not
cross-version evidence; its enlarged allocation supplies address alignment. It
is not runtime-tested and must not be used to promote boot, console, reset, or
display support.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Boot | Planet LK development path | all | boot chain | Retained Android 8 LK requires `bootopt=64...`, gzip plus an appended DTB, its pre-jump CPU/reserved-memory/SCP DT shapes, and a decompressed payload within the MT6797 50 MiB buffer. The stock Gemian Image uses `text_offset=0x80000` with `kernel_addr=0x40080000`; the current Image uses `text_offset=0` and the header-derived `0x40200000`, satisfying the 2 MiB arm64 placement rule. The one-CPU, storage-inert handoff package and diagnostic wrappers pass exact flags, DT-delta, Android-v0 layout/ID, gzip/FDT, size, checksum, and reproducibility gates. Earlier USB, reboot, and simplefb candidates establish only bounded/inconclusive observations; Candidate J has mixed transient fbcon reports. Candidate K was an initramfs-only exact-J derivative, written/read back but cancelled without runtime because it changes no kernel, DT, or configuration input. Candidate L is build-only observability work: corrected UART pinmux, exact primary ramoops-console alignment validated against pinned source plus the active binary and live headers, and watchdog auto-restart plus IRQ-dependent dual-stage policy; pmsg supplies address alignment and is excluded as cross-version evidence. PSCI/TOPRGU runtime behavior and an independent post-black signal remain unresolved | `observed` | `missing` | `required-nonfree` | [Current handoff candidate](../experiments/2026-07-16-lk-handoff-alignment/README.md); [Candidate J](../experiments/2026-07-17-clk-ignore-unused-diagnostic/README.md); [cancelled Candidate K](../experiments/2026-07-17-fbcon-newline-boundary-diagnostic/README.md); [Candidate L](../experiments/2026-07-17-uart-pstore-observability/README.md); build/validate L, then use one attended test to distinguish UART, recovered console, watchdog, and no-entry outcomes; no J/K repetition or matched-I rollback |
...
| UART | MT6797 UART0 debug console; UART1–3 auxiliary ports | all | UART0–3 at `0x11002000`–`0x11005000`, GIC SPIs 91–94 | Live `ttyMT0`–`ttyMT3` bind to vendor `mtk-uart`; Linux 7.1.3 `8250_mtk` and the MT6797 compatible provide the correct 16550/PIO reuse path, and explicitly disable DMA for a console. Vendor VFIFO/AP-DMA windows remain intentionally unrepresented. Captured Gemian and independent reference DT evidence identify UART0 RX GPIO97/function 1 and TX GPIO98/function 1; Candidate L adds that board pinmux correction, but it is build-only until an exact candidate runs | `unknown` | `released` | `none` | [UART/console recovery](../experiments/2026-07-13-uart-console-recovery/README.md); [Candidate L](../experiments/2026-07-17-uart-pstore-observability/README.md); verify early/normal logs and reconcile LK's downstream `ttyMT0` token with the mainline `ttyS*` console name |
| Watchdog | MT6797 TOPRGU at `0x10007000` | all | TOPRGU / SPI137 bark IRQ | Linux 7.1.3 `mtk_wdt` already covers the MT6797 register and reset-controller protocol; the Gemini board DTS patch adds the vendor-confirmed pretimeout IRQ without importing the vendor WDK ABI. A timed `RB_AUTOBOOT` attempt reached an off-like state but did not restart; the next Gemian boot reported no PMIC/AED watchdog reset. Gemian sets TOPRGU mode bit 4 to bypass the power key for normal reboot, while mainline preserved it. Candidate L adds a narrow MT6797 auto-restart policy that sets bit 4 when the watchdog starts, restarts, or adopts an already-running firmware watchdog; it is build-only and not a reset-success claim | `unknown` | `local` | `none` | [Watchdog recovery](../experiments/2026-07-12-mt6797-watchdog-recovery/README.md); [restart source audit](../experiments/2026-07-16-timed-reboot-diagnostic/results/restart-path-source-audit-20260716.txt); [Candidate L](../experiments/2026-07-17-uart-pstore-observability/README.md); use one ownership-handoff ping followed by a bounded watchdog expiry, then recover pstore before inferring restart behavior |
...
| Display pipeline | MT6797 MMSYS/SMI/IOMMU/CMDQ/MM mutex/OVL/PQ/RDMA/DSI | all | multimedia | Local series adds disabled SMI/IOMMU, GCE, mutex, routes/resets, and a descriptor-ready NT36672E framework; native DRM/DSI/PHY nodes remain disabled or module-only. Candidates F–J provide transient/inconclusive loader-simplefb evidence only, not native display support. Candidate K was an initramfs-only exact-J derivative, written/read back but cancelled without runtime. Candidate L does not claim a display change; it supplies independent UART/pstore/watchdog observations before another display-specific test | `unknown` | `local` | `build-only` | [DRM component recovery](../experiments/2026-07-12-mt6797-drm-component-recovery/README.md); [Candidate J](../experiments/2026-07-17-clk-ignore-unused-diagnostic/README.md); [cancelled Candidate K](../experiments/2026-07-17-fbcon-newline-boundary-diagnostic/README.md); [Candidate L](../experiments/2026-07-17-uart-pstore-observability/README.md); do not infer native panel/DRM support; recover L evidence before selecting a new display hypothesis |
