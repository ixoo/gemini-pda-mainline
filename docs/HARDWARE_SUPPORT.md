...
| MT6797 I2C6 native short read | `partial` | `local` | `none` | The native packed/FIFO one-byte pointer plus one-byte read path is proven for the fixed diagnostic and read-only DA921x provider shapes, including an exact 30-transfer attributed ledger with no AP-DMA or write-shaped transfers. Arbitrary lengths, the one-message two-byte write shape, failures, reset recovery, suspend, and stress are unproved. See the [DA921x/I2C6 boundary](hardware/da921x-i2c6-a72.md). |
...
| Legacy DA921x identification/provider | `partial` | `local` | `none` | The dedicated legacy driver has an exact mainline runtime in which two read-only providers bound after 14 identity reads and completed four consistent provider reads with zero writes. A later one-shot runtime preflight retained the exact 20-entry startup ledger, added ten expected reads, observed two stable safe-prestate samples, and returned natively to Gemian with zero write-shaped transfers and CPU8/9 closed. This closes Gate-6 B3/B4, not firmware-writer exclusion, native write transport, rail ownership, rollback, resume, or A72 integration. See the [finalized preflight result](../experiments/2026-08-18-mainline-da921x-runtime-preflight-ledger/results/runtime-attempt-1e-finalized-20260818.txt) and [ordered roadmap](ROADMAP.md#ordered-gates). |
