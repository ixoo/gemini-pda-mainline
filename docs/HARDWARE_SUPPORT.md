...
| Legacy DA921x identification/provider | `partial` | `local` | `none` | The dedicated legacy driver has exact mainline runtimes in which two read-only providers bound after 14 identity reads, completed four consistent provider reads, retained exact bounded ledgers, observed stable safe prestate, and held SCP reset asserted at both edges of all 20 startup transactions. Every write-shaped counter stayed zero and CPU8/9 remained closed. This closes Gate-6 B1/B3/B4 on the named unit, not B2 native write transport, rail ownership, rollback, resume, or A72 integration. See the [transaction-window result](../experiments/2026-08-18-mainline-i2c6-firmware-writer-transaction-window/results/runtime-attempt-1-success-20260819.txt) and [ordered roadmap](ROADMAP.md#ordered-gates). |
