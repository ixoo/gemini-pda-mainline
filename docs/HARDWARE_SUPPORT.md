...
| Built-in keyboard | `partial` | `local` | `none` | AW9523 plus generic matrix polling provides working keyboard input and the current physical map. A bounded attended capture observed correct F1–F10, Home/End/Page Up/Page Down, Shift/Ctrl/Alt/Fn translations and short typing, including recorded repeats. Ctrl-first release ordering, wider physical coverage, rollover, wake and sustained reliability remain open. See the [keyboard facts](hardware/keyboard.md). |
| USB gadget Ethernet and development shell | `working` | `local` | `none` | Peripheral-mode Ethernet now has a named first-boot result for key-authenticated administration, rejection of incorrect keys/host pins, readable console and complete sealed boot-log capture. Gemian return was confirmed; the native-request transport witness remains inconclusive. Host mode, role switching, charging, both ports and hotplug regression remain open. See the [authenticated baseline outcome](../experiments/2026-09-05-owner-away-experiment-preparation/baseline/ATTENDED_OUTCOME.md) and [earlier USB experiment](../experiments/2026-07-21-usb-gadget-ethernet/README.md). |
...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | Isolated candidates demonstrate both A72 CPUs, standard 4+4+2 topology, bounded peer-visible RAM checks, independent scheduler accounting and repeated CPU9 down/restore. One integrated lifecycle/frequency/thermal/load protocol passed; a separate boot reproduced its finite CPU/RAM/frequency result but rejected the thermal comparison. General stress/hotplug, thermal protection, cpufreq/OPP, suspend, default integration and upstream review remain open. See [integrated evidence](../experiments/2026-09-04-mt6797-a72-frequency-observation/results/zero-divider-runtime-pass-20260904.txt), [thermal rejection](../experiments/2026-09-04-mt6797-a72-frequency-observation/results/cold-rise-runtime-thermal-rejected.txt), and [repeated lifecycle](../experiments/2026-09-02-mainline-a72-hotplug-lifecycle-gate/README.md). |
| DVFSP handoff and AP-DMA ownership | `partial` | `local` | `none` | The bounded handoff releases I2C6 while preserving the I2C5 shared AP-DMA owner. Isolated provider/backend binding and an attributable protected-clock snapshot have serviceability evidence. General concurrent ownership, retry/error recovery, resume and writable policy remain open. A72 transaction results are scoped separately in the CPU row. See [resource ownership](hardware/mt6797-live-resource-map.md) and [protected snapshot](../experiments/2026-08-23-mainline-protected-clock-first-dmesg-call/results/runtime-attempt-1-pass-20260823.txt). |
...
| Charger, fuel gauge, and battery telemetry | `unknown` | `local` | `unknown` | One reviewed Gemian identity read matches BQ25896, with the vendor interface's transfer-status limitation. Safe mainline charging, telemetry, thermal protection, suspend behavior, and standard power-supply integration remain unproved. See the [charger identity result](../experiments/2026-07-12-charger-power-recovery/CHARGER_ID.md) and [charger recovery record](../experiments/2026-07-12-charger-power-recovery/README.md). |
| Thermal management | `partial` | `local` | `none` | Isolated candidates provide calibration-gated, read-only temperature observation and complete per-bank snapshots. The corrected conversion passed its bounded, workload-free snapshot regression on the named Gemini; see [corrected runtime evidence](../experiments/2026-09-04-mt6797-thermal-snapshot/results/v4-runtime-pass.txt). Cross-boot thermal comparison remains rejected; conversion age, calibrated accuracy and the physical cause of the workload-associated rise are unresolved. Observation does not establish trips, watchdog/IRQ protection, cooling, throttling, PM, suspend or default-profile support. The standalone AUXADC consumer remains disabled. See [thermal runtime evidence](../experiments/2026-09-04-mt6797-thermal-ledger-live-model-repair/results/runtime-attempt-1-success-20260904.txt), [snapshot evidence](../experiments/2026-09-04-mt6797-thermal-snapshot/results/no-workload-runtime-pass.txt), and [measurement limits](../experiments/2026-09-04-mt6797-thermal-snapshot/PASSIVE_DISCRIMINATOR_DECISION.md). |
...
| Wi-Fi | `unknown` | `missing` | `required-nonfree` | An effect-free CONSYS provider bound its passive WLAN client. Authenticated mainline snapshots found CONN off, with bus-protection status clear, common remap disabled and EMI selector set at late init. Shared ownership, an upstream transport, firmware/calibration application and runtime networking remain unestablished. See the [passive binding result](../experiments/2026-09-06-mt6797-consys-passive-boot/results/runtime-20260906.txt) and [shared-handoff result](../experiments/2026-09-25-mt6797-consys-handoff/results/runtime-20260926.json). |
| Bluetooth, GNSS, and FM | `unknown` | `missing` | `required-nonfree` | The CONSYS/WMT firmware boundary is documented, but maintainable upstream transports and runtime acceptance tests are missing. See the [connectivity recovery record](../experiments/2026-07-12-connectivity-wmt-recovery/README.md). |
