...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | Six fresh current-mainline attempts passed pristine single triggers with CPUs `0-9` online and independent CPU8/CPU9 scheduler accounting. The exact patch-`0482` acceptance boot published the standard MT6797 one-package 4+4+2 topology: package siblings `0-9`, cluster lists `0-3`, `4-7`, and `8-9`, cluster-local core IDs, and self-only thread siblings. Its bounded topology/RAM child passed exact affinity, four bidirectional 1.9 MiB volatile-rootfs hashes, 257/258-tick accounting, cleanup, changed-ID recovery, unchanged `boot2`, and both terminal retained proofs. A separate fresh concurrent child preserved those gates while CPU8/CPU9 completed four simultaneous disjoint-file writes and four simultaneous peer-file reads: all 16 hashes matched, accounting advanced 67/68 ticks, cleanup passed, and changed-ID recovery again proved both terminals and unchanged `boot2`. A later exact lifecycle attempt booted CPU8, booted CPU9 before and after one CPU9-only down transaction, returned binder success at stage 18, left CPUs `0-9` online, and was independently corroborated by changed-boot retained record 4 as `restored-success` with error zero. This establishes bounded dual-A72 execution, correct live topology in the topology candidate, peer-visible RAM data, and one complete CPU9 down/restore transaction on the named device—not stable/default support, repeatability, or general stress. Topology-preserving lifecycle repetition, broader hotplug, cpufreq/OPP, thermal, suspend, default integration, and upstream review remain open. See the [topology/RAM pass](../experiments/2026-09-02-mainline-mt6797-cpu-map/results/runtime-attempt-2-topology-ram-pass-20260902.txt), [concurrent-load pass](../experiments/2026-09-02-mainline-dual-a72-concurrent-multiline/results/runtime-attempt-2-concurrent-multiline-pass-20260902.txt), and [lifecycle pass](../experiments/2026-09-02-mainline-a72-hotplug-lifecycle-gate/results/stage-binding-fix-runtime-attempt-1-success-20260903.txt). |
