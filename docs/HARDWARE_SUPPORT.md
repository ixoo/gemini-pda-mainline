...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | One exact patch-`0481` current-mainline attempt passed a pristine single trigger with CPUs `0-9` online, independent CPU8/CPU9 scheduler-tick advances, one request per A72, and no CPU_OFF or retry. Changed-ID Gemian recovery found unchanged `boot2` and CRC-valid terminal retained proofs for both A72 CPUs. This establishes one bounded dual-A72 online result, not stable/default support. Fresh repeatability, sustained load/coherency, safe CPU_OFF and hotplug, cpufreq/OPP, thermal, suspend, default integration, and upstream review remain open. See the [exact dual-A72 result](../experiments/2026-08-31-mainline-a72-cpu9-same-boot-successor/results/completion-lock-repair-runtime-attempt-1-20260902.txt) and [DA921x/A72 boundary](hardware/da921x-i2c6-a72.md). |
