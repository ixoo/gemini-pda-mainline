...
| Thermal management | `unknown` | `local` | `none` | The supported path runtime-binds the root-only read-only calibration provider, and the exact MT6797 consumer now fails closed before side effects; all nine isolated policy KUnit cases pass. The infracfg provider is repaired hardware-free to expose only the source-proven thermal `0x120/0x124` and PMIC-wrap `0x140/0x144` SET/CLEAR pairs; its exact Buildbox build and all six translation/rejection KUnit cases pass, with RST1 quarantined. Runtime enablement still awaits PMIC-wrapper serviceability under the corrected reset, while AUXADC power/global-idle ownership and the APMIXED, six-bank commit, first-valid, IRQ/protection, unwind, and PM contracts remain open. Both thermal/AUXADC DT nodes stay disabled and no current-mainline temperature is claimed. See the [reset repair](../experiments/2026-09-03-mt6797-infracfg-reset-repair/README.md), [calibration gate](../experiments/2026-09-03-mt6797-thermal-fail-closed-calibration/README.md), [transaction audit](../experiments/2026-09-03-mt6797-thermal-auxadc-transaction-audit/README.md), and [thermal recovery record](../experiments/2026-07-13-mt6797-thermal-recovery/README.md). |
