...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | The current exact mainline serviceability kernel binds the read-only A72 platform-state source plus the read-free clock and BigiDVFS backends cumulatively while CPUs 8/9 remain closed and the Stage-27 USB/T-PHY/I2C5/keyboard baseline survives. No platform snapshot, protected-clock read, BigiDVFS SMC, or CPU request occurred. Separately, an experiment-only Gemian-derived kernel repeatably retains both A72 CPUs and runs one normal-priority bound task per CPU through finite scheduler-context work and cleanup. This is not default-profile or production enablement. Physical-reader qualification, safe CPU_OFF and post-isolation rollback, writable-provider ownership, sustained/userspace load, cpufreq/OPP, thermal, suspend, and default integration remain unproved. See the [three-backend bind result](../experiments/2026-08-24-mainline-a72-bigidvfs-backend-stage27-control/results/runtime-attempt-1-serviceable-20260824.txt), [scheduler-context repeatability result](../experiments/2026-08-03-a72-scheduler-context/results/runtime-unpark-attempt-2-repeatability-pass-20260805.txt), and [DA921x/A72 boundary](hardware/da921x-i2c6-a72.md). |
