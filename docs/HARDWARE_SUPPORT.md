...
| Cortex-A72 CPU8–9 | `unknown` | `local` | `unknown` | Both CPUs remain offline by design. Prior active attempts did not establish a safe boot path. No A72 request is allowed until the legacy regulator provider, rail ownership, rollback, resume, SPM/SRAM, clocks, CCI, and PSCI sequence pass their separate gates. See the [DA921x/A72 boundary](hardware/da921x-i2c6-a72.md). |
...
| Legacy DA921x board-control contract | `partial` | `local` | `none` | One gated invocation repeated the fixed `0x68`/`0x69` DA9213/DA9214/DA9215-compatible tuple. This closes the read-only board-contract gate, not unique silicon identity, page semantics, writes, or rail state. See the [runtime record](../experiments/2026-07-28-da9214-gauss/results/runtime-candidate-gauss-attempt-1-20260728.txt). |
| Legacy DA921x regulator provider | `unknown` | `missing` | `none` | The upstream DA9211/A-family probe is incompatible with the observed legacy protocol. A separated, identification-only driver and binding contract now fixes the exact zero-write probe/bind/unbind boundary, but no driver or provider is implemented. See the [design contract](../experiments/2026-07-29-da921x-legacy-driver-contract/README.md) and [ordered roadmap](ROADMAP.md#ordered-gates). |
