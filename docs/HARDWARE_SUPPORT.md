...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | An exact experiment-only Gemian-derived kernel brought CPU8 online through verified BUCKB, isolation, SRAM-LDO, PSCI secondary completion, and DCM. A fail-closed HPS/public-down child retained CPU8 through three validated synchronous callbacks in two exact runs; late callbacks at 12.415481 and 12.265514 seconds executed on CPU8 while CPU9 remained offline, and both watchdog recoveries preserved boot2. Safe CPU_OFF, CPU9, coherency stress, cpufreq/OPP, thermal, and suspend remain unproved. This is not default-profile or upstream support. See the [repeatability result](../experiments/2026-08-03-a72-cpu8-late-hold/results/runtime-attempt-2-repeatability-pass-20260803.txt) and [DA921x/A72 boundary](hardware/da921x-i2c6-a72.md). |
