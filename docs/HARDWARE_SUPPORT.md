...
| Cortex-A72 CPU8–9 | `unknown` | `local` | `unknown` | Both CPUs remain offline in the mainline path. One exact Gemian run now proves a coherent natural CPU8 up/down owner cycle with immutable regulator, SPM, secure, clock, DCM, reset, PSCI, and hotplug evidence; it does not prove a mainline provider, failure rollback, resume, or CPU9. No mainline A72 request is allowed until those remaining gates pass. See the [DA921x/A72 boundary](hardware/da921x-i2c6-a72.md). |
