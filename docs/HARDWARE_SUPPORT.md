...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | Multiple fresh current-mainline attempts passed pristine single triggers with CPUs `0-9` online and independent CPU8/CPU9 scheduler accounting. The standard MT6797 one-package 4+4+2 topology is live-proven: package siblings `0-9`, cluster lists `0-3`, `4-7`, and `8-9`, cluster-local core IDs, and self-only thread siblings. Bounded topology/RAM and concurrent-load children passed exact affinity, peer-visible volatile-RAM hashes, independent accounting, cleanup, changed-ID recovery, unchanged `boot2`, and terminal retained proofs. The complete CPU9-only down/restore transaction now passes on two fresh boots; the topology-preserving repeat returned binder success at stage 18, left CPUs `0-9` online with the exact 4+4+2 views, and was independently corroborated by changed-boot retained record 4 as `restored-success` with error zero. This establishes bounded dual-A72 execution, correct live topology, peer-visible RAM data, and repeatability of one complete CPU9 down/restore transaction on the named device—not stable/default support or general stress. The exact lifecycle-plus-load combination, broader hotplug, cpufreq/OPP, thermal, suspend, default integration, and upstream review remain open. See the [topology/RAM pass](../experiments/2026-09-02-mainline-mt6797-cpu-map/results/runtime-attempt-2-topology-ram-pass-20260902.txt), [concurrent-load pass](../experiments/2026-09-02-mainline-dual-a72-concurrent-multiline/results/runtime-attempt-2-concurrent-multiline-pass-20260902.txt), [first lifecycle pass](../experiments/2026-09-02-mainline-a72-hotplug-lifecycle-gate/results/stage-binding-fix-runtime-attempt-1-success-20260903.txt), and [topology-preserving lifecycle repeat](../experiments/2026-09-02-mainline-a72-hotplug-lifecycle-gate/results/topology-repeat-runtime-attempt-1-success-20260903.txt). |
