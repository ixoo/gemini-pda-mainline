...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | An exact experiment-only Gemian-derived kernel brings both A72 CPUs into Linux online accounting while retaining the prepared cluster. Retained execution is repeatable across two watchdog-bounded cycles. A subsequent bounded child reproducibly completed a 1,024-round barrier-ordered shared-memory exchange between CPUs 8/9 across two exact cycles, always with zero errors and final sequences 1,024/1,024 while preserving the fail-closed HPS CPU9-down veto; both watchdog recoveries returned with CPUs 8/9 offline and boot2 unchanged. Multi-cacheline/load coherency, long-term stability, safe CPU_OFF, cpufreq/OPP, thermal, suspend, and default-profile integration remain unproved. See the [bounded coherency repeatability result](../experiments/2026-08-03-a72-cpu9-bounded-coherency/results/runtime-attempt-2-pass-20260803.txt) and [DA921x/A72 boundary](hardware/da921x-i2c6-a72.md). |
