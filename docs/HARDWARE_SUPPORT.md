...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Boot | Planet LK development path | all | boot chain | Retained Android 8 LK requires `bootopt=64...`, gzip plus an appended DTB, its pre-jump CPU/reserved-memory/SCP DT shapes, and a decompressed payload within the MT6797 50 MiB buffer. The stock Gemian Image uses `text_offset=0x80000` with `kernel_addr=0x40080000`; the current Image uses `text_offset=0` and the header-derived `0x40200000`, satisfying the 2 MiB arm64 placement rule. The one-CPU, storage-inert handoff package and diagnostic wrappers pass exact flags, DT-delta, Android-v0 layout/ID, gzip/FDT, size, checksum, and reproducibility gates. Earlier USB, reboot, and simplefb candidates supplied bounded evidence through external `/init`. Candidate M is the first complete observable cycle: its exact marker survived in `console-ramoops`, proving Linux 7.1.3 and external `/init` execution, live-DT inspection, watchdog registration, and progress through 30 seconds before an automatic watchdog return to Gemian. Gemian reported a watchdog reboot and recovered the mainline console. This establishes the LK handoff and a bounded recovery loop for the exact one-CPU candidate, not final post-LK memory mutations, SMP, storage, UART, USB, or general userspace. | `observed` | `missing` | `required-nonfree` | [Current handoff candidate](../experiments/2026-07-16-lk-handoff-alignment/README.md); [Candidate L](../experiments/2026-07-17-uart-pstore-observability/README.md); [Candidate M runtime](../experiments/2026-07-18-watchdog-registration-diagnostic/results/runtime-candidate-m-attempt-1-20260718.txt); retain the proven watchdog/pstore recovery foundation and test CPU1 as one isolated next gate |
...
| Watchdog | MT6797 TOPRGU at `0x10007000` | all | TOPRGU / SPI137 bark IRQ | Linux 7.1.3 `mtk_wdt` covers the MT6797 register and reset-controller protocol. Candidate M kept Candidate L's exact kernel, omitted only the optional bark IRQ as its hardware hypothesis, and proved that the live DT omission survived LK. Retained `console-ramoops` records a successful `mtk-wdt` probe, `/dev/watchdog0`, identity `mtk-wdt`, timeout 31, unavailable pretimeout, successful open, one handoff ping, and waits through 30 seconds. The owner observed automatic return and Gemian reported `wdt_by_pass_pwk`, `powerup_reason=reboot`, and both PMIC watchdog-reboot flags set. The basic single-stage timeout/reset and cross-version console retention therefore work for this revision. Comparing exact L/M strongly isolates the optional IRQ-bearing path as L's registration blocker, but the request errno, SPI137 polarity/delivery, bark, and pretimeout remain unproven. Patch 0081's independent counterfactual effect is also not isolated. | `partial` | `local` | `none` | [Watchdog recovery](../experiments/2026-07-12-mt6797-watchdog-recovery/README.md); [registration audit](../experiments/2026-07-17-uart-pstore-observability/results/watchdog-registration-audit-20260718.txt); [Candidate M runtime](../experiments/2026-07-18-watchdog-registration-diagnostic/results/runtime-candidate-m-attempt-1-20260718.txt); retain the basic no-IRQ path for early recovery, defer and instrument bark separately, and do not repeat unchanged M |
...
| Display pipeline | MT6797 MMSYS/SMI/IOMMU/CMDQ/MM mutex/OVL/PQ/RDMA/DSI | all | multimedia | Local series adds disabled SMI/IOMMU, GCE, mutex, routes/resets, and a descriptor-ready NT36672E framework; native DRM/DSI/PHY nodes remain disabled or module-only. Candidates F–L established transient loader-retained simplefb/fbcon output. During Candidate M's one bounded run, the console scrolled and remained visible through watchdog progress until automatic reset; retained pstore independently establishes execution through 30 seconds. This is useful loader-simplefb/fbcon observability, not native display support, and does not isolate why visibility improved or establish repeatability. Native DRM, DSI, panel, backlight ownership, and rotation remain unknown. | `unknown` | `local` | `build-only` | [DRM component recovery](../experiments/2026-07-12-mt6797-drm-component-recovery/README.md); [Candidate M runtime](../experiments/2026-07-18-watchdog-registration-diagnostic/results/runtime-candidate-m-attempt-1-20260718.txt); preserve the loader console as an observation path but do not promote native panel/DRM support |
