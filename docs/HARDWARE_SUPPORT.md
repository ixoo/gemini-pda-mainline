...
| Wi-Fi | `unknown` | `missing` | `required-nonfree` | An effect-free CONSYS provider bound its passive WLAN client, and a separate authenticated mainline boot found both CONN power-status bits off at late init. Shared ownership, a maintained upstream transport, firmware/calibration application and runtime networking remain unestablished. See the [passive binding result](../experiments/2026-09-06-mt6797-consys-passive-boot/results/runtime-20260906.txt) and [power-status result](../experiments/2026-09-25-mt6797-consys-status/results/runtime-20260925.json). |
