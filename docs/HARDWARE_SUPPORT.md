...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | The exact mainline reader baseline completes one stable two-sample A72-off platform-state snapshot while Stage-27 serviceability survives. Separately, an experiment-only Gemian-derived kernel repeatably retains both A72 CPUs and runs one normal-priority bound task per CPU through finite scheduler-context work and cleanup. An exact follow-up changed-cycle capture now validates all 43 phases, both PASS terminals, and eight complete target-local register records: CPU8/9 ran on MPIDRs `0x200`/`0x201`, both report MIDR `0x410fd081`, and both full vectors agree with their secondary-startup `cpu_data`. This is target evidence, not mainline READY or production/default enablement. Mainline system/policy evidence and architecture commit, safe CPU_OFF and rollback, sustained/userspace load, cpufreq/OPP, thermal, suspend, and default integration remain unproved. See the [target-register complete pass](../experiments/2026-08-28-a72-pmsg-witness/results/runtime-attempt-1-complete-pass-20260829.txt), [platform snapshot result](../experiments/2026-08-24-mainline-a72-platform-snapshot-first-read/results/runtime-attempt-1-pass-20260825.txt), and [DA921x/A72 boundary](hardware/da921x-i2c6-a72.md). |
