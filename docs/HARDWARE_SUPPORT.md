...
| Thermal management | `unknown` | `local` | `none` | The current supported boot path preserves the exact 412-byte LK devinfo shape and runtime-binds the root-only read-only `mt6797-atag-calibration0` provider without exposing its payload. The exact successful A72 candidate deliberately disables THERMAL plus the thermal/AUXADC DT nodes and has no runtime thermal zones; consumer fail-closed behavior, the AUXADC register/idle contract, valid temperatures, trips, cooling, and throttling remain unproved. See the [power-observability gate](../experiments/2026-09-03-mainline-power-observability-gate/README.md) and [thermal recovery record](../experiments/2026-07-13-mt6797-thermal-recovery/README.md). |
