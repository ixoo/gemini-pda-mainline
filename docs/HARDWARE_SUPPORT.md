...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | An exact experiment-only Gemian-derived kernel brings both A72 CPUs into Linux online accounting while retaining the prepared cluster. Across two exact watchdog-bounded cycles, CPU8 and CPU9 each completed three synchronous callbacks; durable terminals also attributed 91 then 89 HPS CPU9-down requests to the fail-closed public `-EPERM` veto. Both cycles excluded declared faults and recovered with CPUs 8/9 offline and boot2 unchanged. General coherency/load, long-term stability, safe CPU_OFF, cpufreq/OPP, thermal, suspend, and default-profile integration remain unproved. See the [repeatability result](../experiments/2026-08-03-a72-cpu9-terminal-attribution/results/runtime-attempt-2-pass-20260803.txt) and [DA921x/A72 boundary](hardware/da921x-i2c6-a72.md). |
