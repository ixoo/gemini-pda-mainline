...
| Built-in keyboard | `partial` | `local` | `none` | AW9523 plus generic matrix polling provides working keyboard input and the current physical map. A focused check observed plain 1, Shift+Fn+1 → F1, and correct modifier release. Complete function-key, Page Up/Page Down, modifier, rollover, wake, and event coverage remain open. See the [keyboard facts](hardware/keyboard.md). |
