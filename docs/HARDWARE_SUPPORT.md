...
silent. Manual recovery was required and immediate pstore was empty. No
watchdog, UART, retention, or native-display support state is promoted.
...
| Watchdog | MT6797 TOPRGU at `0x10007000` | all | TOPRGU / SPI137 bark IRQ | Linux 7.1.3 `mtk_wdt` already covers the MT6797 register and reset-controller protocol; the Gemini board DTS patch adds the vendor-confirmed pretimeout IRQ without importing the vendor WDK ABI. A timed `RB_AUTOBOOT` attempt reached an off-like state but did not restart; the next Gemian boot reported no PMIC/AED watchdog reset. Gemian sets TOPRGU mode bit 4 to bypass the power key for normal reboot, while mainline preserved it. Candidate L adds a narrow MT6797 auto-restart policy that sets bit 4 when the watchdog starts, restarts, or adopts an already-running firmware watchdog. That change is in the independently reproduced artifact verified on logical `boot2`. Attempt 2 strongly reached tracked line `watchdog0=waiting remaining=5s`, proving `/dev/watchdog0` was absent at that check. The screen then switched off, automatic return was not observed, and manual recovery reported no PMIC/AED watchdog reset or pstore marker. Source inspection rejects changing the falling-edge flag: the inherited MediaTek SYSIRQ programs its polarity inverter and passes rising edge to the parent GIC. Candidate M instead omits only the optional bark IRQ, matching independent basic-watchdog runtime evidence, and records exact binding/probe state. No open, handoff ping, bark, expiry, or auto-restart is yet established. | `unknown` | `local` | `none` | [Watchdog recovery](../experiments/2026-07-12-mt6797-watchdog-recovery/README.md); [restart source audit](../experiments/2026-07-16-timed-reboot-diagnostic/results/restart-path-source-audit-20260716.txt); [Candidate L](../experiments/2026-07-17-uart-pstore-observability/README.md); [registration audit](../experiments/2026-07-17-uart-pstore-observability/results/watchdog-registration-audit-20260718.txt); run only the changed Candidate M discriminator; do not repeat L |
...
| Display pipeline | MT6797 MMSYS/SMI/IOMMU/CMDQ/MM mutex/OVL/PQ/RDMA/DSI | all | multimedia | Local series adds disabled SMI/IOMMU, GCE, mutex, routes/resets, and a descriptor-ready NT36672E framework; native DRM/DSI/PHY nodes remain disabled or module-only. Candidates F–J provide transient/inconclusive loader-simplefb evidence only, not native display support. Candidate K was an initramfs-only exact-J derivative, written/read back but cancelled without runtime. Candidate L has no display-specific delta. Attempt 2 visibly reached its unique tracked initramfs suffix `remaining 5s` before the screen switched off, strongly supporting loader-simplefb/fbcon output through that boundary. The cause of the later display loss and all native DRM/panel behavior remain unknown. | `unknown` | `local` | `build-only` | [DRM component recovery](../experiments/2026-07-12-mt6797-drm-component-recovery/README.md); [Candidate J](../experiments/2026-07-17-clk-ignore-unused-diagnostic/README.md); [cancelled Candidate K](../experiments/2026-07-17-fbcon-newline-boundary-diagnostic/README.md); [Candidate L](../experiments/2026-07-17-uart-pstore-observability/README.md); do not infer native panel/DRM support or repeat unchanged L |
