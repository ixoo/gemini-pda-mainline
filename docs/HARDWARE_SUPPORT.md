...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | An exact experiment-only Gemian-derived kernel brought CPU8 online through verified BUCKB, isolation, SRAM-LDO, PSCI secondary completion, and DCM. A fail-closed HPS/public-down child then retained CPU8 through three validated synchronous callbacks, with the late callback at 12.415481 seconds executing on CPU8 while CPU9 remained offline; watchdog recovery and boot2 integrity passed. Repeatability, safe CPU_OFF, CPU9, coherency stress, cpufreq/OPP, thermal, and suspend remain unproved. This is not default-profile or upstream support. See the [late-hold result](../experiments/2026-08-03-a72-cpu8-late-hold/results/runtime-attempt-1-pass-20260803.txt) and [DA921x/A72 boundary](hardware/da921x-i2c6-a72.md). |
