...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | Exact mainline CPU8 admission completes P27, provider acquisition, isolation, masked stable SRAM validation, P28, and one zero-returning CPU_ON callback. P30E now proves CPU8 reaches `secondary_start_kernel()` through CPU-operations postboot and CPU-info capture. The exact late-target validator then parks CPU8 for a sole MIDR expectation mismatch: generic model `0x410fd080` versus observed r0p1 `0x410fd081`; every other architectural comparison passed. CPU8 remains offline and CPU9 stays vetoed. Separately, an experiment-only Gemian-derived kernel repeatably retains both A72 CPUs and validates bounded scheduler work plus target-local MPIDR/MIDR records. This is not stable/default support. Expected-pair repair, architecture completion, safe CPU_OFF, sustained load, cpufreq/OPP, thermal, suspend, and default integration remain unproved. See the [mainline mismatch result](../experiments/2026-08-31-mainline-a72-post-capabilities-checkpoints/results/runtime-attempt-1-midr-mismatch-20260831.txt), [target-register complete pass](../experiments/2026-08-28-a72-pmsg-witness/results/runtime-attempt-1-complete-pass-20260829.txt), and [DA921x/A72 boundary](hardware/da921x-i2c6-a72.md). |
