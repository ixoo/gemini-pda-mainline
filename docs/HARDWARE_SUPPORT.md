...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | One exact experiment-only Gemian-derived kernel brought CPU8 online through verified BUCKB, isolation, SRAM-LDO, PSCI secondary completion, and DCM; CPU9 stayed absent. About 1.17 seconds later HPS attempted CPU8 down and faulted in a pre-platform cpuhvfs hotplug notifier, so stability, safe offlining, repeatability, CPU9, coherency stress, cpufreq/OPP, thermal, and suspend remain unproved. This is not default-profile or upstream support. See the [one-way result](../experiments/2026-08-02-a72-one-way-cpu8-boundary/results/runtime-attempt-1-cpu8-online-20260802.txt) and [DA921x/A72 boundary](hardware/da921x-i2c6-a72.md). |
