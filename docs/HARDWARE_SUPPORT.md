...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | Five fresh current-mainline attempts passed pristine single triggers with CPUs `0-9` online and independent CPU8/CPU9 scheduler accounting. The exact patch-`0482` acceptance boot additionally published the standard MT6797 one-package 4+4+2 topology: package siblings `0-9`, cluster lists `0-3`, `4-7`, and `8-9`, cluster-local core IDs, and self-only thread siblings. In the same session, CPU8 and CPU9 ran with exact affinity, exchanged a 1.9 MiB volatile-rootfs payload in both directions with all four hashes matching, advanced 257/258 accounting ticks, cleaned up, and used no storage, CPU_OFF, retry, or reboot action; changed-ID recovery verified unchanged `boot2` and both terminal retained proofs. This is bounded dual-A72 execution, correct live topology, and RAM-integrity support on the named device, not stable/default support or general coherency stress. Concurrent load, safe CPU_OFF/hotplug, cpufreq/OPP, thermal, suspend, default integration, and upstream review remain open. See the [topology/RAM pass](../experiments/2026-09-02-mainline-mt6797-cpu-map/results/runtime-attempt-2-topology-ram-pass-20260902.txt). |
