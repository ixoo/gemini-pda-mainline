...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | The exact mainline reader baseline now completes one stable two-sample A72 platform-state snapshot while Stage-27 serviceability survives and CPUs 8/9 remain closed. Its source-defined masks report both A72 CPUs off, external B isolation asserted, CCI request/pending clear, PWRAP reset deasserted, and MP2 DCM bits 6:0 clear; no DA921x-provider, protected-clock, BigiDVFS, owner, or CPU action occurred. Separately, an experiment-only Gemian-derived kernel repeatably retains both A72 CPUs and runs one normal-priority bound task per CPU through finite scheduler-context work and cleanup. This is not default-profile or production enablement. Composed later-reader qualification, safe CPU_OFF and post-isolation rollback, writable-provider ownership, sustained/userspace load, cpufreq/OPP, thermal, suspend, and default integration remain unproved. See the [platform snapshot result](../experiments/2026-08-24-mainline-a72-platform-snapshot-first-read/results/runtime-attempt-1-pass-20260825.txt), [scheduler-context repeatability result](../experiments/2026-08-03-a72-scheduler-context/results/runtime-unpark-attempt-2-repeatability-pass-20260805.txt), and [DA921x/A72 boundary](hardware/da921x-i2c6-a72.md). |
