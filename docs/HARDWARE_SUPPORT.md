...
| Thermal management | `unknown` | `local` | `none` | The supported path runtime-binds the root-only read-only calibration provider, and the exact MT6797 consumer now fails closed before side effects; all nine isolated policy KUnit cases pass. A source-pinned transaction audit nevertheless rejects runtime enablement: the current infracfg reset-bank model is wrong, AUXADC power/global-idle ownership is incomplete, and the APMIXED, six-bank commit, first-valid, IRQ/protection, unwind, and PM contracts remain open. Both thermal/AUXADC DT nodes stay disabled and no current-mainline temperature is claimed. See the [calibration gate](../experiments/2026-09-03-mt6797-thermal-fail-closed-calibration/README.md), [transaction audit](../experiments/2026-09-03-mt6797-thermal-auxadc-transaction-audit/README.md), and [thermal recovery record](../experiments/2026-07-13-mt6797-thermal-recovery/README.md). |
