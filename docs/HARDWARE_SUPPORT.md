...
| Legacy DA921x regulator provider | `unknown` | `missing` | `none` | The upstream DA9211/A-family probe is incompatible with the observed legacy protocol. Required boundaries include a dedicated legacy-family driver or strictly separated variant, zero probe-time writes, and probe/bind/unbind with no consumer. See the [ordered roadmap](ROADMAP.md#ordered-gates). |
