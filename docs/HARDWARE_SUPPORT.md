...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | Three fresh exact patch-`0481` current-mainline attempts passed pristine single triggers with CPUs `0-9` online and independent CPU8/CPU9 scheduler accounting. The third also pinned work to each A72 and passed bidirectional 1.9 MiB volatile-RAM writer/reader checks with every hash matching, cleanup complete, no device-storage access, and changed-ID recovery with unchanged `boot2` and terminal retained proofs. Exact DT inspection separately found no `cpu-map`; `core_siblings_list=0-9` is correct for the single package, while leaf-cluster description was missing. Canonical patch `0482` and its exact serviceability boot2 container now pass offline validation for clusters `0-3`, `4-7`, and `8-9`, but runtime topology acceptance is pending. This is bounded dual-A72 execution and RAM-integrity support on the named device, not stable/default support or general coherency stress. Runtime CPU-map acceptance, concurrent load, safe CPU_OFF/hotplug, cpufreq/OPP, thermal, suspend, default integration, and upstream review remain open. See the [RAM-integrity result](../experiments/2026-09-02-mainline-dual-a72-ram-coherency/results/runtime-attempt-1-ram-integrity-pass-flat-topology-20260902.txt) and [CPU-map follow-up](../experiments/2026-09-02-mainline-mt6797-cpu-map/README.md). |
