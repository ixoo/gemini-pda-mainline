...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | An exact experiment-only Gemian-derived kernel brings both A72 CPUs into Linux online accounting while retaining the prepared cluster. Retained execution and a 1,024-round scalar shared-memory exchange are repeatable across two watchdog-bounded cycles. A changed bounded child then reproducibly completed 64 alternating rounds over a 16 KiB, 256-cacheline working set across two exact cycles: 262,144 exact cross-CPU word checks per cycle, zero errors or mismatches, and identical cross-matching hashes while preserving the fail-closed HPS CPU9-down veto. Both recoveries returned with CPUs 8/9 offline and boot2 unchanged. Parallel/disjoint load, long-term stability, safe CPU_OFF, cpufreq/OPP, thermal, suspend, and default-profile integration remain unproved. See the [multi-cacheline repeatability result](../experiments/2026-08-03-a72-cpu9-multiline-integrity/results/runtime-attempt-2-pass-20260803.txt) and [DA921x/A72 boundary](hardware/da921x-i2c6-a72.md). |
