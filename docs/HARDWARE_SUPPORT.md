...
| Cortex-A72 CPU8–9 | `partial` | `local` | `unknown` | An exact experiment-only Gemian-derived kernel brought CPU8 online through verified BUCKB, isolation, SRAM-LDO, PSCI secondary completion, and DCM. CPU8 then passed three synchronous callbacks in two exact runs. A separate PSCI-only cluster-reuse child brought CPU9 into Linux online accounting and completed two synchronous callback rounds on both CPU8 and CPU9, but its run was rejected after 83 safely vetoed HPS CPU9-down requests and no in-window third terminal. Safe CPU_OFF, retained CPU9 stability, coherency stress, cpufreq/OPP, thermal, and suspend remain unproved. This is not default-profile or upstream support. See the [CPU9 runtime result](../experiments/2026-08-03-a72-cpu9-cluster-reuse/results/runtime-attempt-1-rejected-20260803.txt) and [DA921x/A72 boundary](hardware/da921x-i2c6-a72.md). |
