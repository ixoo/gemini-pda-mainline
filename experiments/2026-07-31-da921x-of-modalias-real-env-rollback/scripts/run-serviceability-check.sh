...
abort()
{
	printf 'real_env_rollback_result=FAIL\nfailure=%s\n' "$1"
	exit 1
}
...
[ "$(/bin/busybox uname -r)" = 7.1.3-gemini-da921x-ofrollback ] ||
...
marker='GEMINI_DA921X_OF_MODALIAS_REAL_ENV_ROLLBACK alias_length=38 entry_length=47 rollback_bytes=48 envp_delta=0 buflen_delta=0 fallback=i2c'
marker_count="$(/bin/busybox dmesg | /bin/busybox grep -Fc "$marker")"
[ "$marker_count" = 1 ] || abort real-env-rollback-marker-count

printf 'kernel=7.1.3-gemini-da921x-ofrollback\n'
printf 'adapter=%s\nclient=%s\n' "$adapter" "${client##*/}"
printf 'client_name=da9214-legacy\nclient_of_node=present\n'
printf 'client_sysfs_modalias=of-real-compatible\n'
printf 'real_of_modalias_generated=exact\nreal_of_modalias_length=38\n'
printf 'real_uevent_entry=exact\nreal_uevent_entry_length=47\n'
printf 'real_uevent_rollback_bytes=48\n'
printf 'real_uevent_envp_delta=0\nreal_uevent_buflen_delta=0\n'
printf 'client_uevent_modalias=i2c-fallback\nclient_driver=unbound\n'
printf 'i2c_activity=zero\npost_creation_serviceability=PASS\n'
printf 'real_env_rollback_result=PASS\n'
