#!/usr/bin/env python3
"""Generate and replay the localized READY-plan expectation repair."""
...
PATCH = "0440-arm64-repair-Gemini-late-CPU-plan-expectations.patch"
SUBJECT = "arm64: repair Gemini late CPU plan expectations"
...
def commit(root: Path, subject: str, body: str, minute: int) -> None:
    ...
    environment.update({
        ...
        "GIT_AUTHOR_DATE": f"2026-08-30T21:{minute:02d}:00Z",
        "GIT_COMMITTER_DATE": f"2026-08-30T21:{minute:02d}:00Z",
    })
...
def changed_text(path: Path, prefix: str) -> str:
    return "\n".join(
...
def validate_patch(path: Path) -> None:
    ...
    if added.count("ARM64_WORKAROUND_845719,") != 1:
        raise SystemExit("early 845719 addition changed")
    if added.count("ARM64_LATE_CPU_SMCCC_NONE") != 2:
        raise SystemExit("NONE policy additions changed")
    if deleted.count("ARM64_MISMATCHED_CACHE_TYPE,") != 2:
        raise SystemExit("cache mismatch deletions changed")
    if deleted.count("ARM64_LATE_CPU_SMCCC_SMC") != 2:
        raise SystemExit("SMC policy deletions changed")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="a72-ready-repair-") as name:
        temporary = Path(name)
        ...
        commit(
            source, "Gemini READY expectation repair post-0439 parent",
        ...
        commit(
            ...
            "effect model, validator return, and all CPU-action behavior.", 1,
        )
