...
/* Inclusive initialized-record intervals; descriptive, never an access grant
 * or proof that memblock reservation/no-map marking succeeded.
 */
 ...
 * A no-reg node is admitted only as a declaration matching its initialized
 * reserved_mem record; no region callback, exclusive resource claim, mapping
 * or hardware act occurs.
 */
