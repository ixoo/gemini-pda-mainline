...
def generate():
    with scratch("patch") as tree:
        ...
        for name, item in SPEC["predecessor"].items():
        ...
        git("commit", "--quiet", "--no-gpg-sign", "-m",
            "wifi: mediatek: describe dynamic reserved memory declaration\n\n"
            "Retain a passive descriptor for a no-reg reserved-memory node only\n"
            "when its initialized record matches the exact DT declaration.\n"
            "Keep static reg validation, references and active-entry refusal.\n\n"
