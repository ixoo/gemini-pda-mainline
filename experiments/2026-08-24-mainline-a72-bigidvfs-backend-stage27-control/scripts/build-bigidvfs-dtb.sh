...
# Add only the read-free BigiDVFS-backend probe node to the exact passed
# Stage-27 plus platform-state plus clock-backend DTB.
set -euo pipefail
export LC_ALL=C
umask 077

readonly SOURCE_SHA256=5f5cd8b8af73cc1ae77887bb5761b8f1cc6b62e7028a6da24d6f9a3d0f22ab4f
readonly OUTPUT_SHA256=d439ed8f4c226eda49f5bf652f16761ba3400bd0b80685bfc8f8da371d6ed9db
readonly OUTPUT_SIZE=27359
readonly OUTPUT_FILE=mt6797-gemini-pda-a72-bigidvfs-backend-stage27-control.dtb
...
[[ "$(sha256sum "$input" | awk '{print $1}')" == "$SOURCE_SHA256" ]] || die 'passed clock-backend DTB changed'

workdir=$(mktemp -d "$output_parent/.a72-bigidvfs-backend-stage27.XXXXXXXX")
...
derive() {
	...
	cp "$source" "$output"
	fdtput -c "$output" /dvfsp-bigidvfs-backend
	fdtput -t s "$output" /dvfsp-bigidvfs-backend compatible \
		mediatek,mt6797-bigidvfs-backend
	fdtput -t s "$output" /dvfsp-bigidvfs-backend method smc
	fdtput -t s "$output" /dvfsp-bigidvfs-backend status okay
}
...
[[ "$(fdtget -t s "$dtb" /dvfsp-bigidvfs-backend compatible)" == \
	mediatek,mt6797-bigidvfs-backend ]] || die 'BigiDVFS compatible changed'
[[ "$(fdtget -t s "$dtb" /dvfsp-bigidvfs-backend method)" == smc ]] || die 'BigiDVFS method changed'
[[ "$(fdtget -t s "$dtb" /dvfsp-bigidvfs-backend status)" == okay ]] || die 'BigiDVFS backend is not enabled'
[[ "$(fdtget -t s "$dtb" /dvfsp-clock-backend@1001a000 status)" == okay ]] || die 'passed clock backend changed'
[[ "$(fdtget -t s "$dtb" /a72-platform-state@10222000 status)" == okay ]] || die 'passed platform state changed'
...
fdtput -r "$workdir/reverted.dtb" /dvfsp-bigidvfs-backend
...
cmp -s "$workdir/source.sorted.dts" "$workdir/reverted.sorted.dts" ||
	die 'removing the BigiDVFS node does not recover the passed semantic tree'

{
	printf 'experiment=2026-08-24-mainline-a72-bigidvfs-backend-stage27-control\n'
...
output="$output_parent/dtb-a72-bigidvfs-backend-stage27-${OUTPUT_SHA256:0:8}"
...
printf 'validation=a72-bigidvfs-backend-stage27-dtb\nartifact=%s\ndtb_sha256=%s\n' "$output" "$OUTPUT_SHA256"
