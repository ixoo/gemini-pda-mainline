...
def main():
    ...
    for name, digest in spec["adapter_files"].items():
        ...
        additions[name] = data
    managed = ROOT / "artifacts" / "wifi-kernel-compile"
    ...
    marker = "mt6797-hif-text-patch-v1\n"
    ...
    with tempfile.TemporaryDirectory(prefix="patch-", dir=managed) as directory:
        ...
        target = tree / "lib/mt6797-hif-compile"
        target.mkdir()
        for name, data in additions.items():
            (target / name).write_bytes(data)
        with (tree / "lib/Makefile").open("ab") as output:
            output.write(b"\nobj-$(CONFIG_MT6797_HIF_COMPILE_TEST) += mt6797-hif-compile/\n")
        with (tree / "lib/Kconfig.debug").open("ab") as output:
            output.write(b'\nsource "lib/mt6797-hif-compile/Kconfig"\n')
        git("add", "lib")
        git("diff", "--cached", "--check")
        git("commit", "--quiet", "--no-gpg-sign", "-m",
            "lib: compile MT6797 HIF protocol with an ordered MMIO boundary\n\n"
            "Compile the original bounded PIO, INIT and ordinary-section code\n"
            "against real Linux types and MMIO accessors. No registration,\n"
            "initcall, mapping acquisition or runtime caller is provided.\n\n"
