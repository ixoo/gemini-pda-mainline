...
- `scripts/record-layout.py`: exact compact stage-tag wire records, lengths,
...
any kernel implementation existed. Both an initial verbose encoding and a
76-byte positional successor later exceeded arm64's single-page identity-map
link bound when implemented. The frozen wire encoding is therefore the exact
34-byte framed token/stage tag `GAEL-20260816-A E#`. The fixed stage-to-zone
mapping supplies slot identity; byte-exact matching plus the candidate and
deployment checksums supplies integrity without duplicating derived CRC text in
the entry code. The classifier accepts all 16 intentional independent-stage
subsets and rejects malformed, duplicate, wrong-stage, and foreign-identity
records.
