...
# Bounded read-only live probe for the exact failure-stage candidate.
set -eu
export LC_ALL=C
BB=/bin/busybox

readonly INSTALLED_FULL_SHA256=8b6bedfd7187369104250af5524a36dd2339493df95588e372d54e360d6aeabb
readonly TAG=GEMINI_A72_PLATFORM_PROVIDER_CLOCK_SNAPSHOT_V1
readonly FAILURE='platform/provider/clock capture failed:'
...
$BB printf 'composed_observer_devices='; $BB find /sys/bus/platform/devices -maxdepth 1 -name '*a72-platform-provider-clock-observer*' 2>/dev/null | $BB wc -l
$BB printf 'composed_observer_bound='; if $BB test -L /sys/bus/platform/devices/a72-platform-provider-clock-observer/driver; then $BB printf '1\n'; else $BB printf '0\n'; fi
$BB printf 'provider_only_observer_devices='; $BB find /sys/bus/platform/devices -maxdepth 1 -name '*a72-platform-provider-snapshot-observer*' 2>/dev/null | $BB wc -l
...
failure_log=$($BB dmesg 2>/dev/null | $BB grep -aF "$FAILURE" || true)
$BB printf 'failure_log_b64='; $BB printf '%s\n' "$failure_log" | $BB base64 | $BB tr -d '\n'; $BB printf '\n'
$BB printf 'failure_log_lines='; $BB printf '%s\n' "$failure_log" | $BB grep -aFc "$FAILURE" || true
$BB printf 'snapshot_failure_lines='; $BB printf '%s\n' "$failure_log" | $BB grep -aFc "$FAILURE" || true
