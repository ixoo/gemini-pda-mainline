...
"""Shared bounded helpers for the offline EMI service-gate proposal."""
...
def verify_predecessors():
    ...
    expected_entries = [entry[0] for entry in SPEC["series_entries"]]
    require(actual_entries == expected_entries, "named series order changed")
    for path, expected in SPEC["series_entries"]:
        actual = digest(git_show(SPEC["frozen_parent"], "patches/" + path))
        require(actual == expected, "series entry identity changed: " + path)
...
def git_environment():
    ...
    env.update(GIT_CONFIG_NOSYSTEM="1", GIT_CONFIG_GLOBAL=os.devnull,
               GIT_AUTHOR_NAME="MT6797 EMI service-gate experiment",
               GIT_AUTHOR_EMAIL="nobody@example.invalid",
               GIT_COMMITTER_NAME="MT6797 EMI service-gate experiment",
...
def scratch(kind):
    ...
    managed = artifacts / "emi-service-gate-compile"
    ...
    marker = "mt6797-emi-service-gate-compile-" + kind + "-v1\n"
