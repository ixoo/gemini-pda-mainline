...
readonly REMOTE_SHA256=7a89dece63f3d04c75fc74d979bff06f1029045cecb055600abda3b67d4cad6c
readonly CLASSIFIER_SHA256=d89f212c8a9b6be69600a4677431da0e4724a89058ce8fd0a94924cd406ba694
readonly COMMAND_MARKER=__GEMINI_THERMAL_DT_REPAIR_SCRIPT__
...
while (($#)); do
	...
	--recovery-boot-id|--installed-full-sha256|--output)
		...
		shift 2 ;;
	*) die "usage: $0 --recovery-boot-id UUID --installed-full-sha256 SHA256 --output artifacts/runtime-captures/thermal-serviceability-dt-repair-attempt-1" ;;
	esac
done
...
command_file=$(mktemp "${TMPDIR:-/tmp}/.gemini-thermal-dt-repair-observer.XXXXXXXX")
...
set -e
chmod 0600 "$frame"
grep -Fq __GEMINI_THERMAL_DT_REPAIR_BEGIN__ "$frame" || die "observation frame did not begin rc=$nc_rc"
grep -Fq __GEMINI_THERMAL_DT_REPAIR_END__ "$frame" || die "observation frame did not complete rc=$nc_rc"
python3 "$classifier" "$frame" --recovery-boot-id "$recovery_boot_id" >"$classification"
grep -Fqx 'classification=mt6797-thermal-serviceability-dt-repair-pass' "$classification" || die 'observation frame rejected'
