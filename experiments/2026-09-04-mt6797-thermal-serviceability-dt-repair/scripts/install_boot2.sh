...
# Install the exact repaired thermal-serviceability candidate and power off.
set -euo pipefail
export LC_ALL=C
...
readonly CANDIDATE_SHA256=ca3c25889b92673aa341fa97fc347c3469bc3b532d81045659a3afa1f563636a
...
while (($#)); do
	...
	*) die "usage: $0 --output artifacts/device-deployments/thermal-serviceability-dt-repair-attempt-1" ;;
	esac
done
...
source="$repo_root/artifacts/mt6797-pwrap-reset-serviceability/candidate-mt6797-pwrap-reset-305230b1"
candidate_dir="$repo_root/artifacts/mt6797-thermal-serviceability-dt-repair/$CANDIDATE_DIR"
...
[[ "$(basename -- "$output")" == thermal-serviceability-dt-repair-attempt-1 ]] || die 'output directory identity changed'
...
remote_candidate=/tmp/gemini-thermal-dt-repair-ca3c2588.img
ssh "${ssh_options[@]}" "$TARGET" 'umask 077; rm -f /tmp/gemini-thermal-dt-repair-ca3c2588.img; cat >/tmp/gemini-thermal-dt-repair-ca3c2588.img' <"$candidate"
remote_script=$(mktemp "${TMPDIR:-/tmp}/.gemini-thermal-dt-repair-install.XXXXXXXX")
cleanup() {
	rm -f -- "${remote_script:-}"
...
set -eu
export LC_ALL=C
candidate=/tmp/gemini-thermal-dt-repair-ca3c2588.img
expected_sha=ca3c25889b92673aa341fa97fc347c3469bc3b532d81045659a3afa1f563636a
...
printf '%s\n' __GEMINI_THERMAL_DT_REPAIR_DEPLOYMENT_BEGIN__
...
printf '%s\n' __GEMINI_THERMAL_DT_REPAIR_DEPLOYMENT_END__
rm -f "$candidate"
sync
poweroff
REMOTE
...
ssh_rc=$?
set -e
grep -Fq __GEMINI_THERMAL_DT_REPAIR_DEPLOYMENT_BEGIN__ "$evidence" || die "deployment did not begin rc=$ssh_rc"
grep -Fq __GEMINI_THERMAL_DT_REPAIR_DEPLOYMENT_END__ "$evidence" || die "deployment did not complete rc=$ssh_rc"
