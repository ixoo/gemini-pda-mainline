...
# Return the exact rejected live candidate to Gemian through its pinned reboot
# wrapper. This is a recovery action after the read-only frame, not a second
# measurement session, and performs no partition access.
set -euo pipefail
export LC_ALL=C
umask 077
...
readonly MAINLINE_BOOT_ID=3e6d06e4-b89f-4db5-b292-c5df56dc6372
readonly RECOVERY_BOOT_ID=3c428441-2c70-4fec-9b4b-fa9716b142d2
...
readonly EVENTS_SHA256=bed806926ec281d04edddf652174bb29d27439c9032ac357585c58ff02d777bf
...
while (($#)); do
	...
	--capture-dir)
		...
		shift 2
		;;
	*) die "usage: $0 --capture-dir artifacts/runtime-captures/thermal-serviceability-dt-repair-attempt-1" ;;
	esac
done
...
[[ "$(basename -- "$capture_dir")" == thermal-serviceability-dt-repair-attempt-1 ]] || die 'capture identity changed'
...
grep -Fqx 'thermal_driver=none' "$frame" || die 'captured thermal failure changed'
grep -Fqx 'thermal_bind_count=0' "$frame" || die 'captured thermal bind count changed'
grep -Fqx 'thermal_zone_count=0' "$frame" || die 'captured zone count changed'
...
command_file=$(mktemp "${TMPDIR:-/tmp}/.gemini-thermal-dt-repair-recovery.XXXXXXXX")
...
/bin/busybox printf '%s\n' __GEMINI_THERMAL_DT_REPAIR_RECOVERY_BEGIN__
...
/bin/busybox printf '%s\n' __GEMINI_THERMAL_DT_REPAIR_RECOVERY_END__
...
nc_rc=$?
set -e
grep -Fq __GEMINI_THERMAL_DT_REPAIR_RECOVERY_BEGIN__ "$output" || die "native recovery did not begin rc=$nc_rc"
grep -Fq __GEMINI_THERMAL_DT_REPAIR_RECOVERY_END__ "$output" || die "native recovery did not complete rc=$nc_rc"
