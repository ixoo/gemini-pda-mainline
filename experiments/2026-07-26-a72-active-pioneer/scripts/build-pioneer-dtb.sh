...
# Deterministically derive Candidate Pioneer's active A72 DTB from the exact
...
validator="$script_dir/validate-pioneer-dtb.py"
[[ -f "$validator" && ! -L "$validator" && -s "$validator" ]] || die 'Pioneer DT validator is missing or unsafe'
...
temporary="$(mktemp "$output_parent/.candidate-pioneer-dtb.XXXXXX")"
...
[[ -f "$output" && ! -L "$output" && ! -e "$temporary" ]] || die 'exclusive Candidate Pioneer DT publication failed'
temporary=; trap - EXIT

printf 'validation=candidate-pioneer-active-a72-dtb-built\n'
printf 'output=%s\nsha256=%s\n' "$output" "$built_sha256"
