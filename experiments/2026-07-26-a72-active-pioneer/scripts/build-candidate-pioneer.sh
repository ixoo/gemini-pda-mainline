...
# Assemble Candidate Pioneer from a validated kernel package and the exact
...
module="$script_dir/candidate_pioneer.py"
value() { PYTHONPATH="$script_dir" python3 -c 'import candidate_pioneer as c,sys; print(getattr(c,sys.argv[1]))' "$1"; }
...
validator="$script_dir/validate-package-pioneer.py"
dtb_builder="$script_dir/build-pioneer-dtb.sh"
...
workdir="$(mktemp -d "$output_parent/.candidate-pioneer.XXXXXX")"
...
for output in "$stage/$BOOT_MEMBER" "$replica/$BOOT_MEMBER"; do
	python3 "$serializer" --kernel "$stage/Image.gz" --ramdisk "$stage/$INITRAMFS_MEMBER" \
		--dtb "$stage/$DTB_MEMBER" --output "$output" --name gemini-pioneer \
...
python3 "$analyzer" --validate-lk --expected-image-gz "$stage/Image.gz" \
	...
	--expected-name gemini-pioneer --expected-cmdline "$boot_cmdline" \
...
[[ "$actual" == "$expected" ]] || die 'Candidate Pioneer output inventory changed'
(cd "$stage" && find . -type f ! -name SHA256SUMS -print0 | sort -z | xargs -0 sha256sum) >"$stage/SHA256SUMS"
(cd "$stage" && sha256sum --check --strict SHA256SUMS >/dev/null) || die 'Candidate Pioneer manifest failed'
chmod 0600 "$stage"/*
