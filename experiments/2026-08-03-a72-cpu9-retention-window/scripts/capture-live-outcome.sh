...
# Source-pin and derive the read-only retention-window USB/netcat terminal
...
derived="$(mktemp "${TMPDIR:-/tmp}/.derived-a72-cpu9-window-collector.XXXXXXXX")"
...
perl -0pe '
	...
	s#a72-cpu9-attempt-#a72-cpu9-window-attempt-#g;
	s#A72_CPU9#A72_CPU9_WINDOW#g;
	s#a72-cpu9-live#a72-cpu9-window-live#g;
	s#gemini-a72-pair-v1#gemini-a72-pair-v2#g;
	s#validation=a72-cpu9-live-outcome-pass#validation=a72-cpu9-window-live-outcome-pass#g;
' "$source_collector" >"$derived"
chmod 0700 "$derived"

for token in \
	'a72-cpu9-window-attempt-N/runtime.txt' \
	'gemini-a72-pair-v2 result=(pass|fault)' \
	'pair_terminal_line=.*gemini-a72-pair-v2 result=(pass|fault) sample=[123]' \
	'__A72_CPU9_WINDOW_LIVE_TERMINAL_CAPTURED__' \
	'validation=a72-cpu9-window-live-outcome-pass'; do
