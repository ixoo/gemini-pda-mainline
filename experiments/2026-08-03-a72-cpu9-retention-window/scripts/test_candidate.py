#!/usr/bin/env python3
"""Independently validate the CPU9 retention-window Android-v0 candidate."""
...
EXPECTED = {
    "assembler_sha256": "f6d36d5eeafe92936fb8c18bddf34eed92f28dd1b602989fb196e83206812885",
    "parent_assembler_sha256": "dbd00ee1f2dfbec6eb8c2d48a8e65a1f2ca888a5e6be400e05620cd04a597358",
    "builder_sha256": "e1d6a477689dd3f8fe6687f52d4aaa7d3d9b0fc44e7328024e942657003e3ff9",
    ...
    "raw_sha256": "140bed8c432d1ec3ee1d223e06bf51750fc6bbed37cb1d0f596c89f557bffe9c",
    "padded_sha256": "6f1e5f45c8f75cdfde5a996f902f499d685566c3bec227efa6cdb56aaeffa115",
}
RAW_NAME = "gemian-a72-cpu9-retention-window.boot.img"
...
def validate_tools() -> None:
    ...
    exempt = {
        ...
        EXPECTED["builder_sha256"],
        EXPECTED["parent_assembler_sha256"],
    }
    for value in EXPECTED.values():
        require(value in exempt or value.encode() in tooling, f"tooling lacks: {value}")
    for token in (
        b"cpu9-retention-window-compile-review-only",
        b"late-CPU8 parent patchset changed",
        b"CPU9 parent patchset changed",
        b"retention-window patchset changed",
        b"window_patchset_sha256=%s",
        b"gemian-a72-cpu9-retention-window-candidate",
    ):
    ...
    for pattern in (
        ...
        require(re.search(pattern, builder) is None, f"device action: {pattern!r}")
...
def validate_candidate(candidate: Path) -> None:
    ...
    require(padded[: len(raw)] == raw, "padded prefix differs")
    ...
    require(end == len(raw), "Android-v0 extent differs from raw size")
    kernel = raw[kernel_offset : kernel_offset + kernel_size]
    ramdisk = raw[ramdisk_offset : ramdisk_offset + ramdisk_size]
    require(kernel_size == 8_447_137, "kernel field size changed")
    ...
    for token in (
        "experiment=2026-08-03-a72-cpu9-retention-window",
        "device_access=none",
...
def main() -> int:
    ...
    print("validation=cpu9-retention-window-candidate")
