...
def inspect(interfaces, routes, destination, host_address):
    ...
    for line in routes.splitlines():
        ...
        if network == host.network and parts[1].startswith('link#') and 'G' not in parts[2] and same:
            direct = True
        if network.prefixlen >= host.network.prefixlen and (not same or 'G' in parts[2]):
            conflicting = True
