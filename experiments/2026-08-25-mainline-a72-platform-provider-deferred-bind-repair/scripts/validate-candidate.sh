...
# Source-pin the preceding independent container validator and retarget its
# exact package, DT, layout, readiness marker, and candidate identities.
set -euo pipefail
export LC_ALL=C
umask 077

readonly SOURCE_SHA256=be302323abdc2dcae33ec082a08c57c539ac54657b6dc8971f4365085484ca29
die() { printf 'error: %s\n' "$*" >&2; exit 2; }
...
source_validator="$repo_root/experiments/2026-08-25-mainline-a72-platform-provider-snapshot-second-read/scripts/validate-candidate.sh"
...
derived=$(mktemp "$script_dir/.derived-validate-a72-platform-provider-ready.XXXXXXXX")
...
replacements = (
    ("exact A72 platform/provider-snapshot candidate", "exact A72 platform/provider-ready candidate", 1),
    ("6_912_000", "6_909_952", 1),
    ("4_833_733", "4_833_160", 1),
    ...
    ('b"gemini-a72prov"', 'b"gemini-a72ready"', 1),
    ("2a936080d28cba12df241eb19a694fa1d559ee53", "db62ca343b1e0a1a0f6ead4a042d826a2554a6f2", 1),
    ('"a72-platform-provider-snapshot-candidate"', '"a72-platform-provider-ready-candidate"', 1),
    ...
    ("validation=a72-platform-provider-snapshot-candidate", "validation=a72-platform-provider-ready-candidate", 1),
    ("control_dtb=passed-platform-tree-with-composed-observer-replacement", "control_dtb=exact-reversible-provider-ready-derivative", 1),
    ("unsafe platform/provider validator derivation", "unsafe provider-ready validator derivation", 1),
    (".derived-validate-a72-platform-provider-nested.XXXXXXXX", ".derived-validate-a72-platform-provider-ready-nested.XXXXXXXX", 1),
)
for old, new, count in replacements:
    actual = text.count(old)
    if actual != count:
        raise SystemExit(
            f"unsafe provider-ready validator wrapper: expected {count}, found {actual}: {old}"
        )
