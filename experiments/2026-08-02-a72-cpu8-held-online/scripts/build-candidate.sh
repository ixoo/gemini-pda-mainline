...
readonly REPOSITORY_COMMIT=118ff3cb3e9a2fbee10a44ada748e46bbe9b5312
...
readonly KERNEL_SHA256=9158af17b17e483ec68257378e5c4bd923b254e7242b5ba338f1324eec5f960b
...
readonly EXPECTED_RAW_SHA256=53046cf314f76f213abafa53a1e79758ff835941d78a47ecc878d0a2e1ad3789
readonly EXPECTED_PADDED_SHA256=936b9256709514938ddf2f3ab13e63bd9c8d37e991fe40a568aa36a8f8818018
...
[[ "$(jq -er '.held_patchset_sha256' "$bundle/provenance/build.json")" == \
  "$HELD_PATCHSET_SHA256" ]] || die 'held-online patchset changed'
[[ "$(jq -er '.purpose' "$bundle/provenance/build.json")" == \
  held-online-compile-review-only ]] || die 'Buildbox purpose changed'
...
workdir="$(mktemp -d "$output_parent/.gemian-a72-held.XXXXXX")"
cleanup() { [[ ! -d "${workdir:-}" ]] || rm -rf -- "$workdir"; }
trap cleanup EXIT
stage="$workdir/stage"
replica="$workdir/replica"
mkdir "$stage" "$replica"
raw_name=gemian-a72-cpu8-held-online.boot.img
...
{
  printf 'experiment=2026-08-02-a72-cpu8-held-online\n'
...
output_name="gemian-a72-cpu8-held-online-${raw_sha256:0:12}"
...
workdir=
trap - EXIT
printf 'validation=gemian-a72-cpu8-held-online-candidate\n'
printf 'artifact=%s\nraw_sha256=%s\npadded_sha256=%s\n' \
