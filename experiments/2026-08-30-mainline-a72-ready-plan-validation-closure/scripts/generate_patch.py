#!/usr/bin/env python3
"""Generate and replay the stale READY plan-validator repair."""
...
PATCH = "0437-arm64-drop-stale-Gemini-late-CPU-plan-blocker.patch"
SUBJECT = "arm64: drop stale Gemini late CPU plan blocker"
...
def commit(root: Path, subject: str, body: str, minute: int) -> None:
    ...
    environment.update({
        ...
        "GIT_AUTHOR_DATE": f"2026-08-30T15:{minute:02d}:00Z",
        "GIT_COMMITTER_DATE": f"2026-08-30T15:{minute:02d}:00Z",
    })
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="a72-ready-plan-closure-") as name:
        temporary = Path(name)
        ...
        run("git", "config", "user.email", "gemini-mainline@example.invalid",
            cwd=source)
        commit(
            source, "Gemini READY plan-validation post-0436 parent",
        ...
        commit(
            ...
            "rejects every nonzero blocker before plan publication.", 1,
        )
