#!/usr/bin/env python3
"""Classify the exact two-record call ledger from a private pstore capture."""
...
def classify_payload(payload: bytes) -> tuple[str, str, int, int]:
    ...
    if before == 0:
        return "neither", "observer-entry-or-ledger-gate-not-established", before, after
    if after == 0:
        return "before-clock-only", "protected-clock-call-did-not-return", before, after
    return "before-and-after-clock", "clock-returned-failure-at-or-after-bigidvfs", before, after
...
def main() -> int:
    ...
    print("claim_scope=two-checkpoint-retained-ledger-only")
