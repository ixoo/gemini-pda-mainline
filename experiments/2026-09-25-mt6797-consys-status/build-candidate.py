...
"""Compose one private CONSYS snapshot boot2 image from the tested A53 RAM image."""
...
NEW_RELEASE = b'7.1.3-gemini-consys-status-snapshot'
BUILD_COMMIT = 'b3d26e15bc7f90049e919591b97ecf06f77fb0d3'
PACKAGE_ID = '67e2a0b3110bab959e2d973de17686ae14bb43e69f22509faab51e4bf7192599'
...
def main():
    ...
    require(build['repository_commit'] == BUILD_COMMIT and
            ...
            build['build_profile'] == 'mt6797-a53-consys-status-snapshot',
    ...
    for name in (b'mt6797_consys_status_init', b'__arm64_sys_pidfd_open',
    ...
    with tempfile.TemporaryDirectory(prefix='.consys-stage-', dir=output.parent) as tmp:
        stage = Path(tmp)
        ...
        subprocess.run([sys.executable, str(BOOT / 'analyze-lk-boot-image.py'),
                       ...
                       check=True, stdout=subprocess.DEVNULL)
        result = {
            'status': 'offline-consys-snapshot-composition-validated',
