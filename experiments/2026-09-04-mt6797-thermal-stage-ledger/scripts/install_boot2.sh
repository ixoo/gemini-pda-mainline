...
# Install the exact validated candidate to live-GPT boot2 from known-good
# Gemian, verify its full-partition readback, and shut the device down.
set -euo pipefail
export LC_ALL=C
umask 077
...
readonly CANDIDATE_SHA256=dcb2b4e8dd8353177f86a81b3019b42becb286baf2ab0b73aafaf49901780486
...
while (($#)); do
	...
	--output)
		...
		shift 2
		;;
	*) die "usage: $0 --output artifacts/device-deployments/thermal-stage-ledger-attempt-1" ;;
	esac
done
[[ -n "$output" ]] || die '--output is required'
for command in awk basename chmod date dirname git grep ifconfig mkdir mktemp rm \
	sha256sum sleep ssh stat; do
...
candidate_dir="$repo_root/artifacts/mt6797-thermal-stage-ledger/$CANDIDATE_DIR"
candidate="$candidate_dir/boot2-padded.img"
readonly script_dir repo_root identity candidate_dir candidate
[[ "$(git -C "$repo_root" remote get-url origin)" == "$EXPECTED_ORIGIN" ]] || die 'origin URL changed'
git -C "$repo_root" merge-base --is-ancestor "$BUILD_COMMIT" origin/main || die 'build commit is not published at origin/main'
[[ "$(git -C "$repo_root" rev-parse HEAD)" == "$(git -C "$repo_root" rev-parse origin/main)" ]] || die 'current experiment revision is not published at origin/main'
...
[[ "$(basename -- "$output")" == thermal-stage-ledger-attempt-1 ]] || die 'output directory identity changed'
...
remote_identity=$(ssh -n "${ssh_options[@]}" "$TARGET" \
...
ssh -n "${ssh_options[@]}" "$TARGET" 'sudo -n true' >/dev/null 2>&1 || die 'Gemian passwordless sudo is unavailable; no device action taken'

ssh "${ssh_options[@]}" "$TARGET" \
	'umask 077; rm -f /tmp/gemini-thermal-ledger-dcb2b4e8.img; cat > /tmp/gemini-thermal-ledger-dcb2b4e8.img' <"$candidate"
remote_script=$(mktemp "${TMPDIR:-/tmp}/.gemini-thermal-ledger-install.XXXXXXXX")
cleanup() {
	rm -f -- "${remote_script:-}"
	ssh -n "${ssh_options[@]}" "$TARGET" \
		'rm -f /tmp/gemini-thermal-ledger-dcb2b4e8.img' >/dev/null 2>&1 || true
}
trap cleanup EXIT HUP INT TERM
cat >"$remote_script" <<'REMOTE'
set -eu
export LC_ALL=C
candidate=/tmp/gemini-thermal-ledger-dcb2b4e8.img
expected_sha=dcb2b4e8dd8353177f86a81b3019b42becb286baf2ab0b73aafaf49901780486
...
while read -r source mountpoint rest; do
	...
	if [ "$mountpoint" = / ]; then root_source=$source; fi
done </proc/mounts
[ "${root_source:-unknown}" != "$target" ] || die 'boot2 is the active root'
...
[ "$boot_id_after" = "$boot_id_before" ] || die 'boot identity changed before shutdown'

printf '%s\n' __GEMINI_THERMAL_LEDGER_DEPLOYMENT_BEGIN__
...
printf '%s\n' __GEMINI_THERMAL_LEDGER_DEPLOYMENT_END__
rm -f "$candidate"
sync
poweroff
REMOTE
...
printf 'experiment=2026-09-04-mt6797-thermal-stage-ledger\n' >"$evidence"
...
ssh_rc=$?
set -e
grep -Fq __GEMINI_THERMAL_LEDGER_DEPLOYMENT_BEGIN__ "$evidence" || die "deployment did not begin rc=$ssh_rc"
grep -Fq __GEMINI_THERMAL_LEDGER_DEPLOYMENT_END__ "$evidence" || die "deployment did not complete rc=$ssh_rc"
...
disconnected=no
for _ in {1..30}; do
	if ! ssh -n "${ssh_options[@]}" "$TARGET" true >/dev/null 2>&1; then
		disconnected=yes
		break
	fi
	sleep 1
