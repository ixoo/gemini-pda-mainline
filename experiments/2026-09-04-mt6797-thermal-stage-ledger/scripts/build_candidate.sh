...
readonly PACKAGE_DTB=dtbs/mediatek/mt6797-gemini-pda-thermal-serviceability.dtb
readonly CANDIDATE_DTB=mt6797-gemini-pda-thermal-serviceability.dtb
readonly CANDIDATE_INITRAMFS=gemini-mt6797-thermal-stage-ledger-initramfs.img
readonly CANDIDATE_BOOT=gemini-mt6797-thermal-stage-ledger.boot.img
...
readonly CANDIDATE_RAW_SHA256=c015bc244916ed9e62ee03d05a58d5e28141f123a19771ffaa3c19278b03cd0e
readonly CANDIDATE_PADDED_SHA256=dcb2b4e8dd8353177f86a81b3019b42becb286baf2ab0b73aafaf49901780486
...
for command in awk basename cmp find install jq mkdir mktemp mv python3 rm \
	sha256sum sort tr wc xargs; do
...
[[ "$(sha256sum "$initramfs_source/SHA256SUMS" | awk '{print $1}')" == "$SOURCE_MANIFEST_SHA256" ]] ||
	die 'initramfs source manifest changed'
[[ "$(sha256sum "$initramfs_source/$SOURCE_INITRAMFS" | awk '{print $1}')" == "$SOURCE_INITRAMFS_SHA256" ]] ||
	die 'initramfs source image changed'
case "$output_parent" in
"$repository"|"$package"|"$initramfs_source") die 'unsafe output parent' ;;
esac

work=$(mktemp -d "$output_parent/.thermal-stage-ledger-candidate.XXXXXXXX")
...
[[ ! -e "$output" && ! -L "$output" ]] || die 'refusing to overwrite final candidate'
{
	...
	printf 'initramfs_source_manifest_sha256=%s\n' "$SOURCE_MANIFEST_SHA256"
	...
	printf 'thermal_zones=1\n'
	printf 'thermal_trips=0\n'
	printf 'cooling_maps=0\n'
	printf 'standalone_auxadc=disabled\n'
	printf 'thermal_ledger_record=5\n'
	printf 'thermal_ledger_physical_base=0x44415000\n'
	printf 'thermal_ledger_size=4096\n'
	printf 'thermal_ledger_attempt_id=0x54484d4c00000001\n'
	printf 'thermal_ledger_predecessor=empty-only\n'
	printf 'thermal_ledger_max_commits=96\n'
...
python3 "$script_dir/validate_candidate.py" --repository "$repository" --package "$package" \
	--initramfs-source "$initramfs_source" --candidate "$stage"
mv "$stage" "$output"
chmod 0600 "$output"/*
cleanup
