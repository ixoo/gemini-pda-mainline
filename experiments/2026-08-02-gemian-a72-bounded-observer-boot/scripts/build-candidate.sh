...
for command in awk basename chmod cmp dd find grep install jq mkdir mktemp mv \
	python3 rm sha256sum sort tail tr truncate wc xargs; do
...
readonly EXPECTED_RAW_SHA256=d3ec1e13123e662076bdbbbde86f118a46cc30f4490f928e216bd783b37e088a
readonly EXPECTED_PADDED_SHA256=33ace2c30a8877be2a4b917135aa994ad718201f98ec36d8506a3b1f1d03a7aa
...
[[ "$(jq -er '.repository_commit' "$bundle/provenance/build.json")" == \
	bc01fc33ec60540b86cd1133697c6fc4a6d1b857 ]] || die 'repository commit changed'
[[ "$(jq -er '.patchset_sha256' "$bundle/provenance/build.json")" == \
	85ac017a2fec821ee930a74d021cb6d2224ea958929c5bbffaf7ec480e19f9c9 ]] ||
	die 'observer patchset changed'
[[ "$(jq -er '.purpose' "$bundle/provenance/build.json")" == \
	compile-review-only ]] || die 'Buildbox purpose changed'
[[ "$(jq -er '.boot_candidate' "$bundle/provenance/build.json")" == false ]] ||
	die 'compile-review bundle incorrectly claims boot-candidate status'

workdir="$(mktemp -d "$output_parent/.gemian-a72-observer.XXXXXX")"
cleanup() { [[ ! -d "${workdir:-}" ]] || rm -rf -- "$workdir"; }
trap cleanup EXIT
stage="$workdir/stage"
replica="$workdir/replica"
mkdir "$stage" "$replica"
raw_name=gemian-a72-bounded-owner-observer.boot.img
...
[[ "$raw_sha256" == "$EXPECTED_RAW_SHA256" ]] || die 'raw candidate identity changed'
[[ "$padded_sha256" == "$EXPECTED_PADDED_SHA256" ]] ||
	die 'padded candidate identity changed'
{
	printf 'experiment=2026-08-02-gemian-a72-bounded-observer-boot\n'
	printf 'repository_commit=bc01fc33ec60540b86cd1133697c6fc4a6d1b857\n'
	printf 'kernel_field_sha256=5864c083a156fcb023e62a5e8dd3fd4c75d68fb119c82492ed4653065ca39a18\n'
	printf 'active_boot_sha256=1fa78de9f8744a6818bcef2f6773737939f84364de982413910d4958d6d21513\n'
	printf 'active_ramdisk_sha256=a1ee05445e9a2bd8fbc1f75d7cda326b9ca7a6d3b644cbb1d5fc0ac167835be4\n'
	printf 'raw_sha256=%s\nraw_size=%s\n' "$raw_sha256" "$raw_size"
...
output_name="gemian-a72-bounded-observer-${raw_sha256:0:12}"
...
workdir=
trap - EXIT
printf 'validation=gemian-a72-bounded-observer-candidate\n'
printf 'artifact=%s\nraw_sha256=%s\npadded_sha256=%s\n' \
