...
# Run exactly once as root through the direct USB shell on a fresh selected
# boot of the exact name-only-client artifact.
set -eu

abort()
{
	printf 'rw_window_result=FAIL\n'
	printf 'failure=%s\n' "$1"
	exit 1
}
...
{
	# shellcheck disable=SC2016 # The single-quoted program is interpreted by awk.
	/bin/busybox awk '$2 == "/sys" { print $4; found = 1 }
...
[ -e "$control" ] || abort new-device-control-absent
require_sysfs_mode ro

printf 'pre_creation_serviceability=PASS\n'
printf 'adapter=%s\nclient_name=da9214-legacy\nclient_address=0x68\n' "$adapter"
printf 'sysfs_initial_mode=ro\n'
...
trap restore_sysfs EXIT HUP INT TERM
/bin/busybox mount -o remount,rw /sys || abort sysfs-remount-rw
sysfs_rw=yes
require_sysfs_mode rw
