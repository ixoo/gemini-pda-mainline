...
readonly EXPECTED_SIZE=16777216

usage() {
	cat <<'EOF'
usage: install-candidate-w-boot2.sh \
  --target USER@HOST \
  --candidate FILE \
  --expected-candidate-sha256 HEX \
  --expected-current-sha256 HEX \
  --backup-dir DIR

Install one already validated Candidate W image to the live GPT-resolved
logical boot2 partition. The helper requires passwordless sudo and the
repository's mode-0600 Gemini SSH identity. It creates a private full backup,
pads the candidate to exactly 16 MiB, writes only boot2, syncs and flushes, and
requires matching full remote and local readbacks.

The helper never accepts a password, never reboots, and never selects boot2.
DIR must be one new direct child of artifacts/device-partitions/.
EOF
}
...
while (($#)); do
	case "$1" in
	--target|--candidate|--expected-candidate-sha256|--expected-current-sha256|--backup-dir)
		...
		--expected-candidate-sha256)
			[[ -z "$expected_candidate_sha256" ]] || \
				die '--expected-candidate-sha256 was provided more than once'
			expected_candidate_sha256=$2
			;;
		--expected-current-sha256)
			[[ -z "$expected_current_sha256" ]] || \
				die '--expected-current-sha256 was provided more than once'
...
[[ -n "$target" && -n "$candidate" && -n "$expected_candidate_sha256" && \
	-n "$expected_current_sha256" && -n "$backup_dir" ]] || {
	usage >&2
	die 'all five explicit arguments are required'
}
...
[[ "$expected_current_sha256" =~ ^[0-9a-f]{64}$ ]] || \
	die 'expected current checksum must be one lowercase SHA-256'
[[ "$expected_candidate_sha256" =~ ^[0-9a-f]{64}$ ]] || \
	die 'expected candidate checksum must be one lowercase SHA-256'
[[ "$candidate" != *$'\n'* && "$backup_dir" != *$'\n'* ]] || \
...
[[ "$candidate_size" =~ ^[0-9]+$ ]] || die 'candidate size is not numeric'
((candidate_size > 0)) || die 'candidate is empty'
((candidate_size <= EXPECTED_SIZE)) || die 'candidate exceeds the 16-MiB boot2 capacity'
candidate_sha256="$(checked_sha256_file "$candidate")"
[[ "$candidate_sha256" =~ ^[0-9a-f]{64}$ ]] || die 'candidate SHA-256 is malformed'
[[ "$candidate_sha256" == "$expected_candidate_sha256" ]] || \
	die 'candidate does not match the explicit expected Candidate W checksum'
...
padded="$backup_dir/candidate-w-padded-boot2.img"
cp "$candidate" "$padded"
if ((candidate_size < EXPECTED_SIZE)); then
	dd if=/dev/zero of="$padded" bs=1 count=1 seek=$((EXPECTED_SIZE - 1)) \
		conv=notrunc 2>/dev/null
fi
chmod 0600 "$padded"
[[ "$(file_size "$padded")" == "$EXPECTED_SIZE" ]] || die 'padded candidate size mismatch'
head -c "$candidate_size" "$padded" | cmp -s "$candidate" - || \
	die 'padded candidate prefix differs from raw candidate'
[[ "$(checked_sha256_file "$candidate")" == "$candidate_sha256" ]] || \
	die 'raw candidate changed while its padded image was created'
tail_size=$((EXPECTED_SIZE - candidate_size))
if ((tail_size > 0)); then
...
[[ "$padded_sha256" =~ ^[0-9a-f]{64}$ ]] || die 'padded SHA-256 is malformed'

target_user=${target%%@*}
remote_home="/home/$target_user"
remote_stage_prefix="$remote_home/.gemini-candidate-w."
...
remote_gate() {
	...
	"${ssh_command[@]}" "$target" \
		"sudo -n -- env GATE_MODE=$mode EXPECTED_BOOT_ID=$initial_boot_id EXPECTED_TARGET=$expected_target EXPECTED_STAGE=$expected_stage EXPECTED_OWNER=$target_user EXPECTED_ROOT=$initial_root EXPECTED_SIZE=$EXPECTED_SIZE EXPECTED_CURRENT_SHA256=$expected_current_sha256 EXPECTED_CANDIDATE_SHA256=$padded_sha256 /bin/bash -s" <<'REMOTE_GATE'
set -euo pipefail
export LC_ALL=C
umask 077
...
validate_stage() {
	...
	stage_prefix="/home/$EXPECTED_OWNER/.gemini-candidate-w."
	...
	[[ "$stage_suffix" =~ ^[A-Za-z0-9]+$ ]] || \
		fail 'remote staging suffix is unsafe'
	...
	[[ "$stage_owner" == "$EXPECTED_OWNER" && "$stage_mode" == 600 && "$stage_size" == "$EXPECTED_SIZE" ]] || \
	...
	[[ "$stage_sha256" == "$EXPECTED_CANDIDATE_SHA256" ]] || fail 'remote staging checksum mismatch'
}
...
valid_root_stage_dir() {
	local prefix=/run/.gemini-candidate-w-root.
	local suffix
...
create_root_stage() {
	local metadata owner mode root_stage_sha256 root_stage_size run_metadata run_owner
	[[ -d /run && ! -L /run && "$(readlink -f /run)" == /run ]] || \
		fail '/run is not a canonical real directory'
	run_metadata="$(stat -c '%U' /run)" || fail 'cannot inspect /run ownership'
	read -r run_owner <<<"$run_metadata" || fail 'cannot parse /run ownership'
	[[ "$run_owner" == root ]] || fail '/run is not root-owned'
	root_stage_dir="$(mktemp -d /run/.gemini-candidate-w-root.XXXXXXXX)" || \
...
[[ "$already_current" == no && "$probe_sha256" == "$expected_current_sha256" ]] || \
	die 'initial gate returned an inconsistent current checksum'

backup_partial="$backup_dir/boot2-before-candidate-w.img.partial"
backup="$backup_dir/boot2-before-candidate-w.img"
...
[[ "$(file_size "$backup_partial")" == "$EXPECTED_SIZE" ]] || \
...
backup_checksum_file="$backup_dir/boot2-before-candidate-w.img.sha256"
...
[[ "$(checked_sha256_file "$padded")" == "$padded_sha256" ]] || \
	die 'padded candidate changed before remote staging'

readback_partial="$backup_dir/boot2-after-candidate-w.img.partial"
readback="$backup_dir/boot2-after-candidate-w.img"
readback_stats="$backup_dir/boot2-after-candidate-w.dd.txt"
dd if=/dev/zero of="$readback_partial" bs=1048576 count=16 conv=fsync
chmod 0600 "$readback_partial"
: >"$readback_stats"
chmod 0600 "$readback_stats"
[[ "$(file_size "$readback_partial")" == "$EXPECTED_SIZE" ]] || \
	die 'cannot reserve exact local capacity for the full post-write readback'
sync

remote_stage="$("${ssh_command[@]}" "$target" \
	"umask 077; stage=\$(mktemp '$remote_home/.gemini-candidate-w.XXXXXXXX') || exit 2; chmod 600 \"\$stage\"; printf '%s\\n' \"\$stage\"")"
remote_stage=${remote_stage//$'\r'/}
valid_remote_stage "$remote_stage" || \
	die 'remote mktemp returned an unsafe staging path'
...
[[ "$write_sha256" == "$padded_sha256" ]] || die 'remote post-flush checksum mismatch'
...
[[ "$readback_stream_bytes" == "$EXPECTED_SIZE" ]] || \
...
readback_checksum_file="$backup_dir/boot2-after-candidate-w.img.sha256"
...
[[ "$post_sha256" == "$padded_sha256" ]] || die 'final target checksum mismatch'
...

{
	...
	printf 'candidate_padded_size=%s\ncandidate_padded_sha256=%s\n' "$EXPECTED_SIZE" "$padded_sha256"
	printf 'expected_previous_sha256=%s\nbackup_sha256=%s\n' \
		"$expected_current_sha256" "$backup_sha256"
