...
# Source foundation: Candidate AF watcher SHA-256
# 259474002c433e449e065dba9f5116dd4fec82a8880d49143715357c7fbd1805.
# Candidate AG keeps the exact single-connection USB boundary and adds the
# required prior full-partition-readback checksum input.
...
MAC/address/route, and invoke Candidate AG's read-only runtime collector once.
...
SHA256 must be the exact checksum from the prior verified padded boot2
readback; this watcher records the caller-supplied value and does not read any
device partition. Visual evidence alone cannot produce a passing result.
EOF
}
...
[[ "$installed_full_sha256" == "$EXPECTED_INSTALLED_FULL_SHA256" ]] || \
	die '--installed-full-sha256 is not the exact validated AG value'
...
for command in awk basename chmod date dirname git grep ifconfig ioreg mkdir mv \
	ping route shasum sleep stat sudo; do
...
[[ -x "$collector" && ! -L "$collector" ]] || die 'AG runtime collector is absent or unsafe'
...
write_status() {
	...
	fi
	{
		...
		printf 'pass_basis=exact-usb-runtime-at-45-plus-5-seconds\n'
	} >"$temporary"
...
# ShellCheck also cannot prove that the signal traps invoke this callback.
# shellcheck disable=SC2317
on_signal() {
	local name=$1
	local code=$2
...
# successful ICMP response. Do not probe port 2323: the AG collector must make
...
if ((collector_rc == 0)); then
	...
	last_detail=exact-ag-runtime-validator-passed
...
finalized=1
trap - EXIT
printf 'error: Candidate AG runtime collection failed; evidence=%s\n' "$output" >&2
exit "$collector_rc"
