...
- [2026-09-02 MT6797 CPU topology map](2026-09-02-mainline-mt6797-cpu-map/README.md)
  ...
  readback, clean shutdown, live 4+4+2 topology, exact dual-A72 affinity,
  bidirectional RAM integrity, accounting, recovery, and unchanged-boot2 gates
  pass.
...
- [2026-08-28 serviceable same-boot CPU8 one-shot](2026-08-28-mainline-a72-admission-serviceable-one-shot/README.md)
  ...
  `-EPROBE_DEFER` before core consumption with zero CPU requests and no retry;
  CPU8 therefore remains untested. A corrected host wire classifier accepts
  the preserved two-line commit marker while all 13 unsafe mutations still
  fail closed. Read-only USB/netcat evidence localizes the dependency chain to
  an omitted ATAG devinfo NVMEM provider: handoff, I2C6, the clock backend, and
  binder are unbound. The owner observes no working visible console
  framebuffer; this is recorded as a display limitation, not boot failure or
  framebuffer support.
...
- [2026-08-26 Gemini retained transition ledger](2026-08-26-mainline-gemini-transition-ledger/README.md)
  ...
  Checkpatch, replay, exact Buildbox compilation, and package validation. The
  sole bounded no-network QEMU suite passes all six cases with zero failures,
  skips, or new ledger warnings. The physical API remains default-off with no
  production caller, device action, or boot candidate.
...
- [2026-08-25 A72 platform/provider/protected-clock third read](2026-08-25-mainline-a72-platform-provider-protected-clock-third-read/README.md)
  ...
  marker is separately pinned. Corrected attempt 7 passes generation, replay,
  strict style, checksum, byte-identity, and the 136-profile invariant with the
  recursive Kconfig edge absent. The focused build retry is next; no QEMU or
  device action has occurred.
...
- [2026-08-24 A72 clock-backend probe on the passed Stage-27 platform DT](2026-08-24-mainline-a72-clock-backend-stage27-control/README.md)
  ...
  or retained-RAM write. The one pre-armed runtime then proved exact live
  serviceability with both platform-state and clock-backend drivers bound,
  Stage-27 USB/T-PHY/I2C5/keyboard state intact, CPUs 0–7 online and 8–9
  closed, and every platform snapshot, protected read, BigiDVFS, publication,
  and CPU request absent. The artifact is retired; the selected next boundary
  is the BigiDVFS backend's minimum read-free probe/resource contract on this
  exact passed DT.
...
- [2026-08-24 mainline A72 physical-source qualification contract](2026-08-24-mainline-a72-physical-source-qualification-contract/README.md)
  ...
  provider option and prove it hardware-free. Only then may a candidate-only
  owner-held observer bracket the first attributable BigiDVFS call with two
  retained records. This audit performed no build, device action, hardware
  operation, or CPU request.
- [2026-08-24 mainline A72 production-input ownership audit](2026-08-24-mainline-a72-production-input-ownership-audit/README.md)
  ...
  source registration; its follow-up also identifies that the DA921x snapshot
  is coupled to the positive writable provider option. The qualified clock
  record contradicts A34's static zero vector in 17 of 18 raw words, while the
  physical BigiDVFS record remains unqualified. No build, boot candidate,
  device action, or CPU request occurred.
...
- [2026-08-20 mainline positive DA921x provider transaction](2026-08-20-mainline-da921x-positive-provider-transaction/README.md)
  ...
  profiles. The exact KUnit profile compiles and links on Buildbox, and its one
  bounded network-free fake-adapter QEMU run passes all six focused cases with
  no failures or skips. This completes only the hardware-free provider proof;
  there is no boot candidate, device access, CPU request, or physical I2C
  action.
