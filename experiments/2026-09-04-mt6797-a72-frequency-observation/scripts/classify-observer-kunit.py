#!/usr/bin/env python3
"""Classify the focused bounded MT6797 A72 frequency-observer KUnit runtime."""
...
PROFILE = "mt6797-a72-frequency-observer-kunit"
SUITE = "mt6797-a72-frequency-observer"
EXPECTED_CASES = (
...
def main() -> None:
    ...
    require(build["repository_commit"] == args.repository_commit, "package commit mismatch")
    ...
    require(not any(line.startswith("not ok ") for line in lines), "KTAP failure")
    observed = []
    for line in lines:
    ...
    require(
        lines.count(f"# {SUITE}: pass:5 fail:0 skip:0 total:5") == 1,
        "suite totals changed",
    )
    require(
        lines.count("# Totals: pass:5 fail:0 skip:0 total:5") == 1,
        "global totals changed",
    )
    require(lines.count(f"ok 1 {SUITE}") == 1, "suite result absent")
    require(
        any(line.startswith("Kernel panic - not syncing: VFS: Unable to mount root fs") for line in lines),
