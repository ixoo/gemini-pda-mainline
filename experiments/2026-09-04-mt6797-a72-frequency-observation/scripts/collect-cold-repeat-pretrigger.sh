...
# Capture the cold-repeat pristine frame over the direct Gemini USB link.
set -euo pipefail
export LC_ALL=C
umask 077
...
readonly VALIDATOR_SHA256=39fba1bc82080068ccfa90b9a7188e7beb78881b97ab3a5d01f700023feb186c
readonly COMMAND_MARKER=__GEMINI_A72_FREQUENCY_PRETRIGGER_SCRIPT__
readonly EXPECTED_OUTPUT=artifacts/runtime-captures/a72-frequency-thermal-cold-repeat-1
...
[[ "$(basename -- "$output")" == a72-frequency-thermal-cold-repeat-1 ]] || die 'output identity changed'
...
python3 "$script_dir/compare-cold-repeat.py" gate --capture "$output"
