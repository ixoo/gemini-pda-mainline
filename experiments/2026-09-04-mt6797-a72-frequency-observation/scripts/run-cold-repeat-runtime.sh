...
readonly EXPECTED_CAPTURE=artifacts/runtime-captures/a72-frequency-thermal-cold-repeat-1
...
python3 "$script_dir/compare-cold-repeat.py" gate --capture "$capture"
...
python3 "$script_dir/compare-cold-repeat.py" compare --capture "$capture" >"$capture/cold-repeat-comparison.txt"
(cd "$capture" && sha256sum cold-repeat-comparison.txt >>SHA256SUMS)
cat "$capture/cold-repeat-comparison.txt"
printf 'retries=0\nnative_reboot_command_sent=no\ncapture=%s\n' "$capture"
