#!/usr/bin/env python3
"""Transfer and invoke exact Voyager once on the accepted Hubble boot."""
...
PRIOR_PHOTON_RESULT = (
    ...
    "post=a1,b2,c3,d4,e5,f6 post_diff_mask=0x00 page_con_access=none"
)
I2C_PRE_STATUS = (
    ...
    "clock_domains=i2c-appm,ap-dma transfer_attempts=10 dma_starts=10 "
    "nonzero_starts=10 irq_count=10 suspend_checks=0 resume_checks=0 "
    "resume_failures=0"
)

PRIVATE_RELATIVE_ROOT = pathlib.Path("artifacts/runtime-captures")
TRANSCRIPT_NAME = "voyager-runtime-transfer.txt"
STDERR_NAME = "voyager-nc-stderr.txt"
INTERFACE = re.compile(r"[A-Za-z0-9]+")
HEX_BYTE = re.compile(r"[0-9a-f]{2}")


REMOTE_TEMPLATE = r"""/bin/busybox sh <<'__VOYAGER_REMOTE__'
set -eu
umask 077

probe_path=/run/voyager-probe
stage_path=/run/.voyager-probe.stage
guard_path=/run/voyager-probe.invoked
...
for path in "$probe_path" "$stage_path" "$guard_path"; do
	[ ! -e "$path" ] && [ ! -L "$path" ] || abort prior-voyager-path
done
[ ! -e "$photon_path" ] && [ ! -L "$photon_path" ] ||
	abort prior-photon-helper
[ ! -e "$photon_stage" ] && [ ! -L "$photon_stage" ] ||
	abort prior-photon-stage
[ -f "$photon_guard" ] && [ ! -L "$photon_guard" ] ||
	abort prior-photon-guard-type
require_equal "$(/bin/busybox stat -c '%s' "$photon_guard")" 0 \
	prior-photon-guard-size
require_equal "$(/bin/busybox stat -c '%a:%u:%g' "$photon_guard")" \
	400:0:0 prior-photon-guard-mode
[ ! -e "$kepler_path" ] && [ ! -L "$kepler_path" ] ||
	abort prior-kepler-helper
[ ! -e "$kepler_stage" ] && [ ! -L "$kepler_stage" ] ||
	abort prior-kepler-stage
[ -f "$kepler_guard" ] && [ ! -L "$kepler_guard" ] ||
	abort prior-kepler-guard-type
require_equal "$(/bin/busybox stat -c '%s' "$kepler_guard")" 0 \
	prior-kepler-guard-size
require_equal "$(/bin/busybox stat -c '%a:%u:%g' "$kepler_guard")" \
	400:0:0 prior-kepler-guard-mode
...
photon_begin=$(
	/bin/busybox dmesg |
		/bin/busybox grep -c 'GEMINI_PHOTON_BEGIN ' || true
)
photon_pre=$(
	/bin/busybox dmesg |
		/bin/busybox grep -c 'GEMINI_PHOTON_PRE ' || true
)
photon_result=$(
	/bin/busybox dmesg |
		/bin/busybox grep -F -c '__PRIOR_PHOTON_RESULT__' || true
)
photon_all=$(
	/bin/busybox dmesg |
		/bin/busybox grep -c 'GEMINI_PHOTON_' || true
)
require_equal "$photon_begin" 1 prior-photon-begin-count
require_equal "$photon_pre" 6 prior-photon-pre-count
require_equal "$photon_result" 1 prior-photon-result-count
require_equal "$photon_all" 8 prior-photon-total-count
for marker in \
	'GEMINI_PHOTON_BEGIN adapter=i2c-1 of=/i2c@1100e000 address=0x69 transactions=6 registers=05,06,47,05,06,47 prefills=a1,b2,c3,d4,e5,f6' \
	'GEMINI_PHOTON_PRE transaction=1 pass=1 register=0x05 prefill=0xa1 address=0x69 messages=2' \
	'GEMINI_PHOTON_PRE transaction=2 pass=1 register=0x06 prefill=0xb2 address=0x69 messages=2' \
	'GEMINI_PHOTON_PRE transaction=3 pass=1 register=0x47 prefill=0xc3 address=0x69 messages=2' \
	'GEMINI_PHOTON_PRE transaction=4 pass=2 register=0x05 prefill=0xd4 address=0x69 messages=2' \
	'GEMINI_PHOTON_PRE transaction=5 pass=2 register=0x06 prefill=0xe5 address=0x69 messages=2' \
	'GEMINI_PHOTON_PRE transaction=6 pass=2 register=0x47 prefill=0xf6 address=0x69 messages=2'; do
	exact_marker_count=$(
		/bin/busybox dmesg |
			/bin/busybox grep -F -c "$marker" || true
	)
	require_equal "$exact_marker_count" 1 prior-photon-exact-marker
done
...
printf 'prior_photon_begin=1\nprior_photon_pre=6\n'
printf 'prior_photon_result=post-all-equal-pre\nprior_photon_total=8\n'
...
printf '__VOYAGER_GATE_END__\n__VOYAGER_GATE_PASS__\n'

/bin/busybox base64 -d >"$stage_path" <<'__VOYAGER_PAYLOAD__'
__PAYLOAD_BASE64__
__VOYAGER_PAYLOAD__
[ -f "$stage_path" ] && [ ! -L "$stage_path" ] || abort stage-type
require_equal "$(/bin/busybox stat -c '%s' "$stage_path")" \
	"__VOYAGER_SIZE__" stage-size
require_equal "$(file_sha256 "$stage_path")" "__VOYAGER_SHA256__" stage-hash
...
require_equal "$(/bin/busybox stat -c '%s' "$probe_path")" \
	"__VOYAGER_SIZE__" probe-size
require_equal "$(file_sha256 "$probe_path")" "__VOYAGER_SHA256__" probe-hash
require_equal "$(/bin/busybox stat -c '%a:%u:%g' "$probe_path")" \
	500:0:0 probe-mode
printf '__VOYAGER_TRANSFER_PASS__ size=%s sha256=%s mode=500:0:0 path=/run/voyager-probe\n' \
	"__VOYAGER_SIZE__" "__VOYAGER_SHA256__"

printf '__VOYAGER_PRE_BEGIN__\n'
...
printf '__VOYAGER_POST_END__\n'
printf '__VOYAGER_COMPLETE__ probe_rc=%s invocation_count=1 helper_removed=yes guard_mode=%s\n' \
...
def read_exact_helper(path: pathlib.Path) -> bytes:
    ...
    data = ck.regular(path, "Voyager helper")
    if len(data) != ck.PROBE_BINARY_SIZE or ck.digest(data) != ck.PROBE_BINARY_SHA256:
        raise ContractError("helper is not the exact Voyager ELF")
    return data


def read_prior_kepler_capture(path: pathlib.Path) -> bytes:
    if not path.is_absolute():
        raise ContractError("prior Kepler capture path must be absolute")
    info = path.lstat()
    if (
        path.is_symlink()
        ...
        raise ContractError("prior Kepler capture is not exact private mode-0600 data")
    data = path.read_bytes()
    if ck.digest(data) != ck.PRIOR_KEPLER_CAPTURE_SHA256:
        raise ContractError("prior Kepler capture identity differs")
    text = data.decode("ascii", "strict")
    required = (
        f"boot_id={ck.ACCEPTED_BOOT_ID}",
        (
            "GEMINI_KEPLER_RESULT class=split-stable-other "
            "completed_pairs=2 completed_calls=4 ioctl_result=1 errno=0 "
            "pre=a5,5a post=05,05 post_diff_mask=0x03 "
        ...
        (
            "transfer_attempts=10 dma_starts=10 nonzero_starts=10 "
            "irq_count=10"
        ),
    )
    for marker in required:
        if text.count(marker) < 1:
            raise ContractError("prior Kepler capture lost required result evidence")
    return data
...
def build_remote_program(helper: bytes) -> bytes:
    ...
    if (
        ...
        raise ContractError("refusing to serialize non-Voyager helper bytes")
    ...
    replacements = {
        ...
        "__PRIOR_PHOTON_RESULT__": PRIOR_PHOTON_RESULT,
        "__I2C_PRE_STATUS__": I2C_PRE_STATUS,
        "__VOYAGER_SHA256__": ck.PROBE_BINARY_SHA256,
        "__VOYAGER_SIZE__": str(ck.PROBE_BINARY_SIZE),
...
def classify_posts(post0: str, post1: str, mask: int) -> str:
    prefills = ("3c", "a6")
    computed = (post0 != prefills[0]) | ((post1 != prefills[1]) << 1)
    if mask != computed:
...
def validate_transcript(data: bytes) -> dict[str, str]:
    ...
    gate = key_values(
        unique_section(text, "__VOYAGER_GATE_BEGIN__", "__VOYAGER_GATE_END__"),
        "Hubble gate",
    )
    expected_gate = {
    ...
    transfer = next(
        line for line in text.splitlines() if line.startswith("__VOYAGER_TRANSFER_PASS__")
    )
    if marker_values(transfer.split(" ", 1)[1]) != {
        ...
        raise ContractError("volatile Voyager helper identity differs")

    pre = key_values(
        unique_section(text, "__VOYAGER_PRE_BEGIN__", "__VOYAGER_PRE_END__"),
        "pre-probe state",
    )
    post = key_values(
        unique_section(text, "__VOYAGER_POST_BEGIN__", "__VOYAGER_POST_END__"),
        "post-probe state",
    )
    ...
    if post.get("boot_id_post") != ck.ACCEPTED_BOOT_ID:
        raise ContractError("boot ID changed around Voyager")
    ...
    stdout = unique_section(
        text, "__VOYAGER_PROBE_STDOUT_BEGIN__", "__VOYAGER_PROBE_STDOUT_END__"
    )
    if len(stdout) != 6:
        raise ContractError("Voyager successful stdout must contain exactly six lines")
    wanted_line_order = (
        "GEMINI_VOYAGER_BEGIN ",
        "GEMINI_VOYAGER_TX pair=1 ",
        "GEMINI_VOYAGER_RX pair=1 ",
        "GEMINI_VOYAGER_TX pair=2 ",
        "GEMINI_VOYAGER_RX pair=2 ",
        "GEMINI_VOYAGER_RESULT ",
    )
    if tuple(
        next((prefix for prefix in wanted_line_order if line.startswith(prefix)), "")
        for line in stdout
    ) != wanted_line_order:
        raise ContractError("Voyager helper line order differs")
    begin = [line for line in stdout if line.startswith("GEMINI_VOYAGER_BEGIN ")]
    tx = [line for line in stdout if line.startswith("GEMINI_VOYAGER_TX ")]
    rx = [line for line in stdout if line.startswith("GEMINI_VOYAGER_RX ")]
    results = [line for line in stdout if line.startswith("GEMINI_VOYAGER_RESULT ")]
    if len(begin) != 1 or len(tx) != 2 or len(rx) != 2 or len(results) != 1:
        raise ContractError("Voyager line-type inventory differs")
    if marker_values(begin[0].split(" ", 1)[1]) != {
        "adapter": "i2c-1",
        "of": "/i2c@1100e000",
        "address": "0x69",
        "registers": "06,47",
        ...
        raise ContractError("Voyager BEGIN contract differs")
    for index, line in enumerate(tx):
        values = marker_values(line.split(" ", 1)[1])
        if values != {
            "pair": str(index + 1),
            "call": str(index * 2 + 1),
            "address": "0x69",
            ...
            raise ContractError("Voyager TX contract differs")
    posts: list[str] = []
    for index, line in enumerate(rx):
        values = marker_values(line.split(" ", 1)[1])
        prefill = ("3c", "a6")[index]
        post_byte = values.get("post", "").removeprefix("0x")
        if HEX_BYTE.fullmatch(post_byte) is None:
            raise ContractError("Voyager RX post byte is malformed")
        ...
        if values != {
            ...
            "result": "1",
            "errno": "0",
            "post_differs_pre": differs,
        }:
            raise ContractError("Voyager RX contract differs")
        posts.append(post_byte)

    result = marker_values(results[0].split(" ", 1)[1])
    if not re.fullmatch(r"0x0[0-3]", result.get("post_diff_mask", "")):
        raise ContractError("Voyager result mask is malformed")
    mask = int(result["post_diff_mask"], 16)
    ...
    if result != {
        ...
        "post_diff_mask": f"0x{mask:02x}",
        "stop_between_pointer_and_read": "yes",
        "page_con_access": "none",
    }:
        raise ContractError("Voyager aggregate RESULT contract differs")

    complete = next(
        line for line in text.splitlines() if line.startswith("__VOYAGER_COMPLETE__")
    )
    complete_values = marker_values(complete.split(" ", 1)[1])
    wanted_rc = "0" if wanted_class == "split-expected-live" else "2"
    if complete_values != {
        "probe_rc": wanted_rc,
        "invocation_count": "1",
        "helper_removed": "yes",
        ...
        raise ContractError("Voyager one-shot completion contract differs")
    return {
        "boot_id": ck.ACCEPTED_BOOT_ID,
...
def main() -> int:
    ...
    parser.add_argument(
        "--prior-kepler-capture", required=True, type=pathlib.Path
    )
    parser.add_argument("--output-dir", required=True, type=pathlib.Path)
    args = parser.parse_args()
    transcript_path: pathlib.Path | None = None
    try:
        read_prior_kepler_capture(args.prior_kepler_capture)
        ...
        return 2
    print("validation=voyager-exact-hubble-volatile-split-read-one-shot")
