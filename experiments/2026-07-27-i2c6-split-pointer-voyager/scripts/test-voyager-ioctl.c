...
/* Offline contract harness for Voyager's four I2C_RDWR calls. */
...
static int harness_ioctl(int descriptor, unsigned long request, ...)
{
	...
	if (!expected_read &&
	    data->msgs[0].buf[0] != (pair == 0U ? 0x06U : 0x47U))
		abort();
	if (expected_read) {
...
#define main voyager_unused_main
#include "../initramfs/voyager-probe.c"
#undef main
#undef ioctl
...
int main(void)
{
	...
	printf("validation=voyager-offline-ioctl-contract\n");
	printf("successful_path_ioctl_calls=4\n");
	printf("each_ioctl_nmsgs=1\n");
	printf("order=tx06,rx-3c,tx47,rx-a6\n");
	return 0;
}
