 ...
 * independent I2C_RDWR calls: pointer 0x06/read, then pointer 0x47/read.
 * The STOP between each pointer and read is an explicitly supported DA921x
 * register-read form. No page, regulator, CPU, storage, watchdog, reboot, or other
...
#define VOYAGER_I2C_CLASS "/sys/class/i2c-dev"
...
#define VOYAGER_PATH_SIZE 512U

struct voyager_i2c_msg {
	uint16_t addr;
	uint16_t flags;
	uint16_t len;
	uint8_t *buf;
...
struct voyager_i2c_rdwr_ioctl_data {
	struct voyager_i2c_msg *msgs;
	uint32_t nmsgs;
};

struct voyager_observation {
	uint8_t post[VOYAGER_PAIR_COUNT];
...
static const uint8_t voyager_prefills[VOYAGER_PAIR_COUNT] = {
	0x3cU, 0xa6U
};

static const uint8_t voyager_registers[VOYAGER_PAIR_COUNT] = {
	0x06U, 0x47U
};

static const uint8_t voyager_expected[VOYAGER_PAIR_COUNT] = {
	0xd0U, 0xc0U
};
		     ...
		     char *device_path, size_t device_path_size)
{
	...
	directory = opendir(VOYAGER_I2C_CLASS);
	if (directory == NULL)
		return -1;

	for (;;) {
		char link_path[VOYAGER_PATH_SIZE];
		char target[VOYAGER_PATH_SIZE];
		...
		length = snprintf(link_path, sizeof(link_path),
				  VOYAGER_I2C_CLASS "/%s/device/of_node",
				  entry->d_name);
		...
		if (!has_exact_suffix(target, VOYAGER_I2C_OF_SUFFIX))
			continue;
...
{
	struct voyager_i2c_msg message = {
		.addr = VOYAGER_I2C_ADDR,
		.flags = flags,
		.len = 1U,
		.buf = byte,
	};
	struct voyager_i2c_rdwr_ioctl_data request = {
		.msgs = &message,
		.nmsgs = 1U,
	};

	return ioctl(descriptor, VOYAGER_I2C_RDWR, &request);
}

static const char *classify_complete(const struct voyager_observation *observation)
{
	...
	if (memcmp(observation->post, voyager_expected,
		   sizeof(voyager_expected)) == 0)
		return "split-expected-live";
	if (memcmp(observation->post, voyager_registers,
		   sizeof(voyager_registers)) == 0)
...
static int run_split_pairs(int descriptor, struct voyager_observation *observation)
{
	unsigned int pair;
	...
	for (pair = 0U; pair < VOYAGER_PAIR_COUNT; pair++) {
		uint8_t pointer = voyager_registers[pair];
		uint8_t value = voyager_prefills[pair];

		errno = 0;
		observation->ioctl_result =
			one_message_ioctl(descriptor, 0U, &pointer);
		observation->saved_errno = errno;
		printf("GEMINI_VOYAGER_TX pair=%u call=%u address=0x69 "
		       "flags=0x0000 len=1 pointer=0x%02x result=%d errno=%d\n",
		       pair + 1U, pair * 2U + 1U, pointer,
		       observation->ioctl_result, observation->saved_errno);
		(void)fflush(stdout);
	...
	if (observation->completed_pairs == VOYAGER_PAIR_COUNT)
		observation->result_class = classify_complete(observation);
	return strcmp(observation->result_class, "split-expected-live") == 0 ? 0 : 2;
}

static void emit_result(const struct voyager_observation *observation)
{
	printf("GEMINI_VOYAGER_RESULT class=%s completed_pairs=%u "
	       "completed_calls=%u ioctl_result=%d errno=%d pre=3c,a6 "
...
int main(int argc, char **argv)
{
	...
	char device_path[VOYAGER_PATH_SIZE];
	struct voyager_observation observation;
	int descriptor;
	int result;
	...
	printf("GEMINI_VOYAGER_BEGIN adapter=%s of=/i2c@1100e000 "
