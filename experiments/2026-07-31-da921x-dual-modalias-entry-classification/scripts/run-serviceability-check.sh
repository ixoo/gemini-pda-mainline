...
# Read-only acceptance and classification check for one fresh selected boot.
set -eu

abort()
{
	printf 'dual_modalias_entry_classification_result=FAIL\nfailure=%s\n' "$1"
	exit 1
}
...
[ "$(/bin/busybox uname -r)" = 7.1.3-gemini-da921x-entryclass ] ||
...
[ "$state" = pending ] || abort validation-state
[ "$stage" = 2 ] || abort validation-stage
...
present_mask="$(counter "$classification" present_mask)" ||
	abort present-mask-absent
duplicate_mask="$(counter "$classification" duplicate_mask)" ||
	abort duplicate-mask-absent
ordered_prefix="$(counter "$classification" ordered_prefix)" ||
	abort ordered-prefix-absent
seqnum_count="$(counter "$classification" seqnum_count)" ||
	abort seqnum-count-absent
seqnum_index="$(counter "$classification" seqnum_index)" ||
	abort seqnum-index-absent
unexpected_count="$(counter "$classification" unexpected_count)" ||
	abort unexpected-count-absent
printf '%s\n' "$present_mask" | /bin/busybox grep -Eq '^0x[0-9a-f]+$' ||
	abort malformed-present-mask
printf '%s\n' "$duplicate_mask" | /bin/busybox grep -Eq '^0x[0-9a-f]+$' ||
	abort malformed-duplicate-mask
require_uint "$ordered_prefix" ordered-prefix
require_uint "$seqnum_count" seqnum-count
require_uint "$unexpected_count" unexpected-count
case "$seqnum_index" in
-1) ;;
*) require_uint "$seqnum_index" seqnum-index ;;
esac
present_value=$((present_mask))
duplicate_value=$((duplicate_mask))
[ "$present_value" -le 511 ] || abort present-mask-range
[ "$duplicate_value" -le 511 ] || abort duplicate-mask-range
[ $((duplicate_value & ~present_value)) -eq 0 ] || abort duplicate-mask-subset
[ "$ordered_prefix" -le 9 ] || abort ordered-prefix-range
[ "$seqnum_count" -le 9 ] || abort seqnum-count-range
[ "$unexpected_count" -le 9 ] || abort unexpected-count-range
if [ "$seqnum_count" -eq 0 ]; then
	[ "$seqnum_index" = -1 ] || abort absent-seqnum-index
else
	if [ "$seqnum_index" -lt 0 ] || [ "$seqnum_index" -gt 8 ]; then
		abort seqnum-index-range
	fi
fi
if [ "$duplicate_value" -eq 0 ]; then
	unique_expected="$(popcount "$present_value")"
	[ $((unique_expected + seqnum_count + unexpected_count)) -eq 9 ] ||
		abort classification-total
fi
...
printf 'kernel=7.1.3-gemini-da921x-entryclass\n'
printf 'validation_state=%s\nvalidation_stage=%s\n' "$state" "$stage"
printf 'envp_idx=9\nenvp_capacity=64\nterminator_null=1\n'
printf 'buflen=245\nbuf_capacity=2048\n'
printf 'present_mask=%s\nduplicate_mask=%s\n' \
	"$present_mask" "$duplicate_mask"
printf 'ordered_prefix=%s\nseqnum_count=%s\nseqnum_index=%s\n' \
	"$ordered_prefix" "$seqnum_count" "$seqnum_index"
...
printf 'dual_modalias_entry_classification_result=PASS\n'
