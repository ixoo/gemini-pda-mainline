...
validated package was fetched; see the [Buildbox resume receipt](results/buildbox-resume-0d990f7-20260810.txt).
...
or written, and CPU8/CPU9 admission remains closed. The next gate is binding the
pre-lock wrapper at attributable vendor writer sites, followed by separate
...
receipt](results/buildbox-resume-87923a0-20260810.txt). This remains a
source-independent compile contract: the pinned vendor sources are not copied,
no external vendor caller is wired, no setter or provider/runtime registration
...
validated package was fetched; see the [current-head Buildbox receipt](results/buildbox-resume-b2425cd-20260810.txt).
...
Candidate [0002-mt6797-voltage-observer-outer-boundary.patch](patches/0002-mt6797-voltage-observer-outer-boundary.patch)
...
`mt_cpufreq.o` all passed; see the [outer-boundary Buildbox review](results/vendor-voltage-observer-boundary-buildbox-20260810.txt).
...
pushed commit `7926d1ef60d646ad438479429c2a354880866763`. The `dvfsp-owner-kunit`
Buildbox run applied all 257 canonical series entries, compiled the coordinator
and owner-KUnit objects, produced 119 DTBs, passed package/provenance checksums,
and fetched only the validated package; see the [runtime-event forward
Buildbox receipt](results/vendor-writer-runtime-event-forward-buildbox-20260811.txt).
The forwarder validates the coordinator lifetime and event tuple, invalidates
the shared source before forwarding, and refuses calls after stop. This remains
compile-only: Buildbox did not execute KUnit, no actual vendor probe/remove
caller is bound, no runtime owner/provider/setter or hardware action occurred,
