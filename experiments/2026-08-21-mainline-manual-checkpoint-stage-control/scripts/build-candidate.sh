...
# live-stage discriminator package, configuration, markers, and container.
set -euo pipefail
export LC_ALL=C
umask 077
...
derived="$(mktemp "$script_dir/.manual-checkpoint-stage-builder.XXXXXXXX")"
...
replacements = (
    ("manual-checkpoint control identities and closures",
     "manual-checkpoint live-stage identities and closures", 1),
    (".manual-checkpoint-builder.XXXXXXXX", ".manual-checkpoint-stage-builder-inner.XXXXXXXX", 1),
    ...
    ("readonly BOOT_NAME=gemini-chkctl", "readonly BOOT_NAME=gemini-chkstage", 1),
    ("readonly BOOT_FILE=gemini-mt6797-manual-checkpoint-control.boot.img",
     "readonly BOOT_FILE=gemini-mt6797-manual-checkpoint-stage-control.boot.img", 1),
    ("'manual-checkpoint serviceability DTB'", "'manual-checkpoint stage serviceability DTB'", 1),
    (".manual-checkpoint-control.XXXXXXXX", ".manual-checkpoint-stage-control.XXXXXXXX", 1),
    ("portable-fetched-manual-checkpoint-control-package",
     "portable-fetched-manual-checkpoint-stage-control-package", 1),
    ...
    ("runtime_hypothesis=shared-writer-completes-two-local-readbacks-on-serviceable-base",
     "runtime_hypothesis=one-live-stage-localizes-the-first-shared-writer-refusal", 1),
    ("kernel_delta_from-last-runtime-proven=manual-writer-and-late-initcall-only",
     "kernel_delta_from-last-runtime-proven=default-off-live-stage-instrumentation-only", 1),
    ("candidate-manual-checkpoint-control-${RAW_SHA256:0:8}",
     "candidate-manual-checkpoint-stage-control-${RAW_SHA256:0:8}", 1),
    ("validation=manual-checkpoint-control-candidate-build",
     "validation=manual-checkpoint-stage-control-candidate-build", 1),
    ("'CONFIG_PSTORE_GEMINI_PROTECTED_READBACK_MANUAL_CONTROL=y' \\",
     "'CONFIG_PSTORE_GEMINI_PROTECTED_READBACK_MANUAL_CONTROL=y' \\\n\t'CONFIG_PSTORE_GEMINI_PROTECTED_READBACK_MANUAL_STAGE_CONTROL=y' \\", 1),
    ('CONFIG_LOCALVERSION="-gemini-checkpoint-ctl"',
     'CONFIG_LOCALVERSION="-gemini-checkpoint-stage"', 1),
    ("'GEMINI_MANUAL_CHECKPOINT_CONTROL_LIVE_V1'; do",
     "'GEMINI_MANUAL_CHECKPOINT_CONTROL_LIVE_V1' \\\n\t'GEMINI_MANUAL_CHECKPOINT_STAGE_V1'; do", 2),
...
for old, new, count in replacements:
    ...
    if actual != count:
        raise SystemExit(
            f"unsafe stage candidate derivation: expected {count}, found {actual}: {old}"
        )
