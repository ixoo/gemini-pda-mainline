...
# Read the exact serviceability and one-record raw-write result over the
# initramfs netcat shell. This performs no device or power action.
set -eu
export LC_ALL=C
BB=/bin/busybox

readonly INSTALLED_FULL_SHA256=c10f2c03490fe1aa8ded11895a2d1817dd649edaffa307d0635fe2d69ce1c631
readonly CONTROL_PREFIX=GEMINI_MANUAL_CHECKPOINT_CONTROL_LIVE_V1
readonly STAGE_PREFIX=GEMINI_MANUAL_CHECKPOINT_STAGE_V1
readonly RAW_PREFIX=GEMINI_MANUAL_RAW_WRITE_QUALIFICATION_LIVE_V1
...
readonly RAW_EXACT='GEMINI_MANUAL_RAW_WRITE_QUALIFICATION_LIVE_V1 commit=1 stage=success writes=1 protected=0 clock=0 bigidvfs=0 cpu=0'

control_prefix_count="$($BB dmesg | $BB grep -Fc "$CONTROL_PREFIX" || true)"
stage_prefix_count="$($BB dmesg | $BB grep -Fc "$STAGE_PREFIX" || true)"
raw_prefix_count="$($BB dmesg | $BB grep -Fc "$RAW_PREFIX" || true)"
control_exact_count="$($BB dmesg | $BB grep -Fc "$CONTROL_EXACT" || true)"
stage_exact_count="$($BB dmesg | $BB grep -Fc "$STAGE_EXACT" || true)"
raw_exact_count="$($BB dmesg | $BB grep -Fc "$RAW_EXACT" || true)"

$BB printf '%s\n' __MANUAL_CHECKPOINT_RAW_WRITE_RUNTIME_BEGIN__
...
$BB printf '%s\n' reboot_request=none
$BB printf '%s\n' __MANUAL_CHECKPOINT_RAW_WRITE_RUNTIME_END__
