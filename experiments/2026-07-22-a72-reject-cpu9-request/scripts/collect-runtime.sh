...
# Candidate AK's initramfs intentionally retains Candidate AC's inherited USB
# banner. This collector combines the caller-attested exact boot2 readback with
# read-only live kernel/configuration/DT evidence. It never reads a device
# partition and never requests a CPU state transition.
...
readonly VALIDATOR_SHA256=a92e64259e60f05ce5a8b96d2582307fd3d4167eafe9e60295a9bdb8491bdf83
readonly CANDIDATE_AK_SHA256=c52e133767f305045664b2274883e8f145170ee4fd8ae34418b7a14ed42360a0
readonly AI_COLLECTOR_SHA256=5fe46ea345e8ec94ea2253c26e7f359f0ba46fd1e792598086c891806b3617bf
...
for command in awk dirname python3 shasum; do
...
ai_collector="$repo_root/experiments/2026-07-22-a72-reject-gate-kernel-split/scripts/collect-runtime.sh"
[[ -f "$validator" && ! -L "$validator" ]] || die 'AK runtime validator is absent or unsafe'
[[ "$(shasum -a 256 "$validator" | awk '{ print $1 }')" == "$VALIDATOR_SHA256" ]] || \
	die 'AK runtime validator source identity changed'
[[ -f "$candidate_identity" && ! -L "$candidate_identity" ]] || \
	die 'Candidate AK identity module is absent or unsafe'
[[ "$(shasum -a 256 "$candidate_identity" | awk '{ print $1 }')" == \
	"$CANDIDATE_AK_SHA256" ]] || die 'Candidate AK identity module source changed'
[[ -f "$ai_collector" && ! -L "$ai_collector" ]] || \
	die 'source Candidate AI runtime collector is absent or unsafe'
[[ "$(shasum -a 256 "$ai_collector" | awk '{ print $1 }')" == \
	"$AI_COLLECTOR_SHA256" ]] || die 'source Candidate AI runtime collector changed'

# Refuse before inspecting or creating the caller's capture path unless every
# package/artifact identity is selected. An uncalibrated scaffold stops here
# while candidate_ak.py retains any TO_PIN value.
...
spec = importlib.util.spec_from_file_location("candidate_ak_collector_pins", path)
...
[[ "$installed_full_sha256" == "$pinned_full_sha256" ]] || \
	die 'installed full-partition checksum is not Candidate AK'

for command in basename cat chmod git grep ifconfig mktemp nc ping rm route stat; do
	command -v "$command" >/dev/null 2>&1 || die "required command missing: $command"
done
...
[[ -d "$private_root" && ! -L "$private_root" ]] || \
	die 'private runtime-capture root is absent or unsafe; use the guarded cycle collector'
...
command_file="$(mktemp /tmp/candidate-ak-runtime-command.XXXXXX)"
...
printf '__AK_IDENTITY_END__\n'
printf '__AK_STAT1_BEGIN__\n'; /bin/busybox grep '^cpu[0-9]' /proc/stat; printf '__AK_STAT1_END__\n'
/bin/busybox sleep 5
printf '__AK_STAT2_BEGIN__\n'; /bin/busybox grep '^cpu[0-9]' /proc/stat; printf '__AK_STAT2_END__\n'
printf '__AK_STABILITY_BEGIN__\n'
...
printf '__AK_STABILITY_END__\n'
printf '__AK_DMESG_BEGIN__\n'; /bin/busybox dmesg; printf '__AK_DMESG_END__\n'
exit
EOF

{
	printf '__AK_HOST_BEGIN__\n'
