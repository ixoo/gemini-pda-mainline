...
# Invoke AK's inherited /bin/reboot exactly once, but only when an exact,
# independently validated AK runtime capture names the boot ID still served
# over the unit-bound USB interface.
...
# These pins select the reproduced AK artifact and final runtime/native
# validators. Their syntax gate runs before any host or device probe.
readonly INSTALLED_FULL_SHA256=66902cb2f2faa5c4c6457ce89ff67aa25e5345ac43ee0ca8062a55e0fbac870e
readonly CANDIDATE_AK_SHA256=c52e133767f305045664b2274883e8f145170ee4fd8ae34418b7a14ed42360a0
readonly RUNTIME_VALIDATOR_SHA256=a92e64259e60f05ce5a8b96d2582307fd3d4167eafe9e60295a9bdb8491bdf83
...
for pin in "$INSTALLED_FULL_SHA256" "$CANDIDATE_AK_SHA256" \
	"$RUNTIME_VALIDATOR_SHA256" "$NATIVE_VALIDATOR_SHA256"; do
	is_sha256 "$pin" || die 'Candidate AK native-reboot production pins remain unresolved'
done
is_sha256 "$installed_full_sha256" || die 'installed checksum must be one lowercase SHA-256 value'
[[ "$installed_full_sha256" == "$INSTALLED_FULL_SHA256" ]] || die 'installed checksum is not Candidate AK'
...
# probe. No testing override exists in the production requester.
[[ -f "$candidate_identity" && ! -L "$candidate_identity" ]] || die 'Candidate AK identity source is absent or unsafe'
[[ "$(file_sha256 "$candidate_identity")" == "$CANDIDATE_AK_SHA256" ]] || die 'Candidate AK identity source changed'
...
spec = importlib.util.spec_from_file_location("ak_native_request_pins", path)
...
[[ "$pinned_full_sha256" == "$INSTALLED_FULL_SHA256" ]] || die 'Candidate AK padded identity changed'
...
[[ "$(basename -- "$capture_dir")" == candidate-ak-* ]] || die 'runtime capture directory is not Candidate AK'
...
runtime_validation="$(python3 "$runtime_validator" --capture "$runtime_capture" \
	--expected-installed-full-sha256 "$installed_full_sha256")" || die 'runtime capture did not pass exact AK validation'
printf '%s\n' "$runtime_validation" | grep -qx 'validation=candidate-ak-usb-cpu-runtime-subgate' || die 'runtime validation label changed'
...
native_preflight="$(python3 "$native_validator" --preflight-runtime \
	...
	die 'native reboot validator preflight rejected the exact AK runtime'
printf '%s\n' "$native_preflight" | grep -qx 'validation=candidate-ak-native-reboot-preflight' || die 'native reboot preflight label changed'
preflight_boot_id="$(printf '%s\n' "$native_preflight" | awk -F= '$1 == "candidate_boot_id" { print substr($0, length($1) + 2); count++ } END { exit count != 1 }')" || die 'native reboot preflight boot ID is absent or duplicated'
[[ "$preflight_boot_id" == "$candidate_boot_id" ]] || die 'native reboot preflight boot ID differs from the runtime capture'
interface="$(awk '
	$0 == "__AK_HOST_BEGIN__" { inside=1; next }
	$0 == "__AK_HOST_END__" { inside=0 }
...
command_file="$(mktemp /tmp/candidate-ak-native-reboot-command.XXXXXX)"
...
printf '__AK_NATIVE_REQUEST_BEGIN__\\n'
...
printf '__AK_NATIVE_REQUEST_END__\\n'
if [ "\$live_boot_id" != '$candidate_boot_id' ] || [ "\$live_reboot_sha256" != '$REBOOT_SHA256' ]; then
	exit 93
fi
/bin/reboot
printf '__AK_NATIVE_REBOOT_RETURNED__\\n'
exit 94
EOF

{
	printf '__AK_NATIVE_HOST_BEGIN__\n'
	...
	printf 'runtime_validation=candidate-ak-usb-cpu-runtime-subgate\n'
	printf 'native_runtime_preflight=candidate-ak-native-reboot-preflight\n'
...
nc_exit_status=$?
set -e
grep -q '__AK_NATIVE_REBOOT_RETURNED__' "$output" && die 'native reboot wrapper returned unexpectedly'
...
{
	printf '__AK_NATIVE_RESULT_BEGIN__\n'
	...
	printf '__AK_NATIVE_RESULT_END__\n'
} >>"$output"
