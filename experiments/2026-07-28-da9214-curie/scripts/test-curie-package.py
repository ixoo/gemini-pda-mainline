#!/usr/bin/env python3
"""Static and mutation contracts for Curie's package validator."""
...
SPEC = importlib.util.spec_from_file_location(
    "curie_package_validator_test",
    SCRIPT_DIR / "validate-package-curie.py",
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("cannot load Curie package validator")
...
def synthetic_image() -> bytes:
    ...
    required = (
        b"GEMINI_CURIE_NATIVE_DIAGNOSTIC state=ready",
        b"GEMINI_CURIE_DIAGNOSTIC_GATE missing DVFSP handoff",
        b"GEMINI_CURIE_DIAGNOSTIC_GATE target node unavailable",
        b"GEMINI_CURIE_DIAGNOSTIC_GATE node identity mismatch",
        b"GEMINI_CURIE_DIAGNOSTIC_GATE adapter debugfs unavailable",
        b"GEMINI_CURIE_DIAGNOSTIC_GATE debugfs creation failed",
        b"curie-run-native",
...
class CuriePackageContracts(unittest.TestCase):
    ...
    def test_config_rejects_identity_and_safety_mutations(self) -> None:
        ...
        mutations = (
            config.replace(
                b"CONFIG_I2C_MT65XX_CURIE_DIAGNOSTIC=y\n",
                b"# CONFIG_I2C_MT65XX_CURIE_DIAGNOSTIC is not set\n",
                1,
            ),
            config.replace(
            ...
            config + b"CONFIG_I2C_MT65XX_FERMI_DIAGNOSTIC=y\n",
            config.replace(b"maxcpus=8", b"maxcpus=9", 1),
            config.replace(b"GEMINI_CURIE_20260728", b"GEMINI_WRONG", 1),
            config.replace(
    ...
    def test_system_map_requires_curie_and_rejects_predecessors(self) -> None:
        system_map = synthetic_system_map()
        vp.validate_system_map(system_map)
        missing = system_map.replace(
            b" T mtk_i2c_curie_write\n",
            b" T changed\n",
            1,
        )
        with self.assertRaises(ValueError):
    ...
    def test_image_requires_board_control_and_rejects_old_endpoints(self) -> None:
        image = synthetic_image()
        vp.validate_image(image)
        required_mutations = (
            (b"curie-run-native", b"changed"),
            (b"addresses=0x69,0x68 passes=2 ", b"addresses=changed "),
            (
                b"board_control_register=d3 board_control_expected=1f ",
                b"board_control=changed ",
            ),
            (
                b"stability_registers=d3,5e,d9,da stability_validated=%u ",
                b"stability=changed ",
    ...
    def test_package_checksum_manifest_is_canonical(self) -> None:
        with tempfile.TemporaryDirectory(prefix="curie-package-manifest.") as raw:
            root = pathlib.Path(raw)
            member = root / "Image"
            member.write_bytes(b"synthetic Curie Image")
