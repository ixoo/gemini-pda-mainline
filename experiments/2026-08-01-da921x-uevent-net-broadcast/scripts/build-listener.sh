...
# Derive and build the exact static ARM64 net-broadcast listener in the
...
readonly DERIVED_SOURCE_SHA256=4c3448cc30753880d7b9835253b351feab46112a8e613732117061da65356983
...
derived="$(mktemp "$parent/.net-broadcast-listener.XXXXXXXX.c")"
...
chmod 0600 "$derived"
perl -0pe '
	s#gemini_da921x_uevent_untagged_dispatch#gemini_da921x_uevent_net_broadcast#g;
	s#attempts=0 entries=0 returns=0 baseline_sockets=-1 sockets=-1 listeners=-1 allocations=-1 broadcasts=-1 retval=-1#attempts=0 entries=0 returns=0 namespace_checks=0 untagged_routes=0 tagged_routes=0 baseline_sockets=-1 sockets=-1 listeners=-1 allocations=-1 broadcasts=-1 retval=-1#g;
	s#attempts=1 entries=1 returns=1 baseline_sockets=1 sockets=1 listeners=1 allocations=1 broadcasts=1 retval=0#attempts=1 entries=1 returns=1 namespace_checks=1 untagged_routes=1 tagged_routes=0 baseline_sockets=1 sockets=1 listeners=1 allocations=1 broadcasts=1 retval=0#g;
	s#untagged_dispatch_result#net_broadcast_result#g;
	s#"22\\n", "pre-stage"#"23\\n", "pre-stage"#g;
	s#"23\\n", "post-stage"#"24\\n", "post-stage"#g;
' "$source_file" >"$derived"
