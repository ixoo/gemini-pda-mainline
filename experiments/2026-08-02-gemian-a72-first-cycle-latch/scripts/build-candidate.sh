...
readonly REPOSITORY_COMMIT=a5b22fa59a4e45169a5c31f976b3f19df4e00bfa
readonly PATCHSET_SHA256=3584e9dd5ffb041573b851f31f3a96eaa0a684acb880fd59560762e5abc58be0
readonly KERNEL_SHA256=31d288ebac7bfc4b0999b9178e254e6c1493f44cec53a56ccf5d8d795cd57824
...
readonly EXPECTED_RAW_SHA256=003677fddd6e3d785f9d86b9459cf4c12f9ed36f0fd2bcc1b2ddb256e0af3f7b
readonly EXPECTED_PADDED_SHA256=6dcbda0cb264f5b637f6333f4946707d81ea98d38a8d52673b6fbe2cf109e3af
...
[[ "$(jq -er '.patchset_sha256' "$bundle/provenance/build.json")" == \
  "$PATCHSET_SHA256" ]] || die 'observer patchset changed'
...
workdir="$(mktemp -d "$output_parent/.gemian-a72-latch.XXXXXX")"
cleanup() { [[ ! -d "${workdir:-}" ]] || rm -rf -- "$workdir"; }
trap cleanup EXIT
stage="$workdir/stage"
replica="$workdir/replica"
mkdir "$stage" "$replica"
raw_name=gemian-a72-first-cycle-latch.boot.img
...
{
  printf 'experiment=2026-08-02-gemian-a72-first-cycle-latch\n'
  printf 'repository_commit=%s\npatchset_sha256=%s\n' \
    "$REPOSITORY_COMMIT" "$PATCHSET_SHA256"
...
output_name="gemian-a72-first-cycle-latch-${raw_sha256:0:12}"
...
workdir=
trap - EXIT
printf 'validation=gemian-a72-first-cycle-latch-candidate\n'
printf 'artifact=%s\nraw_sha256=%s\npadded_sha256=%s\n' \
