...
grep -Fq '#1 SMP PREEMPT Sun Aug 2 18:14:10 UTC 2026' /proc/version ||
...
first=$(mktemp /tmp/.gemian-a72-latch-first.XXXXXXXX)
second=$(mktemp /tmp/.gemian-a72-latch-second.XXXXXXXX)
cleanup() { rm -f -- "$first" "$second"; }
trap cleanup EXIT HUP INT TERM

# Evidence comes first. In particular, USB/battery/thermal reporting must not
# prevent the immutable latch from being copied, as it did in the parent run.
cat "$observer" >"$first" || fail observer-first-read
case "$(head -n 1 "$first")" in
abi=mt6797-a72-transition-observer-v2\ state=*) ;;
...
printf 'experiment=gemian-a72-first-cycle-latch-passive\n'
printf 'kernel_release=3.18.41+\narchitecture=aarch64\n'
printf 'build_identity=#1 SMP PREEMPT Sun Aug 2 18:14:10 UTC 2026\n'
