...
def main():
    ...
    marker = "mt6797-hif-start-patch-v1\n"
    ...
    with tempfile.TemporaryDirectory(prefix="patch-", dir=managed) as directory:
        ...
        git("commit", "--quiet", "--no-gpg-sign", "-m",
            ...
            "Assisted-by: LLM")
        base = git("rev-parse", "HEAD").decode().strip()
        for name, digest in spec["plan_files"].items():
            data = (HERE.parent / "2026-09-05-mt6797-whole-image-plan" / "src" / name).read_bytes()
            require(hashlib.sha256(data).hexdigest() == digest, "plan hash: " + name)
            (target / name).write_bytes(data)
        with (target / "Makefile").open("a") as output:
            output.write("obj-y += image-plan.o\n")
        git("add", "drivers")
        git("diff", "--cached", "--check")
        git("commit", "--quiet", "--no-gpg-sign", "-m",
            "wifi: mediatek: prevalidate complete MTKE image plans\n\n"
            "Account for all ordinary and EMI sections before publishing a plan.\n"
            "Separate descriptive planning from unresolved EMI execution admission.\n"
            "No transfer, registration, ownership or START path is supplied.\n\n"
            "Internal experiment only; synthetic non-certifying author, no DCO.\n"
            "Assisted-by: LLM")
        base = git("rev-parse", "HEAD").decode().strip()
        for name, digest in spec["start_files"].items():
            data = (HERE / "src" / name).read_bytes()
            require(hashlib.sha256(data).hexdigest() == digest, "START source hash: " + name)
            (target / name).write_bytes(data)
        git("add", "drivers")
        git("diff", "--cached", "--check")
        git("commit", "--quiet", "--no-gpg-sign", "-m",
            "wifi: mediatek: execute private START with bounded readiness observation\n\n"
            "Reuse INIT START credit/sequence primitives and actual ordered PIO.\n"
            "Latch one deadline and attempt, separating submission from WCIR readiness.\n"
            "No runtime caller, whole-image permission or owner acquisition is added.\n\n"
            "Internal experiment only; synthetic non-certifying author, no DCO.\n"
            "Assisted-by: LLM")
