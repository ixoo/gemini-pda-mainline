#!/usr/bin/env python3
"""Mutation tests for the pure A34 eligibility QEMU classifier."""
...
SPEC = importlib.util.spec_from_file_location("a34_classifier", CLASSIFIER)
...
def valid_log() -> str:
    ...
    return f"""Linux version 7.1.3-gemini-a34-test (builder) #1
KTAP version 1
1..1
...
1..5
{cases}
# {MODULE.SUITE}: pass:5 fail:0 skip:0 total:5
...
def rejected(raw: str, release: str = "7.1.3-gemini-a34-test",
...
def main() -> None:
    raw = valid_log()
    MODULE.classify_runtime(raw, "7.1.3-gemini-a34-test", 124)
    mutations = (
        raw.replace("ok 1 mt6797_a34_exact_provenance_test",
                    "not ok 1 mt6797_a34_exact_provenance_test", 1),
        raw.replace(f"# Subtest: {MODULE.SUITE}",
                    f"# Subtest: unexpected\n# Subtest: {MODULE.SUITE}", 1),
        raw.replace("ok 2 mt6797_a34_null_test",
                    "ok 2 mt6797_a34_missing_provenance_test", 1),
        raw.replace("pass:5 fail:0", "pass:4 fail:1", 1),
        raw.replace("1..5", "1..6", 1),
