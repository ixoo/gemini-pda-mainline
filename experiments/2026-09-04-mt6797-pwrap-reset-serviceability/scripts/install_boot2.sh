...
readonly CANDIDATE_SHA256=5c7429b297c718f5af61367588975e292a8c239854ffd5ba527eb86da1e4a5a6
...
while (($#)); do
	...
	--output)
		...
		shift 2
		;;
	*) die "usage: $0 --output artifacts/device-deployments/pwrap-reset-serviceability-attempt-1" ;;
	esac
done
...
candidate_dir="$repo_root/artifacts/mt6797-pwrap-reset-serviceability/$CANDIDATE_DIR"
...
ssh "${ssh_options[@]}" "$TARGET" \
	'umask 077; rm -f /tmp/gemini-pwrap-reset-5c7429b2.img; cat > /tmp/gemini-pwrap-reset-5c7429b2.img' <"$candidate"
remote_script=$(mktemp "${TMPDIR:-/tmp}/.gemini-pwrap-install.XXXXXXXX")
cleanup() {
	rm -f -- "${remote_script:-}"
	ssh -n "${ssh_options[@]}" "$TARGET" \
		'rm -f /tmp/gemini-pwrap-reset-5c7429b2.img' >/dev/null 2>&1 || true
}
trap cleanup EXIT HUP INT TERM
cat >"$remote_script" <<'REMOTE'
set -eu
export LC_ALL=C
candidate=/tmp/gemini-pwrap-reset-5c7429b2.img
expected_sha=5c7429b297c718f5af61367588975e292a8c239854ffd5ba527eb86da1e4a5a6
...
printf '%s\n' __GEMINI_PWRAP_DEPLOYMENT_BEGIN__
...
printf '%s\n' __GEMINI_PWRAP_DEPLOYMENT_END__
rm -f "$candidate"
sync
poweroff
REMOTE
...
printf 'repository_build_commit=%s\ncandidate_sha256=%s\ncandidate_size=%s\n' \
	"$BUILD_COMMIT" "$CANDIDATE_SHA256" "$CANDIDATE_SIZE" >>"$evidence"
...
ssh_rc=$?
set -e
grep -Fq __GEMINI_PWRAP_DEPLOYMENT_BEGIN__ "$evidence" || die "deployment did not begin rc=$ssh_rc"
grep -Fq __GEMINI_PWRAP_DEPLOYMENT_END__ "$evidence" || die "deployment did not complete rc=$ssh_rc"
