...
# Return a successfully classified PWRAP candidate to Gemian through its
...
while (($#)); do
	...
	--capture-dir)
		...
		shift 2
		;;
	*) die "usage: $0 --capture-dir artifacts/runtime-captures/pwrap-reset-serviceability-attempt-1" ;;
	esac
done
...
[[ "$(basename -- "$capture_dir")" == pwrap-reset-serviceability-attempt-1 ]] || die 'capture identity changed'
...
command_file=$(mktemp "${TMPDIR:-/tmp}/.gemini-pwrap-reboot.XXXXXXXX")
...
/bin/busybox printf '%s\n' __GEMINI_PWRAP_NATIVE_REBOOT_BEGIN__
...
/bin/busybox printf '%s\n' __GEMINI_PWRAP_NATIVE_REBOOT_END__
