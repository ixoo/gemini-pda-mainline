...
/*
 * Fixed-function receive-overwrite observer for the Gemini PDA's DA9214.
 *
 ...
 * after successful transfers so distinct sentinels for the same register can
 * distinguish an untouched buffer from a coincidentally equal device byte.
...
static const uint8_t photon_expected[PHOTON_TRANSACTION_COUNT] = {
	0xd9U, 0xd0U, 0xc0U, 0xd9U, 0xd0U, 0xc0U
};

static const uint8_t photon_sentinels[PHOTON_TRANSACTION_COUNT] = {
...
static const char *complete_status(const uint8_t *values,
				   unsigned int overwrite_mask)
{
	if (memcmp(values, photon_expected, PHOTON_TRANSACTION_COUNT) == 0)
		return "expected-signature";
	if (values[0] == 0U && values[1] == 0U && values[2] == 0U &&
	    values[3] == 0U && values[4] == 0U && values[5] == 0U)
		return "all-overwritten-zero";
	if (memcmp(values, values + PHOTON_REGISTER_COUNT,
		   PHOTON_REGISTER_COUNT) == 0)
		return "stable-different";
	if (overwrite_mask == 0U)
		return "all-sentinels-preserved";
	if (overwrite_mask != (1U << PHOTON_TRANSACTION_COUNT) - 1U)
		return "partial-sentinel-match-unstable";
	return "unstable-different";
}
...
int main(int argc, char *argv[])
{
	...
	unsigned int overwrite_mask = 0U;
	...
	if (argc != 1) {
		emit_stdout("GEMINI_PHOTON_RESULT status=argument-error attempted=0");
		if (kmsg_fd >= 0)
			(void)close(kmsg_fd);
		return 2;
	}
	if (kmsg_fd < 0) {
		emit_stdout("GEMINI_PHOTON_RESULT status=kmsg-open-error attempted=0");
		return 2;
	}
	...
	if (find_i2c6(adapter_name, sizeof(adapter_name),
		...
		(void)emit_marker(
			kmsg_fd,
			"GEMINI_PHOTON_RESULT status=adapter-error attempted=0 pre=a1,b2,c3,d4,e5,f6 post=--,--,--,--,--,-- overwrite_mask=0x00");
		(void)close(kmsg_fd);
		return 2;
	}
	...
	if (descriptor < 0) {
		...
		(void)emit_marker(
			kmsg_fd,
			"GEMINI_PHOTON_RESULT status=open-error errno=%d attempted=0 pre=a1,b2,c3,d4,e5,f6 post=--,--,--,--,--,-- overwrite_mask=0x00",
			saved_errno);
		(void)close(kmsg_fd);
		return 2;
	}
	...
	if (!emit_marker(
		    kmsg_fd,
		    "GEMINI_PHOTON_BEGIN adapter=%s of=/i2c@1100e000 address=0x69 transactions=6 registers=05,06,47,05,06,47 sentinels=a1,b2,c3,d4,e5,f6",
		    adapter_name)) {
	...
	for (transaction = 0U; transaction < PHOTON_TRANSACTION_COUNT;
		...
		uint8_t sentinel = photon_sentinels[transaction];

		values[transaction] = sentinel;
		if (!emit_marker(
			    kmsg_fd,
			    "GEMINI_PHOTON_PRE transaction=%u pass=%u register=0x%02x sentinel=0x%02x address=0x69 messages=2",
			    transaction + 1U, pass + 1U,
			    photon_registers[index], sentinel)) {
			status = "pre-marker-error";
			break;
		}
		...
		if (transfer_result != (int)PHOTON_MESSAGE_COUNT) {
			saved_errno = transfer_result < 0 ? errno : 0;
			status = "transfer-error";
			break;
		}
		attempted++;
		if (values[transaction] != sentinel)
			overwrite_mask |= 1U << transaction;
		emit_stdout(
			"GEMINI_PHOTON_READ transaction=%u pass=%u register=0x%02x sentinel=0x%02x value=0x%02x overwritten=%s",
			transaction + 1U, pass + 1U, photon_registers[index],
			sentinel, values[transaction],
			values[transaction] == sentinel ? "no" : "yes");
	}

	if (attempted == PHOTON_TRANSACTION_COUNT) {
		status = complete_status(values, overwrite_mask);
		if (strcmp(status, "expected-signature") == 0)
			result = 0;
	}
	if (!format_byte_list(post_list, sizeof(post_list), values, attempted))
		status = "format-error";
	if (!emit_marker(
		    kmsg_fd,
		    "GEMINI_PHOTON_RESULT status=%s attempted=%u transfer_result=%d errno=%d pre=a1,b2,c3,d4,e5,f6 post=%s overwrite_mask=0x%02x page_con=untouched",
		    status, attempted, transfer_result, saved_errno, post_list,
		    overwrite_mask)) {
		result = 2;
	}
