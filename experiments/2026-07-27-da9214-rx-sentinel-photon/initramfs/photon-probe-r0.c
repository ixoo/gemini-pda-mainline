 ...
 * in each receive buffer immediately before its ioctl. A completed transfer
 * that leaves that byte unchanged stops the probe. No page, regulator, CPU,
 * storage, watchdog, reboot, or other hardware-control operation is reachable.
 */
...
int main(int argc, char *argv[])
{
	...
	for (transaction = 0U; transaction < PHOTON_TRANSACTION_COUNT;
		...
		if (values[transaction] == sentinel) {
			...
			emit_stdout(
				"GEMINI_PHOTON_READ transaction=%u pass=%u register=0x%02x sentinel=0x%02x value=0x%02x overwritten=no",
				transaction + 1U, pass + 1U,
				photon_registers[index], sentinel,
				values[transaction]);
			break;
		...
		emit_stdout(
			"GEMINI_PHOTON_READ transaction=%u pass=%u register=0x%02x sentinel=0x%02x value=0x%02x overwritten=yes",
			transaction + 1U, pass + 1U, photon_registers[index],
			sentinel, values[transaction]);
	}

	if (attempted == PHOTON_TRANSACTION_COUNT) {
		status = complete_status(values);
