...
# Frozen BusyBox-compatible derivative of the corrected-layout acceptance
# check (source SHA-256 d260464c5bfe84e93efdcb0696a33c40b129614c4aaf7d44667473cb6f67e5a8).
# Read-only acceptance check for one fresh selected serialization boot.
set -eu

abort()
{
	printf 'netlink_serialization_result=FAIL\nfailure=%s\n' "$1"
	exit 1
}
...
[ "$(/bin/busybox uname -r)" = 7.1.3-gemini-da921x-skbser ] ||
...
[ "$stage" = 18 ] || abort validation-stage
...
printf 'kernel=7.1.3-gemini-da921x-skbser\n'
...
printf 'event_transport=skb-consumed-before-socket-traversal-or-multicast\nclient_driver=unbound\ni2c_activity=zero\n'
printf 'post_creation_serviceability=PASS\n'
printf 'netlink_serialization_result=PASS\n'
