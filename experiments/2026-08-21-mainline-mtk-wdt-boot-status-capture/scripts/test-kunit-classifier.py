#!/usr/bin/env python3
"""Unit-test the exact watchdog boot-status KTAP classifier."""
...
def fixture() -> str:
    ...
    return f"""Linux version 7.1.3-gemini-mtk-wdt-boot-status-kunit (test)
[    0.1] KTAP version 1
[    0.1] 1..1
...
[    0.1]     1..4
{cases}
[    0.1] # {MODULE.SUITE}: pass:4 fail:0 skip:0 total:4
[    0.1] # Totals: pass:4 fail:0 skip:0 total:4
...
def require_reject(raw: str, label: str) -> None:
    try:
        MODULE.classify_runtime(
            raw, "7.1.3-gemini-mtk-wdt-boot-status-kunit", 124)
...
def main() -> None:
    ...
    require_reject(raw.replace("1..4", "1..5", 1), "changed plan")
