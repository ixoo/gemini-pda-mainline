#!/usr/bin/env python3
"""Classify the focused MediaTek watchdog boot-status KUnit/QEMU proof."""
...
PROFILE = "mtk-wdt-boot-status-kunit"
SUITE = "mtk-wdt-boot-status"
PANIC_PREFIX = "Kernel panic - not syncing: VFS: Unable to mount root fs"
PANIC_END_PREFIX = f"---[ end {PANIC_PREFIX}"
EXPECTED_CASES = (
    "mtk_wdt_boot_status_invalid_test",
    "mtk_wdt_boot_status_exact_test",
    "mtk_wdt_boot_status_every_bit_test",
    "mtk_wdt_boot_status_immutable_test",
)
...
def classify_runtime(raw: str, expected_release: str, qemu_exit: int) -> None:
    ...
    summary = f"# {SUITE}: pass:4 fail:0 skip:0 total:4"
...
def main() -> None:
    ...
    print("vcpus=4")
    print("network=none")
    print("production_capture=not-executed-no-matching-device")
    print("suites=1")
    print("tests=4")
    print("failed=0")
    print("skipped=0")
