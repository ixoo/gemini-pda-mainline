...
# Assemble storage-inert Candidate AR from an exact validated AR kernel package
...
die() { printf 'error: %s\n' "$*" >&2; exit 2; }
usage() {
	printf 'usage: %s --package AR_PACKAGE --ao-artifact AO_ARTIFACT --output-parent DIR\n' "$0" >&2
}
...
candidate_module="$script_dir/candidate_ar.py"
[[ -f "$candidate_module" && ! -L "$candidate_module" &&
	-s "$candidate_module" ]] || die 'Candidate AR identity module is unsafe'
candidate_value() {
	PYTHONPATH="$script_dir" python3 -c \
		'import candidate_ar as ar, sys; print(getattr(ar, sys.argv[1]))' "$1"
}
...
readonly AR_DTB="$(candidate_value DTB_MEMBER)"
readonly AR_INITRAMFS="$(candidate_value INITRAMFS_MEMBER)"
readonly AR_BOOT="$(candidate_value BOOT_MEMBER)"
...
support_dir="$repo_root/experiments/2026-07-24-mt6797-dvfsp-i2c6-consumer"
package_validator="$script_dir/validate-package-ar.py"
dtb_builder="$support_dir/scripts/build-ap-dtb.sh"
dtb_validator="$support_dir/scripts/validate-dtb-delta.py"
boot_validator="$script_dir/validate-boot-ar.py"
handoff_auditor="$script_dir/audit-compiled-handoff-ar.py"
normalizer="$support_dir/scripts/normalize-build-json.py"
...
for input in "$manifest" "$package_validator" "$dtb_builder" "$dtb_validator" \
	"$boot_validator" "$handoff_auditor" "$normalizer" "$serializer" \
		...
		"$patch_0099" "$patch_0100" "$patch_0101" "$patch_0102" "$patch_0103"; do
...
workdir="$(mktemp -d "$output_parent/.candidate-AR-i2c6-baseline-preserve.XXXXXX")"
...
install -m 0600 "$ao_artifact/$AO_INITRAMFS" "$stage/$AR_INITRAMFS"
...
bash "$dtb_builder" --ao-dtb "$ao_artifact/$AO_DTB" \
	--output "$stage/$AR_DTB" >"$stage/dtb-validation.raw"
bash "$dtb_builder" --ao-dtb "$ao_artifact/$AO_DTB" \
	--output "$replica/$AR_DTB" >/dev/null
cmp -s "$stage/$AR_DTB" "$replica/$AR_DTB" || \
	die 'two independent Candidate AR final-DT derivations differ'
sed -e "s|$stage/$AR_DTB|@AR_DTB@|g" \
	"$stage/dtb-validation.raw" >"$stage/dtb-validation.txt"
rm "$stage/dtb-validation.raw"

candidate="$stage/$AR_BOOT"
replica_boot="$replica/$AR_BOOT"
boot_cmdline=bootopt=64S3,32N2,64N2
for output in "$candidate" "$replica_boot"; do
	python3 "$serializer" --kernel "$stage/Image.gz" \
		--ramdisk "$stage/$AR_INITRAMFS" --dtb "$stage/$AR_DTB" \
...
cmp -s "$candidate" "$replica_boot" || \
	die 'two independent Candidate AR container assemblies differ'
...
python3 "$analyzer" --validate-lk --expected-image-gz "$stage/Image.gz" \
	--expected-ramdisk "$stage/$AR_INITRAMFS" \
	--expected-dtb "$stage/$AR_DTB" --expected-name gemini-obs-L \
...
python3 "$boot_validator" --candidate "$candidate" \
	...
	--kernel-config "$stage/kernel.config" --dtb "$stage/$AR_DTB" \
	--ao-dtb "$ao_artifact/$AO_DTB" --initramfs "$stage/$AR_INITRAMFS" \
...
dtb_sha256="$(sha256sum "$stage/$AR_DTB" | awk '{ print $1 }')"
...
expected_pre_manifest="$(printf '%s\n' Image.gz System.map analysis.txt \
	...
	dtb-validation.txt "$AR_BOOT" "$AR_INITRAMFS" gemini-us.bkeymap \
	input-event-capture kernel.config "$AR_DTB" package-validation.txt \
...
[[ "$actual_inventory" == "$expected_pre_manifest" ]] || \
	die 'Candidate AR output inventory changed'
(
	cd "$stage"
	find . -type f ! -name SHA256SUMS -print0 | sort -z | xargs -0 sha256sum
) >"$stage/SHA256SUMS"
(cd "$stage" && sha256sum --check --strict SHA256SUMS >/dev/null) || \
	die 'Candidate AR artifact manifest failed'
chmod 0600 "$stage"/*
...
[[ -d "$output" && ! -e "$artifact" ]] || \
	die 'exclusive Candidate AR publication failed'
rm -rf -- "$replica"
rmdir "$workdir"
workdir=
trap - EXIT
printf 'validation=candidate-ar-mt6797-dvfsp-i2c6-baseline-preserve\n'
printf 'artifact=%s\ncandidate=%s/%s\n' "$output" "$output" "$AR_BOOT"
printf 'candidate_sha256=%s\ncandidate_size=%s\n' \
	"$candidate_sha256" "$candidate_size"
printf 'dtb_sha256=%s\nruntime_result=not-tested\n' "$dtb_sha256"
