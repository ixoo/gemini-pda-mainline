#!/usr/bin/env python3
"""Validate Candidate AR's canonical Android-v0 container and safety boundary."""
...
def validate_system_map(data: bytes) -> None:
    ...
    if missing:
        raise ValueError("System.map lacks Candidate AR owner symbol: " + missing[0])
    if "mtk_i2c_do_transfer" in symbols:
        raise ValueError(
            "Candidate AR transfer helper is not fully inlined as compiled-audited"
        )
    forbidden = {
...
def run_dtb_validator(
    ...
    validator = (
        ...
        / "experiments/2026-07-24-mt6797-dvfsp-i2c6-consumer/scripts/"
    ...
    if result.returncode:
        ...
        raise ValueError("final DT semantic validator rejected Candidate AR: " + detail)


def run_compiled_audit(
    script_dir: pathlib.Path, image: bytes, system_map: pathlib.Path
) -> bytes:
    auditor = script_dir / "audit-compiled-handoff-ar.py"
    ...
    with tempfile.TemporaryDirectory(prefix="candidate-ar-audit-") as temporary:
...
def main() -> int:
    ...
    try:
        candidate = read_regular(args.candidate, "Candidate AR boot")
        image_gz = read_regular(args.image_gz, "Candidate AR Image.gz")
        system_map = read_regular(args.system_map, "Candidate AR System.map")
        kernel_config = read_regular(args.kernel_config, "Candidate AR config")
        dtb = read_regular(args.dtb, "Candidate AR final DT")
        ...
        if pin_state == "source-pinned" and digest(dtb) != ar.FINAL_DTB_SHA256:
            raise ValueError("Candidate AR final-DT identity changed")
        if digest(initramfs) != ar.INITRAMFS_SHA256:
            raise ValueError("Candidate AR initramfs is not byte-exact Candidate AO")
        ...
        for marker in REQUIRED_IMAGE_MARKERS:
            if marker not in image:
                raise ValueError(
                    f"Candidate AR kernel lacks handoff-owner marker: {marker!r}"
                )
        for marker in FORBIDDEN_IMAGE_MARKERS:
            if marker in image:
                raise ValueError(
                    f"Candidate AR kernel contains forbidden marker: {marker!r}"
                )
        compiled_audit = run_compiled_audit(script_dir, image, args.system_map)

        if not 0 < len(candidate) <= ar.BOOT2_SIZE:
            raise ValueError("Candidate AR size is invalid or exceeds boot2")
        if len(candidate) < PAGE_SIZE or candidate[:8] != b"ANDROID!":
            raise ValueError("Candidate AR is not Android boot image v0")
        ...
        print("validation=candidate-ar-mt6797-dvfsp-i2c6-consumer-android-v0")
