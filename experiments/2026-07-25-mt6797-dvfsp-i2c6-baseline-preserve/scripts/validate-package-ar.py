#!/usr/bin/env python3
"""Validate one Candidate AR kernel package against the pinned AR profile."""
...
SERIES_REL = "patches/series-dvfsp-handoff-owner-i2c6-consumer-ap-dma-preserve"
FRAGMENTS = [
...
def validate(repository: pathlib.Path, package: pathlib.Path) -> None:
    ...
    if profile != expected_profile:
        fail("AR profile differs from the repository manifest")

    build = json.loads(regular(package / "provenance/build.json", "build provenance"))
    if build.get("build_profile") != PROFILE:
        fail("package build profile is not Candidate AR")
    repository_series = regular(repository / SERIES_REL, "repository AR series")
    packaged_series = regular(package / "provenance/series", "packaged AR series")
    if repository_series != packaged_series:
        fail("packaged AR series differs from repository")
    entries = series_entries(repository_series)
    if len(entries) != 102 or not entries[-1].endswith("0103-soc-mediatek-preserve-shared-MT6797-AP-DMA-owner.patch"):
        fail("AR series does not end in patch 0103")
    if any(pathlib.PurePosixPath(entry).name.startswith(("0093-", "0096-")) for entry in entries):
        fail("AR series selects a forbidden active-power patch")
    ...
    for marker in REQUIRED_IMAGE_MARKERS:
        if marker not in image:
            fail(f"kernel Image lacks required AR marker: {marker!r}")
    ...
    for marker in (
        ...
        if marker not in system_map:
            fail(f"System.map lacks required AR symbol: {marker!r}")

    print("validation=candidate-ar-package")
