#!/usr/bin/env python3
"""Derive AT's guarded boot2 installer from exact Candidate AO."""
...
# The device is currently running Candidate AS attempt 3 on boot2.
PREVIOUS_AS_PADDED_SHA256 = (
    "82a5248120ad3201d95e25c1604e45e450e61230db7d8c8b3ec4238a22183aa0"
)
...
def validate_calibration(calibration: Calibration) -> None:
    for label, value in (
        ...
        if ar.HEX256.fullmatch(value) is None:
            raise ValueError(f"Candidate AT {label} SHA-256 is unresolved or malformed")
    if (
        not calibration.raw_size.isdecimal()
        or not 0 < int(calibration.raw_size) <= ar.BOOT2_SIZE
    ):
        raise ValueError("Candidate AT raw size is unresolved, malformed, or oversized")
    if calibration.raw_sha256 == ar.AO_RAW_SHA256:
        raise ValueError("Candidate AT raw identity equals Candidate AO")
    if calibration.manifest_sha256 == ar.AO_MANIFEST_SHA256:
        raise ValueError("Candidate AT artifact manifest equals Candidate AO")
...
def identity_replacements(
    ...
    return (
        ...
        ("Candidate AO", "Candidate AT", 8),
        ("candidate-ao", "candidate-at", 14),
        ("AO_RAW", "AS_RAW", 16),
...
def derive_text(source: str, calibration: Calibration) -> str:
    ...
    if restored != source:
        raise ValueError("Candidate AT installer cannot restore exact AO foundation")
    ...
    for token in required:
        if token not in text:
            raise ValueError(
                f"derived Candidate AT installer lost safety token: {token}"
            )
    for token in (
        "Candidate AN",
        "candidate-an",
        ...
        if token in text:
            raise ValueError(f"derived Candidate AT installer retains stale token: {token}")
    if text.count(TARGET_CHECK) != 1:
        raise ValueError("derived Candidate AT installer target is not source-pinned")
    return text
...
def validate_output(path: pathlib.Path) -> pathlib.Path:
    if not path.name or path.name in {".", ".."} or path.exists() or path.is_symlink():
        raise ValueError("Candidate AT installer output is invalid or already exists")
    info = path.parent.lstat()
    if path.parent.is_symlink() or not stat.S_ISDIR(info.st_mode):
        raise ValueError("Candidate AT installer output parent is unsafe")
...
def main() -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".candidate-at-ao-installer.", dir=output.parent
        ) as raw:
        ...
        publish(output, text)
        print("validation=candidate-at-installer-derivation")
