#!/usr/bin/env python3
"""Require exact AO plus only AT's childless I2C6 access-controller link."""
...
def validate(ao_path: pathlib.Path, candidate_path: pathlib.Path) -> None:
    ...
    require_regular(candidate_path, "Candidate AT DT")
    ...
    candidate_header = require_header(candidate_path, AS_HEADER, "Candidate AT")
    if reservation_map_bytes(candidate_path, candidate_header) != (
        reservation_map_bytes(ao_path, ao_header)
    ):
        raise ValueError("Candidate AT changed AO's raw FDT reservation map")
    ...
    if candidate_reservations != ao_reservations:
        raise ValueError("Candidate AT changed AO's FDT reservation map")
    if candidate_boot_cpu != ao_boot_cpu:
        raise ValueError("Candidate AT changed AO's boot_cpuid_phys")
    ...
    if candidate != expected:
        raise ValueError(
            "Candidate AT DT is not exact AO plus the I2C6 dependency: "
    ...
    if handles != expected_handles:
        raise ValueError("Candidate AT phandle map changed beyond handoff/pins 0x2d")
    if candidate[HANDOFF][ACCESS_CELLS_PROPERTY] != fdt.cells(0):
        raise ValueError("Candidate AT handoff access-controller cells changed")
    reference = candidate[I2C6][DEPENDENCY_PROPERTY]
    if len(reference) != 4:
        raise ValueError("Candidate AT I2C6 dependency is not one phandle")
    if handles.get(struct.unpack(">I", reference)[0]) != HANDOFF:
        raise ValueError("Candidate AT I2C6 dependency does not resolve to handoff")

    for forbidden in (A72_POWER, LEGACY_DVFSP, OBSERVER):
        if forbidden in candidate:
            raise ValueError(f"Candidate AT contains forbidden node {forbidden}")
...
def main() -> int:
    ...
    print("validation=candidate-at-exact-ao-plus-legacy-da9214-i2c6-contract")
    print(f"ao_dtb_sha256={AO_DTB_SHA256}")
    print(f"candidate_dtb_sha256={digest(args.candidate)}")
    print("added_nodes=4")
    print("added_properties=10")
