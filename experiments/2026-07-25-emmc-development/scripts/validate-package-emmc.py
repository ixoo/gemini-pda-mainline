#!/usr/bin/env python3
"""Validate one Candidate AT kernel package against the pinned AT profile."""
...
def validate(repository: pathlib.Path, package: pathlib.Path) -> None:
    ...
    if profile != expected_profile:
        fail("AT profile differs from the repository manifest")

    build = json.loads(regular(package / "provenance/build.json", "build provenance"))
    if build.get("build_profile") != PROFILE:
        fail("package build profile is not Candidate AT")
    repository_series = regular(repository / SERIES_REL, "repository AT series")
    packaged_series = regular(package / "provenance/series", "packaged AT series")
    if repository_series != packaged_series:
        fail("packaged AT series differs from repository")
    ...
    if len(entries) != 107 or not entries[-1].endswith("0107-regulator-da9211-use-write-only-legacy-page-selector.patch"):
        fail("AT series does not end in patch 0107")
    if any(pathlib.PurePosixPath(entry).name.startswith("0093-") for entry in entries):
        fail("AT series selects the forbidden active-power patch")
    required_entries = {
    ...
    if not required_entries.issubset(entries):
        fail("AT series omits a required legacy DA9214 patch")
    ...
    for marker in REQUIRED_IMAGE_MARKERS:
        if marker not in image:
            fail(f"kernel Image lacks required AT marker: {marker!r}")
    ...
    for marker in (
        ...
        if marker not in system_map:
            fail(f"System.map lacks required AT symbol: {marker!r}")

    print("validation=candidate-at-package")
