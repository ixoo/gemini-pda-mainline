#!/usr/bin/env python3
"""Apply deterministic MT6797 A72 platform-snapshot observer edits."""
...
config PSTORE_GEMINI_A72_PLATFORM_SNAPSHOT_LEDGER
	bool "Gemini A72 platform-snapshot retained ledger"
	...
	default n
	help
	  Give the candidate-only platform-snapshot observer one retained record
	  immediately before its first platform read and one only after one stable
	  two-sample result. Require both raw headers to be all ones before writing.
	  ...
	  provider, clock, BigiDVFS, owner, publisher, or CPU action. If unsure,
	  say N.
...
''').lstrip("\n")
    replace_once(
        ...
        mode + "config PSTORE_GEMINI_A72_PHYSICAL_SOURCE_LEDGER\n",
    )
    replace_once(
        kconfig,
...
static const char * const gemini_prb_records[] = {
	...
	"checkpoint=before-platform slot=1 crc32=a8bf2262\n",
	"====0.000000-D\n"
	"GEMINI_A72_PLATFORM_SNAPSHOT_V1 token=GAPS-20260824-A "
	"checkpoint=after-platform slot=2 crc32=ca566ccf\n",
};
...
''').lstrip("\n")
    ...
    if text.count(raw_anchor) != 2:
        raise SystemExit("ledger: expected two raw-record conditionals")
...
config MTK_MT6797_A72_PLATFORM_SNAPSHOT_OBSERVER
	bool "MediaTek MT6797 A72 one-shot platform-snapshot observer"
...
''').lstrip("\n")
    replace_once(
        ...
        observer + "config MTK_MT6797_A72_PHYSICAL_SOURCE_OBSERVER\n",
    )
    ...
    for name in (
        "mt6797-a72-platform-snapshot-observer.c",
        "mt6797-a72-platform-snapshot-observer-internal.h",
    ):
        copy_source(source_dir, name, root / "drivers/soc/mediatek" / name)


def apply_binding(root: Path, source_dir: Path) -> None:
    name = "mediatek,mt6797-a72-platform-snapshot-observer.yaml"
    copy_source(
        source_dir,
        name,
...
config MTK_MT6797_A72_PLATFORM_SNAPSHOT_KUNIT_TEST
	bool "KUnit tests for MT6797 A72 platform-snapshot observer"
	depends on KUNIT=y
...
''').lstrip("\n")
    replace_once(
        kconfig,
        "config MTK_MT6797_A72_PHYSICAL_SOURCE_OBSERVER\n",
        tests + "config MTK_MT6797_A72_PHYSICAL_SOURCE_OBSERVER\n",
    )
    makefile = root / "drivers/soc/mediatek/Makefile"
    replace_once(
        makefile,
        ...
        "obj-$(CONFIG_MTK_MT6797_A72_PLATFORM_SNAPSHOT_OBSERVER) += "
        "mt6797-a72-platform-snapshot-observer.o\n"
        "obj-$(CONFIG_MTK_MT6797_A72_PLATFORM_SNAPSHOT_KUNIT_TEST) += "
        "mt6797-a72-platform-snapshot-observer-test.o\n",
    )
    name = "mt6797-a72-platform-snapshot-observer-test.c"
    copy_source(source_dir, name, root / "drivers/soc/mediatek" / name)
...
def main() -> None:
    ...
    parser.add_argument(
        "--phase",
        choices=("ledger", "observer", "binding", "tests"),
        required=True,
    )
