#!/usr/bin/env python3
"""Classify the exact hardware-free A72 platform-snapshot KUnit suite."""
...
PROFILE = "a72-platform-snapshot-kunit"
SUITE = "mt6797-a72-platform-snapshot"
PANIC_PREFIX = "Kernel panic - not syncing: VFS: Unable to mount root fs"
PANIC_END_PREFIX = f"---[ end {PANIC_PREFIX}"
EXPECTED_CASES = (
    "mt6797_platform_snapshot_success_test",
    "mt6797_platform_snapshot_before_failure_test",
    "mt6797_platform_snapshot_read_failure_test",
    "mt6797_platform_snapshot_after_failure_test",
)
...
def classify_runtime(raw: str, expected_release: str, qemu_exit: int) -> None:
    ...
    require(ktap.count("1..4") == 1, "focused test plan changed")
    ...
    suite_summary = f"# {SUITE}: pass:4 fail:0 skip:0 total:4"
    totals = "# Totals: pass:4 fail:0 skip:0 total:4"
...
def main() -> None:
    ...
    print("experiment=2026-08-24-mainline-a72-platform-snapshot-first-read")
    ...
    print("platform_snapshot_symbols=present")
    ...
    for case in EXPECTED_CASES:
        print(f"{case}=pass")
    print("tap_summary=pass:4_fail:0_skip:0_total:4")
