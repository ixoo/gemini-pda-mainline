...
# Bounded read-only live probe for the exact platform-snapshot candidate.
set -eu
export LC_ALL=C
BB=/bin/busybox

readonly INSTALLED_FULL_SHA256=39f801f713a76c616ed8d9282fc0a662fb34c5a766d6839e4c47c757638bae43
readonly TAG=GEMINI_A72_PLATFORM_SNAPSHOT_V1
...
$BB printf 'snapshot_observer_devices='; $BB find /sys/bus/platform/devices -maxdepth 1 -name '*a72-platform-snapshot-observer*' 2>/dev/null | $BB wc -l
$BB printf 'snapshot_observer_bound='; if $BB test -L /sys/bus/platform/devices/a72-platform-snapshot-observer/driver; then $BB printf '1\n'; else $BB printf '0\n'; fi
...
$BB printf 'snapshot_failure_lines='; $BB dmesg 2>/dev/null | $BB grep -aFc 'platform snapshot failed' || true
