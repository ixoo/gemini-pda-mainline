...
abort()
{
	printf 'of_modalias_result=FAIL\nfailure=%s\n' "$1"
	exit 1
}
...
[ "$(/bin/busybox uname -r)" = 7.1.3-gemini-da921x-ofalias ] ||
...
status="$(/bin/busybox cat "$i2c6/handoff_status")" ||
	abort status-read
...
# The read-only modalias attribute intentionally remains OF-derived. The
# diagnostic changes only the add-event environment, proven by its kernel log.
...
/bin/busybox dmesg |
	...
	abort suppression-marker-absent

printf 'kernel=7.1.3-gemini-da921x-ofalias\n'
printf 'adapter=%s\nclient=%s\n' "$adapter" "${client##*/}"
printf 'client_name=da9214-legacy\n'
printf 'client_of_node=present\n'
...
printf 'of_modalias_result=PASS\n'
