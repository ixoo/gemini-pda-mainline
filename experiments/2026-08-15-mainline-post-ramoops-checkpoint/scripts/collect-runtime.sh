...
# Wait for the Gemini USB gadget and stream the exact checkpoint probe into its
# netcat shell. Raw capture stays below ignored artifacts/.
set -euo pipefail
export LC_ALL=C
umask 077
...
readonly PROBE_SHA256=832aa3dfecc4d03101cc1c21d3bae3190ea0d07becfb777d5b29213bdb9b8347
readonly CANDIDATE_SHA256=ae6b354d51a9e5096b9f6f74ee9037c47ba026e00895e6f4c8028f15bc9bd348

die() { printf 'error: %s\n' "$*" >&2; exit 2; }
usage() {
	printf 'usage: %s --output artifacts/runtime-captures/post-ramoops-checkpoint-attempt-N/runtime.txt\n' "$0" >&2
}
...
[[ "$(dirname -- "$capture_dir")" == "$private_root" &&
	"$(basename -- "$capture_dir")" == post-ramoops-checkpoint-attempt-* &&
	"$(basename -- "$output")" == runtime.txt ]] ||
	die 'output must be runtime.txt in one new post-ramoops-checkpoint-attempt-* private child'
...
command_file="$(mktemp "${TMPDIR:-/tmp}/.post-ramoops-checkpoint.XXXXXXXX")"
...
{
	printf '__POST_RAMOOPS_CHECKPOINT_HOST_BEGIN__\n'
	...
	printf '__POST_RAMOOPS_CHECKPOINT_HOST_END__\n'
} >"$output"
chmod 0600 "$output"
...
printf '%s\n' "$classification" | grep -E \
	'^(runtime_classification|runtime_reason|post_ramoops_checkpoint|cpu8_cpu9_admission|claim_scope)=' \
