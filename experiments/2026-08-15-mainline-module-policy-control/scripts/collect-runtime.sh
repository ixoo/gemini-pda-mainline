...
# Wait for the Gemini USB gadget and stream the exact module-policy control
# probe into its netcat shell. Raw capture stays below ignored artifacts/.
set -euo pipefail
export LC_ALL=C
umask 077
...
readonly WAIT_SECONDS=3600
readonly PROBE_SHA256=55c5b01fb3cb70e194701f2bb32b5fcae7a311aabd868ba3f16b52ca5d1a423d
readonly CANDIDATE_SHA256=044461e57d207f5ddd6e68cc463ea3ee1dd65260c27afe5fd00730137d13a2ff

die() { printf 'error: %s\n' "$*" >&2; exit 2; }
usage() {
	printf 'usage: %s --output artifacts/runtime-captures/module-policy-control-attempt-N/runtime.txt\n' "$0" >&2
}
...
[[ "$(dirname -- "$capture_dir")" == "$private_root" &&
	"$(basename -- "$capture_dir")" == module-policy-control-attempt-* &&
	"$(basename -- "$output")" == runtime.txt ]] ||
	die 'output must be runtime.txt in one new module-policy-control-attempt-* private child'
...
command_file="$(mktemp "${TMPDIR:-/tmp}/.module-policy-control.XXXXXXXX")"
...
for ((attempt = 0; attempt < WAIT_SECONDS; attempt++)); do
	# shellcheck disable=SC2046 # macOS ifconfig -l is a space-separated inventory.
...
{
	printf '__DA921X_MODULE_POLICY_CONTROL_HOST_BEGIN__\n'
	...
	printf '__DA921X_MODULE_POLICY_CONTROL_HOST_END__\n'
} >"$output"
chmod 0600 "$output"
...
printf '%s\n' "$classification" | grep -E \
	'^(runtime_classification|runtime_reason|module_policy_control|cpu8_cpu9_admission|claim_scope)=' \
