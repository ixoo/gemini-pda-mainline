#!/usr/bin/env python3
"""Classify one exact DA921x module-policy serviceability capture."""
...
BEGIN = "__DA921X_MODULE_POLICY_CONTROL_RUNTIME_BEGIN__"
END = "__DA921X_MODULE_POLICY_CONTROL_RUNTIME_END__"
DMESG_BEGIN = "__DA921X_MODULE_POLICY_CONTROL_DMESG_BEGIN__"
DMESG_END = "__DA921X_MODULE_POLICY_CONTROL_DMESG_END__"
CANDIDATE = "044461e57d207f5ddd6e68cc463ea3ee1dd65260c27afe5fd00730137d13a2ff"
RELEASE = "7.1.3-gemini-da921x-modctl"
...
def classify_text(text: str) -> tuple[str, str]:
    ...
    if failure_lines:
        reject("module-policy-control-failure", "provider-probe-failure")
    if len(identity_lines) != 1:
        reject("module-policy-control-failure", "non-unique-provider-identity")
    if values.get("bound_i2c_paths") != "1-0068":
        reject("module-policy-control-failure", "provider-driver-binding-missing")
    if set(values.get("regulator_names", "").split()) != {
        "DA9213-legacy-BUCK0", "DA9213-legacy-BUCK1"
    }:
        reject("module-policy-control-failure", "read-only-regulator-pair-missing")
    return "success-module-policy-control", "exact-module-policy-control-serviceable"
...
def main() -> int:
    ...
    print(f"module_policy_control={'accepted' if result == 'success-module-policy-control' else 'not-accepted'}")
    print("cpu8_cpu9_admission=closed")
    print("claim_scope=module-policy-serviceability-control")
    return 0 if result == "success-module-policy-control" else 3
