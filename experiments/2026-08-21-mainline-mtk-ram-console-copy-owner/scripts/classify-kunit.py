#!/usr/bin/env python3
"""Classify the focused MediaTek retained-console reader KUnit/QEMU proof."""
...
PROFILE = "mtk-ram-console-reader-kunit"
SUITE = "mtk-ram-console-reader"
PANIC_PREFIX = "Kernel panic - not syncing: VFS: Unable to mount root fs"
PANIC_END_PREFIX = f"---[ end {PANIC_PREFIX}"
EXPECTED_CASES = (
    "mtk_ram_console_reader_invalid_arguments_test",
    "mtk_ram_console_reader_exact_copy_test",
    "mtk_ram_console_reader_copy_failure_test",
    "mtk_ram_console_reader_parser_failure_test",
    "mtk_ram_console_reader_second_capture_test",
    "mtk_ram_console_reader_source_independence_test",
    "mtk_ram_console_reader_every_bit_test",
)
...
def classify_runtime(raw: str, expected_release: str, qemu_exit: int) -> None:
    ...
    require(ktap.count("1..7") == 1, "focused test plan changed")
...
def main() -> None:
    ...
    print("vcpus=4")
    print("network=none")
    print("physical_mapping=not-executed-dt-consumer-disabled")
    print("suites=1")
    print("tests=7")
    print("failed=0")
    print("skipped=0")
