#!/usr/bin/env python3
"""Classify the two hardware-free CPU-status-mask KUnit suites."""
...
EXPECTED_RELEASE = "7.1.3-gemini-a72-cpumask-kunit"
SUITES = (
    ("mt6797-a72-platform-state-movement", (
        ...
        "mt6797_state_each_a72_identity_bit_test",
...
def manifest_checksum(file_path: Path, entry: str) -> str:
    matches = re.findall(
        rf"^([0-9a-f]{{64}})  {re.escape(entry)}$",
        file_path.read_text(encoding="utf-8"), re.MULTILINE,
    )
...
def classify_runtime(raw: str, expected_release: str, qemu_exit: int) -> None:
    ...
    require(ktap.count("1..6") == 1, "platform-state plan changed")
...
def main() -> None:
    ...
    for file_path in (build_path, image, config, system_map, sums, raw_log):
        require(file_path.is_file() and not file_path.is_symlink(),
                f"required regular file absent or unsafe: {file_path.name}")
    ...
    print("experiment=2026-08-26-mainline-a72-cpu-status-mask-repair")
    ...
    for _, cases in SUITES:
        for case in cases:
            print(f"{case}=pass")
    print("emitted_suite_totals=pass:6_fail:0_skip:0_total:6,"
