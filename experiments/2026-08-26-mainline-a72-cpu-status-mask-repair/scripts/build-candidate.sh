...
# Source-pin the movement-attribution assembler and retarget only the exact
# package, release, status-mask result, and output identities.
set -euo pipefail
export LC_ALL=C
umask 077

readonly SOURCE_SHA256=71f4ecb8900b4a8179aa424706c7e95d9d04f9ecb32af9eaa2b0f7946c6986d6
die() { printf 'error: %s\n' "$*" >&2; exit 2; }
...
source_builder="$repo_root/experiments/2026-08-26-mainline-a72-platform-movement-attribution/scripts/build-candidate.sh"
...
derived=$(mktemp "$script_dir/.derived-build-cpu-status-mask.XXXXXXXX")
...
replacements = (
    ...
    ("f1e98614c54bb063343e5d3b396d2a4f5f1d3fe283efd3782a41dee1dede754f", "84a7a961aacb9136f4d08223ea6a5519d2bc277a0fc30916c0b96170d470ae9e", 1),
    ...
    ("candidate-a72-platform-movement-", "candidate-a72-cpu-status-mask-", 1),
    ("2026-08-26-mainline-a72-platform-movement-attribution", "2026-08-26-mainline-a72-cpu-status-mask-repair", 1),
    ("one-platform-movement-mask-from-existing-two-sample-pair", "a72-identity-mask-on-existing-two-sample-platform-pair", 1),
    ("dtb_delta_from_retired_failure_stage=none-byte-identical", "dtb_delta_from_movement_control=none-byte-identical", 1),
)
for old, new, count in replacements:
    actual = text.count(old)
    if actual != count:
        raise SystemExit(
            f"unsafe CPU-status-mask builder derivation: expected {count}, found {actual}: {old}"
        )
