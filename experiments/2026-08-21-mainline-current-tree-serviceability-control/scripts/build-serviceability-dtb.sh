...
# Derive the last proven serviceability/resource contract from the exact
# current-tree package DTB while preserving every unrelated current DT input.
set -euo pipefail
export LC_ALL=C
umask 077
...
readonly OUTPUT_DTB_SHA256=b638674b9be209219d51b7dd02538f7a0bc8b402bab7336188cb95011cd912dd
...
[[ "$(fdtget -ts "$base_dtb" "$CLOCK_BACKEND" status)" == disabled ]] ||
	die 'clock backend base status changed'
[[ "$(fdtget -ts "$base_dtb" "$BIGIDVFSP_BACKEND" status)" == disabled ]] ||
	die 'BigiDVFS backend base status changed'
[[ "$(fdtget -ts "$base_dtb" "$RAM_CONSOLE" status)" == disabled ]] ||
	die 'ram-console reader base status changed'
if fdtget "$base_dtb" "$SCP" compatible >/dev/null 2>&1; then
	die 'base unexpectedly contains the LK SCP input node'
fi

temporary="$(mktemp "$(dirname -- "$output")/.current-service-dtb.XXXXXXXX")"
...
fdtput -tx "$temporary" "$HANDOFF" reg \
	0 11015000 0 1000 \
	0 100a0000 0 1000 \
	0 1000e000 0 1000
fdtput -ts "$temporary" "$HANDOFF" reg-names cspm scp-cfg devapc-ao
...
[[ "$(fdtget -ts "$temporary" "$HANDOFF" reg-names)" == 'cspm scp-cfg devapc-ao' ]] ||
	die 'handoff register names changed'
[[ "$(fdtget -tx "$temporary" "$HANDOFF" reg)" == \
	'0 11015000 0 1000 0 100a0000 0 1000 0 1000e000 0 1000' ]] ||
	die 'handoff register windows changed'
[[ "$(fdtget -ts "$temporary" "$CLOCK_BACKEND" status)" == disabled ]] ||
	die 'clock backend closure changed'
[[ "$(fdtget -ts "$temporary" "$BIGIDVFSP_BACKEND" status)" == disabled ]] ||
	die 'BigiDVFS backend closure changed'
[[ "$(fdtget -ts "$temporary" "$RAM_CONSOLE" status)" == disabled ]] ||
	die 'ram-console reader closure changed'

output_sha256="$(sha256sum "$temporary" | awk '{print $1}')"
[[ "$output_sha256" == "$OUTPUT_DTB_SHA256" ]] || die 'derived DTB identity changed'
mv "$temporary" "$output"
chmod 0600 "$output"
temporary=
trap - EXIT HUP INT TERM

printf 'validation=current-tree-serviceability-dtb\n'
printf 'base_dtb_sha256=%s\noutput_dtb_sha256=%s\n' \
	"$BASE_DTB_SHA256" "$output_sha256"
printf 'current_tree_inputs=preserved\nserviceability_property_mutations=20\n'
printf 'serviceability_nodes_added=1\nhandoff_reg_names=cspm,scp-cfg,devapc-ao\n'
printf 'clock_backend_status=disabled\nbigidvfs_backend_status=disabled\n'
printf 'ram_console_reader_status=disabled\nCPU8_CPU9_admission=closed\n'
