#!/usr/bin/env python3
"""Mutation tests for the platform-effect QEMU classifier."""
...
SPEC = importlib.util.spec_from_file_location(
    "platform_effect_classifier", SCRIPT_DIR / "classify-kunit.py")
...
def transcript() -> str:
    ...
    return f"""Linux version 7.1.3-gemini-a72-platform-effect-kunit (builder)
KTAP version 1
1..1
    KTAP version 1
...
def rejected(raw: str, release: str =
             "7.1.3-gemini-a72-platform-effect-kunit",
...
def main() -> None:
    ...
    CLASSIFIER.classify_runtime(
        raw, "7.1.3-gemini-a72-platform-effect-kunit", 124)
    mutations = (
        raw.replace("ok 1 mt6797_platform_effect_success_test",
                    "not ok 1 mt6797_platform_effect_success_test", 1),
        raw.replace("    1..8", "    1..7", 1),
        raw.replace(CLASSIFIER.EXPECTED_CASES[3], "renamed_case", 1),
        raw.replace("pass:8 fail:0", "pass:7 fail:1", 1),
        raw.replace("# Subtest: mt6797-a72-platform-effects",
    ...
    print("validation=platform-effect-kunit-classifier")
