...
# Source-pin the durable admission recovery collector and retarget only the
# installed live-trigger candidate, experiment identity, and private work name.
set -euo pipefail
export LC_ALL=C
umask 077
...
derived=$(mktemp "$script_dir/.derived-collect-pretrigger-recovery.XXXXXXXX")
...
replacements = (
    ...
    ("experiment=2026-08-28-mainline-a72-admission-durable-candidate",
     "experiment=2026-08-28-mainline-a72-admission-live-trigger", 1),
    (".a72-admission-recovery.XXXXXXXX",
     ".a72-admission-live-pretrigger-recovery.XXXXXXXX", 1),
)
for old, new, count in replacements:
    actual = text.count(old)
    if actual != count:
        raise SystemExit(
            f"unsafe pre-trigger recovery derivation: expected {count}, "
            f"found {actual}: {old}"
        )
