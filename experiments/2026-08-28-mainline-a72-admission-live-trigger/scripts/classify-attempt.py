#!/usr/bin/env python3
"""Classify one exact live-trigger transcript after an accepted live frame."""
...
def classify(pretrigger: str, trigger: str) -> tuple[str, str]:
    ...
    ret_text, consumed, requests = terminal.groups()
    ret = int(ret_text)
    ...
    if ret == 0 and consumed == "1" and requests == "1" and online == "0-8" and offline == "9":
        return "cpu8-online", "terminal-success-and-live-cpu-list"
    if (online == "0-7" and offline == "8-9" and ret != 0 and
            (consumed, requests) in (("0", "0"), ("1", "0"), ("1", "1"))):
        return "terminal-admission-error", f"ret={ret_text}-consumed={consumed}-requests={requests}"
