...
# Source-pin the passed container assembler and retarget every exact package,
# DT, configuration, marker, and candidate identity for the third reader.
set -euo pipefail
export LC_ALL=C
umask 077
...
derived=$(mktemp "$script_dir/.derived-build-a72-platform-provider-clock.XXXXXXXX")
...
grep -q '^CONFIG_CMDLINE=".*maxcpus=8.*"$' "$config" || die 'exact maxcpus=8 closure absent'
for token in \
	CONFIG_MTK_MT6797_A72_PLATFORM_PROVIDER_SNAPSHOT_OBSERVER \
	CONFIG_PSTORE_GEMINI_A72_PLATFORM_PROVIDER_SNAPSHOT_LEDGER \
	...
	CONFIG_MTK_MT6797_I2C6_FW_WRITER_TRANSACTION_WINDOW \
	CONFIG_REGULATOR_DA9213_LEGACY_SAME_VALUE_WRITE; do
	! grep -qx "$token=y" "$config" || die "physical or later path enabled: $token"
done
for symbol in mt6797_a72_platform_state_snapshot mt6797_a72_provider_snapshot \
	da9213_provider_snapshot mt6797_dvfsp_clock_backend_read \
	mt6797_dvfsp_clock_backend_probe mt6797_bigidvfs_backend_probe; do
	grep -q " ${symbol}$" "$system_map" || die "required symbol absent: $symbol"
done
for symbol in da9213_legacy_provider_write_cont \
	da9213_legacy_provider_transaction_acquire \
	da9213_legacy_provider_transaction_release da9213_legacy_same_value_write \
	mt6797_a72_atomic_publish mt6797_a72_a34_evaluate; do
	! grep -q " ${symbol}$" "$system_map" || die "writer or later symbol linked: $symbol"
done'''
...
new_markers = r'''for marker in \
	'GEMINI_A72_PLATFORM_PROVIDER_CLOCK_SNAPSHOT_V1 token=GAPC-20260825-A checkpoint=before-clock slot=1 crc32=7a63713c' \
	'GEMINI_A72_PLATFORM_PROVIDER_CLOCK_SNAPSHOT_V1 token=GAPC-20260825-A checkpoint=after-clock slot=2 crc32=5773d4f6' \
	'GEMINI_A72_PLATFORM_PROVIDER_CLOCK_SNAPSHOT_V1 platform valid=%u spm=%08x/%08x/%08x/%08x mp2=%08x/%08x/%08x iso=%08x dcm=%08x cci=%08x/%08x/%08x pwrap=%u' \
	'GEMINI_A72_PLATFORM_PROVIDER_CLOCK_SNAPSHOT_V1 provider abi=%u valid=%u raw=%02x/%02x/%02x/%02x/%02x' \
	'GEMINI_A72_PLATFORM_PROVIDER_CLOCK_SNAPSHOT_V1 clock ret=%d abi=%u generation=%llu muxsel=%08x ckdiv=%08x pll_ll=%08x/%08x/%08x pll_l=%08x/%08x/%08x pll_cci=%08x/%08x/%08x cspm_swctrl=%08x/%08x/%08x cspm_hwsta=%08x/%08x/%08x/%08x' \
	'GEMINI_A72_PLATFORM_PROVIDER_CLOCK_SNAPSHOT_V1 state=complete provider_ready_gate=passed clock_ready_gate=passed valid=%u clock_returned=%u after_checkpoint=%u platform_calls=1 platform_samples=2 platform_register_observations=26 provider_snapshots=1 provider_samples=2 provider_i2c_reads=10 provider_i2c_writes=0 retained_write_attempts=2 protected_clock_calls=1 protected_clock_ret=%d protected_clock_abi=%u protected_clock_generation=%llu clock_gate_pairs=1 explicit_mmio_writes_maximum=401 explicit_mmio_reads_maximum=419 observer_retries=0 bigidvfs_reads=0 secure_calls=0 provider_acquires=0 provider_releases=0 publisher_calls=0 owner_mutations=0 cpu_requests=0'; do'''
replacements = (
    ("2dd7b176a2e54e086a0d7acd689e1aa330a4c358", "5e4b0d584f76d4bf5a5e7e924b886d6b65ed4bd5", 1),
    ("readonly PROFILE=a72-platform-snapshot-candidate", "readonly PROFILE=a72-platform-provider-clock-candidate", 1),
    ...
    ("9645c8e7a9f85e0f9550223937b43832b2496bcd1a864f7d3fa7c20ad2cfb526", "fc28c627cacc5c234937d85d8d3e5a342c66a2f35a3903a618a2ec1d741fedf0", 1),
    ...
    ("readonly BOOT_NAME=gemini-a72snap", "readonly BOOT_NAME=gemini-a72pclk", 1),
    ...
    (".a72-platform-snapshot-candidate.XXXXXXXX", ".a72-platform-provider-clock-candidate.XXXXXXXX", 1),
    ("portable-fetched-a72-platform-snapshot-candidate", "portable-fetched-a72-platform-provider-clock-candidate", 1),
    ("experiment=2026-08-24-mainline-a72-platform-snapshot-first-read", "experiment=2026-08-25-mainline-a72-platform-provider-protected-clock-third-read", 1),
    ("one-stable-platform-snapshot-on-passed-three-backend-baseline", "one-protected-clock-snapshot-after-passed-platform-provider-prefix", 1),
    ("dtb_delta_from_passed_bigidvfs=observer-node-plus-source-phandle", "dtb_delta_from_provider_ready=replace-observer-plus-clock-phandle", 1),
    ...
    ("provider_snapshots=0\\nprotected_clock_reads=0", "provider_snapshots=1\\nprovider_samples=2\\nprovider_i2c_reads=10\\nprovider_i2c_writes=0\\nprotected_clock_calls=1\\nclock_gate_pairs=1\\nexplicit_mmio_writes_maximum=401\\nexplicit_mmio_reads_maximum=419", 1),
    ("candidate-a72-platform-snapshot-${RAW_SHA256:0:8}", "candidate-a72-platform-provider-clock-${RAW_SHA256:0:8}", 1),
    ("validation=a72-platform-snapshot-candidate-build", "validation=a72-platform-provider-clock-candidate-build", 1),
)
for old, new, count in replacements:
    actual = text.count(old)
    if actual != count:
        raise SystemExit(
            f"unsafe platform/provider/clock builder derivation: expected {count}, found {actual}: {old}"
        )
