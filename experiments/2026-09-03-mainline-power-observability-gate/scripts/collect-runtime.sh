...
# Capture one redacted, read-only current-candidate observability frame.
set -euo pipefail
export LC_ALL=C
umask 077
...
readonly REMOTE_SHA256=54b7fbd6b9d39fa2b9a5f15275d7dd33546798b6445cd6df359cb176c49ba8c1
readonly CLASSIFIER_SHA256=225f08a9e85c7640127e3ab1fa84dab832a745dada686f9a6f45dc371ec42e5a
readonly COMMAND_MARKER=__GEMINI_POWER_OBSERVABILITY_SCRIPT__
...
while (($#)); do
	case "$1" in
	--recovery-boot-id)
		(($# >= 2)) || die '--recovery-boot-id requires UUID'
		recovery_boot_id=$2
		shift 2
		;;
	--output)
		...
		shift 2
		;;
	*) die "usage: $0 --recovery-boot-id UUID --output artifacts/runtime-captures/power-observability-attempt-1" ;;
	esac
done
...
remote="$script_dir/remote-observe.sh"
classifier="$script_dir/classify-observation.py"
...
[[ "$(basename -- "$output")" == power-observability-attempt-1 ]] || die 'output directory identity changed'
...
command_file=$(mktemp "${TMPDIR:-/tmp}/.gemini-power-observer.XXXXXXXX")
...
printf 'observer=armed\nrecovery_boot_id=%s\narmed_utc=%s\n' \
	"$recovery_boot_id" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >"$events"
...
set -e
chmod 0600 "$frame"
grep -Fq __GEMINI_POWER_OBSERVABILITY_BEGIN__ "$frame" || die "observation frame did not begin rc=$nc_rc"
grep -Fq __GEMINI_POWER_OBSERVABILITY_END__ "$frame" || die "observation frame did not complete rc=$nc_rc"
python3 "$classifier" "$frame" --recovery-boot-id "$recovery_boot_id" >"$classification"
grep -Fqx 'classification=calibration-provider-bound-power-observability-absent' "$classification" || die 'observation frame rejected'
