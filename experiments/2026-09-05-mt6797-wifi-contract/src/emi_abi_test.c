...
int main(void)
{
	...
	assert(!mt6797_emi_prepare(&owner, 0x80000000, 0x8007ffff, 0xb6da2d, &args));
	...
	assert(args.range_word == 0x80008007U && args.region_permission == 0x90000000U);
	...
	/* Full normalized domain edges, including a representable original
	 * address above 4 GiB only when subtraction makes it non-aliasing.
	 */
	for (selector = 1; selector <= 2; selector++) {
		...
		assert(!mt6797_emi_prepare(&owner, base, base + 0xffffffffULL, 0, &args));
		...
		refused(&owner, base + 0x100000000ULL, base + 0x10000ffffULL, 0);
		owner.end++;
		refused(&owner, base, base + 0xffff, 0); /* invalid owner, even valid request */
	...
	for (i = 0; i < sizeof(results) / sizeof(results[0]); i++) {
		struct mt6797_emi_result result = mt6797_emi_decode_result(results[i].raw);

		assert(result.raw == results[i].raw && result.status == results[i].status);
	}
