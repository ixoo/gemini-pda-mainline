...
[CONN power-domain contract](POWER_DOMAIN.md). The reviewed
[deferred-registration kernel proposal](DEFERRED_REGISTRATION.md) implements
provider-probe activation suppression; no SoC selects it yet.
...
| Regulatory/calibration | Vendor test library depends on NVRAM; calibration was excluded from the [July firmware-file inventory](../2026-07-11-gemian-firmware-inventory/README.md), while the later [private partition backup](../2026-07-14-mmc-partition-backup/README.md) includes NVRAM/configuration/protection partitions | Analyze applicable retained captures after exact identity, integrity and access checks; establish mandatory calibration/regulatory inputs without exposing identifiers or inventing defaults. No new capture is presumed necessary, and RF admission still requires this technical contract |
...
The [integration review](INTEGRATION_REVIEW.md) records the later Python 3.5
