...
class GroupOnlyMatch:
    """Python 3.5 regex matches expose group(), without subscripting."""
...
class GroupOnlyPattern:
    def __init__(self, pattern):
        self.pattern = pattern

    def fullmatch(self, value):
        match = self.pattern.fullmatch(value)
        return GroupOnlyMatch(match) if match is not None else None


def python35_matches():
    return mock.patch.multiple(wifi, **{
        name: GroupOnlyPattern(getattr(wifi, name))
        for name in ("HEX", "ALIAS", "SDIO_NAME")
    })
...
class InventoryTests(unittest.TestCase):
    ...
    def test_python35_match_api_with_populated_sdio(self):
        self.fixture.sdio(as_wlan=True)
        with python35_matches():
            result = self.run_reader()
        self.assertEqual(result["classification"], "sdio_wifi_parent_observed")
        self.assertEqual(result["facts"]["sdio"]["functions"][0]["function"], 1)
        self.assertTrue(result["identity_checked_end"])

    def test_python35_match_api_with_conflicting_sdio_ids(self):
