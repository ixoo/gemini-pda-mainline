...
# Capture a one-way CPU8 terminal marker from the direct USB/netcat shell.
# This collector is read-only; pstore after the watchdog cycle remains primary.
set -euo pipefail
export LC_ALL=C
umask 077
...
die() { printf 'error: %s\n' "$*" >&2; exit 2; }
usage() {
	printf 'usage: %s --output artifacts/runtime-captures/a72-one-way-attempt-N/runtime.txt\n' "$0" >&2
}
...
[[ "$(dirname -- "$capture_dir")" == "$private_root" &&
	"$(basename -- "$capture_dir")" == a72-one-way-attempt-* &&
	"$(basename -- "$output")" == runtime.txt ]] ||
	die 'output must be runtime.txt in one new a72-one-way-attempt-* private child'
...
command_file="$(mktemp "${TMPDIR:-/tmp}/.a72-one-way-live.XXXXXXXX")"
cleanup() { [[ ! -e "${command_file:-}" ]] || rm -f -- "$command_file"; }
trap cleanup EXIT
chmod 0600 "$command_file"
cat >"$command_file" <<'DEVICE'
printf '__A72_ONE_WAY_LIVE_BEGIN__\n'
...
i=0
while [ "$i" -lt 30 ]; do
	line="$(/bin/busybox dmesg | /bin/busybox grep -F 'gemini-a72-oneway-v1 result=' | /bin/busybox tail -n 1)"
	if [ -n "$line" ]; then
		printf 'terminal_marker=present\n'
		...
		printf '__A72_ONE_WAY_LIVE_TERMINAL_CAPTURED__\n'
		while :; do /bin/busybox sleep 1; done
	fi
	i=$((i + 1))
	/bin/busybox sleep 1
done
printf 'terminal_marker=absent\n'
exit 93
DEVICE
...
{
	printf '__A72_ONE_WAY_HOST_BEGIN__\n'
	...
	printf '__A72_ONE_WAY_HOST_END__\n'
} >"$output"
chmod 0600 "$output"
...
grep -Fxq 'terminal_marker=present' "$output" || die 'no exact terminal marker was captured'
grep -Eq 'terminal_line=.*gemini-a72-oneway-v1 result=(rejected-prestate|rolled-back-preiso|fault-retain-preiso|fault-retain-postiso|cpu8-online-held) stage=[a-z0-9-]+ error=-?[0-9]+ cpu9=forbidden cpu_off=forbidden$' \
	"$output" || die 'captured terminal marker is malformed'
grep -Fxq '__A72_ONE_WAY_LIVE_TERMINAL_CAPTURED__' "$output" ||
	die 'terminal capture terminator is absent'
printf 'validation=a72-one-way-live-outcome-pass\ncapture=%s\n' "$output"
