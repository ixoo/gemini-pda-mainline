#!/usr/bin/env python3
"""Prove that the positive-provider classifier fails closed."""
...
SPEC = importlib.util.spec_from_file_location("positive_provider_classifier", SCRIPT)
if SPEC is None or SPEC.loader is None:
    raise SystemExit("cannot load classifier")
CLASSIFIER = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(CLASSIFIER)

RELEASE = "7.1.3-gemini-da921x-positive-provider-kunit"
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="gemini-positive-provider-") as temporary:
    ...
    print("validation=mainline-da921x-positive-provider-kunit-classifier")
