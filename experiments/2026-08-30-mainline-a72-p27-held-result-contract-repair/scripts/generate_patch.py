#!/usr/bin/env python3
"""Generate and audit the CPU8 P27 held-result contract patch."""
...
PATCH_NAME = "0450-soc-mediatek-accept-held-P27-acquire-result.patch"
SUBJECT = "soc: mediatek: accept held P27 acquire result"
    ...
    environment.update(
        {
            ...
            "GIT_AUTHOR_DATE": f"2026-08-31T04:{minute:02d}:00Z",
            "GIT_COMMITTER_DATE": f"2026-08-31T04:{minute:02d}:00Z",
        }
    )
...
def prepare_parent(source_root: Path, root: Path) -> None:
    ...
    commit(root, "Gemini post-0449 generation parent", "Synthetic parent only.", 0)
...
def validate(root: Path, parent_counts: dict[str, int]) -> list[str]:
    ...
    if "result->sealed = state->malformed != TEST_MALFORMED_P27;" in test:
        raise SystemExit("historical sealed-success fake remains")
    return [
        "p27_held_result_contract_validation=pass",
        "production_acquire_expected_sealed=false",
        "release_expected_sealed=true",
        "later_effects_expected_sealed=true",
        "success_fake_matches_production_held_state=true",
        "malformed_sealed_acquire_rejected=true",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="a72-p27-held-result-") as name:
        ...
        provenance.write_text(
            "\n".join(
                [
                    "experiment=2026-08-30-mainline-a72-p27-held-result-contract-repair",
                    f"repository_commit={args.repository_commit}",
                    f"prepared_source_state={state}",
                    f"prepared_source_integrity={integrity}",
                    "canonical_parent=0449",
