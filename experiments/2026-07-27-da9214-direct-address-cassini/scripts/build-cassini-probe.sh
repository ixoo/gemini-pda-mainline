...
# Build the fixed-function Cassini helper as a deterministic static AArch64 ELF.
set -euo pipefail
export LC_ALL=C
...
die() { printf 'error: %s\n' "$*" >&2; exit 2; }
[[ $# -eq 2 ]] || die 'usage: build-cassini-probe.sh SOURCE OUTPUT'
source_file=$1
output=$2
...
validator="$script_dir/validate-cassini-probe.py"
[[ -f "$validator" && ! -L "$validator" && -s "$validator" ]] ||
	die 'probe validator is missing or unsafe'

temporary="$(mktemp "$output_parent/.cassini-probe.XXXXXX")"
...
printf 'validation=cassini-probe-built\n'
