#!/usr/bin/env python3
"""Validate Candidate P as an exact two-config-line kernel derivative."""
...
BASELINE_PROFILE = "observability"
CANDIDATE_PROFILE = "observability-fbcon-rotation"
BASELINE_FRAGMENTS = (
...
ROTATION_FRAGMENT = "configs/gemini-fbcon-rotation.fragment"
CANDIDATE_FRAGMENTS = BASELINE_FRAGMENTS + (ROTATION_FRAGMENT,)
EXPECTED_KERNEL_RELEASE = "7.1.3-gemini-observability-L"
EXPECTED_LOCALVERSION = 'CONFIG_LOCALVERSION="-gemini-observability-L"'
BASELINE_ROTATION = "# CONFIG_FRAMEBUFFER_CONSOLE_ROTATION is not set"
CANDIDATE_ROTATION = "CONFIG_FRAMEBUFFER_CONSOLE_ROTATION=y"
BASELINE_CMDLINE = (
...
CANDIDATE_CMDLINE = BASELINE_CMDLINE[:-1] + ' fbcon=rotate:3"'
RETAINED_CONFIG_LINES = (
...
def require_manifest_boundary(
    ...
    current_config = require_mapping(
        require_key(current, "config", "current manifest"), "current manifest config"
    )
    packaged_config = require_mapping(
        require_key(packaged, "config", label), f"{label} config"
    )
    current_profiles = require_mapping(
        require_key(current_config, "profiles", "current manifest config"),
        "current manifest profiles",
    )
    packaged_profiles = require_mapping(
        require_key(packaged_config, "profiles", f"{label} config"),
        f"{label} profiles",
    )
...
def decode_config(data: bytes, label: str) -> list[str]:
    try:
...
def main() -> int:
    ...
    parser.add_argument("--rotation-fragment", type=pathlib.Path, required=True)
    ...
    try:
        ...
        current_manifest_path = args.current_manifest.resolve(strict=True)
        current_manifest = load_json(current_manifest_path, "current manifest")
        ...
        current_config = require_mapping(
            require_key(current_manifest, "config", "current manifest"),
        ...
        if baseline_build.get("build_profile") != BASELINE_PROFILE:
            raise ValueError("baseline package is not the observability profile")
        if candidate_build.get("build_profile") != CANDIDATE_PROFILE:
            raise ValueError("candidate package is not the observability-fbcon-rotation profile")
        ...
        if baseline_build.get("config_fragments") != list(BASELINE_FRAGMENTS):
            raise ValueError("baseline fragment provenance is not exact observability")
        if candidate_build.get("config_fragments") != list(CANDIDATE_FRAGMENTS):
            raise ValueError("candidate fragment provenance is not exact rotation profile")
        for key in ("source_sha256", "patchset_sha256", "compiler", "linker"):
            baseline_value = require_key(baseline_build, key, "baseline build provenance")
            candidate_value = require_key(candidate_build, key, "candidate build provenance")
            if not isinstance(baseline_value, str) or not baseline_value:
                raise ValueError(f"baseline build provenance has invalid {key}")
            if not isinstance(candidate_value, str) or not candidate_value:
                raise ValueError(f"candidate build provenance has invalid {key}")
            if baseline_value != candidate_value:
                raise ValueError(f"package build provenance differs at {key}")
        if baseline_build.get("kernel_release") != EXPECTED_KERNEL_RELEASE:
            raise ValueError("baseline kernel release is not exact Candidate O")
        if candidate_build.get("kernel_release") != EXPECTED_KERNEL_RELEASE:
            raise ValueError("candidate kernel release changed")
        if baseline_build.get("modules_built") is not False or candidate_build.get(
            "modules_built"
        ) is not False:
            raise ValueError("modules_built is not exactly false")

        manifest_kernel = require_mapping(
            require_key(current_manifest, "kernel", "current manifest"),
            "current manifest kernel",
        )
        ...
        repo_root = current_manifest_path.parent.parent
        series_relative = require_key(current_manifest, "patch_series", "current manifest")
        ...
        for relative in BASELINE_FRAGMENTS:
            ...
            if (
                baseline_fragments[name] != current_bytes
                or candidate_fragments[name] != current_bytes
            ):
                raise ValueError(f"packaged fragment differs from current {relative}")

        rotation_path = args.rotation_fragment.resolve(strict=True)
        expected_rotation_path = (repo_root / ROTATION_FRAGMENT).resolve(strict=True)
        if rotation_path != expected_rotation_path:
            raise ValueError("--rotation-fragment is not the current pinned fragment")
        rotation_bytes = read(rotation_path, "fbcon-rotation fragment")
        rotation_name = pathlib.PurePosixPath(ROTATION_FRAGMENT).name
        if candidate_fragments.get(rotation_name) != rotation_bytes:
            raise ValueError("candidate fbcon-rotation fragment is not exact")
        fragment_text = rotation_bytes.decode("utf-8")
        fragment_requests = [
            line
            for line in fragment_text.splitlines()
            if line.startswith("CONFIG_") or line.startswith("# CONFIG_")
        ]
        expected_fragment_requests = {CANDIDATE_ROTATION, CANDIDATE_CMDLINE}
        if len(fragment_requests) != 2 or set(fragment_requests) != expected_fragment_requests:
            raise ValueError("fbcon-rotation fragment requests more than the exact two deltas")
        ...
        baseline_lines = decode_config(baseline_config, "baseline kernel config")
        candidate_lines = decode_config(candidate_config, "candidate kernel config")
        ...
        expected_differences = {
            (BASELINE_CMDLINE, CANDIDATE_CMDLINE),
            (BASELINE_ROTATION, CANDIDATE_ROTATION),
        }
        observed_differences = {(old, new) for _, old, new in differences}
        if observed_differences != expected_differences:
            raise ValueError("resolved config delta is not exact rotation plus command-line append")
        for line in RETAINED_CONFIG_LINES:
            if line not in baseline_lines or line not in candidate_lines:
                raise ValueError(f"required unchanged config line is missing: {line}")
        if BASELINE_ROTATION not in baseline_lines or CANDIDATE_ROTATION not in candidate_lines:
            raise ValueError("fbcon rotation state is not the exact disabled-to-built-in delta")
        if BASELINE_CMDLINE not in baseline_lines or CANDIDATE_CMDLINE not in candidate_lines:
            raise ValueError("forced command line is not the exact fbcon=rotate:3 append")
    ...
    delta_lines = ",".join(str(number) for number, _, _ in differences)
    print("validation=observability-fbcon-rotation-package-delta")
    ...
    print(f"resolved_config_delta_lines={delta_lines}")
    print("resolved_config_delta=FRAMEBUFFER_CONSOLE_ROTATION-y,CMDLINE-append-fbcon-rotate-3")
    print("config_cmdline_force=yes")
    print("localversion_unchanged=-gemini-observability-L")
    print("font_policy_unchanged=8x16")
    ...
    print("two_resolved_config_deltas=passed")
