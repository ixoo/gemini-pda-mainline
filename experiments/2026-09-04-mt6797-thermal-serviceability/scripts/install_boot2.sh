...
readonly CANDIDATE_SHA256=6f3d8d6e94ff1ce587f0189a0c44db2abc7a29f487f1ec33e66e9db5e3505801
...
while (($#)); do
	...
	--output)
		...
		shift 2
		;;
	*) die "usage: $0 --output artifacts/device-deployments/thermal-serviceability-attempt-1" ;;
	esac
done
...
candidate_dir="$repo_root/artifacts/mt6797-thermal-serviceability/$CANDIDATE_DIR"
...
[[ "$(basename -- "$output")" == thermal-serviceability-attempt-1 ]] || die 'output directory identity changed'
...
ssh "${ssh_options[@]}" "$TARGET" \
	'umask 077; rm -f /tmp/gemini-thermal-6f3d8d6e.img; cat > /tmp/gemini-thermal-6f3d8d6e.img' <"$candidate"
remote_script=$(mktemp "${TMPDIR:-/tmp}/.gemini-thermal-install.XXXXXXXX")
cleanup() {
	rm -f -- "${remote_script:-}"
	ssh -n "${ssh_options[@]}" "$TARGET" \
		'rm -f /tmp/gemini-thermal-6f3d8d6e.img' >/dev/null 2>&1 || true
}
trap cleanup EXIT HUP INT TERM
cat >"$remote_script" <<'REMOTE'
set -eu
export LC_ALL=C
candidate=/tmp/gemini-thermal-6f3d8d6e.img
expected_sha=6f3d8d6e94ff1ce587f0189a0c44db2abc7a29f487f1ec33e66e9db5e3505801
...
printf '%s\n' __GEMINI_THERMAL_DEPLOYMENT_BEGIN__
...
printf '%s\n' __GEMINI_THERMAL_DEPLOYMENT_END__
rm -f "$candidate"
sync
poweroff
REMOTE
...
ssh_rc=$?
set -e
grep -Fq __GEMINI_THERMAL_DEPLOYMENT_BEGIN__ "$evidence" || die "deployment did not begin rc=$ssh_rc"
grep -Fq __GEMINI_THERMAL_DEPLOYMENT_END__ "$evidence" || die "deployment did not complete rc=$ssh_rc"
