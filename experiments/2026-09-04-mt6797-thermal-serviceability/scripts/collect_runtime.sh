...
# Capture one bounded, read-only thermal-serviceability frame.
set -euo pipefail
export LC_ALL=C
umask 077
...
readonly REMOTE_SHA256=f65d773a18aa0d2aedc9ea2d938f8426514226192d7d8bb9e419901b1855f98e
readonly CLASSIFIER_SHA256=ad13cd7efe9ffaafab444fdadb417f8f67ca47924ba050aafc60fd5dbc7cfdbe
readonly COMMAND_MARKER=__GEMINI_THERMAL_SERVICEABILITY_SCRIPT__
...
while (($#)); do
	...
	--recovery-boot-id|--installed-full-sha256|--output)
		...
		shift 2
		;;
	*) die "usage: $0 --recovery-boot-id UUID --installed-full-sha256 SHA256 --output artifacts/runtime-captures/thermal-serviceability-attempt-1" ;;
	esac
done
...
command_file=$(mktemp "${TMPDIR:-/tmp}/.gemini-thermal-observer.XXXXXXXX")
...
set +e
nc -4 -b "$interface" -s "$HOST_ADDRESS" -G 5 -w 40 "$DEVICE_ADDRESS" "$DEVICE_PORT" <"$command_file" >"$frame" 2>&1
nc_rc=$?
set -e
chmod 0600 "$frame"
grep -Fq __GEMINI_THERMAL_SERVICEABILITY_BEGIN__ "$frame" || die "observation frame did not begin rc=$nc_rc"
grep -Fq __GEMINI_THERMAL_SERVICEABILITY_END__ "$frame" || die "observation frame did not complete rc=$nc_rc"
python3 "$classifier" "$frame" --recovery-boot-id "$recovery_boot_id" >"$classification"
grep -Fqx 'classification=mt6797-thermal-serviceability-pass' "$classification" || die 'observation frame rejected'
printf 'netcat_complete=yes status=%s\ntrigger_session=none\nload_session=none\n' "$nc_rc" >>"$events"
printf 'temperature_reads=3-read-only\nnative_reboot_command_sent=no\nsuccessful_mainline_left_running=yes\n' >>"$events"
