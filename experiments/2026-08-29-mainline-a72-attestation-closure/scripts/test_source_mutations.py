...
def mutations() -> list[tuple[str, Callable[[Path], None]]]:
    ...
    return [
        ...
        ("current-macro-parameter", lambda r: replace(r / proton, "u8 *value", "u8 *current")),
        ("collapse-vulnerable-state", lambda r: replace(r / proton, "*value = ARM64_LATE_CPU_MITIGATION_VULNERABLE;", "*value = ARM64_LATE_CPU_MITIGATION_MITIGATED;")),
