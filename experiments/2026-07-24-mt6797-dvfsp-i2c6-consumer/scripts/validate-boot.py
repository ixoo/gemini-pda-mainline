#!/usr/bin/env python3
"""Validate Candidate AP's canonical Android-v0 container and safety boundary."""
...
def validate_system_map(data: bytes) -> None:
    ...
    if missing:
        raise ValueError("System.map lacks Candidate AP owner symbol: " + missing[0])
    if "mtk_i2c_do_transfer" in symbols:
        raise ValueError(
            "Candidate AP transfer helper is not fully inlined as compiled-audited"
        )
    forbidden = {
...
def run_dtb_validator(
    ...
    if result.returncode:
        ...
        raise ValueError("final DT semantic validator rejected Candidate AP: " + detail)
...
def run_compiled_audit(
    ...
    if digest(auditor_data) != ap.COMPILED_HANDOFF_AUDITOR_SHA256:
        raise ValueError("source-pinned compiled-handoff auditor changed")
    with tempfile.TemporaryDirectory(prefix="candidate-ap-audit-") as temporary:
    ...
    if result.returncode:
        ...
        raise ValueError("compiled AP dependency auditor rejected kernel: " + detail)
    try:
...
def main() -> int:
    ...
    try:
        candidate = read_regular(args.candidate, "Candidate AP boot")
        image_gz = read_regular(args.image_gz, "Candidate AP Image.gz")
        system_map = read_regular(args.system_map, "Candidate AP System.map")
        kernel_config = read_regular(args.kernel_config, "Candidate AP config")
        dtb = read_regular(args.dtb, "Candidate AP final DT")
        ...
        if digest(ao_dtb) != ap.AO_DTB_SHA256:
            raise ValueError("exact Candidate AO final DT changed")
        pin_state = ap.artifact_pin_state()
        if pin_state == "source-pinned" and digest(dtb) != ap.FINAL_DTB_SHA256:
            raise ValueError("Candidate AP final-DT identity changed")
        if digest(initramfs) != ap.INITRAMFS_SHA256:
            raise ValueError("Candidate AP initramfs is not byte-exact Candidate AO")
        ...
        for marker in REQUIRED_IMAGE_MARKERS:
            if marker not in image:
                raise ValueError(
                    f"Candidate AP kernel lacks handoff-owner marker: {marker!r}"
                )
        for marker in FORBIDDEN_IMAGE_MARKERS:
            if marker in image:
                raise ValueError(
                    f"Candidate AP kernel contains forbidden marker: {marker!r}"
                )
        compiled_audit = run_compiled_audit(script_dir, image, args.system_map)

        if not 0 < len(candidate) <= ap.BOOT2_SIZE:
            raise ValueError("Candidate AP size is invalid or exceeds boot2")
        if len(candidate) < PAGE_SIZE or candidate[:8] != b"ANDROID!":
            raise ValueError("Candidate AP is not Android boot image v0")
        ...
        if candidate[kernel_offset:kernel_end] != kernel:
            raise ValueError("kernel field is not AP Image.gz plus final AP DT")
        ...
        print("validation=candidate-ap-mt6797-dvfsp-i2c6-consumer-android-v0")
        ...
        print(f"initramfs_sha256={ap.INITRAMFS_SHA256}")
        print(f"compiled_handoff_audit_sha256={digest(compiled_audit)}")
        print("dtb_lineage=exact-ao-plus-access-controller-childless-i2c6")
