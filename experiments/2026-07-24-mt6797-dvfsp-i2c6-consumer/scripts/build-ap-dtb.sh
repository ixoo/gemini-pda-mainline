...
# Deterministically enable only the MT6797 I2C6 controller on exact Candidate
# AO and link it to AO's validated one-way handoff access controller. No
# pinctrl, transfer policy, I2C child, regulator, or A72 node is introduced.
...
[[ -f "$validator" && ! -L "$validator" && -s "$validator" ]] || \
	die 'Candidate AP DT validator is missing or unsafe'
...
temporary="$(mktemp "$output_parent/.candidate-ap-dtb.XXXXXX")"
cleanup() { [[ ! -f "${temporary:-}" ]] || rm -f -- "$temporary"; }
trap cleanup EXIT
install -m 0600 "$ao_dtb" "$temporary"

# AO has the exact contiguous phandle allocation 0x01..0x2b. AP adds only the
# handoff access-controller declaration/phandle and the I2C6 reference.
...
[[ -f "$output" && ! -L "$output" && ! -e "$temporary" ]] || \
	die 'exclusive Candidate AP DT publication failed'
temporary=
trap - EXIT

printf 'validation=candidate-ap-dtb-built\n'
printf 'output=%s\nsha256=%s\n' "$output" "$built_sha256"
...
printf 'i2c6_pinctrl_frequency_push_pull=absent\n'
printf 'da9214_regulators_a72=absent\n'
