#!/usr/bin/env python3
"""Compare AP packages and tie each one to a distinct surviving live build."""
...
def load_validator() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location(
        "candidate_ap_package_validator", source
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load Candidate AP package validator")
...
def compare_packages(
    ...
    if set(left) != set(right):
        raise ValueError("Candidate AP package inventories differ")
    ...
    if left_build != right_build:
        raise ValueError("normalized Candidate AP build provenance differs")
    ...
    if set(left_manifest) != set(right_manifest):
        raise ValueError("Candidate AP manifest inventories differ")
    changed_entries = {
        relative
    ...
    if changed_entries != {"provenance/build.json"}:
        raise ValueError(
            "Candidate AP manifests differ outside generated build provenance"
        )
    if (
        ...
        raise ValueError("Candidate AP manifest build entries are inconsistent")
    return left
...
def main() -> int:
    ...
    try:
        ...
        first = validator.resolve_directory(args.first, "first Candidate AP package")
        second = validator.resolve_directory(
            args.second, "second Candidate AP package"
        )
        live_roots = {
        ...
        print("validation=candidate-ap-package-reproduction-calibration")
