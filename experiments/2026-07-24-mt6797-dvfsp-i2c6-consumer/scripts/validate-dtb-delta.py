#!/usr/bin/env python3
"""Require exact AO plus only AP's childless I2C6 access-controller link."""
...
def validate(ao_path: pathlib.Path, candidate_path: pathlib.Path) -> None:
    ...
    require_regular(candidate_path, "Candidate AP DT")
    ...
    candidate_header = require_header(candidate_path, AP_HEADER, "Candidate AP")
    if reservation_map_bytes(candidate_path, candidate_header) != (
        reservation_map_bytes(ao_path, ao_header)
    ):
        raise ValueError("Candidate AP changed AO's raw FDT reservation map")
    ...
    if candidate_reservations != ao_reservations:
        raise ValueError("Candidate AP changed AO's FDT reservation map")
    if candidate_boot_cpu != ao_boot_cpu:
        raise ValueError("Candidate AP changed AO's boot_cpuid_phys")
    ...
    expected[I2C6]["status"] = fdt.string("okay")
    if candidate != expected:
        raise ValueError(
            "Candidate AP DT is not exact AO plus the I2C6 dependency: "
    ...
    if handles != expected_handles:
        raise ValueError("Candidate AP phandle map changed beyond handoff 0x2c")
    if candidate[HANDOFF][ACCESS_CELLS_PROPERTY] != fdt.cells(0):
        raise ValueError("Candidate AP handoff access-controller cells changed")
    reference = candidate[I2C6][DEPENDENCY_PROPERTY]
    if len(reference) != 4:
        raise ValueError("Candidate AP I2C6 dependency is not one phandle")
    if handles.get(struct.unpack(">I", reference)[0]) != HANDOFF:
        raise ValueError("Candidate AP I2C6 dependency does not resolve to handoff")

    if any(path.startswith(I2C6 + "/") for path in candidate):
        raise ValueError("Candidate AP I2C6 is not childless")
    for prop in ACTIVE_I2C6_PROPERTIES:
        if prop in candidate[I2C6]:
            raise ValueError(f"Candidate AP I2C6 must not contain {prop}")
    for forbidden in (DA9214, A72_POWER, LEGACY_DVFSP, OBSERVER):
        if forbidden in candidate:
            raise ValueError(f"Candidate AP contains forbidden node {forbidden}")
...
def main() -> int:
    ...
    print("validation=candidate-ap-exact-ao-plus-childless-i2c6-access-controller")
    print(f"ao_dtb_sha256={AO_DTB_SHA256}")
    print(f"candidate_dtb_sha256={digest(args.candidate)}")
    print("added_nodes=0")
    print("added_properties=3")
    print("changed_properties=1")
    ...
    print("i2c6_pinctrl_frequency_push_pull=absent")
    print("da9214_regulators_a72=absent")
