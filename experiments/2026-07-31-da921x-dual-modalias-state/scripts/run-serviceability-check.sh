...
abort()
{
	printf 'dual_modalias_state_result=FAIL\nfailure=%s\n' "$1"
	exit 1
}
...
[ "$(/bin/busybox uname -r)" = 7.1.3-gemini-da921x-dualstate ] ||
...
printf 'kernel=7.1.3-gemini-da921x-dualstate\n'
printf 'adapter=%s\nclient=%s\n' "$adapter" "${client##*/}"
...
printf 'dual_modalias_state_result=PASS\n'
