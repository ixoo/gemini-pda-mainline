...
# Frozen BusyBox-compatible derivative of the listener-discovery acceptance
# check (source SHA-256 f5a6f58e2ddaee84e6f1653f13945ee9113b385b3c9c75187745125e5d5dc542).
# Read-only acceptance check for one fresh selected no-listener delivery boot.
set -eu

abort()
{
	printf 'uevent_no_listener_delivery_result=FAIL\nfailure=%s\n' "$1"
	exit 1
}
...
[ "$(/bin/busybox uname -r)" = 7.1.3-gemini-da921x-nodeliv ] ||
...
for path in "$state_path" "$stage_path" "$envelope_path" \
	"$classification_path" "$listener_path" "$delivery_path"; do
...
[ "$stage" = 20 ] || abort validation-stage
...
[ "$(counter "$listener_state" sockets)" = 1 ] || abort listener-sockets
[ "$(counter "$listener_state" listeners)" = 0 ] || abort listener-count
...
printf 'kernel=7.1.3-gemini-da921x-nodeliv\n'
...
printf 'event_transport=no-listener-loop-returned-without-allocation-or-broadcast\n'
printf 'client_driver=unbound\ni2c_activity=zero\n'
printf 'post_creation_serviceability=PASS\n'
printf 'uevent_no_listener_delivery_result=PASS\n'
