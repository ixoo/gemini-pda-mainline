...
# Read-only pre-trigger frame for the topology-preserving lifecycle candidate.
set -u
export LC_ALL=C
...
$BB printf '%s\n' __A72_TOPOLOGY_REPEAT_PRETRIGGER_BEGIN__
...
$BB printf '%s\n' __A72_TOPOLOGY_REPEAT_LATE_PROFILE_BEGIN__
$BB dmesg 2>/dev/null | $BB grep 'arm64-late-cpu-profile:' || true
$BB printf '%s\n' __A72_TOPOLOGY_REPEAT_LATE_PROFILE_END__
...
$BB printf '%s\n' __A72_TOPOLOGY_REPEAT_PRETRIGGER_END__
