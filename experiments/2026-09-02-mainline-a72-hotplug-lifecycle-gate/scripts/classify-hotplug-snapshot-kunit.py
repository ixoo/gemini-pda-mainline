#!/usr/bin/env python3
"""Classify the disconnected MT6797 hotplug-snapshot KUnit run."""
...
PROFILE = "gemini-a72-hotplug-snapshot-kunit"
EXPECTED_RELEASE = "7.1.3-gemini-a72-hotplug-snapshot-kunit"
PANIC_PREFIX = "Kernel panic - not syncing: VFS: Unable to mount root fs"
PANIC_END_PREFIX = f"---[ end {PANIC_PREFIX}"
SUITE = "mt6797-a72-hotplug-snapshot"
CASES = (
    "hotplug_snapshot_success_test",
    "hotplug_snapshot_generation_excluded_test",
    "hotplug_snapshot_component_failures_test",
    "hotplug_snapshot_source_guards_test",
    "hotplug_snapshot_shape_refusal_test",
...
def classify_runtime(raw: str, qemu_exit: int) -> None:
    ...
    require(plans == ["1..1", "1..6"], f"KUnit plans changed: {plans}")
    ...
    summary = f"# {SUITE}: pass:6 fail:0 skip:0 total:6"
...
def main() -> None:
    ...
    print("phase=hotplug-snapshot-kunit-qemu")
    ...
    print(f"runner_sha256={sha256(scripts / 'run-hotplug-snapshot-kunit-qemu')}")
    ...
    print("failed=0")
    print("skipped=0")
    print(f"suite_{SUITE}=pass:6_fail:0_skip:0_total:6")
    for case in CASES:
        print(f"{case}=pass")
