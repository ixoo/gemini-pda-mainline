#!/usr/bin/env python3
"""Classify the disconnected bounded CPU8-observer KUnit run."""
...
PROFILE = "gemini-a72-cpu8-observer-kunit"
EXPECTED_RELEASE = "7.1.3-gemini-a72-cpu8-observer-kunit"
PANIC_PREFIX = "Kernel panic - not syncing: VFS: Unable to mount root fs"
PANIC_END_PREFIX = f"---[ end {PANIC_PREFIX}"
SUITE = "mt6797-a72-cpu8-observer"
CASES = (
...
def classify_runtime(raw: str, qemu_exit: int) -> None:
    ...
    require(plans == ["1..1", "1..7"], f"KUnit plans changed: {plans}")
    ...
    summary = f"# {SUITE}: pass:7 fail:0 skip:0 total:7"
    totals = "# Totals: pass:7 fail:0 skip:0 total:7"
...
def main() -> None:
    ...
    print("phase=cpu8-observer-kunit-qemu")
    ...
    print(f"runner_sha256={sha256(scripts / 'run-cpu8-observer-kunit-qemu')}")
    ...
    print("failed=0")
    print("skipped=0")
    print(f"suite_{SUITE}=pass:7_fail:0_skip:0_total:7")
    for case in CASES:
        print(f"{case}=pass")
    print("tap_summary=pass:7_fail:0_skip:0_total:7")
    print("post_test_state=expected_vm_rootfs_panic")
    print("qemu_exit=124")
    print("dispatch_target=cpu8")
    print("dispatch_wait=0")
    print("dispatch_attempts_max=1")
    print("controller_wait_ms_max=250")
    print("timeout_terminal=true")
    print("late_callback_success=false")
