...
# Source-pin the sanitized subsecond host observer and change only its
# experiment label. It never contacts or modifies the device.
set -euo pipefail
export LC_ALL=C
umask 077
...
derived=$(mktemp "$script_dir/.derived-observe-softtrace-serviceable.XXXXXXXX")
...
old = "2026-08-28-mainline-a72-admission-trace-softfail"
new = "2026-08-28-mainline-a72-admission-softtrace-serviceable"
if text.count(old) != 1:
    raise SystemExit("unsafe corrected USB-observer derivation")
