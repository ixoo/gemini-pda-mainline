#!/usr/bin/env python3
"""Prove that the same-value-write classifier fails closed."""
...
SPEC = importlib.util.spec_from_file_location("same_value_kunit_classifier", SCRIPT)
if SPEC is None or SPEC.loader is None:
    raise SystemExit("cannot load classifier")
CLASSIFIER = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(CLASSIFIER)

RELEASE = "7.1.3-gemini-da921x-same-write-kunit"
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="gemini-same-write-manifest-") as temporary:
    ...
    mutations = (
        raw.replace("ok 4 da9213_same_value_transfer_failures\n", "", 1),
        raw.replace("ok 5 da9213_same_value_mismatches",
                    "not ok 5 da9213_same_value_mismatches", 1),
        raw.replace("pass:6 fail:0 skip:0 total:6",
                    "pass:5 fail:0 skip:1 total:6", 1),
        raw.replace("# Subtest: da9213-legacy-same-value-write",
                    "# Subtest: unexpected-suite", 1),
        raw.replace("ok 1 da9213_same_value_success",
                    "ok 1 wrong_case", 1),
    ...
    print("validation=mainline-da921x-same-value-write-kunit-classifier")
