...
[[ -f "$validator" && ! -L "$validator" && -s "$validator" ]] || \
	die 'Candidate AS DT validator is missing or unsafe'
...
temporary="$(mktemp "$output_parent/.candidate-as-dtb.XXXXXX")"
cleanup() { [[ ! -f "${temporary:-}" ]] || rm -f -- "$temporary"; }
trap cleanup EXIT
install -m 0600 "$ao_dtb" "$temporary"

# AO has the exact contiguous phandle allocation 0x01..0x2b. AS adds the
...
[[ -f "$output" && ! -L "$output" && ! -e "$temporary" ]] || \
	die 'exclusive Candidate AS DT publication failed'
temporary=
trap - EXIT

printf 'validation=candidate-as-dtb-built\n'
printf 'output=%s\nsha256=%s\n' "$output" "$built_sha256"
