#!/usr/bin/env python3
"""Validate Candidate AS's canonical Android-v0 container and safety boundary."""
...
def validate_system_map(data: bytes) -> None:
    ...
    if missing:
        raise ValueError("System.map lacks Candidate AS owner symbol: " + missing[0])
    if "mtk_i2c_do_transfer" in symbols:
        raise ValueError(
            "Candidate AS transfer helper is not fully inlined as compiled-audited"
        )
    forbidden = {
...
def run_dtb_validator(
    ...
    validator = script_dir / "validate-dtb-delta-as.py"
    ...
    if result.returncode:
        ...
        raise ValueError("final DT semantic validator rejected Candidate AS: " + detail)


def run_compiled_audit(
    script_dir: pathlib.Path, image: bytes, system_map: pathlib.Path
) -> bytes:
    auditor = script_dir / "audit-compiled-handoff-as.py"
    ...
    with tempfile.TemporaryDirectory(prefix="candidate-as-audit-") as temporary:
    ...
    if result.returncode:
        ...
        raise ValueError("compiled AS dependency auditor rejected kernel: " + detail)
    try:
...
def main() -> int:
    ...
    try:
        candidate = read_regular(args.candidate, "Candidate AS boot")
        image_gz = read_regular(args.image_gz, "Candidate AS Image.gz")
        system_map = read_regular(args.system_map, "Candidate AS System.map")
        kernel_config = read_regular(args.kernel_config, "Candidate AS config")
        dtb = read_regular(args.dtb, "Candidate AS final DT")
        ...
        if pin_state == "source-pinned" and digest(dtb) != ar.FINAL_DTB_SHA256:
            raise ValueError("Candidate AS final-DT identity changed")
        if digest(initramfs) != ar.INITRAMFS_SHA256:
            raise ValueError("Candidate AS initramfs is not byte-exact Candidate AO")
        ...
        for marker in REQUIRED_IMAGE_MARKERS:
            if marker not in image:
                raise ValueError(
                    f"Candidate AS kernel lacks handoff-owner marker: {marker!r}"
                )
        for marker in FORBIDDEN_IMAGE_MARKERS:
            if marker in image:
                raise ValueError(
                    f"Candidate AS kernel contains forbidden marker: {marker!r}"
                )
        compiled_audit = run_compiled_audit(script_dir, image, args.system_map)

        if not 0 < len(candidate) <= ar.BOOT2_SIZE:
            raise ValueError("Candidate AS size is invalid or exceeds boot2")
        if len(candidate) < PAGE_SIZE or candidate[:8] != b"ANDROID!":
            raise ValueError("Candidate AS is not Android boot image v0")
        ...
        if candidate[kernel_offset:kernel_end] != kernel:
            raise ValueError("kernel field is not AS Image.gz plus final AS DT")
        ...
        print("validation=candidate-as-da9214-legacy-identification-android-v0")
