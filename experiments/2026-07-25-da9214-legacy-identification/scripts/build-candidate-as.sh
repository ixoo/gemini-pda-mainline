...
# Assemble storage-inert Candidate AS from an exact validated AS kernel package
...
die() { printf 'error: %s\n' "$*" >&2; exit 2; }
usage() {
	printf 'usage: %s --package AS_PACKAGE --ao-artifact AO_ARTIFACT --output-parent DIR\n' "$0" >&2
}
...
candidate_module="$script_dir/candidate_as.py"
[[ -f "$candidate_module" && ! -L "$candidate_module" &&
	-s "$candidate_module" ]] || die 'Candidate AS identity module is unsafe'
candidate_value() {
	PYTHONPATH="$script_dir" python3 -c \
		'import candidate_as as ar, sys; print(getattr(ar, sys.argv[1]))' "$1"
}
...
readonly AS_DTB="$(candidate_value DTB_MEMBER)"
readonly AS_INITRAMFS="$(candidate_value INITRAMFS_MEMBER)"
readonly AS_BOOT="$(candidate_value BOOT_MEMBER)"
...
package_validator="$script_dir/validate-package-as.py"
dtb_builder="$support_dir/scripts/build-as-dtb.sh"
dtb_validator="$support_dir/scripts/validate-dtb-delta-as.py"
boot_validator="$script_dir/validate-boot-as.py"
handoff_auditor="$script_dir/audit-compiled-handoff-as.py"
...
patch_0103="$repo_root/patches/v7.1.3/0103-soc-mediatek-preserve-shared-MT6797-AP-DMA-owner.patch"
patch_0096="$repo_root/patches/v7.1.3/0096-regulator-da9211-support-legacy-DA9214-interface.patch"
patch_0104="$repo_root/patches/v7.1.3/0104-regulator-da9211-require-exact-legacy-DA9214-signature.patch"
patch_0105="$repo_root/patches/v7.1.3/0105-arm64-dts-mediatek-enable-legacy-Gemini-DA9214-after-handoff.patch"
patch_0106="$repo_root/patches/v7.1.3/0106-regulator-da9211-use-legacy-DA9214-page-selector.patch"
...
for input in "$manifest" "$package_validator" "$dtb_builder" "$dtb_validator" \
	"$boot_validator" "$handoff_auditor" "$normalizer" "$serializer" \
		...
		"$patch_0096" "$patch_0104" "$patch_0105" "$patch_0106" "$patch_0107" "$patch_0108"; do
...
require_source_hash "$patch_0102" "$(candidate_value PATCH_0102_SHA256)" \
	'patch 0102'
require_source_hash "$patch_0103" "$(candidate_value PATCH_0103_SHA256)" \
	'patch 0103'
require_source_hash "$patch_0096" "$(candidate_value PATCH_0096_SHA256)" \
	'patch 0096'
require_source_hash "$patch_0104" "$(candidate_value PATCH_0104_SHA256)" \
	'patch 0104'
require_source_hash "$patch_0105" "$(candidate_value PATCH_0105_SHA256)" \
	'patch 0105'
require_source_hash "$patch_0106" "$(candidate_value PATCH_0106_SHA256)" \
	'patch 0106'
require_source_hash "$patch_0107" "$(candidate_value PATCH_0107_SHA256)" \
	'patch 0107'
require_source_hash "$patch_0108" "$(candidate_value PATCH_0108_SHA256)" \
	'patch 0108'
...
workdir="$(mktemp -d "$output_parent/.candidate-AS-da9214-legacy-identification.XXXXXX")"
...
cmp -s "$candidate" "$replica_boot" || \
	die 'two independent Candidate AS container assemblies differ'
...
python3 "$analyzer" --validate-lk --expected-image-gz "$stage/Image.gz" \
	--expected-ramdisk "$stage/$AS_INITRAMFS" \
	--expected-dtb "$stage/$AS_DTB" --expected-name gemini-obs-L \
...
dtb_sha256="$(sha256sum "$stage/$AS_DTB" | awk '{ print $1 }')"
...
auditor_sha256="$(sha256sum "$handoff_auditor" | awk '{ print $1 }')"
{
	...
	printf 'patch_0103_sha256=%s\n' "$(candidate_value PATCH_0103_SHA256)"
	printf 'patch_0096_sha256=%s\n' "$(candidate_value PATCH_0096_SHA256)"
	printf 'patch_0104_sha256=%s\n' "$(candidate_value PATCH_0104_SHA256)"
	printf 'patch_0105_sha256=%s\n' "$(candidate_value PATCH_0105_SHA256)"
	printf 'patch_0106_sha256=%s\n' "$(candidate_value PATCH_0106_SHA256)"
	...
	printf 'da9214_probe_delta=source-equivalent-page-selector-read-modify-write\n'
	...
	printf 'da9214_node=present\nregulators=BUCKA,BUCKB\na72_power_node=absent\n'
...
[[ "$actual_inventory" == "$expected_pre_manifest" ]] || \
	die 'Candidate AS output inventory changed'
(
	cd "$stage"
	find . -type f ! -name SHA256SUMS -print0 | sort -z | xargs -0 sha256sum
) >"$stage/SHA256SUMS"
(cd "$stage" && sha256sum --check --strict SHA256SUMS >/dev/null) || \
	die 'Candidate AS artifact manifest failed'
chmod 0600 "$stage"/*
...
[[ -d "$output" && ! -e "$artifact" ]] || \
	die 'exclusive Candidate AS publication failed'
rm -rf -- "$replica"
rmdir "$workdir"
workdir=
trap - EXIT
printf 'validation=candidate-as-da9214-legacy-identification\n'
