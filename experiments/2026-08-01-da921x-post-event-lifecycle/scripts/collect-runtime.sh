...
# Source-pin and mechanically derive the exact one-shot USB/netcat lifecycle
# collector. The temporary device-side helper lives only in initramfs /run.
set -euo pipefail
export LC_ALL=C
umask 077
...
perl -0pe '
	...
	s#d4fae94c17bdcd901c6c269b778a1ccd6cbdde37ff646bc503fcf0bef3254bc9#96847deb664bf4efd39983995ce802f879e9304a58c12d6d9c628fe964cddc6d#g;
	...
	s#DUALSTATE#LIFE27#g;
	s#da921x-dual-modalias-state-runtime#da921x-post-event-lifecycle-runtime#g;
...
grep -Fq 'post_event_lifecycle_result=PASS' "$derived" ||
...
grep -Fq 'runtime_check="$repo_root/experiments/2026-08-01-da921x-post-event-lifecycle/scripts/run-lifecycle-check.sh"' \
