#!/usr/bin/env python3
"""Offline contract and mutation tests for Kepler's volatile runner."""
...
RUNNER_PATH = SCRIPT_DIR / "run-kepler-transfer.py"
SPEC = importlib.util.spec_from_file_location("kepler_transfer", RUNNER_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("cannot import Kepler transfer runner")
...
def valid_capture(post0: str = "d9", post1: str = "d9") -> bytes:
    mask = (post0 != "a5") | ((post1 != "5a") << 1)
    result_class = RUNNER.classify_posts(post0, post1, mask)
    probe_rc = "0" if result_class == "split-stable-d9" else "2"
    lines = [
        RUNNER.USB_BANNER,
        ...
        "nproc=8",
        "handoff_state=ready",
        f"i2c6_status_pre={i2c_status(6)}",
        "i2c6_adapter=i2c-1",
        ...
        (
            f"__KEPLER_TRANSFER_PASS__ size={RUNNER.ck.PROBE_BINARY_SIZE} "
        ...
        (
            "GEMINI_KEPLER_BEGIN adapter=i2c-1 of=/i2c@1100e000 "
            "address=0x69 register=0x05 pairs=2 calls=4 layout=split"
        ),
        (
            "GEMINI_KEPLER_TX pair=1 call=1 address=0x69 flags=0x0000 "
            "len=1 pointer=0x05 result=1 errno=0"
        ),
        (
            "GEMINI_KEPLER_RX pair=1 call=2 address=0x69 flags=0x0001 "
            f"len=1 pre=0xa5 post=0x{post0} result=1 errno=0 "
            f"post_differs_pre={'no' if post0 == 'a5' else 'yes'}"
        ),
        (
            "GEMINI_KEPLER_TX pair=2 call=3 address=0x69 flags=0x0000 "
            "len=1 pointer=0x05 result=1 errno=0"
        ),
        (
            "GEMINI_KEPLER_RX pair=2 call=4 address=0x69 flags=0x0001 "
            f"len=1 pre=0x5a post=0x{post1} result=1 errno=0 "
            f"post_differs_pre={'no' if post1 == '5a' else 'yes'}"
        ),
        (
            f"GEMINI_KEPLER_RESULT class={result_class} completed_pairs=2 "
            f"completed_calls=4 ioctl_result=1 errno=0 pre=a5,5a "
        ...
        f"i2c6_status_post={i2c_status(10)}",
        "usb_carrier_post=1",
        "usb_operstate_post=up",
        "udc_state_post=configured",
        "__KEPLER_POST_END__",
        (
            f"__KEPLER_COMPLETE__ probe_rc={probe_rc} invocation_count=1 "
...
class KeplerRunnerContracts(unittest.TestCase):
    def test_exact_pins(self) -> None:
        ...
        self.assertEqual(RUNNER.ck.PRE_COUNTER, 6)
        self.assertEqual(RUNNER.ck.POST_COUNTER, 10)
        self.assertEqual(RUNNER.ck.PROBE_BINARY_SIZE, 537_584)
        self.assertEqual(RUNNER.I2C_PRE_STATUS, i2c_status(6))

    def test_remote_payload_roundtrip_and_one_invocation(self) -> None:
        helper = b"fixed-kepler-test-payload"
        ...
        self.assertLess(
            program.index("__KEPLER_GATE_PASS__"),
            program.index('\n"$probe_path"\n'),
        )
        self.assertLess(
    ...
    def test_valid_all_complete_classes(self) -> None:
        cases = {
            ("a5", "5a"): "split-all-equal-pre",
            ("a5", "d9"): "split-mixed-equal-pre",
            ("d9", "d9"): "split-stable-d9",
            ("33", "33"): "split-stable-other",
            ("11", "22"): "split-unstable",
        }
    ...
    def test_mutations_fail_closed(self) -> None:
        mutations = {
            ...
            b"pre-counter": (b"transfer_attempts=6", b"transfer_attempts=5", 1),
            b"post-counter": (b"dma_starts=10", b"dma_starts=9", 1),
            b"photon-result": (
    ...
    def test_section_and_helper_line_reordering_fail_closed(self) -> None:
        ...
        section_reordered = original.replace(
            b"__KEPLER_PRE_BEGIN__", b"__KEPLER_TEMP_BEGIN__", 1
        ).replace(
            b"__KEPLER_PROBE_STDOUT_BEGIN__", b"__KEPLER_PRE_BEGIN__", 1
        ).replace(
            b"__KEPLER_TEMP_BEGIN__", b"__KEPLER_PROBE_STDOUT_BEGIN__", 1
        )
        with self.assertRaises(RUNNER.ContractError):
        ...
        tx_index = next(
            ...
            if line.startswith(b"GEMINI_KEPLER_TX pair=1 ")
        )
        rx_index = next(
            index
            for index, line in enumerate(lines)
            if line.startswith(b"GEMINI_KEPLER_RX pair=1 ")
        )
    ...
    def test_incomplete_tx_and_rx_results_are_intentionally_rejected(self) -> None:
        original = valid_capture()
        begin = original.index(b"__KEPLER_PROBE_STDOUT_BEGIN__\n")
        end = original.index(b"__KEPLER_PROBE_STDOUT_END__\n")
        prefix = original[:begin] + b"__KEPLER_PROBE_STDOUT_BEGIN__\n"
        suffix = original[end:]
        error_sequences = (
            (
                b"GEMINI_KEPLER_BEGIN adapter=i2c-1 of=/i2c@1100e000 "
                b"address=0x69 register=0x05 pairs=2 calls=4 layout=split\n"
                b"GEMINI_KEPLER_TX pair=1 call=1 address=0x69 flags=0x0000 "
                b"len=1 pointer=0x05 result=-1 errno=5\n"
                b"GEMINI_KEPLER_RESULT class=tx-result-not-one "
                b"completed_pairs=0 completed_calls=0 ioctl_result=-1 errno=5 "
                b"pre=a5,5a post=00,00 post_diff_mask=0x00 "
            ...
            (
                ...
                b"address=0x69 register=0x05 pairs=2 calls=4 layout=split\n"
                b"GEMINI_KEPLER_TX pair=1 call=1 address=0x69 flags=0x0000 "
                b"len=1 pointer=0x05 result=1 errno=0\n"
                b"GEMINI_KEPLER_RX pair=1 call=2 address=0x69 flags=0x0001 "
                b"len=1 pre=0xa5 post=0xa5 result=-1 errno=5 "
                b"post_differs_pre=no\n"
                b"GEMINI_KEPLER_RESULT class=rx-result-not-one "
                b"completed_pairs=0 completed_calls=1 ioctl_result=-1 errno=5 "
                b"pre=a5,5a post=a5,00 post_diff_mask=0x00 "
                b"stop_between_pointer_and_read=yes page_con_access=none\n"
            ),
        )
    ...
    def test_nonexact_helper_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            path = pathlib.Path(temporary) / "not-kepler"
