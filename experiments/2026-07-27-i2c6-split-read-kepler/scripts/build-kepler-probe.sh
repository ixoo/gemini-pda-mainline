...
# Build the fixed-function Kepler helper as a deterministic static AArch64 ELF.
set -euo pipefail
export LC_ALL=C
...
die() { printf 'error: %s\n' "$*" >&2; exit 2; }
[[ $# -eq 2 ]] || die 'usage: build-kepler-probe.sh SOURCE OUTPUT'
source_file=$1
output=$2
...
validator="$script_dir/validate-kepler-probe.py"
[[ -f "$validator" && ! -L "$validator" && -s "$validator" ]] ||
	die 'probe validator is missing or unsafe'

temporary="$(mktemp "$output_parent/.kepler-probe.XXXXXX")"
...
printf 'validation=kepler-probe-built\n'
printf 'compiler=%s\n' "$(gcc -dumpfullversion -dumpversion)"
printf 'binary_sha256=%s\n' "$binary_sha256"
printf 'arguments=none\npairs=2\nioctl_calls=4\n'
printf 'message_count_per_ioctl=1\npointer=05\nreceive_prefills=a5,5a\n'
