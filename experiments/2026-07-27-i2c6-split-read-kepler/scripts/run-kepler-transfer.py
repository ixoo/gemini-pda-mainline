#!/usr/bin/env python3
"""Transfer and invoke exact Kepler once on the accepted Hubble boot."""
...
I2C_PRE_STATUS = (
    ...
    "clock_domains=i2c-appm,ap-dma transfer_attempts=6 dma_starts=6 "
...
TRANSCRIPT_NAME = "kepler-runtime-transfer.txt"
STDERR_NAME = "kepler-nc-stderr.txt"
INTERFACE = re.compile(r"[A-Za-z0-9]+")
HEX_BYTE = re.compile(r"[0-9a-f]{2}")


REMOTE_TEMPLATE = r"""/bin/busybox sh <<'__KEPLER_REMOTE__'
set -eu
umask 077

probe_path=/run/kepler-probe
stage_path=/run/.kepler-probe.stage
guard_path=/run/kepler-probe.invoked
...
for path in "$probe_path" "$stage_path" "$guard_path"; do
	[ ! -e "$path" ] && [ ! -L "$path" ] || abort prior-kepler-path
done
...
printf '__KEPLER_GATE_END__\n__KEPLER_GATE_PASS__\n'

/bin/busybox base64 -d >"$stage_path" <<'__KEPLER_PAYLOAD__'
__PAYLOAD_BASE64__
__KEPLER_PAYLOAD__
[ -f "$stage_path" ] && [ ! -L "$stage_path" ] || abort stage-type
require_equal "$(/bin/busybox stat -c '%s' "$stage_path")" \
	"__KEPLER_SIZE__" stage-size
require_equal "$(file_sha256 "$stage_path")" "__KEPLER_SHA256__" stage-hash
...
require_equal "$(/bin/busybox stat -c '%s' "$probe_path")" \
	"__KEPLER_SIZE__" probe-size
require_equal "$(file_sha256 "$probe_path")" "__KEPLER_SHA256__" probe-hash
require_equal "$(/bin/busybox stat -c '%a:%u:%g' "$probe_path")" \
	500:0:0 probe-mode
printf '__KEPLER_TRANSFER_PASS__ size=%s sha256=%s mode=500:0:0 path=/run/kepler-probe\n' \
	"__KEPLER_SIZE__" "__KEPLER_SHA256__"

printf '__KEPLER_PRE_BEGIN__\n'
...
printf '__KEPLER_POST_END__\n'
printf '__KEPLER_COMPLETE__ probe_rc=%s invocation_count=1 helper_removed=yes guard_mode=%s\n' \
...
def read_exact_helper(path: pathlib.Path) -> bytes:
    ...
    data = ck.regular(path, "Kepler helper")
    if len(data) != ck.PROBE_BINARY_SIZE or ck.digest(data) != ck.PROBE_BINARY_SHA256:
        raise ContractError("helper is not the exact Kepler ELF")
    return data
...
def build_remote_program(helper: bytes) -> bytes:
    ...
    if (
        ...
        raise ContractError("refusing to serialize non-Kepler helper bytes")
    ...
    replacements = {
        ...
        "__KEPLER_SHA256__": ck.PROBE_BINARY_SHA256,
        "__KEPLER_SIZE__": str(ck.PROBE_BINARY_SIZE),
...
def classify_posts(post0: str, post1: str, mask: int) -> str:
    ...
    if post0 == post1:
        return "split-stable-d9" if post0 == "d9" else "split-stable-other"
...
def validate_transcript(data: bytes) -> dict[str, str]:
    ...
    if "__KEPLER_ABORT__" in text:
        raise ContractError("remote Hubble/serviceability gate aborted")
    ordered_markers = (
        "__KEPLER_GATE_BEGIN__",
        "__KEPLER_GATE_END__",
        "__KEPLER_GATE_PASS__",
        "__KEPLER_TRANSFER_PASS__",
        "__KEPLER_PRE_BEGIN__",
        "__KEPLER_PRE_END__",
        "__KEPLER_PROBE_STDOUT_BEGIN__",
        "__KEPLER_PROBE_STDOUT_END__",
        "__KEPLER_POST_BEGIN__",
        "__KEPLER_POST_END__",
        "__KEPLER_COMPLETE__",
    )
    positions: list[int] = []
    ...
    gate = key_values(
        unique_section(text, "__KEPLER_GATE_BEGIN__", "__KEPLER_GATE_END__"),
        "Hubble gate",
    )
    expected_gate = {
    ...
    transfer = next(
        line for line in text.splitlines() if line.startswith("__KEPLER_TRANSFER_PASS__")
    )
    if marker_values(transfer.split(" ", 1)[1]) != {
        "size": str(ck.PROBE_BINARY_SIZE),
        "sha256": ck.PROBE_BINARY_SHA256,
        "mode": "500:0:0",
        "path": "/run/kepler-probe",
    }:
        raise ContractError("volatile Kepler helper identity differs")

    pre = key_values(
        unique_section(text, "__KEPLER_PRE_BEGIN__", "__KEPLER_PRE_END__"),
        "pre-probe state",
    )
    post = key_values(
        unique_section(text, "__KEPLER_POST_BEGIN__", "__KEPLER_POST_END__"),
        "post-probe state",
    )
    ...
    if post.get("boot_id_post") != ck.ACCEPTED_BOOT_ID:
        raise ContractError("boot ID changed around Kepler")
    ...
    stdout = unique_section(
        text, "__KEPLER_PROBE_STDOUT_BEGIN__", "__KEPLER_PROBE_STDOUT_END__"
    )
    if len(stdout) != 6:
        raise ContractError("Kepler successful stdout must contain exactly six lines")
    wanted_line_order = (
        "GEMINI_KEPLER_BEGIN ",
        "GEMINI_KEPLER_TX pair=1 ",
        "GEMINI_KEPLER_RX pair=1 ",
        "GEMINI_KEPLER_TX pair=2 ",
        "GEMINI_KEPLER_RX pair=2 ",
        "GEMINI_KEPLER_RESULT ",
    )
    if tuple(
        next((prefix for prefix in wanted_line_order if line.startswith(prefix)), "")
        for line in stdout
    ) != wanted_line_order:
        raise ContractError("Kepler helper line order differs")
    begin = [line for line in stdout if line.startswith("GEMINI_KEPLER_BEGIN ")]
    tx = [line for line in stdout if line.startswith("GEMINI_KEPLER_TX ")]
    rx = [line for line in stdout if line.startswith("GEMINI_KEPLER_RX ")]
    results = [line for line in stdout if line.startswith("GEMINI_KEPLER_RESULT ")]
    if len(begin) != 1 or len(tx) != 2 or len(rx) != 2 or len(results) != 1:
        raise ContractError("Kepler line-type inventory differs")
    if marker_values(begin[0].split(" ", 1)[1]) != {
        ...
        raise ContractError("Kepler BEGIN contract differs")
    for index, line in enumerate(tx):
        ...
        if values != {
            ...
            raise ContractError("Kepler TX contract differs")
    posts: list[str] = []
    for index, line in enumerate(rx):
        ...
        if HEX_BYTE.fullmatch(post_byte) is None:
            raise ContractError("Kepler RX post byte is malformed")
    ...
    if result != {
        ...
        raise ContractError("Kepler aggregate RESULT contract differs")

    complete = next(
        line for line in text.splitlines() if line.startswith("__KEPLER_COMPLETE__")
    )
    complete_values = marker_values(complete.split(" ", 1)[1])
    wanted_rc = "0" if wanted_class == "split-stable-d9" else "2"
    if complete_values != {
        "probe_rc": wanted_rc,
        "invocation_count": "1",
        "helper_removed": "yes",
        ...
        raise ContractError("Kepler one-shot completion contract differs")
    return {
        "boot_id": ck.ACCEPTED_BOOT_ID,
...
def host_header(interface: str) -> bytes:
    return (
        ...
        "remote_destination=/run/kepler-probe\n"
...
def main() -> int:
    ...
    try:
        ...
        return 2
    print("validation=kepler-exact-hubble-volatile-split-read-one-shot")
