...
# Wait for exactly one fixed-MAC USB link and let collect-runtime.sh make the
# sole TCP connection.  This helper never reads a device partition and never
# retries a rejected runtime capture.
...
MAC/address/route, and invoke Candidate AH's read-only runtime collector once.
...
[[ "$installed_full_sha256" == "$EXPECTED_INSTALLED_FULL_SHA256" ]] || \
	die '--installed-full-sha256 is not the exact validated AH value'
...
[[ -x "$collector" && ! -L "$collector" ]] || die 'AH runtime collector is absent or unsafe'
...
if ((collector_rc == 0)); then
	...
	last_detail='exact-ah-runtime-validator-passed'
...
finalized=1
trap - EXIT
printf 'error: Candidate AH runtime collection failed; evidence=%s\n' "$output" >&2
exit "$collector_rc"
