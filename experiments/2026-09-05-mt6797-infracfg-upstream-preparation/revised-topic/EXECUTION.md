...
then independently passed all 36 host refusal cases, historical-body comparison,
Bash syntax and ShellCheck. The exact proposal and final-tree checks retain the
intended reset changes while preserving upstream binding bytes. No source
checkout is materialized by the revised mode. Backend generation remains a
separately assigned, exact-revision operation after this tooling is published.
