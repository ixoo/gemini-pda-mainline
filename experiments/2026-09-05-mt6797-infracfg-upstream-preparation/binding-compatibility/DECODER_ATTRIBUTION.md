...
Project Planning reviewed the seven-file correction at `0495c39c`, checked the
raw-property lookup and per-case capture against the fixed fixture inventory,
and independently fetched the primary dtschema 2026.6 wheel in memory. Its wheel
and all three module digests match the recorded pins; the unpack fallback agrees
with the documented byte/halfword behavior. No tool or vendor source was stored.

The integration checkout reproduced 20 attribution refusals, 24 comparison
refusals and 17 warning-chain refusals. The host table retains the original
schema errors and uses explicitly synthetic raw/capture attribution; no actual
third decoding run is claimed. The correction is accepted for a separately
assigned focused Buildbox run on a clean published revision. Prior failed
receipts, fixtures, schemas and full-validity expectations remain unchanged.
