...
def command(binary, image, serial, contract=CONTRACT):
    data = Path(binary).parent.parent / 'share/qemu'
...
def run_attempt(binary, package, output, receipt, interrupted):
    ...
    try:
        ...
        receipt['argv'] = command('<qemu-prefix>/usr/bin/' + binary.name,
                                  '<package>/Image.gz', '<output>/serial.log')
        write_receipt(output, receipt)
...
def verify_tool_setup(binary, evidence):
    prefix = Path(evidence['destination'])
    require(binary == prefix / 'usr/bin/qemu-system-aarch64', 'unexpected QEMU prefix')
    require(digest(regular(binary)) == evidence['inspection']['executable_sha256'],
            'QEMU executable identity')
    require(digest(regular(prefix / 'setup-receipt.json')) == evidence['remote_receipt_sha256'],
            'QEMU setup receipt identity')
    data = prefix / 'usr/share/qemu'
    require(data.is_dir() and data.resolve() == data, 'QEMU data directory')
    return evidence['remote_receipt_sha256']
...
def main():
    ...
    evidence = json.loads(regular(HERE.parent / 'results/qemu-debian-setup.json'))
    setup_identity = verify_tool_setup(binary, evidence)
    ...
    with interruption_guard() as interrupted:
        receipt = {'package': identity, 'qemu_sha256': digest(regular(binary)),
                   'qemu_setup_receipt_sha256': setup_identity,
