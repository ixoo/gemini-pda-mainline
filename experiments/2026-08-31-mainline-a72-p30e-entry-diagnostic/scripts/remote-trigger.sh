...
readonly VALIDATOR_SHA256=f3f4067fdb365ea0fc5eee7c2b0176ddb45c69c5ddf68ddf886aad64e3995a7f
...
[[ "$(sha256sum "$validator" | awk '{print $1}')" == "$VALIDATOR_SHA256" ]] || {
	printf 'error: P30E pre-trigger validator changed\n' >&2
	exit 2
}
materialized=$(mktemp "${TMPDIR:-/tmp}/.gemini-a72-p30e-entry-trigger.XXXXXXXX")
...
spec = spec_from_file_location("p30e_pretrigger", sys.argv[2])
...
if text.count(old) != 1:
    raise SystemExit("unsafe P30E trigger derivation")
