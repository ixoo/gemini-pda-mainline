...
abort()
{
	printf 'dual_modalias_pre_dispatch_result=FAIL\nfailure=%s\n' "$1"
	exit 1
}
...
[ "$(/bin/busybox uname -r)" = 7.1.3-gemini-da921x-dualpre ] ||
...
marker='GEMINI_DA921X_DUAL_MODALIAS_PRE_DISPATCH full_env=validated envp_idx=10 buflen=[0-9]+ of_modalias=present i2c_modalias=present seqnum=present transport=skipped return=success cleanup=normal'
marker_count="$(/bin/busybox dmesg | /bin/busybox grep -Ec "$marker")"
[ "$marker_count" = 1 ] || abort dual-modalias-marker-count

printf 'kernel=7.1.3-gemini-da921x-dualpre\n'
printf 'adapter=%s\nclient=%s\n' "$adapter" "${client##*/}"
printf 'client_name=da9214-legacy\nclient_of_node=present\n'
printf 'client_sysfs_modalias=of-real-compatible\n'
printf 'complete_dual_modalias_event=validated\nof_modalias=present\ni2c_modalias=present\nsequence_number=present\n'
printf 'event_transport=skipped\nuevent_return=success\ncleanup=normal\n'
printf 'client_driver=unbound\ni2c_activity=zero\n'
printf 'post_creation_serviceability=PASS\n'
printf 'dual_modalias_pre_dispatch_result=PASS\n'
