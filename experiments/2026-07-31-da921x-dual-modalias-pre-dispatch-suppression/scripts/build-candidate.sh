...
readonly IMAGE_SHA256=f43e65ebdf7be3e94f006235f1230f996dbfb6ef55db3cd3471455f3c103c21e
readonly CONFIG_SHA256=96bb06d56eb4034ff59909fa205675d834241a7eec9bbdd6a86b5719fc39a23f
readonly SYSTEM_MAP_SHA256=4a0582b9522e4bb890b423e8b061da8a320dca8ed766570fbee0e7c22eaf4e67
readonly SOURCE_BUILD_SHA256=22a4491c7c830ad443ec671fb93f17e3c0e3155e79938e6b2a45b0f0050c2a02
readonly OUTPUT_BOOT=gemini-mt6797-da921x-dual-modalias-pre-dispatch-suppression.boot.img
...
[[ "$(jq -er '.kernel_release' "$package/provenance/build.json")" == \
	7.1.3-gemini-da921x-dualpre ]] || die "kernel release changed"
...
workdir="$(mktemp -d "$output_parent/.da921x-dualpre-candidate.XXXXXX")"
...
for destination in "$stage/$OUTPUT_BOOT" "$replica/$OUTPUT_BOOT"; do
	python3 "$serializer" --kernel "$stage/Image.gz" \
		...
		--output "$destination" --name gemini-dualpre \
...
python3 "$analyzer" --validate-lk --expected-image-gz "$stage/Image.gz" \
	...
	--expected-name gemini-dualpre --expected-cmdline "$boot_cmdline" \
...
{
	printf 'experiment=2026-07-31-da921x-dual-modalias-pre-dispatch-suppression\n'
	printf 'candidate_sha256=%s\ncandidate_size=%s\n' "$candidate_sha256" "$candidate_size"
	printf 'padded_sha256=%s\npadded_size=%s\n' "$padded_sha256" "$BOOT2_SIZE"
	printf 'kernel_release=7.1.3-gemini-da921x-dualpre\n'
...
output_name="candidate-Gate3-da921x-dualpre-${candidate_sha256:0:8}"
...
workdir=
trap - EXIT
printf 'validation=da921x-dual-modalias-pre-dispatch-suppression-candidate\n'
