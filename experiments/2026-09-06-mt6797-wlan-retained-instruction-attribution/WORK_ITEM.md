...
- **Owned scope:** create only this experiment's `README.md`, `inputs.json`,
  ...
  `AMENDMENT-5.md`, `AMENDMENT-6.md`, `PREFLIGHT.md`,
  ...
  `bootstrap-v5.json`, optional `VALIDATION-V5-REFUSED.md`, `method.json`,
  `analysis.json`, `edges.json`, `FREEZE.md`, an assert-free normal/optimized
  verifier, final `README.md` and `VALIDATION-RESULT.md`.
...
- **Tool and method freeze:** use the approved `./scripts/dev-vm re-shell`
  ...
  `bootstrap-v5.json` and `method.json`, with SHA-256 values, before the
  ...
  and [AMENDMENT-6.md](AMENDMENT-6.md). Syntax-checking the frozen scripts is
...
- **Refusal and validation:** reject identity, interval, mapping, tool/method or
  ...
  radio authority. Freeze `bootstrap-v5.json`, then `method.json`, then the
