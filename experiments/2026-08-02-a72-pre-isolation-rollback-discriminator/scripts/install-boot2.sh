...
# A72 pre-isolation rollback discriminator.
set -euo pipefail
export LC_ALL=C
umask 077
...
derived="$(mktemp "${TMPDIR:-/tmp}/.derived-a72-rollback-installer.XXXXXXXX")"
...
perl -0pe '
	...
	s#\.gemini-probe-isolation#\.gemini-a72-rollback#g;
	s#probe-isolation#A72 pre-isolation rollback#g;
