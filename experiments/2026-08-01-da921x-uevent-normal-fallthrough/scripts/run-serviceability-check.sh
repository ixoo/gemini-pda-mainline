...
# Continue only from the independently reconstructed stage-24 predecessor.
...
abort()
{
	printf 'uevent_normal_fallthrough_result=FAIL\nfailure=%s\n' "$1"
	exit 1
}
...
listener=/run/.gemini-normal-fallthrough-listener
listener_sha256=28b20119894ee835c48b775fd7edaa570bc9c1a97d1df9aaae0beb3f958e26c9
[ "$(/bin/busybox id -u)" = 0 ] || abort not-root
[ "$(/bin/busybox uname -r)" = 7.1.3-gemini-da921x-fallthrough ] || abort kernel-identity
...
normal_fallthrough_path=/sys/kernel/gemini_da921x_uevent_normal_fallthrough
for path in "$state_path" "$stage_path" "$untagged_path" "$net_broadcast_path" \
	"$normal_fallthrough_path"; do
...
normal_fallthrough_state="$(/bin/busybox cat "$normal_fallthrough_path")" || abort normal-fallthrough-state-read
[ "$state" = validated ] || abort validation-state
[ "$stage" = 24 ] || abort validation-stage-before-trigger
[ "$untagged_state" = \
...
[ "$net_broadcast_state" = \
	...
	abort net-broadcast-predecessor-state
[ "$normal_fallthrough_state" = \
	'attempts=0 callsite_entries=0 callsite_returns=0 public_returns=0 retval=-1' ] ||
	abort normal-fallthrough-state-before-trigger
...
for exact in \
	...
	'duplicate_receipt=none-bounded-timeout' 'normal_fallthrough_result=PASS'; do
...
normal_fallthrough_state="$(/bin/busybox cat "$normal_fallthrough_path")" || abort normal-fallthrough-state-final
[ "$state" = validated ] || abort validation-state-after-trigger
[ "$stage" = 25 ] || abort validation-stage-after-trigger
[ "$net_broadcast_state" = \
	'attempts=1 entries=1 returns=1 namespace_checks=1 untagged_routes=1 tagged_routes=0 baseline_sockets=1 sockets=1 listeners=1 allocations=1 broadcasts=1 retval=0' ] ||
	abort net-broadcast-predecessor-state-after-trigger
[ "$normal_fallthrough_state" = \
	'attempts=1 callsite_entries=1 callsite_returns=1 public_returns=1 retval=0' ] ||
	abort normal-fallthrough-state-after-trigger
...
printf '%s\n' "$listener_result"
printf 'kernel=7.1.3-gemini-da921x-fallthrough\nvalidation_state=%s\nvalidation_stage=%s\n' "$state" "$stage"
printf 'normal_fallthrough_attempts=1\nnormal_fallthrough_callsite_entries=1\n'
printf 'normal_fallthrough_callsite_returns=1\nnormal_fallthrough_public_returns=1\n'
printf 'normal_fallthrough_retval=0\n'
printf 'adapter=%s\nclient=%s\nclient_driver=unbound\n' "$adapter" "${client##*/}"
printf 'i2c_activity=zero\nsysfs_trigger_mount=temporary-rw-restored-ro\n'
printf 'post_creation_serviceability=PASS\nuevent_normal_fallthrough_result=PASS\n'
