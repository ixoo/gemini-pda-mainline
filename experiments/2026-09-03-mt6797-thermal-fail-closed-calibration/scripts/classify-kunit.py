#!/usr/bin/env python3
"""Classify the focused MT6797 thermal calibration KUnit runtime."""
...
PROFILE = "mt6797-thermal-calibration-kunit"
SUITE = "mtk-thermal-calibration-policy"
EXPECTED_CASES = (
    "mtk_thermal_optional_success",
    "mtk_thermal_required_success",
...
def main() -> None:
    ...
    require(re.findall(r"Linux version ([^ ]+)", raw) == [build["kernel_release"]],
    ...
    require(not any(line.startswith("not ok ") for line in lines),
    ...
    require(lines.count(f"# {SUITE}: pass:9 fail:0 skip:0 total:9") == 1,
            "suite totals changed")
    require(lines.count("# Totals: pass:9 fail:0 skip:0 total:9") == 1,
            "global totals changed")
    require(lines.count(f"ok 1 {SUITE}") == 1, "suite result absent")
    require(any(line.startswith("Kernel panic - not syncing: VFS: Unable to mount root fs")
                for line in lines), "post-test rootfs panic boundary absent")
    result = {
