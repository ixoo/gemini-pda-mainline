...
struct persistent_ram_buffer { u32 sig; int start, size; char data[32]; };
...
int main(void)
{
    struct ramoops_context context = { .phys_addr = 4096, .size = 64 };
    for (int failed = 0; failed <= 1; failed++) {
        for (int empty = 0; empty <= 1; empty++) {
            ...
            fail_copy = failed;
            zaps = 0;
            int result = ramoops_init_prz(NULL, &context, &zone, &address, 64, 0);
