...
the selected minimal userspace. They are not yet included in the private input
filesystem, and no session manifest or boot candidate selects them. They must
not run in the current Gemian environment.
...
No selected manifest is generated by this checkpoint. The packager must create
the mount-point directories and install these files before it can produce a
startup filesystem; the earlier input-only package remains unchanged.
