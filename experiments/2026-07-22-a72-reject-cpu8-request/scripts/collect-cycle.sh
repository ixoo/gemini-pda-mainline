...
# Start this watcher before the owner selects boot2. Bind one collection attempt
# to a real USB cycle. A reachable exact fixed-MAC Gemini link must disappear
# before reappearing; an initially absent link must remain cleanly absent across
# two observations before its first exact appearance. Only then is
# collect-runtime.sh invoked, exactly once. This watcher never reads a device
# partition, requests a reboot, or retries a rejected capture.
...
readonly COLLECTOR_SHA256=a14901aba0e3fbbc9f2137c654a0c13be2d3e6fbdcfe84d7bad7065d1437ee19
readonly CANDIDATE_AJ_SHA256=77f29772bafc070da6d0dda621136586348d2d1d1cf0c4cecec6b24800eee3c1
readonly AI_CYCLE_SHA256=6dfb193a2eacf77fb5c588152338d4b304affe14bb81df05d53ee377db057366
...
Start this watcher before selecting boot2. If the exact Gemini USB Ethernet
interface is packet-ready at startup, confirm that it disappears. If it is
initially absent, confirm that absence twice.
Then validate its unique fixed MAC/address/route on appearance and invoke
Candidate AJ's read-only runtime collector once.
...
for command in awk bash basename chmod date dirname git grep ifconfig ioreg mkdir \
	mv ping python3 route shasum sleep stat sudo; do
...
ai_cycle="$repo_root/experiments/2026-07-22-a72-reject-gate-kernel-split/scripts/collect-cycle.sh"
readonly script_dir repo_root collector candidate_identity ai_cycle
[[ -f "$collector" && ! -L "$collector" ]] || die 'AJ runtime collector is absent or unsafe'
collector_sha256="$(shasum -a 256 "$collector" | awk '{ print $1 }')"
[[ "$collector_sha256" == "$COLLECTOR_SHA256" ]] || \
	die 'AJ runtime collector source identity changed'
readonly collector_sha256
[[ -f "$candidate_identity" && ! -L "$candidate_identity" ]] || \
	die 'Candidate AJ identity module is absent or unsafe'
[[ "$(shasum -a 256 "$candidate_identity" | awk '{ print $1 }')" == \
	"$CANDIDATE_AJ_SHA256" ]] || die 'Candidate AJ identity module source changed'
[[ -f "$ai_cycle" && ! -L "$ai_cycle" ]] || \
	die 'source Candidate AI cycle watcher is absent or unsafe'
[[ "$(shasum -a 256 "$ai_cycle" | awk '{ print $1 }')" == "$AI_CYCLE_SHA256" ]] || \
	die 'source Candidate AI cycle watcher changed'

# Resolve all production identities before inspecting or creating the caller's
# output path. A test cannot weaken this gate with a synthetic artifact.
pinned_full_sha256="$(PYTHONDONTWRITEBYTECODE=1 python3 - "$candidate_identity" <<'PY'
import importlib.util
import pathlib
import sys

path = pathlib.Path(sys.argv[1])
spec = importlib.util.spec_from_file_location("candidate_aj_cycle_pins", path)
...
[[ "$pinned_full_sha256" == "$INSTALLED_FULL_SHA256" ]] || \
	die 'Candidate AJ padded identity disagrees with the cycle watcher'
...
runtime_boot_id_before=unavailable
runtime_boot_id_after=unavailable
runtime_boot_id_match=not-checked
...
write_status() {
	...
	{
		printf 'experiment=2026-07-22-a72-reject-cpu8-request\n'
		...
		printf 'runtime_boot_id_before=%s\nruntime_boot_id_after=%s\n' \
			"$runtime_boot_id_before" "$runtime_boot_id_after"
		printf 'runtime_boot_id_match=%s\n' "$runtime_boot_id_match"
...
extract_runtime_value() {
	...
	local end=$2
	local key=$3
	awk -v begin="$begin" -v end="$end" -v key="$key" '
		{
			sub(/\r$/, "")
			while (index($0, "GEMINI-AC-USB# ") == 1) {
				sub(/^GEMINI-AC-USB# /, "")
			}
		}
		$0 == begin {
		...
		END {
			if (begin_count != 1 || end_count != 1 || value_count != 1) {
				exit 1
			}
			print value
		}
...
record_runtime_boot_ids() {
	local before after
	before="$(extract_runtime_value __AJ_IDENTITY_BEGIN__ \
		__AJ_IDENTITY_END__ boot_id 2>/dev/null || true)"
	after="$(extract_runtime_value __AJ_STABILITY_BEGIN__ \
		__AJ_STABILITY_END__ boot_id_after 2>/dev/null || true)"
	[[ -z "$before" ]] || runtime_boot_id_before=$before
	[[ -z "$after" ]] || runtime_boot_id_after=$after
	if [[ "$before" =~ ^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$ &&
		"$after" =~ ^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$ ]]; then
		if [[ "$before" == "$after" ]]; then
			runtime_boot_id_match=yes
			return 0
		fi
		runtime_boot_id_match=no
		return 1
	fi
	runtime_boot_id_match=partial-or-invalid
	return 1
}
	...
	printf '%s\n' "$route_output" | awk -v target="$DEVICE_ADDRESS" '
		...
		END {
			if (route_to_count != 1 || route_to != target ||
			    ...
			    flags_count > 1) {
				print "__candidate_aj_route_invalid__"
				exit
			}
...
assert_route_parseable() {
	[[ "$1" != __candidate_aj_route_invalid__ ]] || \
...
if ((collector_rc == 0)); then
	if ! record_runtime_boot_ids; then
		phase=runtime-rejected
		runtime_identity_capture=complete
		runtime_subgate=rejected
		oracle=INCONCLUSIVE
		last_detail=runtime-boot-id-binding-failed
		log_event "collector_rc=0 runtime_boot_id_before=$runtime_boot_id_before runtime_boot_id_after=$runtime_boot_id_after runtime_boot_id_match=$runtime_boot_id_match"
		write_status failed 2
		finalized=1
		trap - EXIT
		printf 'error: Candidate AJ runtime boot-ID binding failed; evidence=%s\n' \
			"$output" >&2
		exit 2
	fi
	...
	oracle=PENDING_CONSOLE_REBOOT_RECOVERY_AND_BOOT2_INTEGRITY
	last_detail=exact-aj-runtime-validator-passed
	log_event "collector_rc=0 runtime_boot_id=$runtime_boot_id_before identity_and_stability=matched capture_sha256=$(capture_sha256 "$capture")"
	write_status completed 0
	finalized=1
	trap - EXIT
	printf 'operation_status=completed\nruntime_subgate=passed\n'
	printf 'oracle=PENDING_CONSOLE_REBOOT_RECOVERY_AND_BOOT2_INTEGRITY\n'
	printf 'runtime_boot_id=%s\n' "$runtime_boot_id_before"
...
phase=runtime-rejected
runtime_subgate=rejected
record_runtime_boot_ids || true
if [[ -f "$capture" ]] && grep -aFq '__AJ_IDENTITY_BEGIN__' "$capture" && \
	grep -aFq '__AJ_IDENTITY_END__' "$capture"; then
	runtime_identity_capture=complete
...
finalized=1
trap - EXIT
printf 'error: Candidate AJ runtime collection failed; evidence=%s\n' "$output" >&2
exit "$collector_rc"
