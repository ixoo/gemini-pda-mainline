...
die() { printf 'error: %s\n' "$*" >&2; exit 2; }
usage() {
	printf 'usage: %s --package AJ_PACKAGE --ad-package AD_PACKAGE --ai-package AI_PACKAGE --ad-artifact DIR --ah-artifact DIR --af-artifact DIR --ai-artifact DIR --output-parent DIR\n' "$0" >&2
}
...
while (($#)); do
	case "$1" in
	--package|--ad-package|--ai-package|--ad-artifact|--ah-artifact|--af-artifact|--ai-artifact|--output-parent)
...
[[ -f "$pin_validator" && ! -L "$pin_validator" && -s "$pin_validator" ]] || \
	die 'Candidate AJ package-pin validator is missing or unsafe'
python3 "$pin_validator" --package "$package" >/dev/null || \
	die 'Candidate AJ package identities are not reproduced and pinned'

[[ "$(uname -s)" == Linux ]] || die 'run in the Linux recovery VM'
case "$(uname -m)" in aarch64|arm64) ;; *) die 'expected Linux AArch64' ;; esac
for directory in "$package" "$ad_package" "$ai_package" "$ad_artifact" \
	"$ah_artifact" "$af_artifact" "$ai_artifact" "$output_parent"; do
...
"$ai_package"|"$ai_package"/*|"$ad_artifact"|"$ad_artifact"/*|\
...
for input in "$pin_validator" "$profile_validator" "$package_validator" "$boot_validator" \
	"$finalizer" "$ai_lineage_validator" "$gate_auditor" "$normalizer" \
...
workdir="$(mktemp -d "$output_parent/.candidate-AJ-cpu8-request.XXXXXX")"
...
python3 "$package_validator" --ad-package "$ad_package" \
	--ai-package "$ai_package" --candidate-package "$package" \
...
sed -e "s|$repo_root|@REPOSITORY@|g" -e "s|$workdir|@WORK@|g" \
	-e "s|$package|@AJ_PACKAGE@|g" -e "s|$ai_package|@AI_PACKAGE@|g" \
	-e "s|$ad_package|@AD_PACKAGE@|g" \
...
readonly AJ_INITRAMFS=gemini-a72-reject-cpu8-request-initramfs.img
readonly AJ_DTB=mt6797-gemini-pda-a72-reject-cpu8-request.dtb
...
install -m 0600 "$ad_artifact/$AD_INITRAMFS" "$stage/$AJ_INITRAMFS"
install -m 0600 "$ah_artifact/$AH_DTB" "$stage/$AJ_DTB"
...
cmp -s "$candidate" "$replica_boot" || \
	die 'two Candidate AJ container constructions differ'
...
python3 "$analyzer" --validate-lk --expected-image-gz "$stage/Image.gz" \
	--expected-ramdisk "$stage/$AJ_INITRAMFS" --expected-dtb "$stage/$AJ_DTB" \
...
python3 "$boot_validator" --candidate "$candidate" --image-gz "$stage/Image.gz" \
	--dtb "$stage/$AJ_DTB" --initramfs "$stage/$AJ_INITRAMFS" \
	...
	--ai-boot "$ai_artifact/$AI_BOOT" >"$stage/boot-validation.txt"
...
gate_audit_sha256="$(sha256sum "$stage/mt6797-psci-cpu-boot-audit.txt" | awk '{print $1}')"
{
	...
	printf 'kernel_profile=observability-fbcon-rotation-keyboard-wrrd-manual-reboot-smp8-a72-reject-gate-cpu8-request\n'
	...
	printf 'patch_delta_from_ai=none\n'
	printf 'config_delta_from_ai=maxcpus-8-to-maxcpus-9-only\n'
	printf 'config_inputs_sha256=9fa44c817649a81a633b0c2443e2d7bf73008af613431577b1cddc525121f409\n'
	printf 'config_sha256=64f1c3d1b9a506aad5b0ee0549188abac2fbcff12e9e8aacbda015cf4ee7b8cb\n'
	...
	printf 'final_dtb_lineage=byte-exact-candidate-ai\n'
	printf 'initramfs_helpers_lineage=byte-exact-candidate-ai\n'
	printf 'cpu_policy=maxcpus-9-cpu8-request-cpu9-not-requested\n'
	printf 'expected_gate_result=cpu8-eagain-minus-11\n'
...
output_name="candidate-AJ-a72-reject-cpu8-${candidate_sha256:0:8}"
...
workdir=
trap - EXIT
printf 'validation=candidate-aj-a72-reject-cpu8-request\nartifact=%s\n' "$output"
