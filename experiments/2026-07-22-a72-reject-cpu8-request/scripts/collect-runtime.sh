...
# Candidate AJ's initramfs intentionally retains Candidate AC's inherited USB
...
readonly VALIDATOR_SHA256=e7ec6aa3d9d00fdec8c5d7669956c3c979c21bc228278bcc24d973ef85eff089
...
[[ "$installed_full_sha256" == "$INSTALLED_FULL_SHA256" ]] || \
	die 'installed full-partition checksum is not Candidate AJ'
...
[[ -f "$validator" && ! -L "$validator" ]] || die 'AJ runtime validator is absent or unsafe'
[[ "$(shasum -a 256 "$validator" | awk '{ print $1 }')" == "$VALIDATOR_SHA256" ]] || \
	die 'AJ runtime validator source identity changed'
...
# package/artifact identity is selected and the selected padded identity agrees
# with this collector's immutable production boundary.
...
spec = importlib.util.spec_from_file_location("candidate_aj_collector_pins", path)
...
[[ "$pinned_full_sha256" == "$INSTALLED_FULL_SHA256" ]] || \
	die 'Candidate AJ padded identity disagrees with the runtime collector'
...
command_file="$(mktemp /tmp/candidate-aj-runtime-command.XXXXXX)"
...
printf '__AJ_IDENTITY_END__\n'
printf '__AJ_STAT1_BEGIN__\n'; /bin/busybox grep '^cpu[0-9]' /proc/stat; printf '__AJ_STAT1_END__\n'
/bin/busybox sleep 5
printf '__AJ_STAT2_BEGIN__\n'; /bin/busybox grep '^cpu[0-9]' /proc/stat; printf '__AJ_STAT2_END__\n'
printf '__AJ_STABILITY_BEGIN__\n'
...
printf '__AJ_STABILITY_END__\n'
printf '__AJ_DMESG_BEGIN__\n'; /bin/busybox dmesg; printf '__AJ_DMESG_END__\n'
exit
EOF

{
	printf '__AJ_HOST_BEGIN__\n'
