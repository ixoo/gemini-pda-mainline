...
# Wait for the Gemini USB gadget and stream the exact protected-readback probe
...
readonly PROBE_SHA256=190ad3e7b83b90e1517d946780240e135b7c99185d4b0dcd9016fceada5a592c
readonly CANDIDATE_SHA256=30ec9c56d6be78635f0ccf3ea626727763d71590c23778774c5c6366e4a5e75a

die() { printf 'error: %s\n' "$*" >&2; exit 2; }
usage() {
	printf 'usage: %s --output artifacts/runtime-captures/protected-readback-attempt-N/runtime.txt\n' "$0" >&2
}
...
[[ "$(dirname -- "$capture_dir")" == "$private_root" &&
	"$(basename -- "$capture_dir")" == protected-readback-attempt-* &&
	"$(basename -- "$output")" == runtime.txt ]] ||
	die 'output must be runtime.txt in one new protected-readback-attempt-* private child'
...
command_file="$(mktemp "${TMPDIR:-/tmp}/.protected-readback-runtime.XXXXXXXX")"
...
{
	printf '__PROTECTED_READBACK_HOST_BEGIN__\n'
	...
	printf '__PROTECTED_READBACK_HOST_END__\n'
} >"$output"
chmod 0600 "$output"
...
printf '%s\n' "$classification" | grep -E \
	'^(runtime_classification|runtime_reason|protected_readback|cpu8_cpu9_admission|claim_scope)=' \
