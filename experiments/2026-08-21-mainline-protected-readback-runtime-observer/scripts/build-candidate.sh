...
readonly RAW_SIZE=7636992
readonly BOOT2_SIZE=16777216
readonly BOOT_NAME=gemini-prbro
...
check_hash "$dtb" "$DTB_SHA256" 'protected-readback candidate DTB'
...
grep -q '^CONFIG_CMDLINE=".*maxcpus=8.*"$' "$config" ||
	die 'maxcpus=8 serviceability command line changed'
for symbol in mt6797_dvfsp_clock_backend_read mt6797_bigidvfs_backend_read \
...
workdir="$(mktemp -d "$output_parent/.protected-readback-candidate.XXXXXXXX")"
...
{
	printf 'validation=portable-fetched-protected-readback-kernel-package\n'
...
{
	printf 'experiment=2026-08-21-mainline-protected-readback-runtime-observer\n'
	printf 'repository_commit=%s\nprofile=%s\nkernel_release=%s\n' \
		"$REPOSITORY_COMMIT" "$PROFILE" "$RELEASE"
	printf 'image_sha256=%s\nimage_gzip_sha256=%s\ndtb_sha256=%s\n' \
		"$IMAGE_SHA256" "$IMAGE_GZIP_SHA256" "$DTB_SHA256"
	...
	printf 'runtime_hypothesis=one-clock-and-one-bigidvfs-readback-record\n'
	printf 'secure_writes_expected=0\ncpu_requests_expected=0\n'
...
printf 'validation=protected-readback-observer-candidate-build\n'
printf 'artifact=%s\ncandidate_sha256=%s\npadded_sha256=%s\n' \
