...
# Source-pin the passed failure-stage validator and retarget its exact package,
# layout, movement marker, and candidate identities.
set -euo pipefail
export LC_ALL=C
umask 077

readonly SOURCE_SHA256=b8c44549a50eb619970d7c9c0b2d2383b397fdbc6e27ba673ddb7fd37d149ab6
die() { printf 'error: %s\n' "$*" >&2; exit 2; }
...
source_validator="$repo_root/experiments/2026-08-25-mainline-a72-platform-provider-failure-stage-attribution/scripts/validate-candidate.sh"
...
derived=$(mktemp "$script_dir/.derived-validate-platform-movement.XXXXXXXX")
...
old_marker_tail = r'''        b"platform/provider/clock capture failed: stage=%s ret=%d",'''
new_marker_tail = r'''        b"platform/provider/clock capture failed: stage=%s ret=%d",
        b"platform/provider/clock capture failed: stage=platform ret=-11 movement=%03x cpu=%08x/%08x cpu2=%08x/%08x cpusys=%08x/%08x cpu0=%08x/%08x cpu1=%08x/%08x iso=%08x/%08x dcm=%08x/%08x cci-port=%08x/%08x pwrap=%u/%u",'''
replacements = (
    ...
    ("platform_provider_clock_markers=before-clock,after-clock,platform,provider,clock,complete,failure-stage", "platform_provider_clock_markers=before-clock,after-clock,platform,provider,clock,complete,failure-stage,movement-detail", 1),
    ("control_dtb=byte-identical-retired-third-reader-dtb", "control_dtb=byte-identical-retired-failure-stage-dtb", 1),
...
for old, new, count in replacements:
    ...
    if actual != count:
        raise SystemExit(
            f"unsafe movement validator derivation: expected {count}, found {actual}: {old}"
        )
