#!/usr/bin/env python3
"""Validate Candidate Orion's exact kernel package and fail-closed DT."""
...
def validate_fragment_provenance(
    ...
    if len(names) != len(FRAGMENTS):
        raise ValueError("Orion fragment basenames collide")
    packaged = tree_inventory(
...
def validate_config(data: bytes) -> None:
    ...
    if missing:
        raise ValueError("Orion kernel configuration lacks " + missing[0])
    if config.count("maxcpus=8") != 1 or "maxcpus=9" in config:
        raise ValueError("Orion CPU request boundary changed")
    if config.count("GEMINI_ORION_20260727") != 1:
        raise ValueError("Orion USB attribution marker changed")
...
def validate_system_map(data: bytes) -> None:
    ...
    if missing:
        raise ValueError("Orion System.map lacks " + missing[0])
    present = sorted(FORBIDDEN_SYMBOLS & symbols)
    if present:
        raise ValueError("Orion contains forbidden active symbol " + present[0])
...
def validate_image(image: bytes) -> None:
    ...
    if image.count(cmdline) != 2:
        raise ValueError(
            "Orion Image does not contain exactly two forced-cmdline copies"
        )
    markers = (
    ...
    for marker in markers:
        if marker not in image:
            raise ValueError(f"Orion Image lacks marker {marker!r}")
...
def load_fdt_parser(repository: pathlib.Path) -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location("orion_fdt", source)
...
def validate_dtb(repository: pathlib.Path, path: pathlib.Path) -> None:
    ...
    if "access-controllers" not in tree[i2c6]:
        raise ValueError("Orion I2C6 lacks its fail-closed handoff dependency")
    ...
    for forbidden in (
        ...
        if forbidden in tree:
            raise ValueError(f"Orion DT contains forbidden node {forbidden}")
...
def validate(repository: pathlib.Path, package: pathlib.Path) -> None:
    ...
    if manifest["config"]["profiles"].get(co.PROFILE) != {
        ...
        raise ValueError("Orion manifest profile changed")

    fragment = regular(repository / co.CONFIG_FRAGMENT, "Orion config fragment")
    if digest(fragment) != co.CONFIG_FRAGMENT_SHA256:
        raise ValueError("source-pinned Orion config fragment changed")
    repository_series = regular(repository / co.SERIES, "Orion patch series")
    if digest(repository_series) != co.SERIES_SHA256:
        raise ValueError("source-pinned Orion patch series changed")
    ...
    if len(entries) != 106 or tuple(entries[-4:]) != co.ORION_PATCHES:
        raise ValueError("Orion series ending changed")
    ...
    if (
        ...
        raise ValueError("package provenance is not exact Orion")
    ...
    validate_dtb(repository, package / GEMINI_DTB)
...
def main() -> int:
    ...
    print("validation=orion-fixed-i2c6-kernel-package")
    print(f"profile={co.PROFILE}")
    print(f"patch_series={co.SERIES}")
    print("patch_count=106")
