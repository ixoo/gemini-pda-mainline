"""Pinned inputs and safety boundaries for Candidate Orion."""
...
EXPERIMENT = "2026-07-27-mt6797-i2c6-orion"
CANDIDATE = "Orion"
PROFILE = (
    ...
    "ap-dma-preserve-orion"
)
SERIES = "patches/series-orion-i2c6-idvfs-fifo"
SERIES_SHA256 = "3c75e05153ae3c22e4ec1b29819049c83556369cd788a3a199da29e3542941a9"
CONFIG_FRAGMENT = "configs/gemini-i2c6-orion.fragment"
CONFIG_FRAGMENT_SHA256 = (
    "1e2f47a3562ab9d3750dbd8a00d65b147696028c0b850cc6cc0aa383af894a3d"
)
ORION_PATCHES = (
...
# Candidate Orion derives its serviceability payloads from the exact
# hardware-passed Hubble/Cassini rollback. The kernel and one compatible
# property are the only candidate inputs which change.
...
BOOT_MEMBER = "gemini-mt6797-i2c6-orion.boot.img"
DTB_MEMBER = "mt6797-gemini-pda-i2c6-orion.dtb"
INITRAMFS_MEMBER = HUBBLE_INITRAMFS_MEMBER
PADDED_MEMBER = "boot2-padded.img"
ARTIFACT_PREFIX = "candidate-Orion-mt6797-i2c6-"
...
def require_input_pins() -> None:
    ...
    hashes.update(
        {
            f"ORION_PATCH_SHA256[{index}]": value
            for index, value in enumerate(ORION_PATCH_SHA256)
        }
    )
    for name, value in hashes.items():
        if HEX256.fullmatch(value) is None:
            raise ValueError(f"Candidate Orion {name} is malformed")
    if len(ORION_PATCHES) != len(ORION_PATCH_SHA256):
        raise ValueError("Candidate Orion patch-pin inventory is inconsistent")
    if not 0 < HUBBLE_RAW_SIZE < BOOT2_SIZE:
        raise ValueError("Candidate Orion Hubble raw size is invalid")
