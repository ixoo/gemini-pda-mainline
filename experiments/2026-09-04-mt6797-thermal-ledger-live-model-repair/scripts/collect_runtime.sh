...
# Wait for the exact Gemini USB interface and capture one read-only thermal frame.
set -euo pipefail
export LC_ALL=C
umask 077
...
readonly REMOTE_SHA256=c2aad67673f4d1e0ee17ab2bf403efa750f0c5ac16bfc3655fc964f6a6173e3e
readonly CLASSIFIER_SHA256=9a885b5fc567d43fb2d014e9212c69232edc1a8df4263d51494befcb478319cb
readonly COMMAND_MARKER=__GEMINI_THERMAL_LEDGER_LIVE_MODEL_REPAIR_SCRIPT__
...
while (($#)); do
	...
	--recovery-boot-id|--installed-full-sha256|--output)
		...
		shift 2 ;;
	*) die "usage: $0 --recovery-boot-id UUID --installed-full-sha256 SHA256 --output artifacts/runtime-captures/thermal-ledger-live-model-repair-attempt-1" ;;
	esac
done
...
command_file=$(mktemp "${TMPDIR:-/tmp}/.gemini-thermal-ledger-live-model-observer.XXXXXXXX")
...
set -e
chmod 0600 "$frame"
grep -Fq __GEMINI_THERMAL_LEDGER_LIVE_MODEL_REPAIR_BEGIN__ "$frame" || die "observation frame did not begin rc=$nc_rc"
grep -Fq __GEMINI_THERMAL_LEDGER_LIVE_MODEL_REPAIR_END__ "$frame" || die "observation frame did not complete rc=$nc_rc"
python3 "$classifier" "$frame" --recovery-boot-id "$recovery_boot_id" >"$classification"
grep -Fqx 'classification=mt6797-thermal-ledger-live-model-repair-pass' "$classification" || die 'observation frame rejected'
printf 'netcat_complete=yes status=%s\ntrigger_session=none\nload_session=none\n' "$nc_rc" >>"$events"
printf 'temperature_reads=3\nnative_reboot_command_sent=no\nsuccessful_mainline_left_running=yes\n' >>"$events"
