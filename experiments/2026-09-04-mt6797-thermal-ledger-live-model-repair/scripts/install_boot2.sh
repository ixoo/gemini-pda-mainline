...
# Install the exact thermal-ledger live-model repair candidate and power off.
set -euo pipefail
export LC_ALL=C
...
readonly CANDIDATE_SHA256=93a78b490a9ffbf32eb60c5c875f508fd05b43b726220b3ccdbe9277792752a4
...
while (($#)); do
	...
	*) die "usage: $0 --output artifacts/device-deployments/thermal-ledger-live-model-repair-attempt-1" ;;
	esac
done
[[ -n "$output" ]] || die '--output is required'
for command in awk basename chmod date dirname git grep mkdir mktemp nc python3 rm sha256sum sleep ssh stat; do
...
package="$repo_root/artifacts/buildbox/$BUILD_COMMIT/linux-7.1.3-gemini-mt6797-thermal-stage-ledger-eb29842d-f213e5f0"
source="$repo_root/artifacts/mt6797-thermal-serviceability-dt-repair/candidate-mt6797-thermal-serviceability-dt-repair-dd7a6ec4"
candidate_dir="$repo_root/artifacts/mt6797-thermal-ledger-live-model-repair/$CANDIDATE_DIR"
candidate="$candidate_dir/boot2-padded.img"
readonly script_dir repo_root identity package source candidate_dir candidate
...
python3 "$script_dir/validate_candidate.py" --repository "$repo_root" --package "$package" \
	--source "$source" --candidate "$candidate_dir" >/dev/null
...
[[ "$(basename -- "$output")" == thermal-ledger-live-model-repair-attempt-1 ]] || die 'output directory identity changed'
...
remote_candidate=/tmp/gemini-thermal-live-model-repair-93a78b49.img
ssh "${ssh_options[@]}" "$TARGET" 'umask 077; rm -f /tmp/gemini-thermal-live-model-repair-93a78b49.img; cat >/tmp/gemini-thermal-live-model-repair-93a78b49.img' <"$candidate"
remote_script=$(mktemp "${TMPDIR:-/tmp}/.gemini-thermal-live-model-repair-install.XXXXXXXX")
cleanup() {
	rm -f -- "${remote_script:-}"
...
set -eu
export LC_ALL=C
candidate=/tmp/gemini-thermal-live-model-repair-93a78b49.img
expected_sha=93a78b490a9ffbf32eb60c5c875f508fd05b43b726220b3ccdbe9277792752a4
...
root_source=unknown
while read -r source_path mountpoint rest; do
	[ "$source_path" != "$target" ] || die 'boot2 is mounted'
	[ "$mountpoint" != / ] || root_source=$source_path
done </proc/mounts
...
printf '%s\n' __GEMINI_THERMAL_LEDGER_LIVE_MODEL_REPAIR_DEPLOYMENT_BEGIN__
...
printf '%s\n' __GEMINI_THERMAL_LEDGER_LIVE_MODEL_REPAIR_DEPLOYMENT_END__
rm -f "$candidate"
sync
poweroff
REMOTE
...
ssh_rc=$?
set -e
grep -Fq __GEMINI_THERMAL_LEDGER_LIVE_MODEL_REPAIR_DEPLOYMENT_BEGIN__ "$evidence" || die "deployment did not begin rc=$ssh_rc"
grep -Fq __GEMINI_THERMAL_LEDGER_LIVE_MODEL_REPAIR_DEPLOYMENT_END__ "$evidence" || die "deployment did not complete rc=$ssh_rc"
