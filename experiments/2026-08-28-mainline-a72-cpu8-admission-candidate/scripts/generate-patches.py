#!/usr/bin/env python3
"""Generate the binding and DT patch pair for the CPU8 candidate."""
...
PATCHES = (
    "0413-dt-bindings-soc-mediatek-add-MT6797-A72-admission-controller.patch",
    "0414-arm64-dts-mediatek-add-Gemini-CPU8-admission-candidate.patch",
)
SUBJECTS = (
    "dt-bindings: soc: mediatek: add MT6797 A72 admission controller",
    "arm64: dts: mediatek: add Gemini CPU8 admission candidate",
)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mt6797-cpu8-admission-dt-") as name:
        temp = Path(name)
        source = temp / "source"
        ...
        commit(source, "MT6797 CPU8 candidate post-0412 parent",
               "Exact relevant source copied from the canonical prepared tree through 0412.",
               "2026-08-28T21:10:00Z", check_diff=False)
        parent = run("git", "rev-parse", "HEAD", cwd=source)

        validations: list[str] = []
        for stage, subject, body, timestamp in (
            ("binding", SUBJECTS[0],
        ...
        for generated_path, name_out, subject in zip(
            ...
            run("perl", str(source_root / "scripts/checkpatch.pl"), "--strict",
                "--no-tree", "--ignore",
                "MISSING_SIGN_OFF,FILE_PATH_CHANGES,DT_SPLIT_BINDING_PATCH",
        ...
        validations.append(run(
            "python3", str(SCRIPT_DIR / "validate_source.py"),
            "--source-root", str(replay), "--stage", "dts", cwd=REPO_ROOT,
        ))
        ...
        (package / "provenance.txt").write_text(
            ...
            "standalone_observer_nodes=0\ncpu8_request_owner=controller\n"
            "cpu9_request_paths=0\nnative_vm_build=none\ndevice_action=none\n"
            "boot_candidate=false\n",
            encoding="utf-8",
        )
        checksummed = [*targets, package / "series", package / "source-validation.txt",
