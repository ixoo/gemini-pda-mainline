...
# Wait for the exact Gemini USB gadget and stream the read-only provenance
# probe directly into its netcat shell. The capture remains under artifacts/.
set -euo pipefail
export LC_ALL=C
umask 077
...
die() { printf 'error: %s\n' "$*" >&2; exit 2; }
usage() {
	printf 'usage: %s --output artifacts/runtime-captures/provenance-observer-attempt-N/runtime.txt\n' "$0" >&2
}
...
[[ "$(dirname -- "$capture_dir")" == "$private_root" &&
	"$(basename -- "$capture_dir")" == provenance-observer-attempt-* &&
	"$(basename -- "$output")" == runtime.txt ]] ||
	die 'output must be runtime.txt in one new provenance-observer-attempt-* private child'
...
command_file="$(mktemp "${TMPDIR:-/tmp}/.provenance-runtime.XXXXXXXX")"
...
set -e
printf '%s\n' "$classification" | grep -E '^(runtime_classification|runtime_reason|cpu8_cpu9_admission|claim_scope)=' \
