...
[[ "$(dirname -- "$evidence_dir")" == "$evidence_root" &&
	"$(basename -- "$evidence_dir")" == provenance-observer-deployment-* ]] ||
	die 'evidence directory must be one new provenance-observer-deployment-* child'
...
write)
	...
	[[ "$EXPECTED_STAGE" =~ ^/home/gemini/\.gemini-provenance-observer\.[A-Za-z0-9]+$ ]] ||
...
if [[ "$already_current" == no ]]; then
	stage="$("${ssh_command[@]}" "$target" 'umask 077; mktemp /home/gemini/.gemini-provenance-observer.XXXXXXXX')"
	stage=${stage//$'\r'/}
	[[ "$stage" =~ ^/home/gemini/\.gemini-provenance-observer\.[A-Za-z0-9]+$ ]] ||
