...
die() { printf 'error: %s\n' "$*" >&2; exit 2; }
usage() { echo "usage: $0 --package DIR --gate3-artifact DIR --output-parent DIR"; }
...
readonly IMAGE_SHA256=9462d48c8240a656155dc36f1acb5b3a572e7f913847ba8310f396c4ed1345ae
readonly CONFIG_SHA256=288a532de2ac512a1c95e5cf1b4ecfed3bd7a271305cc9abdb528d447872993e
readonly SYSTEM_MAP_SHA256=cb77083d4cff000cec37b1e64dad41e701a1ebae614d724f6fd3b51ed451f169
readonly SOURCE_BUILD_SHA256=524f65d3e16066a9e77aca046c803e06ce24b9a32b0cdcc1e5b051b91fbb4757
readonly OUTPUT_BOOT=gemini-mt6797-da921x-of-modalias-pre-dispatch-suppression.boot.img
...
[[ "$(jq -er '.kernel_release' "$package/provenance/build.json")" == \
	7.1.3-gemini-da921x-ofpredispatch ]] || die "kernel release changed"
grep -qx 'CONFIG_MODULES=y' "$package/kernel.config" || die "module support is not built in"
...
grep -qx 'CONFIG_I2C_GEMINI_DA921X_OF_MODALIAS_PRE_DISPATCH_SUPPRESSION_DIAGNOSTIC=y' \
	"$package/kernel.config" || die "pre-dispatch diagnostic is not enabled"
for symbol in \
	CONFIG_I2C_GEMINI_DA921X_OF_MODALIAS_DIAGNOSTIC \
	CONFIG_I2C_GEMINI_DA921X_OF_MODALIAS_PRIVATE_GENERATION_DIAGNOSTIC \
	CONFIG_I2C_GEMINI_DA921X_OF_MODALIAS_PRIVATE_INSERTION_DIAGNOSTIC \
	CONFIG_I2C_GEMINI_DA921X_OF_MODALIAS_REAL_ENV_ROLLBACK_DIAGNOSTIC; do
	grep -qx "# $symbol is not set" "$package/kernel.config" ||
		die "conflicting diagnostic enabled: $symbol"
done
grep -q 'maxcpus=8' "$package/kernel.config" || die "maxcpus=8 missing"

workdir="$(mktemp -d "$output_parent/.da921x-ofpredispatch-candidate.XXXXXX")"
...
install -m 0600 "$gate3_artifact/$GATE3_INITRAMFS" "$stage/$GATE3_INITRAMFS"
install -m 0600 "$gate3_artifact/$GATE3_INITRAMFS" "$replica/$GATE3_INITRAMFS"
...
for destination in "$stage/$OUTPUT_BOOT" "$replica/$OUTPUT_BOOT"; do
	python3 "$serializer" --kernel "$stage/Image.gz" \
		--ramdisk "$stage/$GATE3_INITRAMFS" --dtb "$stage/$GATE3_DTB" \
		--output "$destination" --name gemini-ofpre \
		--cmdline "$boot_cmdline" --kernel-addr 0x40200000 \
		--ramdisk-addr 0x45000000 --second-addr 0x40f00000 \
		--tags-addr 0x44000000 --lk-android8 >"${destination}.serializer"
done
...
python3 "$analyzer" --validate-lk --expected-image-gz "$stage/Image.gz" \
	--expected-ramdisk "$stage/$GATE3_INITRAMFS" --expected-dtb "$stage/$GATE3_DTB" \
	--expected-name gemini-ofpre --expected-cmdline "$boot_cmdline" \
...
padded_sha256="$(sha256sum "$stage/boot2-padded.img" | awk '{print $1}')"
{
	...
	printf 'kernel_release=7.1.3-gemini-da921x-ofpredispatch\n'
...
output_name="candidate-Gate3-da921x-ofpredispatch-${candidate_sha256:0:8}"
...
workdir=
trap - EXIT
printf 'validation=da921x-of-modalias-pre-dispatch-suppression-candidate\n'
