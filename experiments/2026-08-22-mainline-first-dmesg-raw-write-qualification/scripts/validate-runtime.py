#!/usr/bin/env python3
"""Classify the exact first-dmesg raw-write live result."""
...
BEGIN = "__FIRST_DMESG_RAW_WRITE_RUNTIME_BEGIN__"
END = "__FIRST_DMESG_RAW_WRITE_RUNTIME_END__"
CANDIDATE = "b96ec109b3f020fdaf0cdc6ca1733d012051e6607b5520a11d32a6441f569e96"
RELEASE = "7.1.3-gemini-checkpoint-first-dmesg"
...
def classify_text(text: str) -> tuple[str, str]:
    ...
    safety_keys = {
        ...
        "manual_stage_prefix_count", "first_dmesg_prefix_count",
        "manual_control_exact_count", "manual_stage_exact_count",
        "first_dmesg_exact_count", "block_mounts", "device_storage_writes",
    ...
    return "first-dmesg-raw-write-live-pass", "record-1-full-readback-and-serviceability"
...
def main() -> int:
    ...
    accepted = result == "first-dmesg-raw-write-live-pass"
    print(f"runtime_classification={result}")
    print(f"runtime_reason={reason}")
    print(f"first_dmesg_raw_write_live={'accepted' if accepted else 'not-accepted'}")
    print("retained_record_commits=1" if accepted else "retained_record_commits=unproved")
    print("local_full_readbacks=1" if accepted else "local_full_readbacks=unproved")
    print("protected_calls=0")
    print("clock_reads=0")
    print("bigidvfs_reads=0")
    print("DA921x_register_data_writes=0")
    print("cpu8_cpu9_admission=closed")
    print("claim_scope=record-1-writer-and-serviceability-only")
