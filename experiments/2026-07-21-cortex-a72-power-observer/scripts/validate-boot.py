...
def main() -> int:
    ...
    try:
        ...
        image_gz = read_regular(args.image_gz, "Candidate AE Image.gz")
        dtb = read_regular(args.dtb, "Candidate AE DTB")
        initramfs = read_regular(args.initramfs, "exact Candidate AD initramfs")
        if digest(image_gz) == AD_IMAGE_GZ_SHA256:
            raise ValueError("Candidate AE retained Candidate AD kernel")
        ...
        if digest(initramfs) != AD_INITRAMFS_SHA256:
            raise ValueError("Candidate AE initramfs is not byte-exact Candidate AD")
        ...
        for marker in (
            ...
            if marker not in image:
                raise ValueError(f"Candidate AE kernel lacks observer marker: {marker!r}")
        for marker in (
            b"mediatek,mt6797-a72-power\0",
            b"mediatek,mt6797-psci\0",
            b"dlg,da9214\0",
            ...
            if marker not in dtb:
                raise ValueError(f"Candidate AE DTB lacks observer resource: {marker!r}")
        ...
        if candidate[kernel_offset:kernel_end] != kernel:
            raise ValueError("kernel field is not AE Image.gz plus AE DTB")
        ...
        print("validation=candidate-ae-a72-observer-android-v0")
        ...
        print("container_delta=kernel-and-dtb-only")
