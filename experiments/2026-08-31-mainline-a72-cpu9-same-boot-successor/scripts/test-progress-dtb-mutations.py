#!/usr/bin/env python3
"""Run DT mutations against the CPU9 progress composition validator."""
...
replacements = (
    ('Path(__file__).with_name("validate-composed-dtb.py")',
     'Path(__file__).with_name("validate-progress-composed-dtb.py")', 1),
    ("provenance-serviceability-dtb-mutations",
     "cpu9-progress-dtb-mutations", 1),
)
for old, new, count in replacements:
    actual = text.count(old)
    if actual != count:
        raise SystemExit(
            f"unsafe CPU9 progress DT mutation derivation: expected {count}, "
            f"found {actual}: {old}"
        )
    text = text.replace(old, new)

namespace = {"__file__": str(SCRIPT), "__name__": "cpu9_progress_dtb_mutations"}
