#!/usr/bin/env python3
"""Validate the CPU9 CPUHP lock-repair provenance composition."""
...
SOURCE_SHA256 = "eaaeb7b5793afc2530904f455a9efcdb4838ad20bec21130ceb6ca66485ab53b"
SCRIPT = Path(__file__).resolve()
ROOT = SCRIPT.parents[3]
SOURCE = ROOT / (
    "experiments/2026-08-31-mainline-a72-cpu9-same-boot-successor/"
    "scripts/validate-progress-raw-lane-composed-dtb.py"
)
if hashlib.sha256(SOURCE.read_bytes()).hexdigest() != SOURCE_SHA256:
    raise SystemExit("source progress raw-lane composed-DT validator changed")
...
replacements = (
    ...
    ("cpu9-progress-raw-lane-composed-dtb-independent",
     "cpu9-cpuhp-lock-repair-composed-dtb-independent", 2),
)
for old, new, count in replacements:
    actual = text.count(old)
    if actual != count:
        raise SystemExit(
            f"unsafe CPUHP lock-repair DT validation derivation: "
...
namespace = {
    "__file__": str(SCRIPT),
    "__name__": "cpu9_cpuhp_lock_repair_dtb_validator",
}
...
def main() -> int:
    ...
    validate(args.serviceability_dtb, args.package_dtb, args.record_json,
             args.candidate)
    print("validation=cpu9-cpuhp-lock-repair-composed-dtb-independent")
