#!/usr/bin/env python3
"""Classify exact changed-ID Gemian recovery of global-initcall records."""
...
CANDIDATE = "e9d565021de9ed1164aa78a78795d6a3dabd7af656aaa3df791e23424e66125a"
DEPLOYMENT_BOOT_ID = "6ac4e0b6-2979-4a5b-851c-af7282fec216"
PREFIX = b"====0.000000-D\n"
MARKER_1 = (
    b"GEMINI_A72_INITCALL_V1 token=GAIC-20260824-A "
    b"checkpoint=subsys-init slot=1 crc32=cf2a6946\n"
)
MARKER_2 = (
    b"GEMINI_A72_INITCALL_V1 token=GAIC-20260824-A "
    b"checkpoint=fs-init slot=2 crc32=91ac2a49\n"
)
RECORD_1 = PREFIX + MARKER_1
RECORD_2 = PREFIX + MARKER_2
VALID_HEADER_1 = "444247436900000069000000"
VALID_HEADER_2 = "444247436500000065000000"
...
def record_state(record: bytes, marker: bytes, valid_header: str,
                 label: str) -> str:
...
def classify_text(path: Path) -> tuple[str, str, str]:
    ...
    for marker, record, state, label in (
        ...
        require(not record_count or state == "valid",
                f"{label}-pstore-direct-conflict")

    if state_1 == "empty" and state_2 == "empty":
        return (
            "before-subsys-init-or-writer-refused",
            "both-global-initcall-records-exact-empty",
            "move-earlier-or-expose-writer-refusal-attribution",
        )
    if state_1 == "valid" and state_2 == "empty":
        return (
            "subsys-init-only",
            "subsys-init-committed-fs-init-empty",
            "split-postcore-arch-subsys-fs-initcall-order",
        )
    if state_1 == "valid" and state_2 == "valid":
        return (
            "subsys-and-fs-initcalls",
            "both-global-initcall-records-exact",
            "split-fs-to-device-init-region",
        )
...
def main() -> int:
    ...
    print("claim_scope=global-initcall-order-localization-only")
