#!/usr/bin/env python3
"""Reject unsafe A72 global-initcall retained-recovery mutations."""
...
def load():
    ...
    spec = importlib.util.spec_from_file_location("a72_global_initcall_recovery", path)
...
def capture(state_1: str, state_2: str, pstore: bytes = b"") -> str:
    ...
    values = {
        ...
        "pstore_file_metadata_b64": encoded(b"dmesg-ramoops-0 206\n" if pstore else b""),
...
def result(text: str) -> str:
    with tempfile.TemporaryDirectory(prefix="gemini-a72-global-initcall-recovery-test.") as raw:
...
def main() -> None:
    ...
    subsys_only = capture("valid", "empty", RECOVERY.RECORD_1)
    ...
    require(result(neither) == "before-subsys-init-or-writer-refused",
            "neither branch rejected")
    require(result(subsys_only) == "subsys-init-only",
            "subsys-only branch rejected")
    require(result(both) == "subsys-and-fs-initcalls", "both branch rejected")
    ...
    mutations = (
        ...
        corrupt_payload,
        both.replace("record_1_header=" + RECOVERY.VALID_HEADER_1,
    ...
    print("validation=a72-global-initcall-recovery-tools")
