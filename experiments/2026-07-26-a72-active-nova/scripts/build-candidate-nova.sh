...
# Assemble Candidate Nova from a validated kernel package and the exact
...
module="$script_dir/candidate_nova.py"
value() { PYTHONPATH="$script_dir" python3 -c 'import candidate_nova as c,sys; print(getattr(c,sys.argv[1]))' "$1"; }
...
validator="$script_dir/validate-package-nova.py"
dtb_builder="$script_dir/build-nova-dtb.sh"
...
workdir="$(mktemp -d "$output_parent/.candidate-nova.XXXXXX")"
...
for output in "$stage/$BOOT_MEMBER" "$replica/$BOOT_MEMBER"; do
	python3 "$serializer" --kernel "$stage/Image.gz" --ramdisk "$stage/$INITRAMFS_MEMBER" \
		--dtb "$stage/$DTB_MEMBER" --output "$output" --name gemini-nova \
...
python3 "$analyzer" --validate-lk --expected-image-gz "$stage/Image.gz" \
	...
	--expected-name gemini-nova --expected-cmdline "$boot_cmdline" \
...
experiment=2026-07-26-a72-active-nova
candidate=Nova
...
cpu8=active-experiment-dtb-provider-and-late-hotplug-retry
cpu9=fail-closed
...
[[ "$actual" == "$expected" ]] || die 'Candidate Nova output inventory changed'
(cd "$stage" && find . -type f ! -name SHA256SUMS -print0 | sort -z | xargs -0 sha256sum) >"$stage/SHA256SUMS"
(cd "$stage" && sha256sum --check --strict SHA256SUMS >/dev/null) || die 'Candidate Nova manifest failed'
chmod 0600 "$stage"/*
