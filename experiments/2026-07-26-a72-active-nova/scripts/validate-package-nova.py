#!/usr/bin/env python3
"""Validate the exact Nova kernel package before boot assembly."""
...
def validate(repository: pathlib.Path, package: pathlib.Path) -> None:
    ...
    if profile is None or profile["base"] != "defconfig":
        raise ValueError("Nova profile is missing from the repository manifest")
    if profile["patch_series"] != cg.SERIES:
        raise ValueError("Nova patch-series identity differs")
    build = json.loads(regular(package / "provenance/build.json", "build provenance"))
    if build.get("build_profile") != cg.PROFILE:
        raise ValueError("package build profile is not Nova")
    ...
    if (
        ...
        or not entries[-1].endswith(
            "0112-soc-mediatek-retry-MT6797-A72-CPU8-after-provider.patch"
        )
    ):
        raise ValueError("Nova series does not end in the active CPU8 retry patch")
    config = regular(package / "kernel.config", "kernel config").decode("ascii")
    required = (
        'CONFIG_LOCALVERSION="-gemini-nova"',
        ...
        'g_ether.iProduct=Gemini-L-Nova '
        'g_ether.iSerialNumber=GEMINI_NOVA_20260726 '
    ...
    if missing:
        raise ValueError(f"required Nova config is missing: {missing[0]}")
    ...
    for symbol in (
        ...
        if symbol not in system_map:
            raise ValueError(f"System.map lacks Nova symbol: {symbol}")
    print("validation=candidate-nova-package")
