#!/usr/bin/env python3
"""Require exact AO plus Nova's active A72 provider DT contract."""
...
def validate(ao_path: pathlib.Path, candidate_path: pathlib.Path) -> None:
    ...
    require_regular(candidate_path, "Candidate Nova DT")
    ...
    candidate_header = require_header(candidate_path, AS_HEADER, "Candidate Nova")
    if reservation_map_bytes(candidate_path, candidate_header) != (
        reservation_map_bytes(ao_path, ao_header)
    ):
        raise ValueError("Candidate Nova changed AO's raw FDT reservation map")
    ...
    if candidate_reservations != ao_reservations:
        raise ValueError("Candidate Nova changed AO's FDT reservation map")
    if candidate_boot_cpu != ao_boot_cpu:
        raise ValueError("Candidate Nova changed AO's boot_cpuid_phys")
    ...
    if candidate != expected:
        raise ValueError(
            "Candidate Nova DT is not exact AO plus the active A72 contract: "
    ...
    if handles != expected_handles:
        raise ValueError("Candidate Nova phandle map changed beyond active A72 contract")
    if candidate[HANDOFF][ACCESS_CELLS_PROPERTY] != fdt.cells(0):
        raise ValueError("Candidate Nova handoff access-controller cells changed")
    reference = candidate[I2C6][DEPENDENCY_PROPERTY]
    if len(reference) != 4:
        raise ValueError("Candidate Nova I2C6 dependency is not one phandle")
    if handles.get(struct.unpack(">I", reference)[0]) != HANDOFF:
        raise ValueError("Candidate Nova I2C6 dependency does not resolve to handoff")

    if A72_POWER not in candidate:
        raise ValueError("Candidate Nova is missing the active A72 provider node")
...
def main() -> int:
    ...
    print("validation=candidate-nova-exact-active-a72-dtb-contract")
