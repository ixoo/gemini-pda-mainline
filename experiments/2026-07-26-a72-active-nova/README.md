# Experiment: Candidate Nova — active MT6797 CPU8
...
Assembly is storage-inert. The builder requires a validated Nova VM package,
...
1. Build and validate the Nova kernel package with \`./scripts/dev-vm
   build-kernel\`.
2. Run \`scripts/build-candidate-nova.sh\` with the exact AO artifact and
...
and container validation passed for the corrected image. Nova adds an active A72
...
initialization. Runtime evidence will be recorded in \`results/\` after boot.
