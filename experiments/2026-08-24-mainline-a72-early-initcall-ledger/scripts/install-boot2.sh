...
# early-initcall candidate, evidence names, and experiment identity.
set -euo pipefail
export LC_ALL=C
umask 077
...
derived=$(mktemp "$script_dir/.derived-install-boot2-a72-early-initcall.XXXXXXXX")
...
replacements = (
    (
        ...
        "# Source-pin the guarded two-record installer and retarget only its exact\n"
        "# A72 early-initcall candidate, evidence names, and experiment identity.",
        1,
    ),
    (
        ".derived-install-boot2-a72-physical-source.XXXXXXXX",
        ".derived-install-boot2-a72-early-initcall.XXXXXXXX",
        2,
    ),
    ("exact A72 physical-source", "exact A72 early-initcall", 1),
    (
    ...
    (
        ...
        "7e2c47c9f46e24b0778848bdeec5b303ab0674b5b336cd812fae4336833a0b5f",
        1,
    ),
    (
        "candidate-a72-physical-source-1d0c1420",
        "candidate-a72-early-initcall-8bff9059",
        1,
    ),
    (
        "a72-physical-source-deployment-",
        "a72-early-initcall-deployment-",
        1,
    ),
    (
        r"\.gemini-a72-physical-source\.",
        r"\.gemini-a72-early-initcall\.",
        1,
    ),
    (
        "/home/gemini/.gemini-a72-physical-source.XXXXXXXX",
        "/home/gemini/.gemini-a72-early-initcall.XXXXXXXX",
        1,
    ),
    (
        "experiment=2026-08-24-mainline-a72-physical-source-observer",
        "experiment=2026-08-24-mainline-a72-early-initcall-ledger",
        1,
    ),
    (
        "unsafe A72 physical-source installer derivation",
        "unsafe A72 early-initcall installer derivation",
        1,
    ),
    (
        "unsafe physical-source installer derivation",
        "unsafe early-initcall installer derivation",
        1,
    ),
    (
        "live A72 physical-source preflight failed",
        "live A72 early-initcall preflight failed",
        1,
    ),
)
for old, new, count in replacements:
    ...
    if actual != count:
        raise SystemExit(
            f"unsafe early-initcall installer derivation: expected {count}, "
            f"found {actual}: {old}"
        )
