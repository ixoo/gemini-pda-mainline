...
# live-GPT boot2 write/readback/shutdown workflow for the prefix candidate.
set -euo pipefail
export LC_ALL=C
umask 077
...
derived="$(mktemp "$script_dir/.derived-install-boot2-manual-prefix.XXXXXXXX")"
...
replacements = (
    ...
    ("14b92d7864899601f9d7bd889ecf837483ffc58ced5d880b03eee727d81a4107",
     "bab777146bff18c83c698cfee6f957a806252a696720ae0f1f59d947c8886990", 1),
    ("candidate-manual-checkpoint-control-4338ac1e",
     "candidate-manual-checkpoint-prefix-control-1d69e033", 1),
    ("manual-checkpoint-control-deployment-", "manual-checkpoint-prefix-control-deployment-", 1),
    (r"\.gemini-manual-checkpoint-control\.", r"\.gemini-manual-checkpoint-prefix-control\.", 1),
    ("/home/gemini/.gemini-manual-checkpoint-control.XXXXXXXX",
     "/home/gemini/.gemini-manual-checkpoint-prefix-control.XXXXXXXX", 1),
    ...
    (".derived-install-boot2-manual-checkpoint-inner.XXXXXXXX",
     ".derived-install-boot2-manual-prefix-inner.XXXXXXXX", 1),
)
for old, new, count in replacements:
    actual = text.count(old)
    if actual != count:
        raise SystemExit(
            f"unsafe prefix installer derivation: expected {count}, found {actual}: {old}"
        )
