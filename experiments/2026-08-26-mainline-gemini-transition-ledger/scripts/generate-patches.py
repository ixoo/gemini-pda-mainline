#!/usr/bin/env python3
"""Generate exact Gemini transition-ledger format patches."""
...
PARENT_HASHES = {
    "fs/pstore/Kconfig":
        "06b7cf90786e32c2475c7eb0b0fcc775bdccaa58f304f9220ef11c24fa485a37",
    "fs/pstore/Makefile":
        "cc4f78cdff34d4bc5ea0fdc2246c82966bac77983f9bf22bf4025e46713f386f",
    "fs/pstore/ram.c":
        "e02ac9017852cf4b64298937be7eccc9b9ba0dcd4bc5e4376c40160c39f67688",
}
NEW_PATHS = (
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(
        prefix="gemini-transition-ledger-generation-"
    ) as name:
        temp = Path(name)
        source = temp / "source"
        ...
        run("git", "config", "user.email", "gemini-mainline@example.invalid",
            cwd=source)
        commit(
            source, "Gemini pstore post-0387 parent",
            "Exact relevant source copied from the canonical prepared tree through 0387.",
        ...
        commit(
            source, "pstore: add Gemini mutable transition ledger",
            "Add one default-off retained zone with two alternating, CRC-committed\n"
            "copies for the last attributable CPU8 transition stage.",
            "2026-08-27T02:01:00Z",
        )
        ...
        commit(
            source, "pstore: test Gemini mutable transition ledger",
            "Cover exact sequencing, raw-header commit, refusal, torn writes,\n"
            "corrupt-copy recovery, and terminal sealing in memory.",
        ...
        package.mkdir()
        subjects = (
            "[PATCH 1/2] pstore: add Gemini mutable transition ledger",
            "[PATCH 2/2] pstore: test Gemini mutable transition ledger",
        )
        ...
        provenance = (
            ...
            f"parent_kconfig_sha256={PARENT_HASHES['fs/pstore/Kconfig']}\n"
            f"parent_makefile_sha256={PARENT_HASHES['fs/pstore/Makefile']}\n"
            f"parent_ram_sha256={PARENT_HASHES['fs/pstore/ram.c']}\n"
            "generated_patch_count=2\n"
            "retained_zone_count=1\n"
            "alternating_copies=2\n"
            "wire_words_per_copy=9\n"
            "success_checkpoint_updates=19\n"
            "focused_kunit_cases=6\n"
            "physical_retained_writes=0\n"
