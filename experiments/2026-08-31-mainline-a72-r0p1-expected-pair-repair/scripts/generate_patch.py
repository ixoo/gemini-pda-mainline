#!/usr/bin/env python3
"""Generate and audit the exact A72 r0p1 expected-pair repair."""
...
PATCH_NAME = "0459-arm64-bind-Gemini-A72-r0p1-expected-MIDR.patch"
SUBJECT = "arm64: bind Gemini A72 r0p1 expected MIDR"
ACTION_TOKENS = (
...
def commit(root: Path, subject: str, body: str, minute: int) -> None:
    ...
    environment.update({
        ...
        "GIT_AUTHOR_DATE": f"2026-08-31T21:{minute:02d}:00Z",
        "GIT_COMMITTER_DATE": f"2026-08-31T21:{minute:02d}:00Z",
    })
...
def prepare_parent(source_root: Path, root: Path) -> None:
    ...
    commit(root, "Gemini post-0458 generation parent", "Synthetic parent only.", 14)
...
def validate_result(root: Path, parent_counts: dict[str, int]) -> list[str]:
    ...
    return [
        "r0p1_expected_pair_validation=pass",
        "expected_midr=0x410fd081",
        "capability_model_matching=all-revisions-unchanged",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="a72-r0p1-expected-pair-") as name:
        ...
        commit(
            root,
            SUBJECT,
            "The immutable Gemini expected pair came from r0p1 target-local\n"
            "capsules, but stored the revision-neutral Cortex-A72 model base.\n"
            "Mainline CPU8 consequently reached CPU-info capture and was\n"
            "parked by the exact late-target validator with MIDR as its sole\n"
            "mismatch.\n\n"
            "Bind that expected pair to r0p1. Keep revision-neutral capability\n"
            "matching and every CPU, power, retry, and CPU9 path unchanged.",
            15,
        )
        changed = run(
