...
ROOT = REPO / 'artifacts/consys-handoff/session-1'
    ...
    try:
        ...
        if result.get('preservation', {}).get('log_complete') and log.is_file():
            for label, key, states in (
                ('mt6797-consys-status', 'consys_state',
                 ('off', 'on', 'mixed', 'moved', 'unavailable')),
                ('mt6797-consys-handoff', 'handoff_state',
                 ('matching', 'disabled', 'mismatch', 'unprotected', 'moved', 'unavailable')),
            ):
                lines = [line for line in log.read_bytes().splitlines()
                         if (label + ': state=').encode() in line]
                if len(lines) == 1:
                    record = lines[0].decode('ascii', errors='replace')
                    for state in states:
                        if label + ': state=' + state in record:
                            result[key] = state
                            break
        (ROOT / 'handoff-result.json').write_bytes(encoded(result))
        prepared['finish'].sync_directory(ROOT)
        print(json.dumps(result, sort_keys=True))
        return 0 if result.get('consys_state') and result.get('handoff_state') else 1
