...
# installed trace-softfail candidate, experiment identity, and private name.
set -euo pipefail
export LC_ALL=C
umask 077
...
derived=$(mktemp "$script_dir/.derived-collect-softtrace-recovery.XXXXXXXX")
...
replacements = (
    ...
    ("experiment=2026-08-28-mainline-a72-admission-durable-candidate",
     "experiment=2026-08-28-mainline-a72-admission-trace-softfail", 1),
    (".a72-admission-recovery.XXXXXXXX",
     ".a72-admission-softtrace-recovery.XXXXXXXX", 1),
)
for old, new, count in replacements:
    actual = text.count(old)
    if actual != count:
        raise SystemExit(
            f"unsafe trace-softfail recovery derivation: expected {count}, "
            f"found {actual}: {old}"
        )
