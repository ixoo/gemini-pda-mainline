#!/usr/bin/env python3
"""Generate and replay the two live trace soft-failure patches."""
...
PATCHES = (
    "0421-soc-mediatek-keep-live-admission-past-trace-failure.patch",
    "0422-soc-mediatek-test-live-admission-trace-softfail.patch",
)
SUBJECTS = (
    "soc: mediatek: keep live admission past trace failure",
    "soc: mediatek: test live admission trace softfail",
)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mt6797-admission-softtrace-") as name:
        temp = Path(name)
        source = temp / "source"
        ...
        run("git", "config", "user.email", "gemini-mainline@example.invalid",
            cwd=source)
        commit(
            source, "MT6797 admission trace-softfail post-0420 parent",
            "Exact relevant source copied from the canonical prepared tree through 0420.",
            "2026-08-29T01:00:00Z", check_diff=False,
        )
        ...
        stages = (
            (
                ...
                "Keep trace errors visible while only the explicit live-trigger mode\n"
                "continues to the unchanged one-shot CPU8 admission core.",
                "2026-08-29T01:01:00Z",
            ),
            (
                "tests", SUBJECTS[1],
                "Prove live entry and terminal trace errors are advisory while the\n"
                "automatic policy remains fail-closed.",
        ...
        validations.append(run(
            "python3", str(SCRIPT_DIR / "validate_source.py"),
            "--source-root", str(replay), "--stage", "tests", cwd=REPO_ROOT,
        ))
        ...
        (package / "provenance.txt").write_text(
            ...
            "live_trace_failure_advisory=1\nautomatic_trace_failure_fatal=1\n"
