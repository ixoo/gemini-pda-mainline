...
# Wait for the Gemini USB gadget and stream the exact matched provider-only
# control probe into its netcat shell. Raw capture stays below ignored artifacts/.
set -euo pipefail
export LC_ALL=C
umask 077
...
readonly PROBE_SHA256=17fbc91379f1f67cb03f16870743f6b8c21e4a971807577ca284e40c2db315be
readonly CANDIDATE_SHA256=3188d474f5d6989a0eb0782cdfac781efaf43dd42a0bd11481277050e735f8a2

die() { printf 'error: %s\n' "$*" >&2; exit 2; }
usage() {
	printf 'usage: %s --output artifacts/runtime-captures/da921x-provider-control-attempt-N/runtime.txt\n' "$0" >&2
}
...
[[ "$(dirname -- "$capture_dir")" == "$private_root" &&
	"$(basename -- "$capture_dir")" == da921x-provider-control-attempt-* &&
	"$(basename -- "$output")" == runtime.txt ]] ||
	die 'output must be runtime.txt in one new da921x-provider-control-attempt-* private child'
...
command_file="$(mktemp "${TMPDIR:-/tmp}/.da921x-provider-control.XXXXXXXX")"
...
{
	printf '__DA921X_PROVIDER_CONTROL_HOST_BEGIN__\n'
	...
	printf '__DA921X_PROVIDER_CONTROL_HOST_END__\n'
} >"$output"
chmod 0600 "$output"
...
printf '%s\n' "$classification" | grep -E \
	'^(runtime_classification|runtime_reason|matched_control|cpu8_cpu9_admission|claim_scope)=' \
