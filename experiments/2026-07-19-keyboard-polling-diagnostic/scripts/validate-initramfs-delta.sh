...
die() { printf 'error: %s\n' "$*" >&2; exit 2; }
[[ $# -eq 2 ]] || die "usage: validate-initramfs-delta.sh P_INITRAMFS U_INITRAMFS"
baseline=$1
candidate=$2
...
readonly U_SHA256=d05b29071cc8d2be8795c246b527ed315591a2e8e692cf6d050d4c7915783f4f
...
[[ "$(sha256sum "$candidate" | cut -c1-64)" == "$U_SHA256" ]] || die "candidate initramfs hash mismatch"
gzip -t "$candidate"
...
expected = {".", "bin", "dev", "etc", "proc", "run", "sys", "init",
            ...
            "bin/u-probe", "bin/u-pass", "etc/inittab"}
...
executables = {"init", "bin/busybox", "bin/input-event-capture", "bin/local-shell",
               "bin/u-probe", "bin/u-pass"}
...
grep -Fq 'GEMINI_KEYBOARD_POLLING_20260719_U' "$workdir/root/init" || die "U marker absent"
! grep -Fq 'input-event-capture' "$workdir/root/init" || die "PID1 still blocks on the input probe"
grep -Fxq '::once:/bin/u-probe' "$workdir/root/etc/inittab" || die "probe is not independent"
grep -Fxq 'tty1::respawn:/bin/local-shell' "$workdir/root/etc/inittab" || die "tty1 is not supervised"
# The dollar expression is literal archive content, not validator expansion.
# shellcheck disable=SC2016
grep -Fq 'while [ "$discovery_second" -lt 20 ]' "$workdir/root/bin/u-probe" || \
	die "bounded event discovery retry absent"
# shellcheck disable=SC2016
grep -Fq 'raw_event_window=begin discovery_wait=${discovery_second}s capture_budget=60s' \
	"$workdir/root/bin/u-probe" || die "bounded event window absent"
grep -Fq '/bin/input-event-capture' "$workdir/root/bin/u-probe" || die "probe helper invocation absent"
...
for forbidden in 'watchdog' 'reboot -f' '/sys/devices/system/cpu/' '/dev/mmc' '/dev/mem' \
	...
	! grep -R -Fq -- "$forbidden" "$workdir/root/init" "$workdir/root/bin/local-shell" \
		"$workdir/root/bin/u-probe" "$workdir/root/bin/u-pass" "$workdir/root/etc/inittab" || \
		die "forbidden action: $forbidden"
done
printf 'validation=candidate-u-initramfs-delta\n'
printf 'baseline_sha256=%s\ncandidate_sha256=%s\nbusybox_sha256=%s\n' \
	"$P_SHA256" "$U_SHA256" "$BUSYBOX_SHA256"
...
printf 'pid1_probe_dependency=none\nautomatic_reboot=no\nnetwork_action=none\nstorage_action=none\n'
