#!/usr/bin/env python3
"""Generate and replay the read-only READY predicate diagnostic patch."""
...
PATCH = "0438-arm64-report-Gemini-late-CPU-plan-predicates.patch"
SUBJECT = "arm64: report Gemini late CPU plan predicates"
...
def commit(root: Path, subject: str, body: str, minute: int) -> None:
    ...
    environment.update({
        ...
        "GIT_AUTHOR_DATE": f"2026-08-30T18:{minute:02d}:00Z",
        "GIT_COMMITTER_DATE": f"2026-08-30T18:{minute:02d}:00Z",
    })
...
def validate_patch(path: Path) -> None:
    ...
    for token in (
        ...
        "mt6797_a72_validate_cap_plan_contract",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(
        prefix="a72-ready-plan-predicate-"
    ) as name:
        temporary = Path(name)
        ...
        run("git", "config", "user.email", "gemini-mainline@example.invalid",
            cwd=source)
        commit(
            source, "Gemini READY predicate diagnostic post-0437 parent",
        ...
        commit(
            source, SUBJECT,
            "The exact post-0437 production candidate still rejects READY,\n"
            "but the public blocker mask groups every internal plan predicate\n"
            "under one bit. That result cannot identify a source-local repair.\n\n"
            "Keep the existing validator as the sole return-value owner and\n"
            "report read-only whole-plan and bound-evidence failure bitmaps.\n"
            "The diagnostic runs only after a failed validation and performs\n"
            "no CPU, firmware, power, storage, retry, or CPU_OFF operation.", 1,
        )
