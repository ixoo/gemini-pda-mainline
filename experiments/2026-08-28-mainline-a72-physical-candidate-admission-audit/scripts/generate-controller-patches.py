#!/usr/bin/env python3
"""Generate the two-patch one-shot CPU8 admission-controller series."""
...
from controller_source_edits import PARENT_HASHES


SCRIPT_DIR = Path(__file__).resolve().parent
EXPERIMENT = SCRIPT_DIR.parent
REPO_ROOT = EXPERIMENT.parents[1]
TEMPLATES = EXPERIMENT / "kernel"
PATCHES = (
    "0411-soc-mediatek-add-one-shot-CPU8-admission-controller.patch",
    "0412-soc-mediatek-test-one-shot-CPU8-admission-controller.patch",
)
SUBJECTS = (
    "soc: mediatek: add one-shot CPU8 admission controller",
    "soc: mediatek: test one-shot CPU8 admission controller",
)
...
def main() -> None:
    ...
    if not integrity_path.is_file() or integrity_path.is_symlink():
        raise SystemExit("prepared source integrity is unavailable")
    for template in (
        "mt6797-a72-admission-controller.c",
        "mt6797-a72-admission-controller-internal.h",
        "mt6797-a72-admission-controller-test.c",
    ):
        path = TEMPLATES / template
        if not path.is_file() or path.is_symlink():
            raise SystemExit(f"controller template is unavailable: {template}")

    with tempfile.TemporaryDirectory(
        prefix="mt6797-a72-admission-controller-generation-"
    ) as name:
        temp = Path(name)
        source = temp / "source"
        ...
        commit(
            source,
            "MT6797 admission controller post-0410 parent",
            "Exact relevant source copied from the canonical prepared tree through 0410.",
            "2026-08-28T18:20:00Z", check_diff=False,
        )
        parent = run("git", "rev-parse", "HEAD", cwd=source)

        run(
            "python3", str(SCRIPT_DIR / "controller_source_edits.py"),
            "--source-root", str(source), "--stage", "production",
            "--template-root", str(TEMPLATES), cwd=REPO_ROOT,
        )
        production_validation = run(
            "python3", str(SCRIPT_DIR / "validate_controller_source.py"),
            "--source-root", str(source), "--stage", "production",
            cwd=REPO_ROOT,
        )
        commit(
        ...
        commit(
            source, SUBJECTS[1],
            "Exercise pre-consumption gates, exact same-task order, every\n"
            "terminal boundary, one request, and repeat closure in memory.",
            "2026-08-28T18:22:00Z",
        )
        ...
        package.mkdir()
        targets: list[Path] = []
        for generated_path, output_name, subject in zip(
            generated, PATCHES, SUBJECTS, strict=True
        ):
            target = package / output_name
        ...
        (package / "provenance.txt").write_text(
            ...
            f"prepared_source_integrity={integrity_path.read_text(encoding='utf-8').strip()}\n"
            "generated_patch_count=2\n"
            "binder_ready_accessors=1\n"
            "source_registrations=1\n"
            "controller_kunit_cases=5\n"
            "derived_kunit_cases=5\n"
            "consumed_before_owner_mutation=true\n"
            "same_task_request=true\n"
            "production_cpu8_request_call_sites=1\n"
            "production_cpu9_request_call_sites=0\n"
            "cpu_off_call_sites=0\n"
            "retry_call_sites=0\n"
            "base_dt_enablements=0\n"
            "physical_test_operations=0\n"
