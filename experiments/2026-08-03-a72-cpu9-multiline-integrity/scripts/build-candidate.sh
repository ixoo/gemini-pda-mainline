...
# Source-pin and derive the multiline-integrity Android-v0 candidate builder
# from the accepted pair-v4 constructor.
set -euo pipefail
export LC_ALL=C
umask 077
...
if [[ -n "$bundle" ]]; then
	[[ -f "$bundle/provenance/build.json" && ! -L "$bundle/provenance/build.json" ]] ||
		die 'multiline compile provenance is missing or unsafe'
	...
	[[ "$(jq -er '.coherence_patchset_sha256' "$bundle/provenance/build.json")" == \
		...
		die 'pair-v4 parent patchset changed'
	[[ "$(jq -er '.multiline_patchset_sha256' "$bundle/provenance/build.json")" == \
		...
		die 'multiline patchset changed'
	[[ "$(jq -er '.purpose' "$bundle/provenance/build.json")" == \
		cpu9-multiline-integrity-compile-review-only ]] ||
		die 'multiline compile purpose changed'
fi

derived="$(mktemp "${TMPDIR:-/tmp}/.derived-a72-cpu9-multiline-builder.XXXXXXXX")"
cleanup() { [[ ! -e "${derived:-}" ]] || rm -f -- "$derived"; }
trap cleanup EXIT
chmod 0700 "$derived"
perl -0pe '
	s#script_dir="\$\(cd -- "\$\(dirname -- "\$\{BASH_SOURCE\[0\]\}"\)" && pwd -P\)"#script_dir="\${GEMINI_CPU9_MULTILINE_SCRIPT_DIR:?missing}"#g;
	s#118ff3cb3e9a2fbee10a44ada748e46bbe9b5312#fb647817cd573e3dd8719821da8742bc5433979b#g;
	s#9158af17b17e483ec68257378e5c4bd923b254e7242b5ba338f1324eec5f960b#81f076198ae314d187790beecee8d9b5edda3c4432e51a0f36a22dbe326fc468#g;
	s#53046cf314f76f213abafa53a1e79758ff835941d78a47ecc878d0a2e1ad3789#4e3c1b1095ee87e0af3c45595ea83d859d53188a53aba10672202bf45938986a#g;
	s#936b9256709514938ddf2f3ab13e63bd9c8d37e991fe40a568aa36a8f8818018#5227729e34ca42cf606f43008ec753fce15147693ce7a670818db58c5903fa48#g;
	s#readonly HELD_PATCHSET_SHA256=e6da1e8cc976ad63dd9cf8a254bb7234d73589cfad6afeb07135403a27d03ba3#readonly HELD_PATCHSET_SHA256=e6da1e8cc976ad63dd9cf8a254bb7234d73589cfad6afeb07135403a27d03ba3\nreadonly LATE_PATCHSET_SHA256=f2bebc4a04888a6c83f0dc72c2de56d350c2a52ab9779ef07e3a01cb5b544d91\nreadonly CPU9_PATCHSET_SHA256=17733d2ae50c16f9d0db2d4bd4075fa5a72ce081606db7d3bf1bfe83f4159a2b\nreadonly WINDOW_PATCHSET_SHA256=9ce572fbc87a1444bb71894dd4528f39dc065065a45b36db52a14791f167eeec\nreadonly TERMINAL_PATCHSET_SHA256=2d94a2cd489e33a7df854ffec7533fbf969dc9c810e9eece57d118b905060310\nreadonly COHERENCE_PATCHSET_SHA256=d4c40577b9e91fedfde048b29cb203311de264c526c71e3abd907fc6fafcf67f\nreadonly MULTILINE_PATCHSET_SHA256=c7a9b020563c4abb74059bbf72705839c528a81d577c7031ddfb36de647fd896#g;
	s#held-online-compile-review-only#cpu9-multiline-integrity-compile-review-only#g;
	s#\.gemian-a72-held#\.gemian-a72-cpu9-multiline#g;
	s#gemian-a72-cpu8-held-online\.boot\.img#gemian-a72-cpu9-multiline-integrity.boot.img#g;
	s#experiment=2026-08-02-a72-cpu8-held-online#experiment=2026-08-03-a72-cpu9-multiline-integrity#g;
	s#printf '\''held_patchset_sha256=%s\\n'\'' "\$HELD_PATCHSET_SHA256"#printf '\''held_patchset_sha256=%s\\n'\'' "\$HELD_PATCHSET_SHA256"\n  printf '\''late_patchset_sha256=%s\\n'\'' "\$LATE_PATCHSET_SHA256"\n  printf '\''cpu9_patchset_sha256=%s\\n'\'' "\$CPU9_PATCHSET_SHA256"\n  printf '\''window_patchset_sha256=%s\\n'\'' "\$WINDOW_PATCHSET_SHA256"\n  printf '\''terminal_patchset_sha256=%s\\n'\'' "\$TERMINAL_PATCHSET_SHA256"\n  printf '\''coherence_patchset_sha256=%s\\n'\'' "\$COHERENCE_PATCHSET_SHA256"\n  printf '\''multiline_patchset_sha256=%s\\n'\'' "\$MULTILINE_PATCHSET_SHA256"#g;
	s#gemian-a72-cpu8-held-online-\$\{raw_sha256:0:12\}#gemian-a72-cpu9-multiline-integrity-\${raw_sha256:0:12}#g;
	s#gemian-a72-cpu8-held-online-candidate#gemian-a72-cpu9-multiline-integrity-candidate#g;
' "$source_builder" >"$derived"
chmod 0700 "$derived"

for token in \
	'REPOSITORY_COMMIT=fb647817cd573e3dd8719821da8742bc5433979b' \
	'MULTILINE_PATCHSET_SHA256=c7a9b020563c4abb74059bbf72705839c528a81d577c7031ddfb36de647fd896' \
	'KERNEL_SHA256=81f076198ae314d187790beecee8d9b5edda3c4432e51a0f36a22dbe326fc468' \
	'EXPECTED_RAW_SHA256=4e3c1b1095ee87e0af3c45595ea83d859d53188a53aba10672202bf45938986a' \
	'EXPECTED_PADDED_SHA256=5227729e34ca42cf606f43008ec753fce15147693ce7a670818db58c5903fa48' \
	'cpu9-multiline-integrity-compile-review-only' \
	'multiline_patchset_sha256=%s' \
	'gemian-a72-cpu9-multiline-integrity-candidate'; do
	grep -Fq "$token" "$derived" || die "derived builder lacks: $token"
done
! grep -Fq 'cpu9-bounded-coherency-compile-review-only' "$derived" ||
