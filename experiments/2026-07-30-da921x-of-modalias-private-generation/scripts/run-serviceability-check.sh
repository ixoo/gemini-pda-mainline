...
abort()
{
	printf 'private_generation_result=FAIL\nfailure=%s\n' "$1"
	exit 1
}
...
[ "$(/bin/busybox uname -r)" = 7.1.3-gemini-da921x-ofgen ] ||
...
marker='GEMINI_DA921X_OF_MODALIAS_PRIVATE_GENERATION length=38 fallback=i2c'
marker_count="$(/bin/busybox dmesg | /bin/busybox grep -Fc "$marker")"
[ "$marker_count" = 1 ] || abort private-generation-marker-count

printf 'kernel=7.1.3-gemini-da921x-ofgen\n'
printf 'adapter=%s\nclient=%s\n' "$adapter" "${client##*/}"
...
printf 'private_of_modalias_length=38\n'
...
printf 'private_generation_result=PASS\n'
