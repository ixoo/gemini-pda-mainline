#!/usr/bin/env python3
"""Validate Candidate O as an exact initramfs-only derivative of Candidate N."""
...
EXPECTED_SHA256 = {
    "Candidate N boot": "43aea71224f6261001ff00904b30dae29063334172a2f6b0163b424a84c0e3aa",
    "Candidate N Image.gz": "0c0d0e22c78b5b0d89b7a7363be55850b3f3474d3b4e7f922946747efbe164d3",
    "Candidate N DTB": "c574762aa178cb5a7238400b499d2edcdd3acb3538d2255e916b041f2074c379",
    "Candidate N initramfs": "3351422e594c59e5785e12cac6ffbefa2644bd6c85932ac6825a9b9c5edd6290",
}
...
def require_header_identity(header: bytes, label: str) -> None:
    ...
    if header[NAME_OFFSET : NAME_OFFSET + NAME_SIZE] != expected_name:
        raise ValueError(f"{label} header name is not exact Candidate N")
...
def main() -> int:
    ...
    try:
        baseline = parse(args.baseline, "Candidate N boot image")
        candidate = parse(args.candidate, "Candidate O boot image")
        image_gz = read(args.image_gz, "Candidate N Image.gz")
        dtb = read(args.dtb, "Candidate N DTB")
        baseline_ramdisk = read(args.baseline_ramdisk, "Candidate N initramfs")
        candidate_ramdisk = read(args.candidate_ramdisk, "Candidate O initramfs")
        actual = {
            "Candidate N boot": baseline["sha256"],
            "Candidate N Image.gz": digest(image_gz),
            "Candidate N DTB": digest(dtb),
            "Candidate N initramfs": digest(baseline_ramdisk),
        }
        ...
        if candidate["sha256"] == baseline["sha256"]:
            raise ValueError("Candidate O unexpectedly equals Candidate N")
        if candidate_ramdisk == baseline_ramdisk:
            raise ValueError("Candidate O initramfs unexpectedly equals Candidate N")
        exact_kernel = image_gz + dtb
        if baseline["kernel"] != exact_kernel or candidate["kernel"] != exact_kernel:
            raise ValueError("kernel segment is not exact Candidate N Image.gz plus DTB")
        ...
        if candidate["ramdisk"] != candidate_ramdisk:
            raise ValueError("Candidate O ramdisk does not match its explicit component")

        for label, image in (("Candidate N", baseline), ("Candidate O", candidate)):
        ...
        for field, value in exact_layout.items():
            if candidate["fields"][field] != value:
                raise ValueError(f"Candidate O has unexpected {field}")
    except (OSError, ValueError) as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 2

    print("validation=android-v0-a53-sweep-delta")
    print("baseline_label=N")
    print("candidate_label=O")
    ...
    print("unchanged_candidate_n_kernel_segment=yes")
