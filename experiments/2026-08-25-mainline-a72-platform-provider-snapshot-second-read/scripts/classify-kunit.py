#!/usr/bin/env python3
"""Classify the hardware-free platform/provider composition KUnit suite."""
...
EXPECTED_CASES = (
    "mt6797_platform_provider_success_test",
    "mt6797_platform_provider_platform_error_test",
    "mt6797_platform_provider_platform_invalid_test",
    "mt6797_platform_provider_before_failure_test",
    "mt6797_platform_provider_provider_failure_test",
    "mt6797_platform_provider_after_failure_test",
)
...
def classify_runtime(raw: str, expected_release: str, qemu_exit: int) -> None:
    ...
    require(ktap.count("1..6") == 1, "focused test plan changed")
    ...
    suite_summary = f"# {SUITE}: pass:6 fail:0 skip:0 total:6"
    totals = "# Totals: pass:6 fail:0 skip:0 total:6"
...
def main() -> None:
    ...
    print("experiment=2026-08-25-mainline-a72-platform-provider-snapshot-second-read")
    ...
    print("platform_provider_symbols=present")
    ...
    for case in EXPECTED_CASES:
        print(f"{case}=pass")
    print("tap_summary=pass:6_fail:0_skip:0_total:6")
