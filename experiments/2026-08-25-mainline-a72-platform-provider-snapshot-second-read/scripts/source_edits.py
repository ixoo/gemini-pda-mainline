#!/usr/bin/env python3
"""Apply deterministic MT6797 platform/provider snapshot observer edits."""

from __future__ import annotations

import argparse
import shutil
from pathlib import Path
from textwrap import dedent


def replace_once(path: Path, old: str, new: str) -> None:
    text = path.read_text(encoding="utf-8")
    count = text.count(old)
    if count != 1:
        first = old.splitlines()[0] if old.splitlines() else "<empty>"
        raise SystemExit(
            f"{path}: expected one anchor beginning {first!r}, found {count}"
        )
    path.write_text(text.replace(old, new, 1), encoding="utf-8")


def copy_source(source_dir: Path, name: str, destination: Path) -> None:
    source = source_dir / name
    if not source.is_file() or source.is_symlink():
        raise SystemExit(f"unsafe source template: {source}")
    if destination.exists():
        raise SystemExit(f"destination already exists: {destination}")
    destination.parent.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(source, destination)


def apply_ledger(root: Path) -> None:
    kconfig = root / "fs/pstore/Kconfig"
    mode = dedent(r'''
config PSTORE_GEMINI_A72_PLATFORM_PROVIDER_SNAPSHOT_LEDGER
	bool "Gemini A72 platform/provider-snapshot retained ledger"
	depends on PSTORE_GEMINI_PROTECTED_READBACK_LEDGER=y
	depends on MTK_MT6797_A72_PLATFORM_PROVIDER_SNAPSHOT_OBSERVER=y
	depends on !PSTORE_GEMINI_A72_PLATFORM_SNAPSHOT_LEDGER
	depends on !PSTORE_GEMINI_A72_EARLY_INITCALL_LEDGER
	depends on !PSTORE_GEMINI_A72_GLOBAL_INITCALL_LEDGER
	depends on !PSTORE_GEMINI_A72_PHYSICAL_SOURCE_LEDGER
	depends on !PSTORE_GEMINI_A72_PHYSICAL_SOURCE_PRECAPTURE_LEDGER
	depends on !PSTORE_GEMINI_PROTECTED_READBACK_MANUAL_CONTROL
	depends on !PSTORE_GEMINI_PROTECTED_READBACK_PROBE_GATE_LEDGER
	depends on !PSTORE_GEMINI_CLOCK_BACKEND_ENTRY_LEDGER
	depends on !PSTORE_GEMINI_PROTECTED_READBACK_RAW_ENTRY_LEDGER
	depends on !PSTORE_GEMINI_PROTECTED_READBACK_FIRST_DMESG_WRITE_QUALIFICATION
	depends on !PSTORE_GEMINI_CLOCK_BACKEND_FIRST_DMESG_ENTRY_QUALIFICATION
	depends on !PSTORE_GEMINI_PROTECTED_CLOCK_FIRST_DMESG_CALL_QUALIFICATION
	default n
	help
	  Give the candidate-only platform/provider observer one retained record
	  immediately before its provider snapshot and one only after one valid
	  two-sample provider result. Require both raw headers to be all ones.

	  Reuse the qualified signature-last, full-readback, two-write, no-clear,
	  and no-retry protocol. This mode adds no storage, register-data write,
	  clock, BigiDVFS, owner, publisher, or CPU action. If unsure, say N.

''').lstrip("\n")
    replace_once(
        kconfig,
        "config PSTORE_GEMINI_A72_PLATFORM_SNAPSHOT_LEDGER\n",
        mode + "config PSTORE_GEMINI_A72_PLATFORM_SNAPSHOT_LEDGER\n",
    )
    replace_once(
        kconfig,
        "depends on MTK_MT6797_PROTECTED_READBACK_OBSERVER=y || "
        "MTK_MT6797_A72_PLATFORM_SNAPSHOT_OBSERVER=y || "
        "MTK_MT6797_A72_PHYSICAL_SOURCE_OBSERVER=y || "
        "MTK_MT6797_DVFSP_CLOCK_BACKEND=y || "
        "PSTORE_GEMINI_PROTECTED_READBACK_MANUAL_CONTROL=y\n",
        "depends on MTK_MT6797_PROTECTED_READBACK_OBSERVER=y || "
        "MTK_MT6797_A72_PLATFORM_SNAPSHOT_OBSERVER=y || "
        "MTK_MT6797_A72_PLATFORM_PROVIDER_SNAPSHOT_OBSERVER=y || "
        "MTK_MT6797_A72_PHYSICAL_SOURCE_OBSERVER=y || "
        "MTK_MT6797_DVFSP_CLOCK_BACKEND=y || "
        "PSTORE_GEMINI_PROTECTED_READBACK_MANUAL_CONTROL=y\n",
    )

    ledger = root / "fs/pstore/gemini_protected_readback_ledger.c"
    layout_anchor = (
        "\tdefined(CONFIG_PSTORE_GEMINI_PROTECTED_CLOCK_FIRST_DMESG_CALL_QUALIFICATION) || \\\n"
        "\tdefined(CONFIG_PSTORE_GEMINI_A72_PLATFORM_SNAPSHOT_LEDGER) || \\\n"
    )
    layout_replacement = (
        "\tdefined(CONFIG_PSTORE_GEMINI_PROTECTED_CLOCK_FIRST_DMESG_CALL_QUALIFICATION) || \\\n"
        "\tdefined(CONFIG_PSTORE_GEMINI_A72_PLATFORM_PROVIDER_SNAPSHOT_LEDGER) || \\\n"
        "\tdefined(CONFIG_PSTORE_GEMINI_A72_PLATFORM_SNAPSHOT_LEDGER) || \\\n"
    )
    replace_once(ledger, layout_anchor, layout_replacement)
    records = dedent(r'''
#ifdef CONFIG_PSTORE_GEMINI_A72_PLATFORM_PROVIDER_SNAPSHOT_LEDGER
static const char * const gemini_prb_records[] = {
	"====0.000000-D\n"
	"GEMINI_A72_PLATFORM_PROVIDER_SNAPSHOT_V1 token=GAPP-20260825-A "
	"checkpoint=before-provider slot=1 crc32=0150f9c7\n",
	"====0.000000-D\n"
	"GEMINI_A72_PLATFORM_PROVIDER_SNAPSHOT_V1 token=GAPP-20260825-A "
	"checkpoint=after-provider slot=2 crc32=4fffb31e\n",
};
#elif defined(CONFIG_PSTORE_GEMINI_A72_PLATFORM_SNAPSHOT_LEDGER)
''').lstrip("\n")
    records_anchor = (
        "static bool gemini_prb_armed;\n\n"
        "#ifdef CONFIG_PSTORE_GEMINI_A72_PLATFORM_SNAPSHOT_LEDGER\n"
    )
    replace_once(ledger, records_anchor, "static bool gemini_prb_armed;\n\n" + records)
    raw_anchor = (
        "#if defined(CONFIG_PSTORE_GEMINI_A72_PLATFORM_SNAPSHOT_LEDGER) || \\\n"
    )
    raw_replacement = (
        "#if defined(CONFIG_PSTORE_GEMINI_A72_PLATFORM_PROVIDER_SNAPSHOT_LEDGER) || \\\n"
        "\tdefined(CONFIG_PSTORE_GEMINI_A72_PLATFORM_SNAPSHOT_LEDGER) || \\\n"
    )
    text = ledger.read_text(encoding="utf-8")
    if text.count(raw_anchor) != 2:
        raise SystemExit("ledger: expected two platform raw-record conditionals")
    ledger.write_text(text.replace(raw_anchor, raw_replacement), encoding="utf-8")


def apply_observer(root: Path, source_dir: Path) -> None:
    kconfig = root / "drivers/soc/mediatek/Kconfig"
    observer = dedent(r'''
config MTK_MT6797_A72_PLATFORM_PROVIDER_SNAPSHOT_OBSERVER
	bool "MediaTek MT6797 A72 platform/provider snapshot observer"
	depends on ARM64_MT6797_A72_PROVIDER_OWNER
	depends on MTK_MT6797_A72_PLATFORM_STATE
	default n
	help
	  Build the candidate-only observer that takes one stable platform snapshot,
	  writes one checkpoint, takes one stable DA921x provider snapshot, and
	  writes one completion checkpoint. It performs 26 read-only platform
	  observations and ten fixed read-only I2C transfers with no observer retry.

	  It adds no provider acquire/release, register-data write, protected-clock
	  or BigiDVFS read, secure call, publication, owner mutation, CPU request,
	  reset, or power action. If unsure, say N.

''').lstrip("\n")
    replace_once(
        kconfig,
        "config MTK_MT6797_A72_PLATFORM_SNAPSHOT_OBSERVER\n",
        observer + "config MTK_MT6797_A72_PLATFORM_SNAPSHOT_OBSERVER\n",
    )
    makefile = root / "drivers/soc/mediatek/Makefile"
    replace_once(
        makefile,
        "obj-$(CONFIG_MTK_MT6797_A72_PLATFORM_SNAPSHOT_OBSERVER) += "
        "mt6797-a72-platform-snapshot-observer.o\n",
        "obj-$(CONFIG_MTK_MT6797_A72_PLATFORM_PROVIDER_SNAPSHOT_OBSERVER) += "
        "mt6797-a72-platform-provider-snapshot-observer.o\n"
        "obj-$(CONFIG_MTK_MT6797_A72_PLATFORM_SNAPSHOT_OBSERVER) += "
        "mt6797-a72-platform-snapshot-observer.o\n",
    )
    for name in (
        "mt6797-a72-platform-provider-snapshot-observer.c",
        "mt6797-a72-platform-provider-snapshot-observer-internal.h",
    ):
        copy_source(source_dir, name, root / "drivers/soc/mediatek" / name)


def apply_binding(root: Path, source_dir: Path) -> None:
    name = "mediatek,mt6797-a72-platform-provider-snapshot-observer.yaml"
    copy_source(
        source_dir,
        name,
        root / "Documentation/devicetree/bindings/soc/mediatek" / name,
    )


def apply_tests(root: Path, source_dir: Path) -> None:
    kconfig = root / "drivers/soc/mediatek/Kconfig"
    tests = dedent(r'''
config MTK_MT6797_A72_PLATFORM_PROVIDER_SNAPSHOT_KUNIT_TEST
	bool "KUnit tests for MT6797 A72 platform/provider observer"
	depends on KUNIT=y
	depends on MTK_MT6797_A72_PLATFORM_PROVIDER_SNAPSHOT_OBSERVER
	default n
	help
	  Exercise exact platform/checkpoint/provider/checkpoint order, every
	  failure boundary, and all-zero failure output with injected in-memory
	  operations. No MMIO, retained RAM, I2C, SMC, provider registry, owner,
	  publisher, or CPU action occurs.

''').lstrip("\n")
    replace_once(
        kconfig,
        "config MTK_MT6797_A72_PLATFORM_SNAPSHOT_OBSERVER\n",
        tests + "config MTK_MT6797_A72_PLATFORM_SNAPSHOT_OBSERVER\n",
    )
    makefile = root / "drivers/soc/mediatek/Makefile"
    replace_once(
        makefile,
        "obj-$(CONFIG_MTK_MT6797_A72_PLATFORM_PROVIDER_SNAPSHOT_OBSERVER) += "
        "mt6797-a72-platform-provider-snapshot-observer.o\n",
        "obj-$(CONFIG_MTK_MT6797_A72_PLATFORM_PROVIDER_SNAPSHOT_OBSERVER) += "
        "mt6797-a72-platform-provider-snapshot-observer.o\n"
        "obj-$(CONFIG_MTK_MT6797_A72_PLATFORM_PROVIDER_SNAPSHOT_KUNIT_TEST) += "
        "mt6797-a72-platform-provider-snapshot-observer-test.o\n",
    )
    name = "mt6797-a72-platform-provider-snapshot-observer-test.c"
    copy_source(source_dir, name, root / "drivers/soc/mediatek" / name)


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--source-root", type=Path, required=True)
    parser.add_argument(
        "--phase", choices=("ledger", "observer", "binding", "tests"), required=True
    )
    args = parser.parse_args()
    root = args.source_root.resolve()
    source_dir = Path(__file__).resolve().parents[1] / "source"
    actions = {
        "ledger": lambda: apply_ledger(root),
        "observer": lambda: apply_observer(root, source_dir),
        "binding": lambda: apply_binding(root, source_dir),
        "tests": lambda: apply_tests(root, source_dir),
    }
    actions[args.phase]()


if __name__ == "__main__":
    main()
