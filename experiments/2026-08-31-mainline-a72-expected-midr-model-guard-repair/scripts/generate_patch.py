#!/usr/bin/env python3
"""Generate and audit the A72 expected-policy model-guard repair."""
...
PATCH_NAME = "0460-arm64-accept-Cortex-A72-revisions-in-expected-policy.patch"
SUBJECT = "arm64: accept Cortex-A72 revisions in expected policy"
ACTION_TOKENS = (
...
def commit(root: Path, subject: str, body: str, minute: int) -> None:
    ...
    environment.update({
        ...
        "GIT_AUTHOR_DATE": f"2026-08-31T22:{minute:02d}:00Z",
        "GIT_COMMITTER_DATE": f"2026-08-31T22:{minute:02d}:00Z",
    })
...
def prepare_parent(source_root: Path, root: Path) -> None:
    ...
    commit(root, "Gemini post-0459 generation parent", "Synthetic parent only.", 19)
...
def validate_mutations(text: str) -> int:
    mutations = (
        ...
        ("== MIDR_CORTEX_A72;", "!= MIDR_CORTEX_A72;"),
        ("== MIDR_CORTEX_A72;", "== MIDR_CORTEX_A53;"),
        ("expected->midr & MIDR_CPU_MODEL_MASK", "MIDR_CPU_MODEL_MASK"),
    )
    rejected = 0
    ...
    if rejected != len(mutations):
        raise SystemExit("model-guard mutation was not rejected")
    return rejected
...
def validate_result(root: Path, parent_counts: dict[str, int]) -> list[str]:
    ...
    return [
        "expected_policy_model_guard_validation=pass",
...
def validate_patch(path: Path) -> None:
    ...
    if added != [source_edits.NEW] or removed != [source_edits.OLD]:
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="a72-expected-midr-model-guard-") as name:
