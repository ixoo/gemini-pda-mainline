#!/usr/bin/env python3
"""Validate Candidate AO's private post-LK live FDT without emitting secrets."""
...
def load_inputs() -> tuple[ModuleType, ModuleType, ModuleType]:
    ...
    identity_path = script.with_name("candidate_ao.py")
    common_path = (
        script.parents[2]
    ...
    for path, label in (
        (identity_path, "Candidate AO identity module"),
    ...
    return (
        load_module(identity_path, "candidate_ao_live_fdt_identity"),
        load_module(common_path, "candidate_ao_live_fdt_common"),
        load_module(parser_path, "candidate_ao_live_fdt_parser"),
    )
...
def require_handoff_contract(
    ...
    if live.get(HANDOFF) != artifact.get(HANDOFF):
        raise ValueError("post-LK handoff node is not byte-exact Candidate AO")
    fdt.require_prop(
        live,
        HANDOFF,
        "compatible",
...
def validate(
    ...
    require_regular(artifact_path, "Candidate AO artifact DTB")
    ...
    if digest(artifact_path) != identity.FINAL_DTB_SHA256:
        raise ValueError("artifact DTB is not exact Candidate AO")
    if digest(live_path) != EXPECTED_LIVE_FDT_SHA256:
        raise ValueError("private live FDT is not the audited Candidate AO capture")
    ...
    return {
        ...
        "live_size": EXPECTED_LIVE_FDT_SIZE,
    }
...
def main() -> int:
    ...
    print("validation=candidate-ao-post-lk-live-fdt-allowlisted")
    ...
    print("handoff_node=byte-exact-pre-lk-candidate-ao")
