...
# Collect Candidate AO's final one-way handoff publication over the inherited
...
die() { printf 'error: %s\n' "$*" >&2; exit 2; }
usage() {
	printf 'usage: %s --interface IFACE --output FILE --installed-full-sha256 SHA256 --expected-config-sha256 SHA256 --expected-live-fdt-sha256 SHA256\n' "$0" >&2
}
...
while (($#)); do
	case "$1" in
	--interface|--output|--installed-full-sha256|--expected-config-sha256|--expected-live-fdt-sha256)
...
command_file="$(mktemp /tmp/candidate-ao-runtime-command.XXXXXX)"
cleanup() { [[ ! -f "$command_file" ]] || rm -f -- "$command_file"; }
trap cleanup EXIT
cat >"$command_file" <<'EOF'
/bin/busybox sh <<'__AO_REMOTE_SCRIPT__'
property_hex() {
	if [ -f "$1" ]; then
...
while :; do
	...
	case "$state" in
	ready|inconclusive|faulted) break ;;
	esac
...
read_state() {
	...
	printf 'handoff_state='; /bin/busybox cat "$handoff/state" 2>/dev/null || true
	printf 'handoff_status='; /bin/busybox cat "$handoff/status" 2>/dev/null || true
	...
	for attribute in state status snapshots; do
...
printf '__AO_IDENTITY_END__\n'
printf '__AO_STATE1_BEGIN__\n'; read_state; printf '__AO_STATE1_END__\n'
printf '__AO_STAT1_BEGIN__\n'
/bin/busybox grep '^cpu[0-9]' /proc/stat
printf '__AO_STAT1_END__\n'
/bin/busybox sleep 5
printf '__AO_STATE2_BEGIN__\n'; read_state; printf '__AO_STATE2_END__\n'
printf '__AO_STAT2_BEGIN__\n'
/bin/busybox grep '^cpu[0-9]' /proc/stat
printf '__AO_STAT2_END__\n'
printf '__AO_DMESG_BEGIN__\n'; /bin/busybox dmesg; printf '__AO_DMESG_END__\n'
exit
__AO_REMOTE_SCRIPT__
exit
EOF

{
	printf '__AO_HOST_BEGIN__\n'
