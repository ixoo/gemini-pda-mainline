...
# Assemble storage-inert Candidate AO from an exact validated AO kernel package
# and the exact hardware-passed Candidate AH DT/initramfs baseline. This script
...
die() { printf 'error: %s\n' "$*" >&2; exit 2; }
usage() {
	printf 'usage: %s --package AO_PACKAGE --ah-artifact AH_ARTIFACT --output-parent DIR\n' "$0" >&2
}
...
candidate_module="$script_dir/candidate_ao.py"
[[ -f "$candidate_module" && ! -L "$candidate_module" &&
	-s "$candidate_module" ]] || die 'Candidate AO identity module is unsafe'
candidate_value() {
	PYTHONPATH="$script_dir" python3 -c \
		'import candidate_ao as ao, sys; print(getattr(ao, sys.argv[1]))' "$1"
}
...
readonly AH_NAME="$(candidate_value AH_ARTIFACT_DIR)"
readonly AH_MANIFEST_SHA256="$(candidate_value AH_MANIFEST_SHA256)"
readonly AH_DTB_SHA256="$(candidate_value AH_DTB_SHA256)"
readonly AH_INITRAMFS_SHA256="$(candidate_value INITRAMFS_SHA256)"
readonly AH_KEYMAP_SHA256="$(candidate_value KEYMAP_SHA256)"
readonly AH_DTB="$(candidate_value AH_DTB_MEMBER)"
readonly AH_INITRAMFS="$(candidate_value AH_INITRAMFS_MEMBER)"
readonly AO_DTB="$(candidate_value DTB_MEMBER)"
readonly AO_INITRAMFS="$(candidate_value INITRAMFS_MEMBER)"
readonly AO_BOOT="$(candidate_value BOOT_MEMBER)"
...
dtb_builder="$script_dir/build-ao-dtb.sh"
...
for input in "$manifest" "$package_validator" "$dtb_builder" "$dtb_validator" \
	...
	"$analyzer" "$patch_0094" "$patch_0095" "$patch_0097" "$patch_0098"; do
...
workdir="$(mktemp -d "$output_parent/.candidate-AO-dvfsp-handoff.XXXXXX")"
...
install -m 0600 "$ah_artifact/$AH_INITRAMFS" "$stage/$AO_INITRAMFS"
...
bash "$dtb_builder" --ah-dtb "$ah_artifact/$AH_DTB" \
	--output "$stage/$AO_DTB" >"$stage/dtb-validation.raw"
bash "$dtb_builder" --ah-dtb "$ah_artifact/$AH_DTB" \
	--output "$replica/$AO_DTB" >/dev/null
cmp -s "$stage/$AO_DTB" "$replica/$AO_DTB" || \
	die 'two independent Candidate AO final-DT derivations differ'
sed -e "s|$stage/$AO_DTB|@AO_DTB@|g" \
	"$stage/dtb-validation.raw" >"$stage/dtb-validation.txt"
rm "$stage/dtb-validation.raw"

candidate="$stage/$AO_BOOT"
replica_boot="$replica/$AO_BOOT"
boot_cmdline=bootopt=64S3,32N2,64N2
for output in "$candidate" "$replica_boot"; do
	python3 "$serializer" --kernel "$stage/Image.gz" \
		--ramdisk "$stage/$AO_INITRAMFS" --dtb "$stage/$AO_DTB" \
...
cmp -s "$candidate" "$replica_boot" || \
	die 'two independent Candidate AO container assemblies differ'
...
python3 "$analyzer" --validate-lk --expected-image-gz "$stage/Image.gz" \
	--expected-ramdisk "$stage/$AO_INITRAMFS" \
	--expected-dtb "$stage/$AO_DTB" --expected-name gemini-obs-L \
...
python3 "$boot_validator" --candidate "$candidate" \
	...
	--kernel-config "$stage/kernel.config" --dtb "$stage/$AO_DTB" \
	--ah-dtb "$ah_artifact/$AH_DTB" --initramfs "$stage/$AO_INITRAMFS" \
...
dtb_sha256="$(sha256sum "$stage/$AO_DTB" | awk '{ print $1 }')"
...
auditor_sha256="$(sha256sum "$handoff_auditor" | awk '{ print $1 }')"
{
	...
	printf 'ah_raw_sha256=%s\n' "$(candidate_value AH_RAW_SHA256)"
	...
	printf 'functional_baseline=exact-hardware-passed-candidate-ah-contract\n'
	printf 'final_dtb_baseline=exact-candidate-ah-final-dtb\n'
	printf 'final_dtb_delta=one-dvfsp-one-way-handoff-owner-node\n'
	printf 'initramfs_keyboard_console_usb_reboot=byte-exact-candidate-ah\n'
	printf 'handoff_initial_samples=3\n'
	printf 'handoff_initial_contract=exact-candidate-an-signature\n'
	printf 'handoff_normalization=one-way-ccf-temporary-reference\n'
	printf 'handoff_ccf_transition_attempts_max=1\n'
	printf 'handoff_retry=none\n'
	printf 'handoff_success_path_ccf_balance=one-prepare-enable-one-disable-unprepare\n'
	printf 'handoff_late_revalidation_delay_ms=45000\n'
	printf 'handoff_late_revalidation=read-only\n'
	printf 'handoff_direct_mmio=read-only\n'
	printf 'handoff_writable_control=none\n'
...
expected_pre_manifest="$(printf '%s\n' Image.gz System.map analysis.txt \
	...
	dtb-validation.txt "$AO_BOOT" "$AO_INITRAMFS" gemini-us.bkeymap \
	input-event-capture kernel.config "$AO_DTB" package-validation.txt \
...
[[ "$actual_inventory" == "$expected_pre_manifest" ]] || \
	die 'Candidate AO output inventory changed'
(
	cd "$stage"
	find . -type f ! -name SHA256SUMS -print0 | sort -z | xargs -0 sha256sum
) >"$stage/SHA256SUMS"
(cd "$stage" && sha256sum --check --strict SHA256SUMS >/dev/null) || \
	die 'Candidate AO artifact manifest failed'
chmod 0600 "$stage"/*
...
[[ -d "$output" && ! -e "$artifact" ]] || \
	die 'exclusive Candidate AO publication failed'
rm -rf -- "$replica"
rmdir "$workdir"
workdir=
trap - EXIT
printf 'validation=candidate-ao-mt6797-dvfsp-one-way-handoff\n'
printf 'artifact=%s\ncandidate=%s/%s\n' "$output" "$output" "$AO_BOOT"
printf 'candidate_sha256=%s\ncandidate_size=%s\n' \
	"$candidate_sha256" "$candidate_size"
printf 'dtb_sha256=%s\nruntime_result=not-tested\n' "$dtb_sha256"
