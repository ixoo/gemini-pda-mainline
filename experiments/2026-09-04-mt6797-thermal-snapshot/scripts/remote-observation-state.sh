...
# Read-only state frame; never read the thermal snapshot attribute here.
set -u
export LC_ALL=C
...
$BB printf '%s\n' __THERMAL_SNAPSHOT_STATE_BEGIN__
...
snapshot_status_mode=missing
snapshot_status=missing
for item in /sys/bus/platform/devices/*/mt6797_temperature_snapshot; do
	[ -r "$item" ] || continue
	snapshot_count=$((snapshot_count + 1))
	snapshot_path=$item
	snapshot_mode=$($BB stat -c '%a' "$item" 2>/dev/null || $BB printf 'missing')
	snapshot_status_mode=$($BB stat -c '%a' "${item}_status" 2>/dev/null || $BB printf 'missing')
	snapshot_status=$($BB cat "${item}_status" 2>/dev/null || $BB printf 'missing')
done
$BB printf 'thermal_snapshot_count=%s\n' "$snapshot_count"
$BB printf 'thermal_snapshot_path=%s\n' "$snapshot_path"
$BB printf 'thermal_snapshot_mode=%s\n' "$snapshot_mode"
$BB printf 'thermal_snapshot_status_mode=%s\n' "$snapshot_status_mode"
$BB printf 'thermal_snapshot_status=%s\n' "$snapshot_status"

$BB printf '%s\n' __THERMAL_SNAPSHOT_LATE_PROFILE_BEGIN__
$BB dmesg 2>/dev/null | $BB grep 'arm64-late-cpu-profile:' || true
$BB printf '%s\n' __THERMAL_SNAPSHOT_LATE_PROFILE_END__
...
$BB printf '%s\n' __THERMAL_SNAPSHOT_STATE_END__
