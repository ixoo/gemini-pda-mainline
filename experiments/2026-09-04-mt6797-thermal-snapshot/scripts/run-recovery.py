...
"""Frozen one-shot cycle preparation and bounded recovery over direct USB."""
import argparse
import hashlib
...
CYCLE=CAPTURE_ROOT/'thermal-snapshot-recovery-cycle-1'
RUN=CAPTURE_ROOT/'thermal-snapshot-workload-recovery-1'
...
def cycle_expected():
    return shutdown_fields() | {'protocol':'thermal-snapshot-workload-recovery-1',
        'candidate_sha256':'666961b636b21b8598a64999e9dbf72af280ad99f07a6b745045320f24ca361b',
        'deployment_receipt_sha256':DEPLOYMENT_SHA,'source_runtime_classification_sha256':SOURCE_EVIDENCE_SHA,'post_shutdown_reachability':'unreachable','reboot_requested':'no'}
...
def read_cycle():
    required={'source-runtime-classification.json','deployment-summary.txt','shutdown.requested','transport-1.txt','transport-1-meta.json','cycle-receipt.json','classification.json'}
    if CYCLE.is_symlink() or not CYCLE.is_dir() or CYCLE.stat().st_mode & 0o777!=0o700:
        raise ValueError('unsafe cycle capture')
    if {p.name for p in CYCLE.iterdir()} != required | {'SHA256SUMS'}:
        raise ValueError('unexpected cycle capture inventory')
    ...
    meta=unique_json((CYCLE/'transport-1-meta.json').read_text())
    if set(meta)!={'timeout','returncode'} or type(meta['timeout']) is not bool or not (
            meta['timeout'] and meta['returncode'] is None or
            not meta['timeout'] and type(meta['returncode']) is int):
        raise ValueError('cycle transport metadata malformed')
    ...
    if entries['source-runtime-classification.json']!=SOURCE_EVIDENCE_SHA:raise ValueError('source runtime evidence changed')
...
def validate_shutdown_frame(raw):
    ...
    text=state.bounded(raw.replace('\r\n','\n'),'__THERMAL_RECOVERY_SHUTDOWN_BEGIN__','__THERMAL_RECOVERY_SHUTDOWN_END__')
    fields={}
...
class Capture:
    ...
    def transport(self,script,workload=False,shutdown=False):
        self.sessions+=1
        marker='__THERMAL_RECOVERY_HOST_SCRIPT__'
...
def workload(capture,deployment,cycle):
    ...
    classifier=module('classify-recovery-runtime');builder=module('build-recovery-runtime')
    ...
    if pre['boot_id'] in classifier.CLOSED_BOOTS | {SOURCE_BOOT}:raise ValueError('consumed boot identity')
    ...
    result.update({'postflight':'pass','host_postflight':'pass','transport_sessions':capture.sessions,'ordinary_thermal_reads':2,
                   'workload_requests':1,'retries':0,'candidate_sha256':protocol.CANDIDATE})
    return result


def prepare_cycle(capture):
    raw=capture.transport((HERE/'remote-recovery-shutdown.sh').read_text(),shutdown=True)
...
def main():
    ...
    source_evidence=SOURCE_EVIDENCE.read_text()
    if SOURCE_EVIDENCE.is_symlink() or hashlib.sha256(source_evidence.encode()).hexdigest()!=SOURCE_EVIDENCE_SHA:
        raise ValueError('published source runtime evidence changed')
    ...
    try:
        ...
        capture.save('source-runtime-classification.json',source_evidence)
    ...
    return 3 if result.get('classification')=='bounded-recovery-comparison-rejected' else 0
