...
spec=importlib.util.spec_from_file_location('host',HERE/'run-attribution.py')
host=importlib.util.module_from_spec(spec);spec.loader.exec_module(host)
host.sources()
subprocess.run(json.loads(os.environ.get('GEMINI_TEST_SHELL','["sh"]'))+['-n',str(HERE/'remote-attribution-shutdown.sh')],check=True)
subprocess.run(['shellcheck',str(HERE/'remote-attribution-shutdown.sh')],check=True)
fixture=runpy.run_path(str(HERE/'test-attribution-runtime.py'))
...
shutdown='__THERMAL_ATTRIBUTION_SHUTDOWN_BEGIN__\n'+''.join(k+'='+v+'\n' for k,v in host.shutdown_fields().items())+'__THERMAL_ATTRIBUTION_SHUTDOWN_END__\n'
...
with tempfile.TemporaryDirectory(prefix='gemini-attribution-host-',dir='/tmp') as tmp:
    ...
    for scenario in scenarios:
        ...
        terminal=host.module('classify-attribution-runtime').scalar(runtime,'post_status')
        def fake_run(args,**kwargs):
            if args[0]=='git' and 'check-ignore' in args:return subprocess.CompletedProcess(args,0)
            if args[0]!='nc':return real_run(args,**kwargs)
        ...
        with patch.object(host,'CYCLE',case/'cycle'),patch.object(host,'RUN',case/'run'),patch.object(host,'DEPLOYMENT',deployment),patch.object(host,'module',side_effect=modules),patch.object(subprocess,'run',side_effect=fake_run),patch.object(sys,'argv',['runner','run','--execute']):
            ...
            assert len(calls)==count
print('cycle_scenarios=4 workload_scenarios=13 restart_refusals=17 receipt_field_mutations='+str(len(host.cycle_expected()))+' receipt_duplicate_refusals=1 manifest_tamper_refusals=1 device_action=none')
