#!/usr/bin/env python3
"""Mutation tests for Fermi's exact topology-result classifier."""
...
SPEC = importlib.util.spec_from_file_location(
    "fermi_result_validator_test",
    SCRIPT_DIR / "validate-fermi-result.py",
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("cannot load Fermi result validator")
vr = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = vr
SPEC.loader.exec_module(vr)

VALUES = (0xD9, 0xD0, 0xC0, 0x15, 0x81, 0x22, 0x33) * 2
...
class FermiResultContracts(unittest.TestCase):
    def test_success_accepts_distinct_buckb_selectors(self) -> None:
        result = vr.validate_text(success_result())
        self.assertEqual(result.classification, "complete-success")
        self.assertEqual(result.samples[5]["value"], "22")
        self.assertEqual(result.samples[6]["value"], "33")
    ...
    def test_topology_signature_stability_and_prefill_fail_closed(self) -> None:
        original = success_result()
        mutations = (
            (b"value=d9", b"value=d8"),
            (b"sample=3 pass=0 index=3 address=68 register=d3 expected_kind=topology-stable expected=05 prefill=96 value=15",
             b"sample=3 pass=0 index=3 address=68 register=d3 expected_kind=topology-stable expected=05 prefill=96 value=14"),
            (b"sample=11 pass=1 index=4 address=68 register=5e expected_kind=stable expected=00 prefill=2d value=81",
             b"sample=11 pass=1 index=4 address=68 register=5e expected_kind=stable expected=00 prefill=2d value=82"),
            (b"sample=4 pass=0 index=4 address=68 register=5e expected_kind=stable expected=00 prefill=69 value=81",
             b"sample=4 pass=0 index=4 address=68 register=5e expected_kind=stable expected=00 prefill=69 value=69"),
            (b"address=69 register=05", b"address=68 register=05"),
            (b"expected_kind=topology-stable", b"expected_kind=stable"),
        )
    ...
    def test_grammar_truncation_and_duplicate_sample_fail_closed(self) -> None:
        ...
        mutations = (
            success.rstrip(b"\n"),
            b"\n" + success,
            success.replace(b"candidate=Fermi ", b"candidate=Fermi  ", 1),
