#!/usr/bin/env python3
"""Derive Vega's guarded boot2 installer from exact Orion machinery."""
...
def reconstruct_orion(work: pathlib.Path) -> str:
    root = pathlib.Path(__file__).resolve().parents[3]
    scripts = (
        root / "experiments/2026-07-27-mt6797-i2c6-orion/scripts"
    )
    deriver = scripts / "derive-installer.py"
    pins = scripts / "installer_orion.py"
    io.read_regular(deriver, "Candidate Orion installer deriver")
    io.read_regular(pins, "Candidate Orion installer pins")
    if io.digest_path(deriver) != io.ORION_DERIVER_SHA256:
        raise ValueError("source-pinned Candidate Orion installer deriver changed")
    if io.digest_path(pins) != io.ORION_PINS_SHA256:
        raise ValueError("source-pinned Candidate Orion installer pins changed")

    output = work / "install-candidate-orion-boot2.sh"
    ...
    if result.returncode:
        ...
        raise ValueError(f"Orion installer reconstruction failed: {detail}")
    info = output.lstat()
    if (
        output.is_symlink()
        or not stat.S_ISREG(info.st_mode)
        or stat.S_IMODE(info.st_mode) != 0o700
        or io.digest_path(output) != io.ORION_INSTALLER_SHA256
    ):
        raise ValueError("exact Candidate Orion installer changed")
...
def calibrated_replacements(
    ...
    return (
        (
            "readonly VEGA_RAW_SHA256="
            "6724eb7bccc5179955681a156468af53ec60557242b50e555cb50a02769a04db",
            f"readonly VEGA_RAW_SHA256={pins.raw_sha256}",
            1,
        ),
        (
            "readonly VEGA_RAW_SIZE=7747584",
            f"readonly VEGA_RAW_SIZE={pins.raw_size}",
            1,
        ),
        (
            "readonly VEGA_PADDED_SHA256="
            "74f9d9c8cae1213665db2100dda72e0531e0b221cd74a660fc183edcd7bb50d4",
            f"readonly VEGA_PADDED_SHA256={pins.padded_sha256}",
            1,
        ),
        (
            "readonly VEGA_ARTIFACT_MANIFEST_SHA256="
        ...
        (
            "readonly EXPECTED_CURRENT_ORION_PADDED_SHA256="
            "febe4d44b14b899cb357fae1b3ecda9bdb687c0c3e1f9e4b3cee30bc04f13cf1",
            "readonly EXPECTED_CURRENT_ORION_PADDED_SHA256="
            f"{io.ORION_PADDED_SHA256}",
            1,
        ),
        (
            'expected_artifact_name="candidate-Vega-mt6797-i2c6-6724eb7b"',
...
def validate_contract(text: str, pins: io.ArtifactPins) -> None:
    ...
    exact_counts = {
        f"readonly VEGA_RAW_SHA256={pins.raw_sha256}": 1,
        f"readonly VEGA_RAW_SIZE={pins.raw_size}": 1,
        f"readonly VEGA_PADDED_SHA256={pins.padded_sha256}": 1,
        "readonly VEGA_ARTIFACT_MANIFEST_SHA256="
        f"{pins.manifest_sha256}": 1,
        "readonly EXPECTED_CURRENT_ORION_PADDED_SHA256="
        f"{io.ORION_PADDED_SHA256}": 1,
        f'expected_artifact_name="{pins.artifact_dir}"': 1,
        f'[[ "$candidate_name" == {io.BOOT_MEMBER} ]]': 1,
        f"experiment={io.EXPERIMENT}": 2,
        "candidate_label=Vega": 2,
        "EXPECTED_CURRENT_ORION_PADDED_SHA256": 8,
        "Orion-installed-readback-verified": 4,
    ...
    for token, expected in exact_counts.items():
        ...
        if actual != expected:
            raise ValueError(
                f"derived Vega installer count changed for {token!r}: "
    ...
    for token, expected in CRITICAL_TOKEN_COUNTS.items():
        ...
        if actual != expected:
            raise ValueError(
                f"derived Vega safety gate changed for {token!r}: "
                f"expected {expected}, found {actual}"
            )
    ...
    for token in required:
        if token not in text:
            raise ValueError(f"derived Vega installer lost contract token: {token}")

    stale = (
        "EXPECTED_CURRENT_HUBBLE",
        "Hubble-installed-readback-verified",
        "candidate-Orion-mt6797-i2c6-6724eb7b",
        "gemini-mt6797-i2c6-orion.boot.img",
        "2026-07-27-mt6797-i2c6-orion",
        "boot2-before-candidate-orion",
        "boot2-after-candidate-orion",
        ".gemini-candidate-orion",
    )
    for token in stale:
        if token in text:
            raise ValueError(f"derived Vega installer retains stale token: {token}")
    ...
    for token in forbidden:
        if token in text:
            raise ValueError(f"derived Vega installer gained forbidden token: {token}")
...
def derive_text(source: str, pins: io.ArtifactPins) -> str:
    ...
    for old, new, count in reversed(generic_replacements()):
        restored = replace_exact(restored, new, old, count)
    if restored != source:
        raise ValueError("Vega installer cannot restore exact Orion foundation")
...
def main() -> int:
    ...
    try:
        ...
        io.require_artifact_pins(pins)
        reproducibility_record_sha256 = io.require_reproducibility_record(
            repository,
            pins,
        )
        ...
        with tempfile.TemporaryDirectory(
            prefix=".vega-orion-installer.", dir=output.parent
        ) as raw:
            source = reconstruct_orion(pathlib.Path(raw))
        text = derive_text(source, pins)
        digest = hashlib.sha256(text.encode()).hexdigest()
        if io.INSTALLER_SHA256 != "UNRESOLVED" and digest != io.INSTALLER_SHA256:
            raise ValueError("derived Vega installer identity changed")
        publish(output, text)
        print("validation=vega-installer-derived")
        ...
        print(
            ...
            f"{reproducibility_record_sha256}"
        )
