...
# Assemble storage-inert Candidate Vega from a validated kernel package and
...
value() {
	PYTHONPATH="$script_dir" python3 -c \
		'import candidate_vega as c,sys; print(getattr(c,sys.argv[1]))' "$1"
}
...
package_validator="$script_dir/validate-package-vega.py"
orion_scripts="$repo_root/experiments/2026-07-27-mt6797-i2c6-orion/scripts"
dtb_builder="$orion_scripts/build-orion-dtb.sh"
dtb_validator="$orion_scripts/validate-orion-dtb.py"
dtb_lineage_validator="$orion_scripts/validate-orion-dtb-lineage.py"
...
[[ "$(sha256sum "$dtb_builder" | awk '{print $1}')" == \
	"$(value ORION_DTB_BUILDER_SHA256)" ]] ||
	die 'source-pinned Orion DT builder changed'
[[ "$(sha256sum "$dtb_validator" | awk '{print $1}')" == \
	"$(value ORION_DTB_VALIDATOR_SHA256)" ]] ||
	die 'source-pinned Orion boot-DT validator changed'
[[ "$(sha256sum "$dtb_lineage_validator" | awk '{print $1}')" == \
	"$(value DTB_LINEAGE_VALIDATOR_SHA256)" ]] ||
	die 'source-pinned Orion DT lineage validator changed'
...
workdir="$(mktemp -d "$output_parent/.candidate-vega.XXXXXX")"
...
cmp -s "$stage/$DTB_MEMBER" "$replica/$DTB_MEMBER" ||
	die 'independent Vega DT derivations differ'
[[ "$(sha256sum "$stage/$DTB_MEMBER" | awk '{print $1}')" == \
	"$(value ORION_BOOT_DTB_SHA256)" ]] ||
	die 'Vega boot DT differs from exact Orion boot DT'
...
for output in "$stage/$BOOT_MEMBER" "$replica/$BOOT_MEMBER"; do
	python3 "$serializer" --kernel "$stage/Image.gz" \
		...
		--output "$output" --name gemini-vega \
...
cmp -s "$stage/$BOOT_MEMBER" "$replica/$BOOT_MEMBER" ||
	die 'independent Vega Android-v0 assemblies differ'
...
python3 "$analyzer" --validate-lk --expected-image-gz "$stage/Image.gz" \
	...
	--expected-dtb "$stage/$DTB_MEMBER" --expected-name gemini-vega \
...
((raw_size > 0 && raw_size <= 16 * 1024 * 1024)) ||
	die 'Vega image does not fit boot2'
...
compiled_dtb_delta=exact-orion-byte-identical
boot_dtb_delta=exact-orion-byte-identical
...
[[ "$actual" == "$expected" ]] || die 'Vega output inventory changed'
...
(cd "$stage" && sha256sum --check --strict SHA256SUMS >/dev/null) ||
	die 'Vega artifact manifest failed'
chmod 0600 "$stage"/*
...
printf 'validation=candidate-vega-assembled\n'
printf 'artifact=%s\nraw_sha256=%s\nraw_size=%s\n' \
