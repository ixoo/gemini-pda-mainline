...
- [Candidate H](experiments/2026-07-16-simplefb-mm-root-retention/README.md) preserves G's exact kernel and initramfs and appends only `CLK_TOP_MUX_MM` to the simplefb clock references. Two builds are byte-identical, every semantic/container gate passes, and the exact image is synchronized to logical `boot2` with a matching full-partition readback. Runtime is the next gate; rotation remains deferred until display retention is stable.
