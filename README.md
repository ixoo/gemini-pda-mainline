...
- [Candidate N](experiments/2026-07-18-cpu1-online-diagnostic/README.md) is the next recovery-backed platform gate. It retains exact M's kernel, embedded configuration, no-IRQ DTB, pstore, fbcon, watchdog, and LK container, changing only external `/init`. Exact-binary validation confirms SMP, CPU hotplug, PSCI, forced `maxcpus=1`, and the intended first secondary DT CPU as Cortex-A53 MPIDR `0x1`; N requires the live Linux CPU1 `of_node` to match before acting. It arms the proven watchdog first, writes `1` once to CPU1's standard `online` control, captures the return/masks/kernel lines/accounting, and never retries or pings again. Two clean VM builds are recursively identical; the raw image SHA-256 is `43aea71224f6261001ff00904b30dae29063334172a2f6b0163b424a84c0e3aa`. It is exported privately but has not yet been written or tested. See the [build reproduction](experiments/2026-07-18-cpu1-online-diagnostic/results/final-build-reproduction-20260718.txt).
