...
- Patches 0077–0078 and the [USB gadget diagnostic](experiments/2026-07-16-usb-gadget-diagnostic/README.md) add a peripheral-mode MTU3/T-PHY test path. Its exact candidate is written and fully read back from non-primary `boot2`, but it has not run on hardware. The next evidence gate is USB enumeration. Ping and the TCP marker shell are separate later gates for initramfs execution; this diagnostic makes no host-mode, VBUS, Type-C policy, or charging claim.
...
- A focused review of [bsg100's working native-fbcon commit](experiments/2026-07-13-bsg100-gemini-linux-comparison/results/fbcon-commit-035d4b0-20260716.md) identifies a one-property backlight-clock retention test for the next simplefb candidate and a staged native-DRM port. Its generic MT6797 handoff/PHY findings are useful evidence; its SSD2092 panel data is not assumed to match this unit.
...
- [Candidate G](experiments/2026-07-16-fbcon-text-diagnostic/README.md) is the controlled follow-up: the exact F kernel and DTB, with only the initramfs changed to remove every raw framebuffer access and hold a distinctive console banner. Two builds are byte-identical and the image is synchronized with a matching full readback from logical `boot2`. Sideways output is expected because the exact kernel has fbcon rotation compiled out; rotation is the next isolated kernel-config change after this runtime A/B.
