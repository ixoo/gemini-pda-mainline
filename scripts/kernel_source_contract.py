...
"""Shared kernel source selection and package provenance contract."""
import argparse
import datetime
import json
...
def check_provenance(manifest, build):
    ...
    # Historical packages require only the global digest. Do not retroactively
    # apply modern version/date/URL validation when no complete tuple was stored.
    config = manifest.get('config', {})
    entry = config.get('profiles', {}).get(profile, {})
    if not isinstance(entry, dict):
        raise ValueError('selected profile must be an object')
    if 'kernel' not in entry and 'kernel_source' not in build:
        if 'profiles' in config and profile not in config['profiles']:
            raise ValueError('unknown selected profile')
        if 'profiles' not in config and profile is not None:
            raise ValueError('legacy manifest cannot select a profile')
        source = manifest.get('kernel', {})
        digest = source.get('sha256') if isinstance(source, dict) else None
        if not isinstance(digest, str) or not re.fullmatch(r'[0-9a-f]{64}', digest):
            raise ValueError('source checksum must be SHA-256')
        if build.get('source_sha256') != digest:
            raise ValueError('provenance source_sha256 does not match the packaged kernel manifest')
        return {'profile': profile, 'selection': 'global', 'kernel': {'sha256': digest}}
    selected = resolve(manifest, profile)
    expected = selected['kernel']
    if build.get('source_sha256') != expected['sha256']:
        raise ValueError('provenance source_sha256 does not match selected profile')
